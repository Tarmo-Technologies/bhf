...
- `staticScan`: run the same full offline static engine as the CLI for
  ...
  or `bhf_work/static`, and return `summary`, `report`, and `exit_code`. It
...
`bhf-daemon --mcp`. That is a separate protocol mode from the LSP-framed
...
The host model performs any reasoning, so BHF receives no host API token.
...
Authenticated responses include a top-level `bhfAudit` object with tenant,
role, method, and workspace metadata. Auth failures use BHF server error
