...
# `--force` reach: what was fixed, and what bhf still cannot fuzz
...
the value: `<Type> _bhf_ret = {0}; return _bhf_ret;`. `{0}` zero-initializes any
...
bhf auto clay --work-dir wk --per-target-time 1 --single-pass --jobs 1 \
...
harvests the actual `error:` lines out of every harness BHF GAVE UP on, and
...
BHF's own errors rather than project limitations:
...
- **`unknown type name` (17) is the largest, and it splits.** Roughly half is a
  ...
  the manifest, not a repair. The other half is bhf's own: an export
...
- `cannot combine with previous declaration specifier` (7, all C, all tmux)
  smells like a synthesized typedef colliding with a real one, i.e. a bhf
...
- **Go** (`crates/cli/src/auto/go_build.rs`): an undrivable parameter becomes its
  ...
  method touched a field, and that panic would be bhf's fault, not the
  target's.
...
- **Both A/B arms must run the same binary**, and the sweep must not point at
  `target/release/bhf` while you are still building into it — an enumeration
  run that way silently changes binary mid-sweep and is not an A/B at all. Copy
  the binary to `/home/ubuntu/bhf-sweep-bin/` first and pass it explicitly.
- **`bhf --version` is identical across rebuilds** (`v0.2.20-18-g893b68c-dirty`).
