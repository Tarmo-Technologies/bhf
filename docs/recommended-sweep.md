...
# The recommended BHF sweep

One command for the usual job: point BHF at a source tree you control, let it
...
Read `bhf_work/FINDINGS.md` first; the top-level `findings.csv` is the
...
| Flag | Why it is here |
|---|---|
| `--work-dir bhf_work` | Everything the run produces — harnesses, corpora, findings, reports. **Keep it OUTSIDE the tree being scanned**, or the next run discovers BHF's own generated harnesses as targets. |
| `--max-work-dir-mb 4096` | Default campaign-wide allocated-size ceiling. Once reached, BHF preserves completed/in-flight evidence but starts no new targets; `0` disables it. Parallel in-flight targets can cause a bounded overshoot. |
...
| `--unsafe-search-and-run-build-commands` | **Runs code from the scanned tree.** Finds the project's own build entry point (`build.sh`, autotools, CMake, Meson, SCons, Waf, Bazel) and executes it under BHF's compiler-interception shim to recover the real compile flags. This is what turns "cannot build" into "builds" on most real C/C++ trees. Only use it on sources you trust; drop it for untrusted code and pass `--build-command` instead if you know the command. |
...
| `--debug` | Capture a backtrace if BHF itself panics, keep going past a file that crashes it, and enrich `bug-report.json`. Cheap; leave it on. |

## Variations worth knowing

- **Reclaiming disk safely.** `bhf clean bhf_work --compact` removes
...
- **Untrusted source.** Drop `--unsafe-search-and-run-build-commands`. BHF
...
- **static-only / report_only** — BHF could not build it and fell back to
