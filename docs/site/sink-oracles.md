...
connect to *this* host, load *this* library, execute *this* SQL. bhf calls
...
attacker-controlled on a reachable path — so it produces noise. bhf confirms
...
That "never untainted" clause is what keeps false positives at zero. bhf
...
|---|---|---|---|
| **File-open path traversal** | `open`, `openat` | BHF-405 | CWE-22 |
| **Process execution** | `system`, `popen`, `execv`, `execvp`, `execvpe`, `execve`, `fexecve`, `posix_spawn`, `posix_spawnp` | BHF-431 | CWE-78 |
| **Network egress / SSRF** | `getaddrinfo` (hostname), `connect` (destination) | BHF-433 | CWE-918 |
| **Controlled library load** | `dlopen`, `dlmopen` | BHF-435 | CWE-427 |
| **SQL injection** | `sqlite3_exec`, `sqlite3_prepare[_v2/_v3]`, `PQexec`, `PQexecParams`, `mysql_query`, `mysql_real_query` | BHF-441 | CWE-89 |
| **Destructive filesystem op** | `unlink`, `unlinkat`, `remove`, `rename`, `renameat`, `mkdir`, `mkdirat`, `rmdir`, `symlink`, `symlinkat`, `link`, `linkat`, `truncate` | BHF-440 | CWE-73 |
| **Format string** | `printf`, `fprintf`, `sprintf`, `snprintf`, `dprintf` | BHF-408 | CWE-134 |
...
- **XML external entity (CWE-611)** is not discriminating as a taint sink: a
  ...
  true on every iteration. XXE is better caught by the static scanner (BHF-430),
...
- **LDAP injection (CWE-90)** and other niche library sinks are extensible by the
  ...
  they are rare enough in the government/legacy corpus bhf targets that the
