...
# Fuzzing C# / .NET with bhf

bhf fuzzes C# with **no harness to write** — point it at a .NET source tree and
...
```sh
bhf auto path/to/dotnet-src --languages csharp
```

## How it works

A `public` method taking a byte buffer (`byte[]`, `ReadOnlySpan<byte>`,
`Memory<byte>`), a `string`, or a `System.IO.Stream` is the fuzzable unit. bhf:
...
1. **Discovers** each such method, tracking the namespace and enclosing type by
   ...
   scalar siblings (an `int` length operand), so bhf never invents a
...
2. **Builds** the target with `dotnet build -c Release` through a
   ...
   `BhfEntry.Run(byte[])` shim that decodes the fuzz bytes to the parameter's
...
4. **Fuzzes** it over bhf's own coverage-guided fork-server engine. The driver
   `mmap`s bhf's file-backed `BHF_COV_SHM` edge bitmap (64 KB — exactly the
   AFL map size SharpFuzz targets) into `SharpFuzz.Common.Trace.SharedMem`, so the
   instrumented target writes coverage straight into bhf's cumulative bitmap.
...
driver hard-halts with exit 86 and a `== bhf csharp finding:` marker). The
.NET exception type maps to a BHF rule + CWE:

| Exception | Rule | CWE |
|---|---|---|
| `IndexOutOfRangeException` | BHF-201 | CWE-125/787 (out-of-bounds) |
| `NullReferenceException` | BHF-206 | CWE-476 (null dereference) |
| `DivideByZeroException` / `OverflowException` | BHF-205 | CWE-369/190 |
| `StackOverflowException` | BHF-207 | CWE-674 (uncontrolled recursion) |
| `OutOfMemoryException` | BHF-209 | CWE-789 |
| any other / custom throwable | BHF-210 | reachable crash |
...
library bhf builds on. SharpFuzz is excellent, but it is a *library*: to use it
...
bhf automates the entire SharpFuzz workflow end to end:

| | SharpFuzz (raw) | **bhf `auto --languages csharp`** |
|---|---|---|
...
| Coverage plumbing | `__AFL_SHM_ID` / shmat | **automatic (`BHF_COV_SHM` bridge)** |
...
- **End-to-end on YamlDotNet**: 14 of 21 methods were built, IL-instrumented, and
  ...
  real coverage — and bhf **fuzz-confirmed 8 real bugs**: a
