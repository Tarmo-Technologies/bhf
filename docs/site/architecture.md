...
BHF is an offline fuzzing toolchain for sixteen languages — Ada, C, C++,
...
`bhf auto` runs steps 1–7 in one shot. Ada, C, and C++ targets are
...
PHP under their own interpreters. Every lane drives bhf's own builtin engine
...
- `crates/bhf_runtrace_shim` — LD_PRELOAD shim
  (`libbhf_runtrace.so`) that audits and fakes the runtime
...
- `crates/finding_rules` — finding-class registry (BHF-101…BHF-563, plus BHF-668,
...
- `crates/cli` — the `bhf` command-line entry point. Houses the
...
and GitHub Pages serves the generated artifact at `docs.buildharnessfuzz.dev` when
...
`BHF_PAGES_ENABLED` repository variable so the workflow stays green
