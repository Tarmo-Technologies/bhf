...
# Fuzzing Fortran with bhf

bhf fuzzes Fortran with **no harness to write** — point it at Fortran source
...
4. **Fuzzes** it on bhf's C fork-server engine at native speed (thousands of
...
path injection) also apply, since the harness runs on bhf's C path.
...
- **Standalone free-form subroutines fuzz and find real bugs**: a heap out-of-bounds
  array access is caught as an ASan crash (BHF-201, CWE-122/787) attributed to the
