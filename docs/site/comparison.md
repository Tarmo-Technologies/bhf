...
# bhf vs the field: a measured comparison

This page compares **bhf** against the most widely used fuzzer for each of
the eight languages covered by this benchmark suite, on identical planted-bug
targets, with real measurements. BHF now has sixteen fuzzing lanes; COBOL,
...
Every number here is produced by the scripts under [`benchmarks/`](https://github.com/Tarmo-Technologies/bhf/tree/main/benchmarks)
...
bhf removes the plumbing. It discovers fuzzable entry points across a whole
...
`TTFC` for bhf is **end-to-end** (raw source → discovery → build → crash);
...
parity bhf also ships the AFL++ engine adapter. An experimental LibAFL
adapter crate can be built with its cargo feature, but it is not a selectable
`bhf fuzz --engine` value. The built-in engine is the zero-config default
measured here.
...
| **bhf** | **0** | **stable** | **yes** | 1.61 s (end-to-end) |

cargo-fuzz needs a nightly toolchain, a `fuzz/` crate, and a `fuzz_target!`
harness per target. bhf fuzzes the library as-is on stable Rust, harness-free.

### Java — vs Jazzer

bhf fuzzes Java with its **own JVM bytecode coverage agent**
...
| **bhf** (native JVM agent) | **0** | **yes** | 2.62 s (end-to-end) |

bhf discovers the receiver method, **synthesises the receiver object**, and
...
| **bhf** | **0** | **yes** (CONSTRAINT_ERROR) | 1.41 s (end-to-end) |

bhf discovered `Parse_Frame`, decoded fuzz bytes into its `String` parameter,
...
C++ service, bhf ranks `Gov::TelemetryService::Submit` as attacker-reachable
and **synthesises the receiver object** to drive it. With libFuzzer/AFL++ a human
must write a harness that constructs the object and calls the method; bhf
does it automatically.

### Python, Perl, and Go — three more lanes, same zero-harness workflow

bhf also fuzzes **Python**, **Perl**, and **Go**, each driven by its own
...
writes is **0**: bhf discovers the entry points, generates the typed driver,
...
`-d:BhfCov` (`DB::DB`) tracer, both into the same shared coverage map the
...
input, and folds the executed-block set into BHF's shared edge map. A failed
...
A fuzzer that needs one harness per entry point scales with human effort. bhf
scales with one command:

| library | targets bhf auto-harnessed (one command, 0 harnesses) | equivalent hand-written harness code |
|---|---|---|
...
- **Fairness.** Each competitor runs in its best documented configuration (AFL++
  ...
  input that trips no gate. bhf runs with no flags but `--per-target-time`.
- **TTFC.** bhf's is end-to-end and *includes* the build of the harness it
