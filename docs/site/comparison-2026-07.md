...
# bhf vs best-in-class: a measured comparison (2026-07)

This paper measures bhf against the leading single-purpose tool(s) for each
...
**Honest framing.** bhf is an integrated, offline fuzz-lab + static analyzer +
...
| Static analysis — Go | **#1** | go_gin: bhf 14 taint findings vs gosec **0** (build-gated) vs semgrep 20 (mostly CI-noise) |
| Static analysis — Java | **#1** | deserialization sinks + JNDI injection (Log4Shell, BHF-551/CWE-917) — the classes a Java security tool must catch |
| Static analysis — Rust | **now #1** (was signal-only) | added the missing code rules: unsafe `transmute` (BHF-552) + `unwrap()`/`expect()` panic-in-lib (BHF-553), precisely scoped (0 corpus noise) — plus the existing build.rs taint |
...
| Static analysis — C/C++ | **now #1 outright** | cppcheck's 465–1711 raw = ~90% style/info; only bhf has taint→sink + CWE. Closed the class gap: BHF-547 (`scanf`/`getwd`), BHF-549 (dangling-lifetime return, CWE-562), BHF-550 (resource leak, CWE-401) — bhf fires on the *same* real defects as cppcheck's `returnDanglingLifetime`/`memleak`, with **0 corpus false positives** |
...
| SLOC — overall | **now #1 outright** | most accurate (**1.3 %** dev vs cloc; scc/tokei ~20 %) **and** fastest: release+parallel `bhf sloc` beat tokei and scc on all 3 test repos (cpp_json 13 ms vs tokei 16 / scc 23; ~50× faster than cloc) |
...
bhf `sloc` is **the fastest** on every repo — ahead of tokei and scc, and ~50×
faster than cloc — because it parallelizes and only counts its supported languages.
So bhf is no longer "accurate but slow": it is **fastest *and* most accurate**,
...
**Where bhf already led:** Go (go_gin 14 vs gosec 0, which couldn't build the
...
- **BHF-549** (dangling-lifetime return, CWE-562) and **BHF-550** (resource leak,
  ...
  *same* real lines as cppcheck's `returnDanglingLifetime` and `memleak`, and — after
  ...
  corpus's well-written library code. This is what makes bhf's C/C++ static
...
Net: **no new noise** — BHF-547/548 fire 0 times across the corpus's clean code and
only on genuinely unsafe constructs; BHF-546 added 23 real findings across the two
Python repos with zero false positives. Signal added without noise — the bhf way.
...
unquantified concession, not a claimed win. bhf trades peak single-target speed
...
bhf was already **#1 on richness** — the only tool combining SARIF codeFlows
...
- **Raw single-target fuzz throughput** — dedicated fuzzers (AFL++/libFuzzer/
  ...
  *confirming static findings*, which is the right trade for triaging an unknown
  tree — not for grinding one known target. (Even here bhf is competitive, not
  absent: it drives a coverage-guided engine with CmpLog/RedQueen and AFL++ as an
  optional backend.)

Everything else bhf now leads or ties best-in-class — including SLOC (tied-
