...
BHF is designed to run with no network access. This page is the operational
guide for **installing and updating BHF on a disconnected (air-gapped)
...
BHF fuzzes sixteen languages (Ada, C, C++, Rust, Java, Python, Perl, Go,
...
There is no BHF auto-updater and the tool never phones home. Updating is a
...
|---|---|---|
| You move | The built `bhf`, `bhf-daemon`, runtrace shim, and compiler-interception shim | The repo + a vendored crate cache |
...
the *generated harnesses*; they are separate from building BHF itself.

## Optional LLM assistance in an enclave

The complete deterministic BHF workflow needs no model and makes no network
...
endpoint inside the enclave. BHF's distribution does not bundle weights or
a model server.

`bhf-daemon --mcp` can also serve its five read-only tools locally, but the
...
BHF application source tree. It does include the harness runtime support
files that generated harnesses and interpreter drivers need at build/run time.

The package contains:

- `tool/bhf`, `tool/bhf-daemon`, the runtrace shim, and the
...
|---|---|---|
| `rules` | `rules/static.json` | Built-in finding/static-analysis rule catalog exported by `bhf rules list --json` |
| `cve` | `cve/sbom-cves.json` | Offline CVE/SCA database used by `bhf sbom --vuln-db` |
...
BHF and the bundled rule catalog; there is no separate CWE pack to generate.
...
- `bhf-*` — the CLI
- `bhf-daemon-*` — the JSON-RPC/read-only-MCP daemon (only for IDE or MCP use)
- `bhf_runtrace_shim-*` — the `LD_PRELOAD` shim `bhf auto` uses for
...
- `bhf_cc_intercept-*` — the `LD_PRELOAD` shim C/C++ build recovery uses to
...
files. For the usual full Linux `bhf auto` deployment, transfer the CLI,
...
gh release download vX.Y.Z --repo Tarmo-Technologies/bhf \
  --pattern 'bhf-x86_64-unknown-linux-gnu.tar.xz*' \
  --pattern 'bhf_runtrace_shim-x86_64-unknown-linux-gnu.tar.xz*' \
  --pattern 'bhf_cc_intercept-x86_64-unknown-linux-gnu.tar.xz*'

# Only if the disconnected host will provide JSON-RPC/MCP:
gh release download vX.Y.Z --repo Tarmo-Technologies/bhf \
  --pattern 'bhf-daemon-x86_64-unknown-linux-gnu.tar.xz*'
...
sha256sum -c bhf_runtrace_shim-*.tar.xz.sha256
sha256sum -c bhf_cc_intercept-*.tar.xz.sha256
# If downloaded:
...
```sh
sha256sum -c bhf-x86_64-unknown-linux-gnu.tar.xz.sha256 # re-verify after transfer
tar xf bhf-x86_64-unknown-linux-gnu.tar.xz
tar xf bhf_runtrace_shim-*.tar.xz
tar xf bhf_cc_intercept-*.tar.xz
...
```
target/release/bhf
target/release/libbhf_runtrace_shim.so # recommended: runtime audit/fake resources
target/release/libbhf_cc_intercept.so  # optional: complex C/C++ build recovery
target/release/bhf-daemon              # optional: IDE JSON-RPC or MCP
```

> **Match the runtime, not just the CPU.** The runtrace shim is `LD_PRELOAD`-ed
> into the harness processes BHF compiles on the offline host, so it must be
...
host (BHF pins all dependency versions in `Cargo.lock`).
...
BHF discovers, ranks, and recovers source with no external tools, but to
...
| Perl | `perl` (per-statement edge coverage via the bundled `-d:BhfCov` debugger) | harness build (`perl -c` + `require` smoke-test) + fuzz |
...
mirror, transfer, `dpkg -i`). Without them, `bhf auto` still runs discovery
...
> **License profile reminder.** GNAT and GPRbuild are GPL. BHF only ever
...
offline until those are staged. BHF turns this into a single scan-then-stage
...
bhf auto path/to/src --work-dir bhf_work --deps-only
#   → writes bhf_work/auto/missing-deps.txt (human list)
#           bhf_work/auto/missing-deps.json (machine-readable)
...
bhf auto path/to/src --work-dir bhf_work \
...
project's scripts. Without them, BHF stubs the generated deps and records
them in the manifest instead of running anything.

---

## Updating the bhf tool in place (data-safe)
...
| Action | Items |
|---|---|
| **Replaced** (updated to the new version) | `bhf`, `bhf-daemon`, the `libbhf_runtrace*.so` and `libbhf_cc_intercept.so` shims, and every `*_runtime/` codegen tree. Re-installed even if unchanged — a re-run is a clean overwrite. |
...
corpora, and all prior run data. `bhf --version` (or `cat <prefix>/VERSION`)
...
## When bhf itself errors: `--debug` and `bug-report.json`

If bhf *itself* fails on your tree — an internal panic, or a harness it
...
bug report is bhf's *own* defects, boiled down to what a maintainer needs to
fix them offline.
...
- A bhf-internal panic while parsing or fuzzing **one** file is caught at that
  ...
  the whole run.
- Whenever a run hits any internal defect, bhf writes
...
# → bhf_work/auto/bug-report.md   (paste an entry to the maintainer)
```

`bug-report.md` lists each issue with its category (`PANIC` / `codegen`), the
source file and target it happened on, the language, the bhf version +
...
creates and installs this pack for you. Run `bhf pack create` directly only
...
bhf pack verify packs/current/update-pack.json \
  --root packs/current --policy bhf-policy.json
```
...
```sh
bhf --help                                  # CLI resolves
bhf auto <a small bundled source tree> \    # full pipeline end-to-end
  --work-dir /tmp/bhf-smoke --per-target-time 5 --verbose
```
...
see `libbhf_runtrace_shim.so not found` or every target reports build
failure, re-check the shim location (`BHF_RUNTRACE_SHIM`) and the
