...
**bhf sees them.** On top of the same coverage-guided engine it runs byte-origin
...
In a measured head-to-head on a library with one CWE per function, bhf found
...
**Runtime taint.** bhf's `LD_PRELOAD` instrumentation records the byte origin
...
events into findings: path-controlled open (`BHF-405`/CWE-22), command injection
(`BHF-304`/CWE-78), insecure temp file (`BHF-417`/CWE-377), sensitive-environment
access (`BHF-305`/CWE-522), format string (`BHF-408`/CWE-134), SSRF
(`BHF-303`/CWE-918), and more — each with a severity, a confidence, a sink
...
bhf discovers and harnesses *every* fuzzable function, so a bug anywhere in
...
reaches one, bhf auto-harnesses all — with zero hand-written harness code:

| language | **bhf** (0 harnesses) | competitor (1 harness) |
|---|:--:|:--:|
| Rust vs cargo-fuzz | **3 / 3** | 1 / 3 |
| Java vs Jazzer | **3 / 3** | 1 / 3 |

**Ada.** bhf found both planted `CONSTRAINT_ERROR`s; no other fuzzer supports
Ada.

**Timing.** bhf writes the harness on the first run and reuses it on the
...
number of executions, and the **same** bhf run also reports three behavioral
CWEs libFuzzer finds in *no* amount of time. On a behavioral CWE, bhf wins the
...
bhf fuzzes the whole tree across sixteen current lanes and adds runtime
...
*Reproduce every figure: `benchmarks/cwe/` in the bhf repository. clang/
