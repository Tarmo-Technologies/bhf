...
BHF's `runtrace_shim` virtualises the libc boundary so a target
...
1. Create `crates/bhf_runtrace_shim/src/hooks/clock.rs` with:
   ...
   - An `impl crate::sdk::FakeResource for Clock` returning
     ...
     `BHF_FAKE_CLOCK` env var, and a one-line description.
...
`bhf auto --list-fakes` prints the inventory:
...
identity            env-gated    BHF_FAKE_IDENTITY       getpid getuid getgid getppid
cmplog              env-gated    BHF_CMPLOG              strcmp strncmp memcmp
...
`BHF_FAKE_IDENTITY=1` is set the four POSIX identity calls
