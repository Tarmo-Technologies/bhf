...
BHF's built-in engine is coverage-guided and runs offline with no libFuzzer
...
BHF supplies its own mutator and coverage feedback instead of libFuzzer's.
This page maps the familiar libFuzzer knobs to their BHF equivalents so users
...
| `-max_total_time=N` | `--time 30s` (manual `fuzz`); `auto --per-target-time N` | Whole-campaign budget for a single manual `fuzz` run. Under `bhf auto`, `--per-target-time` (default 60s) is the per-target TOTAL fuzz wall, split evenly across the three passes (empty/rng/fuzz_driven) under one shared deadline — so the per-target wall ≈ this value, **not** × pass count. This is the per-target `-max_total_time` / AFL `-V` parity knob. |
...
instrumented by BHF's own ASM agent (not Jazzer), feeding edge coverage into
...
BHF reaches the same goal through **cmplog/RedQueen** rather than a distance
...
- **String / buffer gates** (`memcmp`, `strcmp`, `strncmp`, …): the generated
  ...
  LD_PRELOAD runtrace shim records the same under `BHF_CMPLOG=1`.
...
Both feed a per-exec operand ring (`BHF_CMP_SHM`). The engine arms capture for
...
nudging an input toward a comparison value over many generations, BHF injects
