...
The parity benchmark measures how fast BHF's **built-in** engine finds a
planted bug **cold** — with no trigger seed — on a corpus of gated-parser gap
classes, the metric that matters for [#376](https://github.com/Tarmo-Technologies/bhf/issues/376)
(libFuzzer parity).
...
its own gate test
([#400](https://github.com/Tarmo-Technologies/bhf/issues/400)):

```sh
cargo test -p bhf --test redqueen_cmplog -- --ignored --nocapture
```

It asserts the gate is solved cold with per-input capture **on** and **not** with
it disabled (`BHF_DISABLE_REDQUEEN=1`, the dictionary-only path).

## Running it

```sh
cargo test -p bhf --test engine_parity -- --ignored --nocapture
```
...
solved cold** — the parity gate. The `BHF_PARITY_SECS` env var sets the
...
([#379](https://github.com/Tarmo-Technologies/bhf/issues/379)), with cmplog
([#378](https://github.com/Tarmo-Technologies/bhf/issues/378)), edge
coverage ([#381](https://github.com/Tarmo-Technologies/bhf/issues/381)),
and the entropic schedule
([#382](https://github.com/Tarmo-Technologies/bhf/issues/382)). The missing
piece was **throughput**: generated C direct harnesses used to run per-spawn
(~tens of execs/s). [#399](https://github.com/Tarmo-Technologies/bhf/issues/399)
