...
`docs.buildharnessfuzz.dev` is the public documentation site for BHF
(Build Harness Fuzz) operators, release engineers, and IDE integrators.
BHF builds the harness so you do not have to: it is an offline fuzz lab
...
- [Auto](./auto/) — `bhf auto <PATH>` sweeps a source tree in any of the
...
- [Comparison](./comparison/) — bhf measured head-to-head against the most
...
- [Release Packaging](./release-packaging/) — distributed archives,
  checksums, and checksum-verified content packs.
...
- [Offline / Air-Gapped Deployment](./offline-deployment/) — installing and
  updating BHF on a disconnected host: build-vs-transfer, the artifact set,
