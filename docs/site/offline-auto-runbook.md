...
BHF can recover compile flags, generate harnesses, repair broken build
...
BHF creates `auto/missing-deps.txt` and `auto/missing-deps.json` before the
...
drop are listed separately as vendor/project source; BHF does not replace
...
target discovery begins. If the BHF parent is OOM-killed, the latest project
...
proprietary SDK/source has no identifying metadata, BHF records the
...
```sh
bhf auto /src/project \
  --work-dir /results/bhf-real \
  --languages ada,c,cpp \
  --run-untrusted \
...
execute in the isolated VM, let BHF find it:
...
- `--include-dir NAME` restores discovery under a directory that BHF would
  normally prune.
...
Helper/Skel/Stub mapping units under `bhf_work/fake_corba`, adds that source
...
present. When the generator or its inputs are absent, BHF can synthesize
...
- Size `--jobs` against memory: `jobs * --rss-limit-mb` is the fuzz-child
  allowance, not total peak. Also reserve RAM for BHF's declaration index,
...
The script writes and prints `bhf-support-report.txt`, capped at 4,000 bytes
...
Use `/results/bhf-real/auto/run.json` as the machine-readable evidence.
...
- `auto/bug-report.md` for defects in BHF itself.
