...
# Fuzzer Research And BHF Dogfood - 2026-07-01

This memo records a July 2026 ecosystem review plus a broad BHF dogfood
campaign. It complements the May 2026 landscape memo by validating current
BHF behavior across fuzzing, SBOM/vulnerability matching, reporting, static
scan, and binary fuzzing.

Artifacts from this run are intentionally outside the repository:

- Broad campaign root: `/tmp/bhf-dogfood-2026-07-01`
- Extended soak root: `/tmp/bhf-dogfood-long-2026-07-01`
...
| Area | Mature examples | BHF implication |
|---|---|---|
...
| Corpus lifecycle | Distributed fuzzing writeups emphasize saving corpora, merging findings back, and restoring the latest corpus in CI. | BHF should make corpus persistence/merge/reuse explicit in `auto` and CI reports. |
| Structure-aware fuzzing | Community discussions repeatedly note that raw byte mutation stalls on JSON/protobuf-like and protocol inputs. | BHF should grow schema/grammar/message-sequence lanes, especially for protocol and legacy record formats. |
| Triage/reproduction | Community issues show pain around minimization, non-reproducible findings, timeouts, and unclear progress/stalls. | Reporting is a BHF strength, but progress health and replay bundle quality need continued work. |
| Language/runtime coverage | OSS-Fuzz covers C/C++, Rust, Go, Python, Java/JVM, JavaScript, Lua; syzkaller handles OS kernels; Jazzer covers JVM code and bug detectors. | BHF's broad language discovery is useful, but non-C reporting and SBOM mapping need polish. |
...
- `bhf report` over the CWE findings emitted Markdown, JSON, SARIF, JUnit,
  and CSV.
...
- `bhf sbom tests/fixtures/rust_discovery` emitted BHF SBOM JSON,
...
- `bhf static-scan benchmarks/cwe/targets --sarif` emitted JSON, Markdown,
  and SARIF.
- It found one medium-confidence `BHF-406` environment-dependent configuration
  issue at `c/libcwe.c:52`.
- `--fail-on medium` exited 1 after writing artifacts, as expected.

Binary surface:

- `bhf binary scan target/release/bhf` emitted a binary inventory for
  the BHF ELF.
- A tiny compiled crashing fixture passed to `bhf binary fuzz` produced
  `BF-0001` with rule `BHF-501`, severity high, and a testcase artifact.
...
BHF reported these as external dependencies:
...
For source packages under test, BHF should map fuzzed harness/source paths
back to SBOM components before issuing "not reachable" VEX statements.

### P2: `bhf --version` Is Missing

`target/release/bhf --version` exits with an unexpected-argument error.
...
found. BHF can improve Java depth by synthesizing object construction from
...
reuse across CI. BHF has corpus artifacts, but `auto` reports should make
...
instability, and confusing minimization/replay behavior. BHF should surface
...
```sh
target/release/bhf auto benchmarks/targets \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-2026-07-01/work/benchmarks-all \
  --per-target-time 20 \
  --max-repair-rounds 6 \
  --jobs 2 \
  --no-discovery-cache \
...
target/release/bhf auto benchmarks/cwe/targets \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-2026-07-01/work/cwe-mixed \
  --per-target-time 20 \
  --max-repair-rounds 6 \
  --jobs 2 \
  --no-discovery-cache \
...
target/release/bhf auto benchmarks/targets/c \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-2026-07-01/work/afl-c \
  --per-target-time 24 \
  --max-targets 3 \
  --single-pass \
  --engine builtin,afl++ \
...
target/release/bhf auto benchmarks/targets \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-long-2026-07-01/work/extended-benchmarks \
  --per-target-time 90 \
  --max-repair-rounds 6 \
  --jobs 3 \
  --no-discovery-cache \
...
target/release/bhf auto benchmarks/cwe/targets \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-long-2026-07-01/work/extended-cwe \
  --per-target-time 90 \
  --max-repair-rounds 6 \
  --jobs 3 \
  --no-discovery-cache \
...
target/release/bhf auto benchmarks/targets/c \
  --profile external-tools \
  --work-dir /tmp/bhf-dogfood-long-2026-07-01/work/extended-afl-c \
  --per-target-time 90 \
  --max-targets 3 \
  --jobs 2 \
  --single-pass \
...
```sh
target/release/bhf report \
  --findings /tmp/bhf-dogfood-2026-07-01/work/cwe-mixed/findings \
  --out /tmp/bhf-dogfood-2026-07-01/reports/cwe \
  --run cwe-mixed \
  --sarif \
  --junit \
  --csv

target/release/bhf sbom tests/fixtures/rust_discovery \
  --out /tmp/bhf-dogfood-2026-07-01/sbom/rust-vuln \
  --run-json /tmp/bhf-dogfood-2026-07-01/work/rust-discovery/auto/run.json \
  --vuln-db /tmp/bhf-dogfood-2026-07-01/vulns-rust.json

target/release/bhf sbom tests/fixtures/rust_discovery \
  --out /tmp/bhf-dogfood-2026-07-01/sbom/rust-vuln-fail \
  --run-json /tmp/bhf-dogfood-2026-07-01/work/rust-discovery/auto/run.json \
  --vuln-db /tmp/bhf-dogfood-2026-07-01/vulns-rust.json \
  --fail-on high

target/release/bhf static-scan benchmarks/cwe/targets \
  --out /tmp/bhf-dogfood-2026-07-01/static/cwe \
  --sarif

target/release/bhf static-scan benchmarks/cwe/targets \
  --out /tmp/bhf-dogfood-2026-07-01/static/cwe-fail \
  --sarif \
  --fail-on medium

target/release/bhf binary scan target/release/bhf \
  --out /tmp/bhf-dogfood-2026-07-01/binary/scan-bhf

target/release/bhf binary fuzz /tmp/bhf-dogfood-2026-07-01/binary/crasher \
  --work-dir /tmp/bhf-dogfood-2026-07-01/binary/fuzz-crasher \
  --engine builtin \
  --iterations 3 \
  --seed-input ok \
  --seed-input 'GFZ!' \
