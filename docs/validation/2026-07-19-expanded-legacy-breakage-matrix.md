...
This campaign exercises BHF discovery, build repair, and fuzz execution
...
python3 scripts/validation/legacy-breakage-matrix.py \
  ...
  --materialized-root /tmp/bhf-legacy-campaign-2026-07-19/repos \
  --offline \
  --workspace /tmp/bhf-expanded-full-v49 \
  --jobs 3 \
  --json-out /tmp/bhf-expanded-full-v49-result.json \
  --markdown-out /tmp/bhf-expanded-full-v49-result.md
```
...
clone, deletes the proved artifact, verifies its absence, and runs BHF
...
The local Git clone matters: it lets BHF restore exact deleted content from
