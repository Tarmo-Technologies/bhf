...
This validation covers the optional LLM-assistance and MCP interfaces. BHF's
...
| --- | --- | ---: | ---: |
| `bhf llm test --provider codex` | connected; 15-byte sentinel | 6.73 s | 170,392 KiB |
| `bhf llm test --provider claude` | connected; 15-byte sentinel | 3.47 s | 319,464 KiB |
| Codex host -> BHF MCP preflight | `CODEX_READONLY_ANNOTATION_OK` | 15.46 s | not recorded |
| Claude host -> BHF MCP preflight | `CLAUDE_BHF_MCP_OK` | 15.00 s | 346,204 KiB |
...
standalone BHF daemon footprint and they do not sum the resident memory of
...
bypass. Claude was restricted to the one BHF MCP preflight tool during its
...
- Timed-out CLI provider process groups are terminated instead of leaving model
  descendants behind.
- The combined `bhf`, `bhf-daemon`, and `llm_harness_gen` all-target
...
should run `bhf llm test` with their selected model and credentials before a
production campaign.
