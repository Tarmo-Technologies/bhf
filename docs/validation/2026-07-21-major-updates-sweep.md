...
workspace and distribution checks so both legacy recovery and BHF's wider
...
python3 scripts/validation/legacy-breakage-matrix.py \
  ...
  --workspace /tmp/bhf-expanded-major-updates-20260721 \
  --offline --jobs 3 \
  --json-out /tmp/bhf-expanded-major-updates-20260721-result.json \
  --markdown-out /tmp/bhf-expanded-major-updates-20260721-result.md
```
...
reported GLIBC 2.16 for `bhf` and `bhf-daemon`, and GLIBC 2.14 for the
