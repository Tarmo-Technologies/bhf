...
- Impact: code that builds under its actual project fails under bhf or binds
...
### ZF-17 — Discovery cache is not invalidated by a bhf upgrade
...
- Evidence: generic-local type rewriting depended only on `TypeOwner::Package`.
  ...
  `Bhf_Generic_Instance`, producing invalid Ada.
...
| ZF-17 | `load_rejects_cache_from_a_different_bhf_producer_or_semantics` | incompatible-resume acceptance campaign |
...
`bhf-bug-report.sh` is installed and packaged in the offline release; its
...
7. `bhf bug-report` over the campaigns remains within its byte cap, contains
...
- `cargo test -p bhf --test zero_fuzz_acceptance -- --nocapture`: 6/6
  fresh end-to-end campaigns passed.
- `cargo test -p bhf --test legacy_knr_c`: 2/2 passed, including the
...
- `offline_dist_scripts`: 14/14 passed, including release installation and
  packaging of `bhf-bug-report.sh`.
- `cargo test --workspace --lib --no-fail-fast -q`: all workspace library tests
  passed (`bhf` 1,295/1,295 and `harness_gen` 555/555 among them).
