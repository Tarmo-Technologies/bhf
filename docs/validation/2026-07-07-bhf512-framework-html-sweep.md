...
# BHF-512 Framework Raw-HTML Sweep - 2026-07-07

This memo records validation for the `BHF-512` JavaScript raw-HTML sink expansion:
...
- Sweep root: `/tmp/bhf-sast-bhf512-framework-sweep-2026-07-07`
- Scanner: `target/debug/bhf static-scan <repo> --debug --enable-rule BHF-512`
...
- Initial real `BHF-512` findings: 20

Initial hit repos:

| Repo | Initial `BHF-512` findings | Triage |
|---|---:|---|
...
- Treat Gatsby page-data loader helpers as JSON/data object producers, not raw
  HTML producers.
- Suppress `BHF-512` in `e2e-test` / `e2e-tests` fixture directories.

## Final Focused Rescan

After the fixes, the hit repos were rescanned into `out-fixed/`:

| Repo | Final scan status | Final `BHF-512` findings |
|---|---|---:|
| `primeng` | ok | 0 |
| `storybook` | ok | 2 |
| `gatsby` | ok | 0 |
...
They are defensible `BHF-512` reports for the current source-only rule because the
...
```text
/tmp/bhf-sast-bhf512-framework-sweep-2026-07-07/summary-corrected.tsv
/tmp/bhf-sast-bhf512-framework-sweep-2026-07-07/out-fixed/
```
