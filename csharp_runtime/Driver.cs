...
//
// bhf C# / .NET fork-server driver — the managed analog of
// python_runtime/bhf_driver.py and java_runtime Driver.java. Speaks the SAME
// BHF_FRAMED protocol so the builtin engine drives a warm, long-lived CLR one
...
// which is exactly bhf's BHF_COV_BITS. We mmap the engine's file-backed
// BHF_COV_SHM (MAP_SHARED, 65536 bytes) and point Trace.SharedMem at it, so
...
// rejected.
//
// Without BHF_FRAMED, argv[0] is a single input file to replay once (the
...
internal static unsafe class Driver
{
    ...
    private const int CovBits = 1 << 16; // matches BHF_COV_BITS and the AFL map
    ...
    // never a target bug — mirrors the Python driver's REJECTION_EXC. bhf
    ...
    //     bhf `new`s a fresh receiver and calls one method on it, so a
    ...
    // we feed → BHF-201), NullReferenceException (BHF-206), DivideByZero/Overflow
    // (BHF-205), OutOfMemory (BHF-209), and any other/custom throwable (BHF-210).
    ...
    // synthesized-receiver artifact. bhf `new`s a fresh receiver (or a default
    ...
    /// (System.*, Microsoft.*, Internal.*), reflection glue, and the bhf driver
    ...
    private static int TargetFrameDepth(Exception exc)
    {
        ...
        foreach (var frame in frames)
        {
            ...
            if (full.Length == 0
                ...
                || full.StartsWith("Bhf", StringComparison.Ordinal))
            {
                continue;
            }
            depth++;
    ...
    {
        // Marker mirrors the JVM driver's `== bhf JVM finding:`; the engine's
        // `parse_csharp_finding` maps the exception type -> BHF rule -> CWE.
        var msg = (exc.Message ?? "").Replace('\n', ' ').Replace('\r', ' ');
        Console.Error.WriteLine($"== bhf csharp finding: {exc.GetType().FullName}: {msg}");
    ...
    // Map BHF_COV_SHM into Trace.SharedMem. On any failure fall back to a
    // private anonymous page so the instrumented target's writes never null-deref.
    private static void SetupCoverage()
    {
        var path = Environment.GetEnvironmentVariable("BHF_COV_SHM");
    ...
    {
        _expected = (Environment.GetEnvironmentVariable("BHF_EXPECTED_EXCEPTIONS") ?? "")
            .Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
        for (int i = 0; i < _expected.Length; i++)
        {
            _expected[i] = _expected[i].Trim();
        }
        _targetNamespace = (Environment.GetEnvironmentVariable("BHF_CS_NAMESPACE") ?? "").Trim();
        if (int.TryParse(
                (Environment.GetEnvironmentVariable("BHF_NPE_MIN_DEPTH") ?? "").Trim(),
        ...
        if (Environment.GetEnvironmentVariable("BHF_FRAMED") != null)
        {
            FramedLoop();
            return 0;
        }
