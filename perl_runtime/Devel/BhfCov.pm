...
package Devel::BhfCov;
use strict; use warnings;
our $COV_BITS = 1 << 16;
our $MASK = $COV_BITS - 1;
my $map = "\0" x $COV_BITS;
my $prev = 0;
my $prefix = $ENV{BHF_TRACE_PREFIX} // '';
# Covered "file:line" set for negative fuzz-confirmation (see bhf_cov.py).
my %covered;
my $covered_dumped = 0;
sub _fnv {
    my ($s, $line) = @_;
...
sub dump_covered_lines {
    my $path = $ENV{BHF_COVERED_LINES} or return;
...
sub flush {
    my $path = $ENV{BHF_COV_SHM} or return;
    my $fh;
