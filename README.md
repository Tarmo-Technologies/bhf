...
in a distribution), and `bhf auto --help` prints the same command at the end:
...
ceiling). BHF removes transient Rust Cargo caches automatically. To compact an
older or interrupted work directory without deleting findings, corpora, reports,
checkpoints, or replay binaries, run `bhf clean bhf_work --compact`.
...
`bhf auto` checkpoints every completed target atomically. After a process
...
#    already-completed target is durable in bhf_work/harnesses/<id>/.
...
BHF that predates the build-context fingerprint is treated conservatively as
...
targets rather than trusting stale results, and a compatible BHF build is
required.
...
co-locate `bhf`, `libbhf_runtrace_shim.so`, and
`libbhf_cc_intercept.so`. For a normal **Windows CLI installation**, install
only `bhf`; add `bhf-daemon` only for IDE/MCP use. The two shims are
...
The lightweight GitHub Release installer installs the BHF executable only:
...
`BHF_MAX_CORPUS_BYTES`, `BHF_MAX_CORPUS_ENTRIES`,
`BHF_MAX_FILE_BYTES`, and `BHF_MAX_SOURCE_FILE_BYTES`. Captured
subprocess, harness, runtrace, and external-analyzer output budgets are also
memory-scaled and have `BHF_MAX_*_BYTES` overrides; see the
...
BHF can use an authenticated Codex or Claude CLI, the OpenAI Responses API,
the Anthropic Messages API, or a local OpenAI-compatible server. For interactive
work, the recommended mode is `bhf-daemon --mcp`: the Codex/Claude session
you are already in performs the reasoning and calls deterministic BHF tools,
so BHF needs no API token. Direct CLI-provider calls start a separate,
...
- **No harness to write.** `bhf auto` discovers fuzzable subprograms, generates typed
...
- **Fuzzing** — `bhf auto` across all sixteen lanes, with build recovery, typed harness/stub
...
| `bhf auto <src> --force` | Second phase over what the normal run could not fuzz — fabricated parameters and stubs across C/C++/Ada, Go, and C# (Low-confidence findings) |
...
Every subcommand is documented in [docs/site/cli.md](docs/site/cli.md); `bhf --help` lists
them all.
...
- [`bhf auto`](docs/site/auto.md) — end-to-end, scaling to large trees, force-fuzz, static integration.
