...
/**
 * Joins bhf's file-backed edge-coverage map and records AFL-style edge hits
 * into it. The bhf engine names the map via the {@code BHF_COV_SHM}
 * environment variable, which is a plain FILE PATH the engine/driver mmap with
 * {@code MAP_SHARED} (see {@code c_runtime/bhf_driver.c}). Because it is a
 ...
 *
 * <p>Layout MUST match {@code BHF_COV_BITS} (1 &lt;&lt; 16) in the C driver.
 */
public final class Coverage {
    /** Must equal BHF_COV_BITS in c_runtime/bhf_driver.c. */
    private static final int COV_BITS = 1 << 16;
    private static final int COV_MASK = COV_BITS - 1;

    /** The shared edge map, or null when BHF_COV_SHM is unset (coverage off). */
    ...
    private static MappedByteBuffer open() {
        try {
            String path = System.getenv("BHF_COV_SHM");
    ...
    /** True when a coverage map is attached (BHF_COV_SHM was set + mappable). */
