...
/**
 * Native bhf JVM fork-server driver — the Java analog of the persistent
 * {@code main} in {@code c_runtime/bhf_driver.c}. It speaks the SAME
 * {@code BHF_FRAMED} protocol so the bhf builtin engine drives a warm,
 ...
 *
 * <p>Without {@code BHF_FRAMED}, {@code args[1]} is a single input file to
...
public final class Driver {
     ...
     * from BHF_EXPECTED_EXCEPTIONS. These are expected rejections of bad input
     ...
     * Tunable via {@code BHF_NPE_MIN_DEPTH} (default 3).
     */
    private static final int NPE_MIN_DEPTH = loadNpeMinDepth();

    /**
     * True when the target has no attacker byte/char channel (BHF_SCALAR_ONLY_TARGET),
    ...
    private static final boolean SCALAR_ONLY_TARGET =
            "1".equals(System.getenv("BHF_SCALAR_ONLY_TARGET"));

    private Driver() {}

    private static java.util.Set<String> loadExpectedExceptions() {
        java.util.Set<String> set = new java.util.HashSet<>();
        String raw = System.getenv("BHF_EXPECTED_EXCEPTIONS");
        if (raw != null) {
    ...
    private static int loadNpeMinDepth() {
        String raw = System.getenv("BHF_NPE_MIN_DEPTH");
        if (raw != null) {
            try {
    ...
    public static void main(String[] args) throws Exception {
        String harnessClass = args.length > 0 ? args[0] : System.getenv("BHF_HARNESS_CLASS");
        if (harnessClass == null || harnessClass.isEmpty()) {
            System.err.println("bhf Driver: no harness class (argv[0] / BHF_HARNESS_CLASS)");
            System.exit(2);
        }
        Method runOne = Class.forName(harnessClass).getMethod("bhfRunOne", byte[].class);

        if (System.getenv("BHF_FRAMED") != null) {
     ...
     * JDK, reflection glue, and the bhf driver/harness/agent — a proxy for how
    ...
    private static int targetFrameDepth(Throwable t) {
        ...
        for (StackTraceElement frame : t.getStackTrace()) {
            ...
            if (cls.startsWith("java.")
                    ...
                    || cls.startsWith("com.bhf.")
                    || cls.startsWith("bhfgen.")) {
                continue;
            }
            depth++;
        }
    ...
    private static void reportFinding(Throwable t) {
        System.err.println("== bhf JVM finding: " + t.getClass().getName()
