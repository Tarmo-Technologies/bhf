...
/**
 * bhf's JVM RedQueen/cmplog operand capture — the Java side of the engine's
 * comparison-solving mutator. When the engine is about to mutate a corpus entry it
 * ARMS the {@code BHF_CMP_SHM} ring; the instrumented JVM pushes the operands
 ...
 *
 * <p>The ring layout MUST match {@code c_runtime/bhf_driver.c} /
 * {@code CmpShmReader}: {@code [u32 armed][u32 count]} then {@code BHF_CMP_CAP}
...
public final class Cmplog {
    ...
    private static MappedByteBuffer open() {
        try {
            String path = System.getenv("BHF_CMP_SHM");
