...
[`docs/site/`](docs/site/) and live `bhf <command> --help` output for current
behavior. Current optional LLM/MCP workflows are documented in
[`docs/site/llm.md`](docs/site/llm.md).

BHF currently fuzzes sixteen languages: Ada, C, C++, Rust, Java, Python,
...
- `bhf auto` is the complete sixteen-lane entry point. The narrower manual
...
- The C/C++ lane includes direct-call harnesses for common byte-oriented APIs,
  ...
  audit, and optional AFL++ targets through `bhf build --c-engine afl++`
...
- The Rust and Java lanes are native fuzzing lanes (not SBOM-only). Both are
  ...
  bhf's own ASM bytecode JVM coverage agent (not Jazzer) over
...
- The Python, Perl, and Go lanes (M3.1–M3.3) are native fuzzing lanes driven by
  bhf's own built-in engine over the framed fork-server protocol — no
  ...
  `sys.settrace` edge counts and Perl runs under `perl -d:BhfCov` with
  `DB::DB` per-statement edges, both into the shared `BHF_COV_SHM` map; their
  ...
  execution maps to one edge set) into the shared `BHF_COV_SHM` edge map —
...
1. `bhf scan` finds the target package and its public subprograms using a from-scratch lexer plus tree-sitter-ada.
2. `bhf instrument` rewrites a copy of the source so every executable statement leaves a breadcrumb and every `when ... =>` handler logs a structured event before running its original body.
3. `bhf generate-harness` emits a standalone Ada main that decodes stdin bytes into the target's parameter types and calls it.
4. `bhf build` produces a synthetic `.gpr`, calls user-installed FSF GNAT/GPRbuild as a subprocess, and parses diagnostics.
5. `bhf fuzz` runs the built harness using a built-from-scratch mutational engine, retains testcases by exception signature, and writes JSON findings.
6. `bhf replay` and `bhf report` reproduce and explain a finding.
...
|---|---|---|---|---|---|
| From-scratch scanner (bhf) | Permissive Ada lexer + structural parser | Apache-2.0 (ours) | **Core** | Grammar drift | Author in-house. Only path to true permissive guarantee. |
...
| ASM (org.ow2.asm) | JVM bytecode instrumentation for bhf's own coverage agent (native Java fuzzing, M2.1) | BSD-3-Clause | **Core (agent runtime, NOT linked)** | Low (permissive, strict-permissive clean) | Bundled only into the standalone `bhf-jvm-agent.jar` (`-javaagent` for the user's JVM — agent/subprocess posture, never linked into the Rust binary). Built from `java_runtime/` by `build-agent.sh`; ASM fetched to a cache or pre-staged for air-gapped builds. |
...
The native Rust lane (Phase 1) emits a `fuzz/` cargo-fuzz crate whose generated harness depends on `libfuzzer-sys` and `arbitrary` (both **MIT OR Apache-2.0**). These live in the *generated* `fuzz/Cargo.toml`, not the bhf workspace — they are linked only into the user-compiled harness, never into the bhf binary, so they need no `deny.toml` workspace entry (same posture as the `adafuzz-*` runtime copies above). The libFuzzer runtime cargo-fuzz links is **Apache-2.0-with-LLVM-exception**, also permissive. Generated `fuzz_targets/*.rs` carry the Apache-2.0 SPDX header.
...
Out-of-process JSON-RPC; never linked. Reference implementation lives in a separate repo `bhf-lal-plugin` under GPLv3 (matching Libadalang). The `strict-permissive` build never invokes it.
...
- Original sources are never modified; instrumented copies live in `bhf_work/src_instrumented/`.
...
```
Header (16B)  : magic "BHFZC", version, flags
...
```json
{
  ...
  "replay": { "command": "bhf replay --finding F-2026-04-30-0001" },
...
- **Per-tenant retraining**: enterprise users retrain locally via `bhf model train --labels labels.json --out model.bin` without sharing data.
- **Auditable**: every learned score ships with `model_id` and feature vector for the finding.

### 15.3 Output formats

JSON (always), Markdown (always), **SARIF 2.1.0** when `--sarif`, JUnit XML when `--junit`, seed file, optional standalone Ada reproducer (`repro.adb`). Findings include `result.kind = "fail"`, `properties.bhfExceptionSignature`, `locations[]` for handler site + last breadcrumb + explicit raise, `relatedLocations[]` for stubs/fake CORBA used. SARIF schema validation is a CI gate.
...
- **`bhf license-audit`** — In: project + manifests. Out: pass/fail. `bhf license-audit --profile strict-permissive`. Fail: exit 2 on any disallowed SPDX.
- **`bhf scan <path>`** — In: Ada/C/C++/Rust/Java/Python/Perl/Go file or directory tree. Out:
  `bhf_work/scan_index.json` plus JSON summary on stdout. Fail: exit 1
  when no supported source file was scanned.
- **`bhf list-targets <path>`** — In: Ada/C/C++/Rust/Java/Python/Perl/Go file or directory tree.
  Out: ranked targets, stable harness ids, and JSON/table output. `--top 20
  --format json`.
- **`bhf instrument --target <id>`** — In: target id. Out: rewritten files in `src_instrumented/`, `breadcrumbs.json`. Fail: prior instrumentation intact; exit 1 on any rewrite failure.
- **`bhf generate-harness <source> --target <name>`** — In: source file,
  target name, harness kind. Out: Ada `main.adb` + `.gpr`, or C/C++ `main.c`
  / `main.cpp` + `Makefile`. `--kind direct|sequence|servant_direct`.
- **`bhf generate-stubs --target <id>`** — In: target id. Out: `generated_stubs/...`.
- **`bhf fake-corba --idl <file>`** — In: IDL file (or scan-detected CORBA artifacts). Out: `fake_corba/...`.
- **`bhf build <work-dir> --harness <id>`** — In: harness id. Out: built
...
- **`bhf fuzz <work-dir> --harness <id>`** — In: harness id, engine,
  time/iter budget. Out: corpus, findings. Current engines:
  `--engine builtin|afl++`.
- **`bhf replay --finding <id>`** — In: finding id (or testcase file). Out: stdout exception classification, identical signature. Fail: exit 3 on signature mismatch.
- **`bhf minimize --finding <id>`** — In: finding id. Out: `min_testcase.bin`, updated finding record.
- **`bhf report`** — In: optional run id. Out: JSON+MD+SARIF+JUnit. `--run last`.
- **`bhf model train`** — In: labeled findings. Out: per-tenant learned-confidence model. `--labels labels.json --out model.bin`.
- **`bhf daemon`** (M18) — In: `--listen 127.0.0.1:port`. Out: JSON-RPC service.
- **`bhf clean`** — `--all`/`--build`/`--corpus`.
...
- **Deliverables**: `crates/target_rank` with §5.2 scoring, `bhf list-targets`.
...
- **Acceptance**: `bhf generate-harness && bhf build && bhf fuzz` runs.
...
- **Deliverables**: corpus retention by signature; finding record; `bhf replay`.
...
- **Acceptance**: SARIF validates; Markdown renders; `repro.adb` builds with FSF GNAT; `bhf model train` produces a model file used by subsequent `report` runs.
...
- **Goal**: fuzz Windows-native (`_WIN32`) C/C++ targets as real PEs, and build
  bhf itself to run natively on Windows (in addition to Linux).
- **Deliverables**:
  - Cross-fuzz path (Linux host): the generated fork-server driver
    (`c_runtime/bhf_driver.c` + `direct_harness.{c,cpp}.tera`) is
  ...
  - Native build: `cargo build --target x86_64-pc-windows-gnu -p bhf` →
    `bhf.exe`; crash classification recognizes the driver's `0x39` fault
    sentinel off-Unix.
- **Acceptance**: `bhf auto` on a `_WIN32`-guarded C and C++ fixture
  cross-builds a PE, fuzzes it under wine with real edge coverage, and reports the
  planted crash (`crates/cli/tests/windows_cross_fixture.rs`); `bhf.exe` builds
...
- **Follow-ups**: coverage-guided feedback + persistent fork-server on the
  *native-Windows* build (Win32 SHM readers + a Windows handshake timeout);
  general `bhf auto` multi-file-library build-context (project include-dir
...
- **Goal**: shippable.
- **Deliverables**: `cargo dist` packaging; `bhf license-audit` exit codes; signed releases; docs site; full CI matrix (GNAT 11..14 × dialects 95/2005/2012/2022 × profiles).
...
9. Additional executable-oracle classes (reclassified from §25 #343) and deeper
   ...
   - **Advanced (2026-06-19).** Added BHF-417 insecure-temporary-file oracle
...
SAST incumbents) found bhf already ahead of the field on its core
...
2. **Structure-aware input layer + automatic dictionary generation** from
   ...
   PNG/RIFF/ZIP/network-framing structure of the legacy binary parsers bhf
...
scaffolds) is the least urgent: bhf's own thesis correctly says
...
A native-Windows `bhf auto` campaign (run on a real Win10 host against a
...
3. **Ranking attempts non-public / internal symbols.** `auto` ranks and tries
   ...
   **SHIPPED 2026-06-28**: `name_has_helper_marker` (target_rank/c_rank) now also
   ...
   already soft-demoted (not skipped, since bhf reaches a static target via
...
6. **`double` / `float` parameters are reported "unsupported" in the C++ lifecycle
   ...
   **SHIPPED 2026-06-28**: by-value `float`/`double`/`long double` decode through
   `legacy_select_c_decoder` (`bhf_i32`/`bhf_i64` reinterpreted), so the registry-less
...
Environment note (not a bhf defect): one pugixml target failed with
...
4. **C++ abstract-receiver substitution — #456, deferred phases.** A method on an
   ...
   `<Subclass> _bhf_receiver; _bhf_receiver.method(..)` (the virtual call dispatches
   ...
   arguments (`<Subclass> _bhf_receiver(args); _bhf_receiver.method(..)`). (b) when no
   ...
   for the abstract base (`Base *make()` -> null-guarded `_bhf_receiver->method(..)`).
...
9. **Inline function-pointer C parameters — #466, parser fix (deferred).** The C
   ...
   `bhf_i32` harness (the part-3 safety net). The deeper fix is upstream: keep an
   ...
   — align them), then synthesise a `_bhf_cb_<name>` typedef + trampoline in
...
10. **In-crate Rust harness build mode — #463, deferred.** The external-crate
    ...
    target crate, injects the harness as a `#[doc(hidden)] pub mod __bhf_harness;`
...
11. **Configurable decoder caps via CLI flags — #464 (C) / #465 (C++). DELIVERED.**
    ...
    flattened (`DecoderLimitArgs`) into both `bhf auto` and
    `bhf generate-harness`. Defaults reproduce the historical hardcoded caps
...
**zero bhf panics** across ~6000 candidates and shipped the following fixes.
...
1. **Reporting: CWE on EVERY finding + group findings to one row per root-cause
   ...
   *issue* (cluster key, else finding id) with a member count, the `fix_file`/
   ...
   (idiomatic) with a cluster `bhfIssueKey` fingerprint, CWE properties, and
...
7. **Ada generic-package constructors** named through the synthesised instance,
   ...
   `Bhf_Generic_Instance.Create_Null`; json-ada). Area: `harness_gen/generate`.
...
15. **Never instantiate an abstract Java class.** bhf emitted
...
substring C rules, e.g. BHF-401 strcpy, fire on K&R source → CWE findings).
...
**Goal:** running bhf on every pull request must take one `uses:` line and zero
configuration. The design bar is *extreme usability*, not just capability.

- **`bhf ci` mode** — diff-scoped. Given a base ref (`--diff <ref>`, defaulting
...
- **Action** — `Tarmo-Technologies/bhf-action` (composite/Docker): one `uses:`
...
**Shipped 2026-07-12.** `bhf ci --changed-since <ref>` (+ `--changed-paths-from`,
...
`.github/actions/bhf-pr/` (base-ref resolve, release-binary install with source
...
- **`bhf auto --differential <a>:<b>`** — two targets (two libraries, or old vs
...
**Shipped 2026-07-12 (two-compiler variant).** The standalone `bhf differential`
subcommand (arbitrary two-harness + metamorphic, stdout compare) already existed; the
new piece is auto-integration: `bhf auto --differential clang:gcc` rebuilds each
C/C++ harness under both compilers (a portable `make diff` target) and replays the
fuzz corpus through both, flagging **exit/crash divergence** as a BHF-301 finding in
the normal report (bhf harnesses suppress target stdout, so exit status is the
...
- **JavaScript / Node.js** — **Shipped 2026-07-12.** Node interpreted lane (like
  ...
  taken-bit)` — into the shared `BHF_COV_SHM` map, so the engine gets genuine
  branch feedback. An uncaught non-rejection exception hard-halts (exit 86); stack
  overflow → BHF-207, resource `RangeError`/OOM → BHF-209, else BHF-210. `TypeError`
  ...
  a taint-confirmed **command-injection detector** (BHF-431/CWE-78). See
...
- **C#** — **Shipped 2026-07-12.** .NET lane over the shared fork-server engine: a
  ...
  into bhf), and the driver `mmap`s `BHF_COV_SHM` into
  `SharpFuzz.Common.Trace.SharedMem` (the AFL 64 KB map == `BHF_COV_BITS`) to
  bridge real edge coverage. A warm CLR is kept alive over the framed protocol; an
  uncaught non-rejection exception hard-halts (exit 86) and maps to a BHF rule + CWE.
