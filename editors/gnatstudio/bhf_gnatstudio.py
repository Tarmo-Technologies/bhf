...
"""GNAT Studio plugin for BHF findings."""
...
CATEGORY = "BHF"
PREF_DAEMON = "BHF/daemon-path"
PREF_CLI = "BHF/cli-path"
...
def initialize() -> None:
    create_preferences()
    refresh = GPS.Action("BHF refresh findings")
    if not refresh.exists():
        refresh.create(
            on_activate=refresh_findings,
            category=CATEGORY,
            description="Refresh BHF findings",
        )
        refresh.menu("/Tools/BHF/Refresh Findings")
...
def create_preferences() -> None:
    create_preference(
        ...
        "Daemon executable",
        "string",
        "Path to the BHF daemon executable.",
        "bhf-daemon",
    )
    create_preference(
        PREF_CLI,
        "CLI executable",
        "string",
        "Path to the BHF CLI executable.",
        "bhf",
    )
    create_preference(
        PREF_FINDINGS,
...
def refresh_findings() -> None:
    ...
    try:
        ...
        console_write(f"BHF refresh failed: {error}\n")
        return
    ...
    console_write(f"BHF loaded {len(findings)} finding(s).\n")


def register_finding_actions(finding, config: BhfConfig) -> None:
    finding_id = str(finding.get("id", "unknown-finding"))
    menu_base = f"/Tools/BHF/Findings/{safe_label(finding_id)}"
...
def run_process(args: list[str], config: BhfConfig, task_name: str) -> None:
    GPS.Process(
        args,
        task_manager=True,
        show_command=True,
        directory=config.workspace_root,
        task_manager_name=f"BHF {task_name}",
    )


def open_reproducer(finding, config: BhfConfig) -> None:
    path = resolve_reproducer_path(finding, config)
    if not path:
        console_write(f"BHF finding {finding.get('id')} has no repro.adb.\n")
        return
...
def current_config() -> BhfConfig:
    return BhfConfig(
        daemon_path=string_preference(PREF_DAEMON, "bhf-daemon"),
        cli_path=string_preference(PREF_CLI, "bhf"),
