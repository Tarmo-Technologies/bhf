    ...
    lenses.push(
      {
        ...
        command: "bhf.replayFinding",
        findingId: finding.id,
        range,
      },
      {
        title: "Minimize",
        command: "bhf.minimizeFinding",
        findingId: finding.id,
        range,
      },
    );
...
export function findingById(
  findings: BhfFinding[],
  findingId: string,
): BhfFinding | undefined {
  return findings.find((finding) => finding.id === findingId);
}

export function primaryLocation(finding: BhfFinding): FindingLocation | undefined {
...
function messageForFinding(finding: BhfFinding): string {
  const parts = [
    `BHF ${finding.classification ?? "finding"}`,
    finding.id,
  ];
