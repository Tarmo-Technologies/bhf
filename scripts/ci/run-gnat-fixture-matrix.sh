...
exec /usr/bin/gprbuild --config="${BHF_M19_GPR_CONFIG:?}" "$@"
EOF
  chmod +x "$wrapper_dir/gprbuild"

  export BHF_M19_GPR_CONFIG="$config_path"
...
main() {
  require_env BHF_GNAT_VERSION
  require_env BHF_ADA_DIALECT
  require_env BHF_PROFILE

  validate_value BHF_GNAT_VERSION "$BHF_GNAT_VERSION" 11 12 13 14
  validate_value BHF_ADA_DIALECT "$BHF_ADA_DIALECT" ada95 ada2005 ada2012 ada2022
  validate_value BHF_PROFILE "$BHF_PROFILE" strict-permissive external-tools

  local gnatmake="gnatmake-${BHF_GNAT_VERSION}"
  ...
  local config_path="${temp_root}/bhf-gnat-${BHF_GNAT_VERSION}.cgpr"
  local wrapper_dir="${temp_root}/bhf-gnat-${BHF_GNAT_VERSION}-bin"
  ...
  gprconfig --batch \
    "--config=Ada,,,,$gnatmake" \
    "--config=C,,,,gcc-${BHF_GNAT_VERSION}" \
    -o "$config_path"
  link_versioned_gnat_tools "$wrapper_dir" "$BHF_GNAT_VERSION"
  if [ "$BHF_ADA_DIALECT" = "ada2022" ] && ! gnatmake_supports_ada2022 "$gnatmake"; then
    install_ada2022_compat_wrapper "$wrapper_dir" "$config_path" "$BHF_GNAT_VERSION"
  fi
  write_gprbuild_wrapper "$wrapper_dir" "$config_path"

  printf 'GNAT matrix cell: GNAT %s / %s / %s\n' \
    "$BHF_GNAT_VERSION" "$BHF_ADA_DIALECT" "$BHF_PROFILE"
  "$gnatmake" --version | sed -n '1p'
  printf 'fixture pipelines: %s\n' "$FIXTURE_PIPELINES"

  BHF_M19_FULL_CI=1 \
    cargo test -p bhf --test m19_full_ci_matrix \
