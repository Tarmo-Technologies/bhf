...
DEFAULT_WORKSPACE = Path(
    os.environ.get(
        "BHF_LEGACY_BREAKAGE_WORKSPACE", "/tmp/bhf-legacy-breakage"
    )
)
SCHEMA_VERSION = "bhf.legacy_breakage_matrix.v2"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "BHF to build and fuzz the surviving real target."
        )
    )
...
def bhf_binary() -> Path:
    ...
    binary = Path(override) if override else REPO_ROOT / "target/release/bhf"
    if not binary.is_file():
        raise RuntimeError(
            f"BHF binary not found at {binary}; run cargo build --release -p bhf"
        )
...
def execute_scenario_run(
    ...
    *,
    ...
    try:
        ...
        # disposable scenario export instead of leaking them into the BHF
        ...
        if not run_path.is_file():
            raise RuntimeError("BHF did not write auto/run.json")
...
def run_scenario(
    ...
    try:
        ...
        with lock:
            ...
            log = workspace / "scenarios" / scenario["id"] / "bhf.log"
