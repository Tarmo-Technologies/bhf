...
Usage:
    finding-parity.py capture <tree> <digest.json> [--bhf PATH] [--jobs N]
...
def capture(tree: pathlib.Path, out: pathlib.Path, bhf: str, jobs: int) -> int:
...
def main() -> int:
    ...
    cap.add_argument("--bhf", default="./target/release/bhf")
    ...
    if args.mode == "capture":
        return capture(args.tree, args.out, args.bhf, args.jobs)
