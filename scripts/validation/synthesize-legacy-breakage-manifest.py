...
def bhf_binary() -> Path:
    binary = REPO_ROOT / "target/release/bhf"
    if not binary.is_file():
        raise RuntimeError("target/release/bhf is missing; build it first")
    return binary
...
def discover_targets(repo: Path, top: int) -> tuple[list[dict[str, Any]], str | None]:
    with tempfile.TemporaryDirectory(prefix="bhf-auto-discovery-") as work_dir:
        command = [
            str(bhf_binary()),
            "auto",
            str(repo),
