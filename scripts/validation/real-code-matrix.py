...
DEFAULT_WORKSPACE = Path(os.environ.get("BHF_REAL_CODE_WORKSPACE", "/tmp/bhf-real-code-validation"))


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run BHF against pinned real codebases.")
    ...
    return {
        ...
        "evidence_claim": {
            ...
            "limitations": [
                ...
                "Toolchain gaps are reported separately from BHF defects.",
            ],
        },
    }
...
def bhf_cmd(args: list[str]) -> list[str]:
    override = os.environ.get("BHF_BIN")
    if override:
        return [override, *args]
    return ["cargo", "run", "--quiet", "-p", "bhf", "--", *args]
...
def check_list_targets(repo: dict[str, Any], repo_dir: Path, check: dict[str, Any]) -> dict[str, Any]:
    ...
    proc = run(bhf_cmd(["list-targets", str(target_path), "--format", "json", "--top", top]))
...
def check_scan(repo: dict[str, Any], repo_dir: Path, check: dict[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    target_path = repo_dir / check.get("path", ".")
    run(bhf_cmd(["scan", str(target_path), "--work-dir", str(work_dir)]))
...
def check_generate_harness_build(repo: dict[str, Any], repo_dir: Path, check: dict[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    run(bhf_cmd(["build", str(work_dir), "--harness", check["harness_id"]]))
...
def check_instrument(repo: dict[str, Any], repo_dir: Path, check: dict[str, Any], workspace: Path) -> dict[str, Any]:
    ...
    run(bhf_cmd(["instrument", str(repo_dir / check["source"]), "--output", str(out_dir)]))
...
def check_auto_target_entry(repo: dict[str, Any], repo_dir: Path, check: dict[str, Any], workspace: Path) -> dict[str, Any]:
    # #104: run `bhf auto` end-to-end on a real target and PROVE it entered the
    ...
    if check.get("project"):
        cmd.extend(["--project", str(repo_dir / check["project"])])
    run(bhf_cmd(cmd), allow_failure=True)
...
def scenario_auto_missing_file(repo: dict[str, Any], repo_dir: Path, scenario: dict[str, Any], workspace: Path, default_time: int) -> dict[str, Any]:
    ...
    for rel in scenario.get("remove_files", []):
        ...
        path.unlink()
    work_dir = scenario_dir / "bhf_work"
    cmd = [
        "auto",
        str(scenario_dir),
        "--work-dir",
