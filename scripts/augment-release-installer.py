...
"""Add BHF's EL7 prerequisite guidance to dist's generated CLI installer."""
...
BEGIN_MARKER = "# BHF_RHEL7_GUIDANCE_BEGIN"

GUIDANCE = r'''# BHF_RHEL7_GUIDANCE_BEGIN
...
bhf_release_installer_os_value() {
    _bhf_key="$1"
    awk -F= -v key="$_bhf_key" '$1 == key { sub(/^[^=]*=/, ""); print; exit }' \
        "$_bhf_os_release" | sed 's/^"//; s/"$//'
}

bhf_release_installer_rhel7_guidance() {
    _bhf_os_release="${BHF_OS_RELEASE_FILE:-/etc/os-release}"
    [ -r "$_bhf_os_release" ] || return 0

    _bhf_os_id="$(bhf_release_installer_os_value ID)"
    _bhf_os_like="$(bhf_release_installer_os_value ID_LIKE)"
    _bhf_os_version="$(bhf_release_installer_os_value VERSION_ID)"
    case " $_bhf_os_id $_bhf_os_like " in
        *" rhel "*|*" centos "*|*" redhat "*) ;;
        *) return 0 ;;
    esac
    case "$_bhf_os_version" in
        7|7.*) ;;
        *) return 0 ;;
    esac
    ...
    say "  This bhf-installer.sh asset installs the CLI only. It does not enable"
    say "  repositories, install compiler packages, or install Linux preload shims."
    if [ "$_bhf_os_id" = "rhel" ]; then
    ...
    if [ -x /opt/rh/llvm-toolset-7.0/root/usr/bin/clang ] \
        ...
        say "  LLVM Toolset 7 detected; BHF activates it automatically."
    else
    ...
    for _bhf_base_url in $ARTIFACT_DOWNLOAD_URLS; do
        break
    done
    say "  Full bhf auto runtime coverage also needs the separate runtrace shim:"
    say "    curl --proto '=https' --tlsv1.2 -LsSf \"$_bhf_base_url/bhf_runtrace_shim-installer.sh\" | sh"
    say "  Recommended for C/C++ build-command recovery:"
    say "    curl --proto '=https' --tlsv1.2 -LsSf \"$_bhf_base_url/bhf_cc_intercept-installer.sh\" | sh"
    say ""
}

bhf_release_installer_rhel7_guidance
# BHF_RHEL7_GUIDANCE_END
'''
