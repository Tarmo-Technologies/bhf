...
Install or update a binary-only BHF distribution.
...
neither whiptail nor dialog is installed (or BHF_INSTALL_NO_GUI is set) it
...
resolve_bundle_root() {
  local script_dir="$1"
  if [[ -f "$script_dir/tool/bhf" ]]; then
    printf '%s' "$script_dir"
    return 0
  fi
  if [[ -f "$PWD/tool/bhf" ]]; then
    printf '%s' "$PWD"
    return 0
  fi
  die "could not find distribution root; expected tool/bhf beside install.sh or in the current directory"
}
...
if [[ "$NON_INTERACTIVE" -eq 0 ]]; then
  ...
  FUZZERS="$(ask_checklist "Fuzzers to install" "$FUZZERS" \
    "builtin|BHF built-in coverage-guided fork-server engine|on" \
...
printf 'BHF distribution install\n'
printf '  bundle:    %s\n' "$BUNDLE_ROOT"
printf '  prefix:    %s\n' "$PREFIX"
printf '  languages: %s\n' "${LANGUAGES:-none}"
printf '  targets:   %s\n' "${TARGETS:-none}"
...
case "$PACKAGE_MANAGER" in
  apt-get)
    ...
    # Warn and continue when the mirror cannot be reached; the bundled BHF
...
if [[ "$DRY_RUN" -eq 0 ]] \
  ...
  if command -v clang >/dev/null 2>&1 \
    && clang_supports_bhf_coverage "$(command -v clang)"; then
    COVERAGE_CLANG_OK=1
  else
    for SCL_ROOT in /opt/rh/llvm-toolset-7.0/root/usr /opt/rh/llvm-toolset-7/root/usr; do
      if [[ -x "$SCL_ROOT/bin/clang" ]] \
        && clang_supports_bhf_coverage "$SCL_ROOT/bin/clang" "$SCL_ROOT/lib64"; then
        COVERAGE_CLANG_OK=1
        break
      fi
    done
...
if [[ "$NO_SYMLINK" -eq 0 ]]; then
  ...
  run_with_optional_prefix "$SUDO_BIN_WORD" ln -sfn "$PREFIX/bhf" "$BIN_DIR/bhf"
  if [[ -f "$PREFIX/bhf-daemon" ]]; then
    run_with_optional_prefix "$SUDO_BIN_WORD" ln -sfn "$PREFIX/bhf-daemon" "$BIN_DIR/bhf-daemon"
  fi
