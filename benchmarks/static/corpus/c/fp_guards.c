...
/* Campaign 2026-07-03 regression guards: real-tree false-positive patterns that
   the BHF-401 / BHF-405 / BHF-424 heuristics used to fire on. Lines WITHOUT an
...
/* BHF-401: the SAFE bounded API `fgets` contains the substring `gets(`, and
...
/* BHF-401 positive: a real strcpy of a runtime-length (variable) source. */
void copy_user(char *dst, const char *src) {
    strcpy(dst, src);                /* EXPECT BHF-401 */
}

/* BHF-405: a string-LITERAL path is author-chosen, never attacker-controlled;
...
/* BHF-405 positive: a non-literal path argument (variable) reaches open(). */
int open_user_file(const char *name) {
    return open(name, 0);            /* EXPECT BHF-405 */
}

/* BHF-424: a for-loop induction variable is defined by its init clause, and an
...
/* BHF-422 (weak crypto) and BHF-428 (weak PRNG in a secret context). A random()
...
void make_keys(unsigned char *out, unsigned long n) {
    MD5(out, n, out);                /* EXPECT BHF-422 */
    unsigned int nonce = rand();     /* EXPECT BHF-428 */
