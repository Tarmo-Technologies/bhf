...
// Uncontrolled allocation size (BHF-436, CWE-789): an attacker-controlled value
...
char *grow(const char *user_input) {
    ...
    return (char *)malloc(n);            /* EXPECT BHF-436 */
}
...
char *fit_copy(const char *user_input) {
    /* Allocate-to-fit: sized to the existing string, not amplified — the SAFE
       idiom, so BHF-436 must NOT fire even though user_input is tainted. */
