...
void unsafe_qt_process(QProcess *process, const QString &userCommand, const QString &userProgram) {
    process->startCommand(userCommand);                         // EXPECT BHF-304
    process->start("sh", QStringList() << "-c" << userCommand); // EXPECT BHF-404
    QProcess::startDetached(userProgram, QStringList());        // EXPECT BHF-304
}
...
void unsafe_qt_sql(QSqlDatabase db, const QString &userName) {
    QSqlQuery query(db);
    query.exec("SELECT * FROM users WHERE name='" + userName + "'"); // EXPECT BHF-419
    QSqlQuery inlineQuery("DELETE FROM audit WHERE actor='" + userName + "'", db); // EXPECT BHF-419
}

void unsafe_cpp_log(const QString &userInput) {
    spdlog::warn("{}", userInput); // EXPECT BHF-544
}
