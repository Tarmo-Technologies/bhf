...
# bhf vs cppcheck / flawfinder / semgrep — C/C++ static scan (2026-07-09)

All numbers are real, measured on this box. Tools: cppcheck 2.13.0, flawfinder 2.0.20, semgrep 1.168.0, bhf `target/debug/bhf`. Corpus: `benchmarks/campaign-2026-07-08/corpus/{c_zlib,c_jansson,cpp_json,cpp_fmt}`.

## Metrics table (finding counts)

| Repo | bhf | cppcheck (raw `wc -l`) | cppcheck (real `<error>`) | flawfinder (task cmd `grep -c ':'`) | flawfinder (real hits) | semgrep (`results`) |
|---|---|---|---|---|---|---|
...
3. **Resource-leak (CWE-772) on `fopen`/`malloc` without matching close/free** — cppcheck's `resourceLeak` fires in zlib; bhf has no leak rule in the C lane. Worth a heuristic pass on the file-open sink it already tracks for BHF-405.

Artifacts: bhf reports at `/tmp/bhf_{repo}/static-report.json`; cppcheck XML at `/tmp/ccz.xml` (zlib) and `/tmp/cc_{jansson,cpp_json,cpp_fmt}.xml`; semgrep JSON at `/tmp/sg_{repo}.json`.