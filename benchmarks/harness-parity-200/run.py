...
# These lanes write BHF_TARGET_ENTRY_SHM immediately before the selected
    ...
    *,
    ...
    with log.open("wb") as output:
        ...
        try:
            ...
            # BHF/build tools spawn compiler and linker grandchildren. Killing
...
def run_one(project: dict[str, str], args, output: Path) -> dict[str, object]:
    ...
    row: dict[str, object] = {
        ...
        "bhf_version": args.bhf_version,
        "bhf_sha256": args.bhf_sha256,
    }
    if source_error:
...
def write_results(rows: list[dict[str, object]], output: Path) -> None:
    ...
    lines = [
        ...
        "",
        f"- BHF versions: {', '.join(sorted({str(row.get('bhf_version', 'unknown')) for row in rows}))}",
        f"- BHF binary SHA-256 values: {', '.join(sorted({str(row.get('bhf_sha256', 'unknown')) for row in rows}))}",
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("/tmp/bhf-harness-parity-200")
    )
    ...
    args.bhf_sha256 = hashlib.sha256(args.bhf.read_bytes()).hexdigest()
    try:
        args.bhf_version = subprocess.check_output(
            [str(args.bhf), "--version"], text=True, timeout=10
        ).strip()
    except (OSError, subprocess.SubprocessError):
        args.bhf_version = "unknown"
