...
# The 2026-07-28 full sweep (v0.2.22): same budgets and same corpus as launch_full.sh, but
...
cd "$(dirname "$0")" || exit 1
BHF_BIN="${BHF_BIN:-/home/ubuntu/bhf-sweep-bin/bhf-0728}"
export BHF_BIN
"$BHF_BIN" --version >/dev/null 2>&1 || { echo "pinned binary missing: $BHF_BIN"; exit 1; }
mkdir -p results-0728
nohup python3 -u run_sweep.py \
    ...
    --max-repair-rounds 4 \
    --auto-slack 420 \
    --results-dir results-0728 \
    --rerun \
    > /tmp/full-0728.log 2>&1 &
echo "launched pid $!"
