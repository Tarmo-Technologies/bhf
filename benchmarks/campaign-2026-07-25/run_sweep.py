...
"""Drive bhf over the sweep corpus, one project at a time, and measure it.
...
findings, per-surface exit codes and wall times, and any bhf panic --
...
WORK_ROOT = Path("/home/ubuntu/bhf-sweep-work")
RESULTS = HERE / "results"
VALIDATED = HERE / "validated.json"

LANES = list(dict.fromkeys(bc.LANE.values()))

# bhf's own panic hook CATCHES an internal panic, records it in the bug
...
def sh(
    ...
    """Run one bhf invocation, capturing what the sweep needs to judge it."""
    env = dict(os.environ)
    env.setdefault("BHF_PROFILE", "external-tools")
    env["RUST_BACKTRACE"] = "1"
    # A C# shop has a current SDK; the distro one is .NET 8 and most of the
    # corpus now targets 9/10. bhf picks the newest it finds on PATH.
...
def run_project(row: dict, args: argparse.Namespace) -> dict:
    ...
    try:
        ...
        if "sloc" in surfaces:
            r = sh(
                [str(BHF), "sloc", str(tree), "--json", "--out", str(scratch / "sloc.json")],
                scratch,
                300,
            )
        ...
        if "sbom" in surfaces:
            sbom_out = work / "sbom"
            r = sh(
                [str(BHF), "sbom", str(tree), "--out", str(sbom_out)],
                scratch,
                600,
            )
        ...
        if "fuzz" in surfaces:
            ...
            if args.max_targets:
                ...
                # FUZZ, backfilling past the nonviable ones. Answers "can bhf
...
def main() -> int:
    ...
    ap.add_argument("--inner-jobs", type=int, default=1, help="bhf auto --jobs")
    ...
    if not BHF.exists():
        print(f"missing binary: {BHF}", file=sys.stderr)
        return 2
