...
A single sweep of bhf over 500 pinned open-source projects spanning all
...
The binary is pinned to a copy (`/home/ubuntu/bhf-sweep-bin/bhf`) for
...
| The external-class degradation matched compiler wording | the substitution it was written for (an opaque scalar) is no longer what bhf emits, so an unsuppliable MFC class stayed a bare failed_build | decide on provenance: a diagnostic in the project's own source is not a bhf codegen bug |
