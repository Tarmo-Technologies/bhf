...
#
# The sweep's own blocker table says the largest REAL bhf limit (as opposed
...
# is not overwritten:
#
#   aggregate.py --results results --compare results-plain2
#
...
cd "$(dirname "$0")" || exit 1
BHF_BIN="${BHF_BIN:-/home/ubuntu/bhf-sweep-bin/bhf-twophase}"
export BHF_BIN
"$BHF_BIN" --version >/dev/null 2>&1 || { echo "pinned binary missing: $BHF_BIN"; exit 1; }
echo "force arm binary: $("$BHF_BIN" --version)"
mkdir -p results-plain2
