...
BHF = Path(os.environ.get("BHF_BIN", "/home/ubuntu/github/tarmo/bhf/target/release/bhf"))
...
# Outcomes that mean BHF gave up on the target. Anything else — above all
...
def harvest(work: Path) -> list[dict]:
    """One record per harness BHF GAVE UP on, with its compiler errors."""
    out = []
...
def main() -> int:
    ...
    if not BHF.is_file():
        print(f"no bhf binary at {BHF}", file=sys.stderr)
        return 1
