...
"""Measure bhf against the leading tool for each job it does.

Four comparisons, each run against the same inputs for every tool:

  sloc     bhf sloc vs cloc (the accuracy reference) and tokei
  static   bhf static-scan vs cppcheck / flawfinder / semgrep / bandit /
           gosec, on findings and on throughput
  sbom     bhf sbom vs syft, on components identified
  fuzz     bhf auto vs libFuzzer and AFL++ on the SAME C function --
           including the harness each competitor needs and bhf does not
...
BHF = Path(os.environ.get("BHF_BIN", REPO / "target" / "release" / "bhf"))
...
def compare_sloc(projects: list[Path]) -> list[dict]:
    ...
    for project in projects:
        ...
        code, out, wall = timed([str(BHF), "sloc", str(project), "--json"], 600)
        row["bhf"] = {"wall_s": wall, "code": parse_bhf_sloc(out)}
        ...
        rows.append(row)
        print(
            f"{project.name[:34]:34s} bhf={row['bhf']['code']:>9} "
            f"cloc={row['cloc']['code']:>9} tokei={row['tokei']['code']:>9}  "
            f"({row['bhf']['wall_s']}s / {row['cloc']['wall_s']}s / {row['tokei']['wall_s']}s)"
        )
    return rows


def parse_bhf_sloc(out: str) -> int:
    try:
...
def compare_static(projects: list[Path], work: Path) -> list[dict]:
    ...
    for project in projects:
        ...
        code, out, wall = timed(
            [str(BHF), "static-scan", str(project), "--out", str(out_dir), "--jobs", "2"], 1800
        )
        ...
        if report.is_file():
            try:
                ...
                findings = 0
        row["bhf"] = {"wall_s": wall, "findings": findings}
        ...
        print(
            f"{project.name[:30]:30s} {lane:7s} bhf={findings:5d}"
            f"({row['bhf']['wall_s']}s)  {others}"
        )
    return rows
...
def compare_sbom(projects: list[Path], work: Path) -> list[dict]:
    ...
    for project in projects:
        ...
        shutil.rmtree(out_dir, ignore_errors=True)
        code, out, wall = timed([str(BHF), "sbom", str(project), "--out", str(out_dir)], 900)
        components = 0
        for name in ("sbom.json", "sbom.cdx.json", "sbom.spdx.json"):
            ...
            if path.is_file():
                ...
                break
        row = {"project": project.name, "bhf": {"wall_s": wall, "components": components}}
        ...
        rows.append(row)
        print(
            f"{project.name[:34]:34s} bhf={components:4d}({row['bhf']['wall_s']}s)  "
...
// Hand-written libFuzzer harness -- the work bhf does not ask for.
#include <stdint.h>
#include <stddef.h>
%(decl)s
...
def compare_fuzz(target_dir: Path, source: Path, function: str, seconds: int, work: Path) -> dict:
    """bhf (no harness) vs libFuzzer and AFL++ (one hand-written harness each)."""
    work.mkdir(parents=True, exist_ok=True)
    result: dict = {"target": str(source), "function": function, "seconds": seconds}

    bhf_work = work / "bhf-work"
    shutil.rmtree(bhf_work, ignore_errors=True)
    code, out, wall = timed(
        [str(BHF), "auto", str(target_dir), "--work-dir", str(bhf_work),
         "--per-target-time", str(seconds), "--max-targets", "1", "--single-pass"],
        seconds + 900,
    )
    execs, findings = 0, 0
    run_json = bhf_work / "auto" / "run.json"
    if run_json.is_file():
    ...
    print(f"bhf    execs/s={execs:8d} findings={findings} harnesses=0 ({wall}s)")
    return result
...
def main() -> None:
    ...
    ap.add_argument("--projects", type=Path, default=Path("/home/ubuntu/bhf-corpus-500"))
    ap.add_argument("--limit", type=int, default=10)
    ap.add_argument("--work", type=Path, default=Path("/tmp/bhf-compare"))
