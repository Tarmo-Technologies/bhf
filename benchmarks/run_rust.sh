...
# Rust head-to-head: bhf (builtin Rust lane, zero harness) vs cargo-fuzz
...
# --- bhf: only the library crate (NO fuzz/ harness dir) ---
mkdir -p "$SCRATCH/bhf"
cp "$ROOT/targets/rust/Cargo.toml" "$SCRATCH/bhf/"; cp -r "$ROOT/targets/rust/src" "$SCRATCH/bhf/"
s=$(now)
...
echo "bhf rust: crash=$crash ttfc=$ttfc"; grep -iE "built\+fuzzed|finding|failed" "$SCRATCH/bhf.log" | head -3
