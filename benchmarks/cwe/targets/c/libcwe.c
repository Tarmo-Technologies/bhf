...
// crashes — only a taint/runtime-aware fuzzer (bhf) reports them, even when
...
// CWE-22: input bytes flow into a path passed to open() — NOT a crash. [bhf only]
...
// CWE-377: insecure temp file — O_CREAT without O_EXCL in a world-writable dir. [bhf only]
int write_temp(const unsigned char *d, size_t n) {
    if (n < 1) return -1;
    int fd = open("/tmp/bhf_cwe_predictable", O_WRONLY | O_CREAT, 0644);  // no O_EXCL
    if (fd >= 0) { (void)write(fd, d, n > 16 ? 16 : n); close(fd); }
    return 0;
}

// CWE-522: reads a secret-like environment variable on an input-driven path. [bhf only]
