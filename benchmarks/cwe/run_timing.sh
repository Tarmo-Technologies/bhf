...
# First-run (cold) vs second-run (warm/reuse) time-to-first-finding for bhf,
...
#   - behavioral bug (CWE-22): only bhf finds it -> competitor = "not found".
set -u
...
echo -e "bug\tbhf_run1_cold_s\tbhf_run2_warm_s\tlibFuzzer_s\tnote" > "$RES"

bhf_ttfc() {  # $1=src-dir(contains one .c) $2=workdir : echo seconds to first finding (no build on warm if WD exists)
  local src="$1" wd="$2"; rm -rf "$wd/findings"
  local s=$(date +%s.%N)
  "$BHF" auto --per-target-time "$BUDGET" --reuse-discovery --work-dir "$wd" "$src" >/dev/null 2>&1 &
...
# CWE-121 memory: extract just parse_header into its own dir so bhf fuzzes one target.
...
WDm="$SCRATCH/wd_mem"
m1=$(bhf_ttfc "$SCRATCH/mem" "$WDm"); m2=$(bhf_ttfc "$SCRATCH/mem" "$WDm"); mlf=$(lf_ttfc parse_header)
echo -e "CWE-121 (memory, both find)\t$m1\t$m2\t$mlf\tfair head-to-head; libFuzzer pre-built harness" >> "$RES"

# CWE-22 path-control: only bhf finds; libFuzzer reports nothing (no crash).
...
WDb="$SCRATCH/wd_beh"
b1=$(bhf_ttfc "$SCRATCH/beh" "$WDb"); b2=$(bhf_ttfc "$SCRATCH/beh" "$WDb"); blf=$(lf_ttfc load_resource)
echo -e "CWE-22 (path-control, bhf only)\t$b1\t$b2\t$blf\tlibFuzzer cannot detect (no crash)" >> "$RES"
