...
# CWE-coverage head-to-head (C). bhf auto-harnesses the whole library and
...
echo -e "cwe\tfunction\tbhf\tlibFuzzer\tAFL++" > "$RES"
...
# oracle/rule -> CWE map for bhf findings
bhf_cwes() { python3 - "$1" <<'PY'
import json,os,sys
root=sys.argv[1]; m={
...
echo ">> bhf: run 1 (cold) on the whole library"
WD="$SCRATCH/gw"; mkdir -p "$SCRATCH/src"; cp "$LIB" "$SCRATCH/src/"
s=$(date +%s.%N)
"$BHF" auto --per-target-time "$BUDGET" --reuse-discovery --work-dir "$WD" "$SCRATCH/src" >"$SCRATCH/bhf1.log" 2>&1
R1=$(echo "$(date +%s.%N) - $s"|bc -l)
BHF_CWES="$(bhf_cwes "$WD/findings")"
echo "   bhf run1 wall=${R1}s  CWEs=[$BHF_CWES]"
echo ">> bhf: run 2 (warm, reuse harnesses)"
s=$(date +%s.%N)
"$BHF" auto --per-target-time "$BUDGET" --reuse-discovery --work-dir "$WD" "$SCRATCH/src" >"$SCRATCH/bhf2.log" 2>&1
R2=$(echo "$(date +%s.%N) - $s"|bc -l)
echo "   bhf run2 wall=${R2}s  (rebuilt harnesses: $(grep -c 'generating harness' "$SCRATCH/bhf2.log"))"
...
for fn in parse_header set_level load_resource write_temp read_secret; do
  cwe="${CWE[$fn]}"
  bhf=$([[ " $BHF_CWES " == *" $cwe "* ]] && echo 1 || echo 0)
  echo ">> competitor on $fn ($cwe)"
  lf=$(lf_finds "$fn"); afl=$(afl_finds "$fn")
  echo -e "$cwe\t$fn\t$bhf\t$lf\t$afl" >> "$RES"
done

# timings row
echo -e "TIMING\tbhf_run1_wall=${R1}s\tbhf_run2_wall=${R2}s\t-\t-" >> "$RES"
