...
bugs and bugs that produce **no crash** — fuzzed by bhf vs the most popular
fuzzer for that language.

| script | comparison | what it shows |
|---|---|---|
| `run_c.sh` | bhf vs libFuzzer + AFL++ | **detection**: behavioral CWEs (path traversal / insecure-temp / sensitive-env) that crash-only fuzzers miss even with a per-function harness |
| `run_rust.sh` | bhf vs cargo-fuzz | **coverage**: bugs in 3 functions; one hand-written harness reaches 1, bhf auto-harnesses all 3 |
| `run_java.sh` | bhf vs Jazzer | **coverage**: same, for Java |
...
- Each tool gets the same small seed corpus (`seeds_*/`) so coverage is measured
  without engine-to-engine gate-cracking variance; bhf also cracks the gates
...
- **Run the work dir OUTSIDE the scanned tree** (the runners do). A `--work-dir`
  *inside* the tree contaminates the per-target build with bhf's own
