...
# C head-to-head: libFuzzer vs AFL++ vs bhf(builtin) vs bhf(afl++) on
# planted-bug targets with a UNIFORM entry `target_one_input`. Measures, per tool:
# crash-found, wall time-to-first-crash (TTFC), executions, exec/s. bhf needs
...
run_bhf() {
  ...
  local dir="$SCRATCH/$name/bhf_${engine}"; mkdir -p "$dir/src"
  cp "$tgt" "$dir/src/"
  local s; s=$(now)
  "$BHF" auto --engine "$engine" --per-target-time "$BUDGET" --work-dir "$dir/gw" "$dir/src" >"$dir/log" 2>&1 &
  local pid=$!
  local ttfc="-" crash=0
  # Record end-to-end TTFC (source -> first crash, incl. auto-build) when the
  # first finding appears, but let bhf finish so run.json finalizes (execs).
...
PY
  ...
  echo -e "$name\tbhf($engine)\t0\t$crash\t$ttfc\t${execs:-0}\t${eps:-0}" >>"$RES"
}
...
for tgt in "$ROOT"/targets/c/*.c; do
  ...
  run_afl "$tgt" "$name"
  run_bhf "$tgt" "$name" builtin
  run_bhf "$tgt" "$name" afl++
done
echo "=== RESULTS ($RES) ==="
column -t -s $'\t' "$RES"
rm -rf "$SCRATCH"
