...
For each language we compare **bhf** against the most widely used fuzzer for
...
- **human-authored harness LOC** — lines of fuzz-harness code a person must write
  before fuzzing can start. bhf needs **zero**; the others need a harness per
  entry point.
- **crash found** within a fixed wall budget.
- **TTFC** — wall-clock time to first crash. For bhf this is *end-to-end*
...
crashing seed). bhf is run with no flags beyond `--per-target-time`.
...
```sh
cargo build --workspace                 # builds bhf + the cc-intercept shim
