...
This suite compares a bhf-generated harness with a small, reviewed expert
harness for the same API. Projects and revisions are pinned in `projects.tsv`.
The runner sets `BHF_BLIND_EXPERT_HARNESSES=1`, so maintained fuzz
entrypoints are excluded from recipe mining, and sets `BHF_EXPERT_HARNESS`
...
`/tmp/bhf-harness-parity-20`). A project counts as measured only when both
