...
Thanks for your interest in bhf. This guide covers how to build, test, and submit
changes. By contributing you agree that your contributions are licensed under the
project's [Apache-2.0](./LICENSE) license.

## Development setup

bhf is a Rust workspace of ~40 crates. You need a recent stable Rust toolchain
...
```sh
cargo build --workspace     # also produces libbhf_runtrace_shim.so next to the binaries
cargo test  --workspace     # full suite (~3850 tests)
cargo test -p c_parser      # a single crate
cargo test -p bhf --test auto_attempt name_substring   # a single test
```
...
- Treat all scanned trees, manifests, corpus files, and child-process output as
  **untrusted input** — bhf's threat model is analyzing code you don't trust.
