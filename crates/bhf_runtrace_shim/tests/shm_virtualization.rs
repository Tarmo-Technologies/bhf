...
fn shm_open_is_virtualized_to_private_memory_under_a_fuzz_pass() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-shm-vtest-{}", std::process::id()));
    ...
    let virtualized = Command::new(&bin)
        ...
        .env("BHF_RUNTRACE_MODE", "empty") // any faking mode
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .status()
...
fn sysv_shmget_is_virtualized_to_private_memory_under_a_fuzz_pass() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-sysv-vtest-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    ...
    let run = |with_shim: bool, role: &str, log: Option<&std::path::Path>| -> Option<i32> {
        ...
        c.arg(role)
            ...
            .stderr(Stdio::null());
        if with_shim {
            c.env("LD_PRELOAD", &shim).env("BHF_RUNTRACE_MODE", "empty");
            if let Some(l) = log {
                c.env("BHF_RUNTRACE_LOG", l);
            }
        }
...
fn anonymous_map_shared_is_privatized_under_a_fuzz_pass() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-anonmmap-vtest-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
...
#include <unistd.h>
int main(void) {
    int fd = shm_open("/bhf_shm_content", O_CREAT | O_RDWR, 0600);
    if (fd < 0) return 2;
...
fn shm_content_is_fuzz_driven_under_a_content_bearing_pass() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-shm-content-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
