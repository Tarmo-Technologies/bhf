...
#include <fcntl.h>
int main(void) {
    mqd_t q = mq_open("/bhf_mq_vtest", O_RDONLY);
...
fn mq_receive_delivers_bounded_fuzz_messages_under_a_fuzz_pass() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-mq-vtest-{}", std::process::id()));
    ...
    let mut child = Command::new(&bin)
        ...
        .env("BHF_RUNTRACE_MODE", "rng") // deterministic non-empty messages
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .spawn()
