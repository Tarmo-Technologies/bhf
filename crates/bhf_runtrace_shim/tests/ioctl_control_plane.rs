...
#define BHF_CAPS _IOR('g', 1, unsigned char[16])
int main(void) {
    ...
    if (ioctl(pipefd[0], BHF_CAPS, caps) == 0) return 6;
    ...
    memset(caps, 0, sizeof caps);
    if (ioctl(fd, BHF_CAPS, caps) != 0) return 3; /* memfd alone: ENOTTY */
    int nonzero = 0;
...
fn a_capability_ioctl_is_answered_so_the_driver_reaches_the_register_window() {
    ...
    let dir = std::env::temp_dir().join(format!("bhf-ioctl-test-{}", std::process::id()));
