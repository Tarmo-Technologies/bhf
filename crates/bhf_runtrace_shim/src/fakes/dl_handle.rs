...
//! returns the address of `bhf_fake_dlsym_stub` — a stub that
...
/// must explicitly opt out via `BHF_DISABLE_DLOPEN_FAKE=1`.
#[no_mangle]
pub unsafe extern "C" fn bhf_fake_dlsym_stub() -> *mut libc::c_void {
    std::ptr::null_mut()
}
...
mod tests {
    ...
    fn rtld_next_is_not_synthetic() {
        ...
        // wrongly redirect to bhf_fake_dlsym_stub.
