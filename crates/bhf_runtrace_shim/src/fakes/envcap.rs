...
//!
//! * **Record** — when `BHF_ENVCAP_RECORD=<path>` is set, every
...
//! * **Replay** — when `BHF_ENVCAP_REPLAY=<path>` points at a recorded capsule,
...
//!
//! Together these let `bhf env-capsule` bundle a self-contained, replayable
...
fn record_fd() -> i32 {
    *RECORD_FD.get_or_init(|| {
        let Some(path) = std::env::var_os("BHF_ENVCAP_RECORD") else {
            return -1;
        };
...
fn replay_map() -> Option<&'static HashMap<Vec<u8>, Vec<u8>>> {
    REPLAY_MAP
        .get_or_init(|| {
            let path = std::env::var_os("BHF_ENVCAP_REPLAY")?;
