...
const SUPPRESSED_VARS: &[&[u8]] = &[
    ...
    // LD_PRELOAD + BHF_RUNTRACE_LOG and probe these config families. The
...
mod tests {
    ...
    fn ld_preload_is_suppressed() {
        ...
        assert!(!should_log(b"BHF_RUNTRACE_LOG"));
        assert!(!should_log(b"AFL_FUZZ_DIR"));
        assert!(!should_log(b"ASAN_OPTIONS"));
    }

    #[test]
    fn bhf_internal_env_vars_are_suppressed() {
        assert!(!should_log(b"BHF_FAKE_IDENTITY"));
        assert!(!should_log(b"BHF_CMPLOG"));
        assert!(!should_log(b"BHF_FUZZ_INPUT_FD"));
        assert!(!should_log(b"BHF_RUNTRACE_MODE"));
    }
