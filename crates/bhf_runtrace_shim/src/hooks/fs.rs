...
fn log_insecure_tempfile(api: &[u8], path: &[u8], flags: libc::c_int) {
    // BHF's own infrastructure files (the coverage/value-profile/cmplog
...
/// path-controlled-open candidate (BHF-405) with a source→sink path.
...
/// registers) so the CLI's destructive-path sink oracle (BHF-440) can confirm a
...
/// function, auditing the controlled path (BHF-440) before returning. If the real
...
/// controlled destination picks what to clobber (BHF-440). The caller has already
