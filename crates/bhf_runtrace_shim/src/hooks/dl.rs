...
/// (BHF-435) can confirm a fuzz-controlled path and suppress a constant plugin
...
pub unsafe extern "C" fn dlopen(
    ...
    // #422: emit the controlled-library-load sink event on every load (BHF-435).
    ...
    if result.is_null() && !filename.is_null() {
        ...
        if crate::fakes::mode::current().is_faking()
            && std::env::var_os("BHF_DISABLE_DLOPEN_FAKE").is_none()
        {
