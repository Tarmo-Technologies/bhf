...
/// BHF's own per-harness instrumentation directory, derived ONCE from
/// the `BHF_RUNTRACE_LOG` path. `bhf auto` writes the runtrace log,
...
/// when the var is unset (host is not running under `bhf auto`) or has
...
fn owned_dir_prefix() -> Option<&'static [u8]> {
    ...
    PREFIX
        .get_or_init(|| {
            let log = std::env::var_os("BHF_RUNTRACE_LOG")?;
...
fn should_audit_path_inner(path: &[u8], owned_dir: Option<&[u8]>) -> bool {
    ...
    // BHF's own per-harness instrumentation files (coverage.shm, vp.shm,
    ...
    // self-trip the insecure-temp-file oracle (BHF-417 / CWE-377, #403) on
    ...
    // checks below only catch the default `./bhf_work` name, not an
    ...
    // Belt-and-braces for the same #403 self-FP: BHF's own per-harness
    ...
    // BHF_RUNTRACE_LOG being resolvable from *this* hooked process — which
    ...
    // observable), so a /tmp work-dir self-trips BHF-417 on every target. Match
    ...
    // (BHF-418) on every sanitizer crash.
    if path.starts_with(b"/usr/lib/debug/") || path_contains_segment(path, b".build-id") {
        return false;
    }
    // BHF's own synthetic fixtures. setenv injection points env
    // vars at /tmp/bhf/fake_env/<NAME>, and Slice C's fake_fs
    // lives at /tmp/bhf/fake_fs/...; both are BHF-owned and
    // their ENOENTs are noise, not real "missing config" misses. The
    // local ./bhf_work/ dir is the same story for relative-path
    // invocations.
    if path.starts_with(b"/tmp/bhf/") || path.starts_with(b"./bhf_work/") {
        return false;
    }
    // BHF's own work artefacts — the shim must not log its own
    // log file open, the fuzz harness's runtime dir, etc.
    if path_contains_segment(path, b"bhf_work")
...
mod tests {
    ...
    #[test]
    fn bhf_work_path_not_audited() {
        assert!(!should_audit_path(
            b"/home/u/proj/bhf_work/auto/H-X/runtrace.jsonl"
        ));
    }
    ...
    fn segment_match_requires_slash_boundary() {
        // "bhf_workshop" should NOT match bhf_work.
        assert!(should_audit_path(b"/work/bhf_workshop/foo"));
    }
    ...
    fn fake_env_path_not_audited() {
        assert!(!should_audit_path(b"/tmp/bhf/fake_env/ACME_CONFIG"));
    }

    #[test]
    fn fake_fs_path_not_audited() {
        assert!(!should_audit_path(b"/tmp/bhf/fake_fs/foo/bar"));
    }
    ...
    fn engine_shm_basenames_not_audited_even_without_owned_dir() {
        ...
        // otherwise BHF-417 self-fires on coverage.shm for 100% of targets.
        for f in [
    ...
    fn no_owned_dir_falls_through_to_default_policy() {
        // When BHF_RUNTRACE_LOG is unset there is no engine dir; behaviour
