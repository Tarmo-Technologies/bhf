...
//! (Kept as compile-time Rust `const` data rather than a runtime TOML file: bhf
...
pub(crate) struct TaintSink {
    ...
    /// The finding rule this sink graduates a tainted flow to (BHF-304 command,
    /// BHF-405 path, BHF-419 SQL); its CWE (used to dedup the pattern hit at the same
...
/// A server-side HTTP-request sink (SSRF, BHF-427): a tainted URL reaching it lets
...
mod tests {
    ...
    /// entry silently drops that lane's BHF-304 detection).
    #[test]
    ...
    fn matches_case_insensitively_and_honors_gates() {
        ...
        assert_eq!((sink.display, sink.rule_id), ("Runtime.exec", "BHF-304"));
    ...
    fn multi_class_sinks_carry_their_rule() {
        ...
        assert_eq!((sql.rule_id, sql.needs_dynamic_string), ("BHF-419", true));
    }
