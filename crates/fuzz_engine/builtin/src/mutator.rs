...
pub enum MutationKind {
    ...
    /// dominant structure of the legacy binary parsers bhf targets.
    StructuredChunked,
...
impl MutatorSuite {
    ...
    fn structured_multipart(
        ...
        // Two bytes, deliberately. The boundary is emitted twice (opening
        // delimiter + closing delimiter), so every byte here costs two from the
        // caller's max-length budget — enough, at small budgets, to crowd out
        // the dictionary tokens that make the body worth generating. The value
        // itself is arbitrary: the parser reads the boundary back off the first
        // line rather than matching a fixed string.
        let boundary = b"bh";
