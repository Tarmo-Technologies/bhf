...
pub const PERSISTENT_SHIM_C_FILENAME: &str = "bhf_afl_persistent_shim.c";
...
pub fn persistent_shim_c_source() -> &'static str {
    include_str!("../c/bhf_afl_persistent_shim.c")
}
...
impl Default for AflFeedbackHookPlan {
    fn default() -> Self {
        Self {
            ...
            event_path_env_var: "BHF_EVENTS_PATH",
...
mod tests {
    ...
    fn build_plan_defaults_to_user_installed_afl_clang_fast() {
        let plan = AflShimBuildPlan::new("shim.c", "bhf-afl-shim");
    ...
    fn write_persistent_shim_c_writes_apache_source() {
        let root = std::env::temp_dir().join(format!("bhf-afl-shim-test-{}", std::process::id()));
    ...
    fn feedback_hook_plan_names_afl_custom_mutator_hooks() {
        ...
        assert_eq!(plan.event_path_env_var, "BHF_EVENTS_PATH");
    ...
    fn translates_exception_and_breadcrumb_feedback_for_afl_hooks() {
        ...
        assert_eq!(translated.afl.describe, "bhf:exception-signature+bitmap");
    ...
    fn empty_feedback_disables_afl_queue_get() {
        ...
        assert_eq!(translated.describe, "bhf:none");
    }
    ...
    fn custom_mutator_hook_state_tracks_last_translated_feedback() {
        ...
        assert_eq!(state.afl_custom_describe(), "bhf:none");
        ...
        assert_eq!(
            ...
            "bhf:exception-signature+bitmap"
        );
    }
    ...
    fn fallback_shim_propagates_child_signal_to_own_status() {
        ...
        let shim_bin = root.join("bhf-afl-shim");
...
"#,
    ...
    fn temp_dir(name: &str) -> std::path::PathBuf {
        let root = std::env::temp_dir().join(format!("bhf-afl-shim-{name}-{}", std::process::id()));
