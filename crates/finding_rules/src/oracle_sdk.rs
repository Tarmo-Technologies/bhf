...
pub trait BugOracle: Sync {
    ...
    /// Numeric rule id (`BHF-NNNN`) the oracle's findings map to.
...
pub enum OracleRuntimeEvent {
    ...
    /// `path-controlled-open-runtime` oracle (BHF-405); produced only by
    ...
    /// oracle (BHF-431); produced only by correlation, never by a single raw
    /// command event.
    TaintedCommand {
        api: String,
        command: String,
    ...
    /// `ssrf-controlled-runtime` oracle (BHF-433, CWE-918). Produced only by
    ...
    /// Drives the `library-load-controlled-runtime` oracle (BHF-435, CWE-427).
    TaintedLibrary {
        api: String,
        library: String,
        taint_offset: u32,
    ...
    /// cross-execution correlation has confirmed is fuzz-controlled (#422).
    /// Drives the `sql-injection-runtime` oracle (BHF-441, CWE-89).
    TaintedSqlQuery {
        api: String,
        query: String,
        taint_offset: u32,
    ...
    /// `destructive-path-controlled-runtime` oracle (BHF-440, CWE-73).
...
mod tests {
    ...
    fn bug_oracle_is_object_safe() {
        ...
        assert_eq!(plugin.rule_id(), "BHF-101");
