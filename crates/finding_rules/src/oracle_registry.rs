...
impl BugOracle for MetamorphicRelationRuntime {
    ...
    fn dangerous_apis(&self) -> &'static [&'static str] {
        &["bhf differential --metamorphic-transform"]
    }
    fn describe(&self) -> &'static str {
...
/// candidate (BHF-405, #422). Fires when an `open`/`openat` path argument
...
/// `..`-pattern path-traversal heuristic (BHF-101, `PathTraversalAda`):
...
/// shell-execution API (BHF-431, #422). Fires when a contiguous run of the
...
/// Distinct from the shell-metacharacter heuristic (BHF-304,
...
/// egress API (BHF-433, #422, CWE-918 SSRF). Fires when a `getaddrinfo`
...
/// that taint. Distinct from the presence heuristic (BHF-303, `SsrfAda`): a
...
/// loader (BHF-435, #422, CWE-427). Fires when the `dlopen`/`dlmopen` filename
...
/// database-execution API (BHF-441, #422, CWE-89). Fires when the query string
...
/// filesystem API (BHF-440, #422, CWE-73). Fires when the path passed to
...
/// oracle (BHF-405, read/open sink): this covers *mutating* operations where a
...
mod tests {
    ...
    fn path_controlled_open_oracle_confirms_tainted_open() {
        ...
        let hit = super::PathControlledOpenRuntime
            ...
            .expect("a confirmed fuzz-controlled open path should confirm BHF-405");
    ...
    fn path_controlled_open_oracle_ignores_plain_file_path() {
        ...
        // A raw (uncorrelated) FilePath event never confirms BHF-405 — only a
    ...
    fn command_controlled_runtime_oracle_confirms_tainted_command() {
        ...
        let hit = super::CommandInjectionRuntime
            ...
            .expect("a confirmed fuzz-controlled command should confirm BHF-431");
    ...
    fn command_controlled_runtime_oracle_ignores_plain_command() {
        ...
        // A raw (uncorrelated) Command event never confirms BHF-431 — only a
        // cross-execution-confirmed TaintedCommand does. The shell-metacharacter
        // heuristic (BHF-304) still handles the untainted case; this taint-gated
    ...
    fn differential_oracle_matches_output_divergence() {
        ...
        assert_eq!(hit.rule_id, "BHF-301");
        assert_eq!(hit.api, "bhf differential");
    ...
    fn insecure_temp_file_oracle_flags_world_writable_dirs_only() {
        ...
        let hit = super::InsecureTempFileRuntime.evaluate(&OracleRuntimeEvent::InsecureTempFile {
            api: "open".to_owned(),
            path: "/tmp/bhf-pidXXXX.tmp".to_owned(),
        });
        let hit = hit.expect("/tmp creation must be flagged");
        assert_eq!(hit.rule_id, "BHF-417");
    ...
    fn metamorphic_oracle_matches_relation_violation() {
        ...
        let hit = super::MetamorphicRelationRuntime
            .evaluate(&OracleRuntimeEvent::Metamorphic {
                api: "bhf differential metamorphic".to_owned(),
        ...
        assert_eq!(hit.rule_id, "BHF-307");
        assert_eq!(hit.api, "bhf differential metamorphic");
