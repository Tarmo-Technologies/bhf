...
//! files from AFL/libFuzzer/Honggfuzz/bhf, deduplicates by
...
/// bhf naming, deduped by content hash. `source_format` is
...
pub fn import_dir(
    ...
    copy_with_dedup(src, dst, source_format, Format::Bhf)
}
...
pub fn merge_dirs(srcs: &[PathBuf], dst: &Path) -> Result<ImportSummary, BridgeError> {
    ...
    let mut ord = next_afl_ordinal(dst, Format::Bhf);
    for (i, src) in srcs.iter().enumerate() {
        ...
        for (bytes, hash) in iter_files(src)? {
            ...
            let name = output_name(Format::Bhf, &hash, &mut ord);
...
fn output_name(format: Format, hash: &[u8; 32], next_afl_ord: &mut u32) -> String {
    ...
    match format {
        Format::Bhf | Format::Unknown => format!("{}.bin", &hex[..16]),
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-bridge-{name}-{nonce}"));
    ...
    fn import_dir_copies_unique_files() {
        ...
        assert_eq!(summary.target_format, Format::Bhf);
