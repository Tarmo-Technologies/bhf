...
//!
//! bhf instruments Ada sources by inserting breadcrumb calls, which shifts
...
//! instrumenter writes a `<file>.bhf-lines.json` sidecar next to each
...
const SIDECAR_SUFFIX: &str = ".bhf-lines.json";
...
impl SourceLineMaps {
    /// Load every `*.bhf-lines.json` sidecar in `dir`. Missing dir / bad
...
mod tests {
    ...
    fn load_parses_sidecars_from_dir() {
        let dir = std::env::temp_dir().join(format!("bhf-linemap-{}", std::process::id()));
        let _ = fs::create_dir_all(&dir);
        fs::write(
            dir.join("p.adb.bhf-lines.json"),
