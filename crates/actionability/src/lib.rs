...
pub struct EntryPath {
    ...
    /// * `Some(false)` — the bhf harness drove this function directly but the
...
fn impact_from(raw: &Value) -> Impact {
    ...
    // stops a swallowed Constraint_Error (BHF-102, inherently "high") from
...
fn sanitizer_stack_location(raw: &Value) -> Option<FixLocation> {
    ...
    for frame in frames {
        ...
        // Never resolve the fix to bhf's own generated harness (the
        // `bhf_run_one`/`main` frames in `<work>/auto/<id>/main.c`): that
...
/// `bhf_run_one(unsigned char const*, unsigned long)` becomes
/// `bhf_run_one`. The literal `(anonymous namespace)` scope token is NOT an
...
/// (non-runtime, non-allocator) frame is a bhf harness frame, and no target
...
/// double-/invalid-frees. bhf manufactured the fault, so it is NOT an
...
/// A native C/C++ assertion (BHF-415) whose failed expression is an ENUM-DOMAIN
...
fn is_native_assertion_enum_domain_precondition(raw: &Value) -> bool {
    let is_native_assert = string_at(raw, &["oracle", "name"]).as_deref()
        ...
        || string_at(raw, &["rule_id"]).as_deref() == Some("BHF-415");
    if !is_native_assert {
        return false;
    }
...
/// The bhf harness scaffolding (entry shims + the generated `main`), which
...
pub fn is_harness_frame(function: &str, file: Option<&str>) -> bool {
    let head = function_head(function);
    if head.starts_with("bhf_")
        // The generated decode helpers (`bhf_c_string`, `bhf_take_*`, ...) live in
        // the copied-in runtime header `bhf_decode.h`; a leak/crash whose top
        // frame is one of them is bhf scaffolding, not a target site.
        || head.starts_with("bhf_")
    ...
    file.is_some_and(|file| {
        ...
        (lower.contains("/auto/")
            ...
            // The copied-in decode/runtime headers (`bhf_decode.h`,
            // `bhf_cov.*`) and any `*_runtime/` tree (`c_runtime/`,
            // `rust_runtime/`, `java_runtime/`, ...) are bhf's own code.
            || lower.contains("bhf_decode")
            || lower.contains("bhf_cov")
            || lower.contains("_runtime/")
    })
}

/// Where execution goes wrong: the top project frame on the crash stack. Skips
/// sanitizer-runtime frames, allocator primitives, and the bhf harness.
...
/// (BHF-415) records — so oracle findings populate the report's sink columns
...
fn cwe_for_finding(raw: &Value) -> (Vec<String>, Option<String>) {
    ...
    let (primary, secondary): (&str, Option<&str>) = match class {
        ...
        // BHF-415 oracle hits used to emit a blank CWE in findings.csv). Mirrors
