...
fn fix_location_skips_generated_harness_frame() {
    ...
    // cleanup (bhf_run_one in <work>/auto/<id>/main.c) must NOT resolve the
    // fix location to bhf's own code — that frame is the harness freeing
...
fn fix_location_uses_target_frame_below_harness_frame() {
    ...
    let raw = json!({
        "exception": {
            "stack": [
                { "function": "bhf_run_one", "file": "/w/auto/H/main.c", "line": 64 },
