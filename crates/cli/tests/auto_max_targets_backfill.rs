...
fn tmpdir(tag: &str) -> std::path::PathBuf {
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-maxtargets-{tag}-{nonce}"));
...
/// unsupported any more: bhf drives C callbacks, so every function in the
 ...
 * cap. (A function-POINTER parameter no longer belongs here: bhf drives
