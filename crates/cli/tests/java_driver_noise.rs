...
// (`com.bhf.Driver.isFinding`), in particular the M2.2 NullPointerException
...
public final class NpeNoiseProbe {
    ...
    public static void main(String[] args) {
        ...
        StackTraceElement[] deep = {
            ...
            f("com.acme.Middle"),
            f("com.acme.Outer"),
            new StackTraceElement("bhfgen.Harness", "bhfRunOne", "Harness.java", 1),
            new StackTraceElement("jdk.internal.reflect.DirectMethodHandleAccessor", "invoke", null, 1),
            new StackTraceElement("com.bhf.Driver", "runInput", "Driver.java", 1),
        };
...
/// A probe for the scalar-only-target suppression (BHF_SCALAR_ONLY_TARGET=1): a
...
fn driver_noise_policy_promotes_deep_npe_and_suppresses_shallow() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-jvm-noise-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    let src = tmp.join("com/bhf");
    ...
    let run = Command::new("java")
        .args(["-cp", classes.to_str().unwrap(), "com.bhf.NpeNoiseProbe"])
        .output()
        .expect("java");
    let combined = format!(
        "{}\n{}",
    ...
    // Second run with BHF_SCALAR_ONLY_TARGET=1: the container range/size
    ...
    let scalar_run = Command::new("java")
        .env("BHF_SCALAR_ONLY_TARGET", "1")
        .args(["-cp", classes.to_str().unwrap(), "com.bhf.ScalarOnlyProbe"])
        .output()
        .expect("java");
