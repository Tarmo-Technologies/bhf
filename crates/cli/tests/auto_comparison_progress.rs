...
//! (`BHF_CMP_PROGRESS_SHM`). The engine folds a newly-reached leading-match
...
/// `BHF_CMPP_BITS` in the driver templates and the `CoverageTracker` reader.
const BHF_CMPP_BITS: usize = 1 << 16;

/// True when this host can build a C sanitizer/libFuzzer target the way
/// `bhf auto` does. When true, a failed build is a real regression.
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-cmpp-{prefix}-{n}"));
...
/* A normal library parse entry, NOT a pre-written `LLVMFuzzerTestOneInput`:
   ...
   so `bhf auto` would discover 0 candidates and exit non-zero. A plain
   `(const uint8_t*, size_t)` function is discovered, and bhf wraps it in a
...
fn standalone_progress_sum(main: &Path, work: &Path, tag: &str, input: &[u8]) -> u64 {
    ...
    assert_eq!(
        bytes.len(),
        BHF_CMPP_BITS,
        "progress map must be BHF_CMPP_BITS-sized"
    );
...
fn comparison_progress_rewards_more_leading_bytes_matched() {
    ...
    // The built harness binary lives at `bhf_work/harnesses/<id>/main`
    // (`find_named` looks one level down, at `<root>/<id>/main`).
    let main = find_named(&root.join("bhf_work/harnesses"), "main")
