...
//
// `bhf auto` legacy-C++ dialect ladder. The fixture uses `register` — a storage
...
fn run_auto(bin: &Path, src: &Path, work: &Path, extra: &[&str]) -> std::process::Output {
    ...
    Command::new(bin).args(args).output().expect("run bhf auto")
}
...
fn auto_cxx_std_ladder_builds_legacy_cpp_and_rejects_bogus() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-cxxstd-{}", std::process::id()));
...
fn auto_builds_pre_cpp11_adjacent_format_macro_under_modern_clang() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-cxx-adjacent-literal-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    let src = tmp.join("src");
    std::fs::create_dir_all(&src).unwrap();
