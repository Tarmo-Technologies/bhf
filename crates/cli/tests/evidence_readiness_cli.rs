...
fn real_code_matrix_reports_language_coverage_and_offline_subset_metadata() {
    ...
    assert_eq!(value["schema_version"], "bhf.real_code_matrix.v1");
...
fn seeded_benchmark_tracks_recall_precision_and_fails_on_missed_expected_findings() {
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.seeded_benchmark.v1",
            ...
            "cases": [
                {
                    ...
                    "finding_kind": "static",
                    "source": "unsafe.c",
                    "expected_findings": [{ "rule_id": "BHF-401", "path": "unsafe.c" }]
                },
                {
                    "id": "cpp-nonliteral-format",
                    "finding_kind": "static",
                    "source": "service.cpp",
                    "expected_findings": [{ "rule_id": "BHF-408", "path": "service.cpp" }]
                },
                {
                    "id": "ada-tasking",
                    "finding_kind": "static",
                    "source": "tasking.adb",
                    "expected_findings": [{ "rule_id": "BHF-411", "path": "tasking.adb" }]
                },
                {
                    "id": "clean-c",
                    "finding_kind": "static",
                    "source": "clean.c",
                    "expected_non_findings": [{ "rule_id": "BHF-401", "path": "clean.c" }]
                },
                {
                    "id": "dynamic-crash-triage",
                    "finding_kind": "dynamic",
                    "observed_findings": [{ "rule_id": "BHF-101", "path": "replay/run-last.json" }],
                    "expected_findings": [{ "rule_id": "BHF-101", "path": "replay/run-last.json" }]
                },
                {
                    "id": "binary-crash",
                    "finding_kind": "binary",
                    "observed_findings": [{ "rule_id": "BHF-501", "path": "bin/legacyd" }],
                    "expected_findings": [{ "rule_id": "BHF-501", "path": "bin/legacyd" }]
                },
                {
                    "id": "sca-cve",
                    "finding_kind": "sca",
    ...
    assert_eq!(value["schema_version"], "bhf.seeded_benchmark.metrics.v1");
    ...
    assert_eq!(value["metrics"]["by_rule"]["BHF-401"]["true_positives"], 1);
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&json!({
            ...
            "cases": [{
                ...
                "finding_kind": "static",
                "source": "clean.c",
                "expected_findings": [{ "rule_id": "BHF-999", "path": "clean.c" }]
            }]
        }))
        .unwrap(),
    )
...
fn government_legacy_pattern_corpus_reports_required_track_coverage() {
    ...
    assert_eq!(manifest_json["schema_version"], "bhf.legacy_patterns.v1");
    ...
    assert_eq!(
        ...
        "bhf.legacy_patterns.coverage.v1"
    );
...
fn readiness_scorecard_uses_evidence_and_blocks_unsupported_enterprise_scanner_claims() {
    ...
    fs::write(
        &validation,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.real_code_matrix.v1",
            "summary": {
                "repositories": 4,
                "checks": 8,
                "scenarios": 3,
    ...
    fs::write(
        &metrics,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.seeded_benchmark.metrics.v1",
            "metrics": {
                "recall": 1.0,
                "precision_proxy": 1.0,
                "false_negatives": 0,
    ...
    fs::write(
        &patterns,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.legacy_patterns.coverage.v1",
    ...
    fs::write(
        ...
        "BHF is a government legacy software fuzzer for Ada, C, C++, binaries, and enterprise offline workflows.\n",
    )
    .unwrap();
    fs::write(
        claim_root.join("bad.md"),
        "BHF is a complete enterprise scanner with exhaustive vulnerability detection.\n",
    )
    .unwrap();
    ...
    assert_eq!(scorecard["schema_version"], "bhf.readiness.v1");
    for category in [
        "static_scanning",
        "binary_scanning",
        "ada_depth",
...
fn temp_dir(name: &str) -> PathBuf {
    ...
    let unique = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    path.push(format!("bhf-{name}-{unique}"));
