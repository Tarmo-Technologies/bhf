...
//! #437: ASan fiber-switch annotations (`c_runtime/bhf_asan_fiber.h`).
//!
...
fn tmpdir() -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-asan-fiber-{n}"));
...
static bhf_fiber_t main_fiber = {0, 0, 0};
static bhf_fiber_t co_fiber;
...
static void coroutine(void) {
    bhf_fiber_after_switch(&co_fiber, &main_fiber); /* learn main's region */
    touch_stack(6);
    ...
    touch_stack(6);
    bhf_fiber_before_switch(0, &main_fiber); /* exiting: drop fake stack */
    swapcontext(&co_uc, &main_uc);
}
