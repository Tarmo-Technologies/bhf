...
/// Run `bhf auto` on `fixture_dir` and return (combined stdout+stderr, run.json).
fn run_auto(
    fixture_dir: &Path,
    work: &Path,
    per_target: &str,
...
fn reader_trait_method_builds_fuzzes_and_finds_planted_crash() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-reader-{}", std::process::id()));
...
fn private_module_target_builds_and_fuzzes_in_crate() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-incrate-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    ...
    let module = tmp
        .join("harnesses")
        .join(harness_id)
        .join("incrate/src/__bhf_harness.rs");
