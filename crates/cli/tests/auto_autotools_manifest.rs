...
//! Regression test for #418: `bhf auto` on an autotools-style C tree whose
...
//!
//! End-to-end (shells the built `bhf` binary). Gated on clang + make so a
...
///
/// The guard is deliberately COMPOUND. BHF repairs the simple
...
/// that decides it, so BHF refuses to guess and the build still fails: the
