...
// listed by `bhf auto`, then pre-skipped cleanly at attempt time (the build +
...
fn java_static_target_builds_and_fuzzes_natively() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-java-disc-{}", std::process::id()));
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on java fixture");
    let combined = format!(
        "{}\n{}",
