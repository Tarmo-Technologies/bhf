...
fn tmpdir(case: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-parity-{case}-{n}"));
...
fn engine_parity_cold_solve_sweep() {
    ...
    let budget_secs = std::env::var("BHF_PARITY_SECS").unwrap_or_else(|_| "8".to_owned());
    ...
    for case in CASES {
        ...
        // BHF_PARITY_SECS on a slow host.
        assert!(
            findings > 0,
...
/// Run one fixture cold through `bhf auto` and return `(built, execs, findings)`.
fn run_parity_case(case: &str, budget_secs: &str) -> (bool, u64, usize) {
    ...
    assert!(
        ...
        "bhf auto crashed on {case} ({:?})",
        status.code()
    );

    let run_json: serde_json::Value =
        serde_json::from_slice(&std::fs::read(work.join("bhf_work/auto/run.json")).unwrap())
            .unwrap();
