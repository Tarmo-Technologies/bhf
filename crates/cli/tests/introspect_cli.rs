...
fn tempdir(prefix: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-introspect-{prefix}-{nonce}"));
...
fn introspect_markdown_guides_first_run_when_no_auto_report_exists() {
    ...
    // this the default `bhf_work` resolves against the process CWD, and a stray
    // `bhf_work/auto/run.json` left in the crate dir by any earlier `bhf auto`
    ...
    assert!(stdout.contains("# BHF Introspection"));
    assert!(stdout.contains("Prior auto run: not found"));
    assert!(stdout.contains("bhf auto"));
