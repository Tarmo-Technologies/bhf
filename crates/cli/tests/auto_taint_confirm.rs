...
//! `path-controlled-file-open` candidate (BHF-405) to a runtime-confirmed
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-auto-taint-{prefix}-{n}"));
...
#[test]
fn auto_confirms_path_controlled_open_bhf405() {
    ...
    // is fuzz-controlled, so the open path is tainted and BHF-405 confirms.
    fs::write(
        src.join("opener.c"),
        "#include <fcntl.h>\n\
         #include <unistd.h>\n\
    ...
    let findings = finding_jsons(&root.join("bhf_work"));
    let bhf405: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-405")
        .collect();
    assert!(
        !bhf405.is_empty(),
        "expected a runtime-confirmed BHF-405 finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf405.iter().find(|f| f["confirmation"] == "runtime");
    let confirmed = confirmed.expect("BHF-405 finding must be runtime-confirmed (#422)");
    ...
    assert!(
        ...
        "BHF-405 finding must carry a source→sink taint path, got: {taint_path:?}"
    );
}

#[test]
fn auto_sanitized_open_reports_no_bhf405() {
    ...
    // input. The byte-origin match is severed, so no BHF-405 confirmation.
    fs::write(
        src.join("opener.c"),
        "#include <fcntl.h>\n\
         #include <unistd.h>\n\
         int parse_path(const unsigned char *d, unsigned long n) {\n\
             (void)d;\n\
             int fd = open(\"/etc/bhf-sanitized-fixed.conf\", 0);\n\
             if (fd >= 0) close(fd);\n\
             return (int)n;\n\
         }\n",
    )
    ...
    let findings = finding_jsons(&root.join("bhf_work"));
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-405"),
        "sanitized fixed-path open must not produce a BHF-405 finding; findings: {:#?}",
        findings
    );
}
