...
fn policy_validate_writes_governance_summary() {
    ...
    let policy = root.join("bhf-policy.json");
    fs::write(
        &policy,
        serde_json::to_vec_pretty(&json!({
            ...
            "languages": ["ada", "c", "cpp"],
            "rules": { "enabled": ["BHF-401", "BHF-402"], "disabled": [] },
    ...
    assert_eq!(summary["schema_version"], "bhf.policy.validation.v1");
...
fn runners_validate_writes_capability_summary() {
    ...
    let summary: serde_json::Value = read_json(&out);
    assert_eq!(summary["schema_version"], "bhf.runners.validation.v1");
    assert_eq!(summary["valid"], true);
...
fn runners_plan_assigns_queue_with_policy_and_capacity_evidence() {
    ...
    fs::write(
        &queue,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.runner_queue.v1",
            "jobs": [
                {
                    "id": "ada-stateful",
                    "language": "ada",
    ...
    let planned = Command::new(bhf_bin())
        .args([
            "runners",
            "plan",
            manifest.to_str().unwrap(),
    ...
    assert_eq!(plan["schema_version"], "bhf.runners.plan.v1");
...
fn pack_verify_checks_air_gapped_item_hashes() {
    ...
    fs::write(&rules_path, b"{\"rules\":[\"BHF-401\"]}\n").unwrap();
    ...
    let summary: serde_json::Value = read_json(&out);
    assert_eq!(summary["schema_version"], "bhf.update_pack.verification.v1");
    assert_eq!(summary["valid"], true);
...
fn pack_create_builds_deterministic_integrity_digest_manifest() {
    ...
    assert_eq!(manifest["schema_version"], "bhf.update_pack.v1");
...
fn export_bundle_writes_manifest_for_reports_static_policy_and_pack() {
    ...
    fs::write(&pack, b"{\"schema_version\":\"bhf.update_pack.v1\"}\n").unwrap();
...
fn ci_and_export_include_runner_plan_governance_evidence() {
    ...
    fs::write(
        &runner_plan,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.runners.plan.v1",
            "valid": false,
...
fn ci_policy_requires_runner_plan_and_full_assignment() {
    ...
    let missing = Command::new(bhf_bin())
        .args([
            "ci",
            root.to_str().unwrap(),
            "--work-dir",
    ...
    let incomplete = Command::new(bhf_bin())
        .args([
            "ci",
            root.to_str().unwrap(),
            "--work-dir",
    ...
    let complete = Command::new(bhf_bin())
        .args([
            "ci",
            root.to_str().unwrap(),
            "--work-dir",
...
fn sbom_generates_components_matches_offline_cves_and_gates_ci() {
    ...
    let out = root.join("bhf_work/sbom");
    ...
    assert_eq!(sbom["schema_version"], "bhf.sbom.v1");
    assert_eq!(
    ...
    assert_eq!(vulns["schema_version"], "bhf.vulnerabilities.v1");
...
fn enterprise_ops_support_runners_packs_policy_ci_audit_and_dashboard() {
    ...
    assert_eq!(lease_json["schema_version"], "bhf.runners.lease.v1");
    ...
    fs::write(&rules_path, b"{\"rules\":[\"BHF-777\"]}\n").unwrap();
    ...
    .unwrap();
    let denied = Command::new(bhf_bin())
        .args([
            "pack",
            "verify",
    ...
    assert_eq!(ci_json["schema_version"], "bhf.ci.dashboard.v1");
    ...
    let ci_after_evidence = Command::new(bhf_bin())
        .args([
            "ci",
            root.to_str().unwrap(),
            "--work-dir",
    ...
    let dashboard_json = read_json(&dashboard);
    assert_eq!(dashboard_json["schema_version"], "bhf.dashboard.v1");
    ...
    let exported = read_json(&export);
    assert_eq!(exported["schema_version"], "bhf.export.v1");
...
fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-enterprise-ops-{name}-{nonce}"));
