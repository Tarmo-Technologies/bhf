...
const FRAME_PROBE_JAVA: &str = r#"package bhffix;
...
fn coverage_agent_emits_valid_frames_for_uninitialized_stackmap() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-java-frames-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    let src = tmp.join("src/bhffix");
    ...
    let agent_jar = tmp.join("bhf-jvm-agent.jar");
    ...
    assert!(
        combined.contains("OK loaded+verified: bhffix.FrameProbe"),
