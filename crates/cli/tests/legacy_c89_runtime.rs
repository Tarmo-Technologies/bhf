...
//! historical contradiction — `bhf_decode.h` advertised C89 yet used a
...
fn decode_runtime_compiles_strict_c89() {
    ...
    let header = rt.join("bhf_decode.h");
    assert!(
        header.is_file(),
        "c_runtime/bhf_decode.h must exist: {}",
        header.display()
    );

    let dir = std::env::temp_dir().join(format!("bhf-c89-{}", std::process::id()));
    ...
    std::fs::write(
        ...
        "#include \"bhf_decode.h\"\n\
         ...
         \x20   bhf_cursor c = bhf_open(b, sizeof b);\n\
         \x20   (void) bhf_i32(&c);\n\
         \x20   (void) bhf_i64(&c);\n\
         \x20   (void) bhf_bounded_i32(&c, 0, 3);\n\
         \x20   { char *s = bhf_c_string(&c, 4); free(s); }\n\
         \x20   return 0;\n\
         }\n",
    )
    .unwrap();
    ...
    assert!(
        out.status.success(),
        "bhf_decode.h must compile strict-C89 (-std=c89 -ansi -pedantic-errors):\n{}",
