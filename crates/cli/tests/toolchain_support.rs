...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-toolchain-support-{prefix}-{n}"));
...
fn libfuzzer_probe_args_thread_libstdcxx_fallback() {
    let bin = Path::new("/tmp/bhf-probe-bin");
    let src = Path::new("/tmp/bhf-probe.c");
...
#[test]
fn bhf_cargo_command_uses_manifest_path() {
    let cmd = support::bhf_cargo_command();
    let rendered = cmd
        .get_args()
        .map(|arg| arg.to_string_lossy().into_owned())
        .collect::<Vec<_>>();
...
fn c_runtime_decoder_compiles_under_c89_project_flags() {
    ...
    let src = c_runtime.join("bhf_decode_test.c");
    let out_dir = tmpdir("c89-runtime");
    let bin = out_dir.join("bhf_decode_test");
    ...
    assert!(
        ...
        "bhf_decode.h must compile when real C compile databases carry -std=c89\nstdout:\n{}\nstderr:\n{}",
