...
fn write_fixture(root: &Path) {
    ...
    // BHF forces, and the honest answer is "you need that library".
    std::fs::write(
        root.join("session.h"),
        "#ifndef SESSION_H\n#define SESSION_H\n\
         #include <stddef.h>\n\
...
fn manifest_for(args: &[&str], tag: &str) -> (usize, String) {
    let tmp = std::env::temp_dir().join(format!("bhf-force-manifest-{tag}-{}", std::process::id()));
    ...
    cmd.arg("auto")
        ...
        .args(args);
    let _ = cmd.output().expect("run bhf auto");
