...
//! injection (BHF-431, the process-exec sink class), and reports nothing for a
//! hardcoded exec.
//!
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-auto-exectaint-{prefix}-{n}"));
...
#[test]
fn auto_confirms_exec_program_controlled_bhf431() {
    ...
    assert!(
        !bhf431.is_empty(),
        "expected a runtime-confirmed BHF-431 finding for exec; findings: {:#?}",
        findings
    );
    let confirmed = bhf431
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-431 exec finding must be runtime-confirmed (#422)");
    ...
    assert!(
        ...
        "BHF-431 exec finding must carry an execv source→sink taint path, got: {taint_path:?}"
    );
}

#[test]
fn auto_hardcoded_exec_reports_no_bhf431() {
    ...
    fs::write(
        ...
        "#include <unistd.h>\n\
         int run_exec(const unsigned char *d, unsigned long n) {\n\
             (void)d;\n\
             char *const argv[] = { (char *)\"/nonexistent/bhf-fixed\", (char *)0 };\n\
             execv(\"/nonexistent/bhf-fixed\", argv);\n\
             return (int)n;\n\
         }\n",
    )
    .unwrap();
    ...
    assert!(
        ...
        "hardcoded exec must not produce a BHF-431 finding; findings: {:#?}",
        findings
    );
}
