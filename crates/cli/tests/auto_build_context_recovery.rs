...
//!   own source and fails with undefined externals; bhf must link the
...
fn tmpdir() -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-bcr-{n}"));
...
/// harness link fails with `undefined reference to bhf_helper`; bhf must link
...
fn undefined_external_links_recovered_static_library() {
    ...
    fs::write(inc.join("helper.h"), "int bhf_helper(int x);\n").unwrap();
    // The target the harness will exercise; references bhf_helper so its object
    // carries an undefined reference the link must resolve.
    fs::write(
        src.join("target.c"),
        "#include \"helper.h\"\n\
         int bhf_target(const unsigned char *data, unsigned long len) {\n\
         \x20   int acc = 0;\n\
         \x20   for (unsigned long i = 0; i < len; i++) acc = bhf_helper(acc + data[i]);\n\
         \x20   return acc;\n\
         }\n",
    )
    .unwrap();
    ...
    // the project's build dir — so decl_index never sees bhf_helper's body and the
    // ONLY way to resolve it is to link the recovered archive.
    let ext = root.join("external");
    fs::create_dir_all(&ext).unwrap();
    fs::write(
        ext.join("helper.c"),
        "int bhf_helper(int x){ return x + 1; }\n",
    )
    .unwrap();
    ...
    // Sanity: bhf_helper has NO definition source in the swept tree (only a decl).
    assert!(
        idx.lookup_c_definition_source("bhf_helper").is_none(),
        "fixture invalid: bhf_helper body must not be in the tree"
    );

    // `source_root` is the project root bhf was pointed at — where the build
    // dir (and its recovered archive) lives, as in a real `bhf auto <tree>`.
    ...
    // And bhf_helper must NOT have been blind/declared-stubbed — the real library
    // was linked, not faked.
    assert!(
        !repairs.iter().any(|r| matches!(r,
            Repair::StubBlind { symbol } | Repair::StubDeclared { symbol, .. }
                if symbol == "bhf_helper")),
        "bhf_helper must be resolved from the archive, not stubbed; repairs = {repairs:?}"
    );

    let _ = fs::remove_dir_all(&root);
}

/// §26.8: a CMake-`generate_export_header`-style header (`bhf_export.h`) is dropped
...
fn generated_export_header_resolves_from_probe_dir() {
    ...
    let probe = tree.join(".bhf-build");
    fs::create_dir_all(&src).unwrap();
    fs::create_dir_all(&probe).unwrap();

    // Generated export header: defines the export macro AND a value macro the
    // target body uses — an empty placeholder could not supply BHF_MAGIC, so a
    // clean build proves the REAL generated header was on the include path.
    fs::write(
        probe.join("bhf_export.h"),
        "#ifndef BHF_EXPORT_H\n#define BHF_EXPORT_H\n\
         #define BHF_EXPORT\n\
         #define BHF_MAGIC 0x5a\n\
         #endif\n",
    )
    .unwrap();
    fs::write(
        src.join("codec.c"),
        "#include \"bhf_export.h\"\n\
         BHF_EXPORT int bhf_decode(const unsigned char *data, unsigned long len) {\n\
         \x20   return len ? (data[0] ^ BHF_MAGIC) : BHF_MAGIC;\n\
         }\n",
    )
    .unwrap();
    ...
    assert!(
        !repairs.iter().any(|r| matches!(r,
            ...
            | Repair::ConfigHeaderSynth { virtual_path }
                if virtual_path.contains("bhf_export.h"))),
        "bhf_export.h must resolve from the probe dir, not be placeholdered; repairs = {repairs:?}"
    );
