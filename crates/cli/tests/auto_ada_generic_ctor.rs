...
//!
//! bhf already instantiates such a generic and qualifies the operation and
//! the parameter TYPE through the synthesised instance
//! (`Bhf_Generic_Instance.Get`, `Bhf_Generic_Instance.Value`). The bug
...
//!
//! This drives the real `bhf auto` against the bundled fixture and asserts:
...
fn generic_package_operation_with_private_ctor_param_builds_through_instance() {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("bhf-jsonada-generic-ctor-")
        .tempdir()
        .expect("tempdir");
    ...
    assert!(
        main_adb.contains("Bhf_Generic_Instance.Create_Null"),
    ...
    assert!(
        main_adb.contains("Bhf_Generic_Instance.get"),
