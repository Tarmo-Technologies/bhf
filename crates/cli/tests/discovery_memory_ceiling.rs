...
// during discovery in BOTH `list targets` and `auto`, so bhf produced no
...
//
// `BHF_MAX_MEMORY_KB=1` forces pressure on the first sample (any live
...
fn discovery_degrades_under_memory_pressure_instead_of_being_killed() {
    ...
    let out = Command::new(&bin)
        ...
        .env("BHF_MAX_MEMORY_KB", "1")
        .output()
        .expect("run bhf list targets under a 1 kB ceiling");
...
fn discovery_is_unaffected_without_pressure() {
    ...
    let out = Command::new(&bin)
        .args(["list", "targets", fixture().to_str().unwrap()])
        .env("BHF_MAX_MEMORY_KB", "8000000")
        .output()
        .expect("run bhf list targets at a normal ceiling");
