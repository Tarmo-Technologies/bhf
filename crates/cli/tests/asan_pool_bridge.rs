...
//! #436: the custom-allocator ASan poison bridge (`c_runtime/bhf_asan.h`).
//!
...
fn tmpdir() -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-asan-pool-{n}"));
...
int main(void) {
#ifdef USE_BRIDGE
    bhf_asan_pool_init(pool, sizeof pool);
#endif
    {
