...
//! End-to-end: `bhf auto --build-command "<cmd>"` — the universal escape
//! hatch for build systems bhf doesn't natively probe (a bare `build.sh`,
...
//! `build.sh`. bhf runs that script under a front-of-PATH compiler shim,
...
fn write_fixture(root: &Path) {
    ...
    // real flags (`-Iinc`). Named arbitrarily — the user points bhf at it.
    std::fs::write(
        root.join("build.sh"),
...
fn build_command_interception_recovers_flags_and_fuzzes() {
    ...
    // include dir + define.
    let db_path = root.join(".bhf-build/compile_commands.json");
...
fn build_command_intercepts_an_absolute_path_compiler() {
    ...
    // the binary (e.g. a non-workspace `-p bhf` build).
    let so = Path::new(env!("CARGO_BIN_EXE_bhf"))
        .parent()
        .unwrap()
        .join("libbhf_cc_intercept.so");
    if !so.is_file() {
        eprintln!(
