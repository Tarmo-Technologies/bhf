...
//
// M2.1b/d end-to-end native Java fuzzing: `bhf auto` discovers + ranks the
// `java_fuzz` fixture, compiles it with javac, generates + compiles a bhf
// harness, and drives a persistent JVM under bhf's OWN bytecode coverage agent
// over the framed fork-server protocol — finding + classifying the planted
// ArrayIndexOutOfBoundsException (BHF-201). No Jazzer, no libFuzzer.
...
fn java_target_builds_and_fuzzes_natively_and_finds_planted_crash() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-java-m21d-{}", std::process::id()));
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on java_fuzz fixture");
    let combined = format!(
        "{}\n{}",
    ...
    assert!(
        found_bhf201,
        "expected a BHF-201 ArrayIndexOutOfBoundsException finding under {}, output:\n{combined}",
        findings.display()
    );
    assert!(
        crash_starts_with_g,
