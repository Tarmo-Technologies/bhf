...
//! undefined external bhf stubs), but only one crash CAUSALLY needs the
...
fn stub_fabricated_crash_is_demoted_to_lab_only() {
    ...
    let work = std::env::temp_dir().join(format!("bhf-prov-artifact-{}", std::process::id()));
...
fn stub_independent_crash_is_certified_real_defect() {
    ...
    let work = std::env::temp_dir().join(format!("bhf-prov-real-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&work);
