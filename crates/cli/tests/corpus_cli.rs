...
//! End-to-end checks that `bhf corpus import|export|merge`
...
fn tempdir(prefix: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-corpus-cli-{prefix}-{nonce}"));
    fs::create_dir_all(&dir).unwrap();
    dir
}

#[test]
fn corpus_import_afl_dir_writes_bhf_dir() {
    ...
    let out = Command::new(bhf_bin())
        ...
        .output()
        .expect("spawn bhf corpus import");
    assert!(
        out.status.success(),
        "exit={:?} stderr={}",
        out.status.code(),
    ...
    assert!(stdout.contains("afl -> bhf"));

    let written: Vec<_> = fs::read_dir(&dst).unwrap().collect();
    assert_eq!(written.len(), 2);
    for entry in written {
        let name = entry.unwrap().file_name().into_string().unwrap();
        assert!(name.ends_with(".bin"), "bhf output: {name}");
    }
}
...
fn corpus_export_to_libfuzzer_uses_40_char_names() {
    ...
    let out = Command::new(bhf_bin())
        ...
        .output()
        .expect("spawn bhf corpus export");
