...
//! coverage from the bhf driver. Before the fix the C++ build linked
...
//! collapsed, bhf MISSED reachable bugs, and the libFuzzer-main mis-drive
...
/// True when this host can build a C++ sanitizer target the way `bhf auto`
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-cpp-amalg-{prefix}-{n}"));
...
/// mis-drive) bhf missed this class of bug; now it must find it.
...
fn read_run_json(root: &Path) -> serde_json::Value {
    serde_json::from_slice(&fs::read(root.join("bhf_work/auto/run.json")).unwrap()).unwrap()
}
...
fn cpp_amalgamation_detects_planted_bug_with_nonempty_testcase() {
    ...
    assert!(
        findings > 0,
        "bhf must detect the reachable C++ heap-overflow (#408 AC2): {run_json}"
    );
...
fn cpp_amalgamation_benign_target_reports_no_findings() {
    ...
    // findings — no spurious BHF-206 from a mis-driven libFuzzer binary. This
...
/// First `<root>/<id>/<name>` in the bhf_work layout (one `<id>` subdir).
...
fn fork_server_state_masked_crash_is_caught_by_fresh_reverify() {
    ...
    let main = find_named(&root.join("bhf_work/harnesses"), "main").expect("built harness binary");
    ...
    if let Some(queue) = find_named(&root.join("bhf_work/corpus"), "queue") {
