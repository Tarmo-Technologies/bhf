...
// a framed `perl -d:BhfCov` launcher, fuzzed by the builtin engine, and the
...
fn auto_builds_fuzzes_and_finds_divide_by_zero_cwe369() {
    ...
    let src = std::env::temp_dir().join(format!("bhf_perllane_it_{}", std::process::id()));
    ...
    .unwrap();
    let work = std::env::temp_dir().join(format!("bhf_perllane_w_{}", std::process::id()));
