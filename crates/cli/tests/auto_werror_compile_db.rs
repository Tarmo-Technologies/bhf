...
//! (`-Werror -Wmissing-prototypes`). bhf applies it to the generated harness
//! translation unit too, so the generated `bhf_run_one` /
//! `LLVMFuzzerTestOneInput` must not trip `-Wmissing-prototypes` — bhf
...
fn tmpdir(tag: &str) -> std::path::PathBuf {
    let nonce = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-werror-{tag}-{nonce}"));
