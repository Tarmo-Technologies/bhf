...
fn report_subcommand_writes_json_and_markdown_reports() {
    ...
    assert!(markdown.contains("# BHF Report: ci"));
    assert!(markdown
...
fn report_subcommand_writes_junit_when_requested() {
    ...
    assert!(junit.contains("<testsuite name=\"bhf.ci\" tests=\"1\" failures=\"1\""));
    assert!(junit.contains("<testcase classname=\"bhf.unknown\" name=\"F-0001-junit [unknown]\">"));
    assert!(junit.contains("bhfExceptionSignature=abcd"));
}
...
fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-cli-report-{name}-{nonce}"));
