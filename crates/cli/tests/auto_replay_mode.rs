...
//!      returning non-NULL.
//!   2. bhf auto cascade: pass 1 (Empty mode) logs the env miss,
...
//!      env_injected: { ACME_REPLAY_TRIGGER: "/tmp/bhf/fake_env/..." } }.
//!   4. bhf replay reads the stamp, reconstructs the env, re-execs
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-replay-{prefix}-{n}"));
...
fn replay_reproduces_env_triggered_crash_via_runtime_mode_stamp() {
    ...
    //              SEGV_ON_UNKNOWN_ADDRESS -> BHF-206.
    fs::write(
        src.join("probe.c"),
        "#include <stdlib.h>\n\
         #include <string.h>\n\
    ...
    // Phase 1: drive bhf auto. The cascade injects ACME_REPLAY_TRIGGER
    ...
    assert!(
        status.success(),
        "bhf auto exited non-zero ({:?}); see {}/bhf_work/auto/run.md",
        status.code(),
        root.display()
    );
    ...
    // Phase 3: invoke bhf replay with the stamped finding.
    let harness_binary = root
        .join("bhf_work/harnesses")
    ...
    let replay_status = support::bhf_cargo_command()
        .current_dir(&root)
        .args([
            "replay",
            "--harness",
