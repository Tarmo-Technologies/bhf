...
//! End-to-end check that `bhf differential` finds output
...
fn tempdir(prefix: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-diff-cli-{prefix}-{nonce}"));
...
fn divergent_harnesses_emit_findings_and_nonzero_exit() {
    ...
    assert_eq!(finding_json["oracle"]["rule_id"], "BHF-301");
    assert_eq!(finding_json["oracle"]["api"], "bhf differential");
...
fn metamorphic_transform_emits_oracle_finding() {
    ...
    assert_eq!(finding_json["oracle"]["rule_id"], "BHF-307");
    assert_eq!(
        finding_json["oracle"]["api"],
        "bhf differential metamorphic"
    );
