...
//! (BHF-431), and reports nothing for a hardcoded command that merely contains
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-auto-cmdtaint-{prefix}-{n}"));
...
#[test]
fn auto_confirms_command_controlled_bhf431() {
    ...
    // carries byte-origin taint and BHF-431 confirms. The single-quote wrap plus
    ...
    fs::write(
        ...
        "#include <stdlib.h>\n\
         ...
         int run_cmd(const unsigned char *d, unsigned long n) {\n\
             ...
             memcpy(arg, d, n);\n\
             arg[n] = 0;\n\
             snprintf(cmd, sizeof(cmd), \"echo 'bhf_%s' > /dev/null\", arg);\n\
             system(cmd);\n\
             return (int)n;\n\
         }\n",
    )
    ...
    let bhf431: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-431")
        .collect();
    assert!(
        !bhf431.is_empty(),
        "expected a runtime-confirmed BHF-431 finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf431
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-431 finding must be runtime-confirmed (#422)");
    ...
    assert!(
        ...
        "BHF-431 finding must carry a source→sink taint path, got: {taint_path:?}"
    );
}

#[test]
fn auto_hardcoded_command_reports_no_bhf431() {
    ...
    // run on every input, so any dictionary echo is suppressed) — no BHF-431.
    fs::write(
        src.join("runner.c"),
        "#include <stdlib.h>\n\
         int run_cmd(const unsigned char *d, unsigned long n) {\n\
             (void)d;\n\
             system(\"echo bhf-fixed | grep bhf > /dev/null\");\n\
             return (int)n;\n\
         }\n",
    )
    .unwrap();
    ...
    let findings = finding_jsons(&root.join("bhf_work"));
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-431"),
        "hardcoded command must not produce a BHF-431 finding; findings: {:#?}",
        findings
    );
}
