...
// writes a BHF-558 finding for an input whose memory is far above baseline AND
...
fn mem_profile_flags_amplified_allocation_input() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-mem-e2e-{}", std::process::id()));
    ...
    // The replay binary: reads argv[1] (the input file, as the bhf driver passes
    ...
    assert_eq!(
        written, 1,
        "expected exactly one BHF-558 uncontrolled-consumption finding, got {written}"
    );
