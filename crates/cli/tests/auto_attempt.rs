...
fn tmpdir() -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-auto-it-{n}"));
...
// OOM finding (BHF-209) instead of OOM-killing the host. A target that grows the
// resident set past the cap on a marker input must be caught + classified.
#[test]
fn attempt_rss_limit_classifies_oom_as_bhf209() {
    use cli::auto::{attempt::*, candidate::*, decl_index::DeclarationIndex, pass::Pass};
    ...
    fs::write(
        src.join("bomb.c"),
        "#include \"bomb.h\"\n#include <stdlib.h>\n#include <string.h>\n\
         volatile unsigned char bhf_oom_sink;\n\
         int eat_memory(const unsigned char *d, unsigned long n){\n\
           if(n>=4 && d[0]=='B'&&d[1]=='O'&&d[2]=='M'&&d[3]=='B'){\n\
             unsigned long chunk=16ul*1024*1024;\n\
             for(int i=0;i<64;i++){ unsigned char*p=(unsigned char*)malloc(chunk); if(!p)break; memset(p,(unsigned char)(i+1),chunk); bhf_oom_sink+=p[(unsigned long)i*7919u%chunk]; }\n\
           }\n return 0; }\n",
    )
    .unwrap();
    let work = root.join("work");
    ...
    assert!(saw_bhf209, "OOM finding must carry rule id BHF-209");
}
...
fn attempt_uses_direct_harness_for_generic_void_pointer_c_target() {
    ...
    assert!(
        !main.contains("void _bhf_handle"),
...
fn attempt_builds_and_fuzzes_c_scalar_and_enum_output_pointers() {
    ...
    let main = fs::read_to_string(result.harness_dir.join("main.c")).unwrap();
    assert!(main.contains("mz_uint32 _bhf_out_pIndex"));
    assert!(main.contains("mz_zip_error _bhf_out_pErr"));
...
fn attempt_builds_and_fuzzes_c_miniz_file_macro_pointer() {
    ...
    let main = fs::read_to_string(result.harness_dir.join("main.c")).unwrap();
    assert!(main.contains("MZ_FILE * pFile = (MZ_FILE *)(_bhf_file_buf_pFile ?"));
    assert!(main.contains("fmemopen(_bhf_file_buf_pFile, Size, \"r+\")"));
    assert!(main.contains("parse_cfile(pFile)"));
    assert!(main.contains("if (pFile) fclose(pFile);"));
    assert!(main.contains("free(_bhf_file_buf_pFile);"));
}
...
fn attempt_builds_and_fuzzes_c_miniz_time_pointer_alias() {
    ...
    let main = fs::read_to_string(result.harness_dir.join("main.c")).unwrap();
    assert!(main.contains("MZ_TIME_T _bhf_out_pFile_time"));
    assert!(main.contains("const MZ_TIME_T * pFile_time = &_bhf_out_pFile_time"));
...
fn attempt_builds_and_fuzzes_c_void_pointer_output_slot() {
    ...
    let main = fs::read_to_string(result.harness_dir.join("main.c")).unwrap();
    assert!(main.contains("void * _bhf_out_ppBuf = NULL"));
    assert!(main.contains("void * * ppBuf = &_bhf_out_ppBuf"));
    assert!(main.contains("size_t *pSize = &_bhf_out_pSize"));
...
fn attempt_builds_and_fuzzes_c_void_output_capacity_and_input_pair() {
    ...
    assert!(main.contains("size_t out_buf_len = (size_t)(pOut_buf ? _bhf_cap_pOut_buf : 0)"));
...
fn attempt_builds_and_fuzzes_cpp_void_output_capacity_and_input_pair() {
    ...
    assert!(main.contains("void * pOut_buf = (void *)malloc"));
    assert!(
        main.contains("std::size_t out_buf_len = (std::size_t)(pOut_buf ? _bhf_cap_pOut_buf : 0)")
    );
    assert!(main.contains("const void * pSrc_buf = (const void *)Data"));
...
fn attempt_builds_and_fuzzes_cpp_standard_scalar_aliases() {
    ...
    let main = fs::read_to_string(result.harness_dir.join("main.cpp")).unwrap();
    assert!(main.contains("std::uint32_t flags = (std::uint32_t)bhf_bounded_i32"));
    assert!(main.contains("std::size_t count = (std::size_t)bhf_bounded_length"));
    assert!(main.contains("std::uint16_t code = (std::uint16_t)bhf_bounded_i32"));
    assert!(main.contains("bool enabled = (bool)(bhf_u8(&Cur) & 1)"));
...
fn attempt_builds_and_fuzzes_cpp_file_pointer_target() {
    ...
    assert!(main.contains("FILE * stream = _bhf_file_buf_stream ?"));
