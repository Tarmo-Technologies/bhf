...
fn full_ci_fixture_pipelines_cover_active_matrix_cell() {
    if std::env::var_os("BHF_M19_FULL_CI").is_none() {
        eprintln!("skipping: BHF_M19_FULL_CI is not set");
        return;
    }
    assert!(
    ...
    assert_eq!(cli::run_from(["bhf", "--profile", cell.profile()]), 0);
...
fn ci_workflow_defines_gnat_dialect_profile_matrix() {
    ...
    assert!(runner.contains("gnatmake-${BHF_GNAT_VERSION}"));
    ...
    assert!(
        runner.contains("--config=C,,,,gcc-${BHF_GNAT_VERSION}"),
...
impl MatrixCell {
    fn from_env() -> Self {
        let dialect = AdaDialect::parse(&required_env("BHF_ADA_DIALECT"));
        let profile = required_env("BHF_PROFILE");
        assert!(
            matches!(profile.as_str(), "strict-permissive" | "external-tools"),
            "unsupported BHF_PROFILE={profile}"
        );
...
impl AdaDialect {
    fn parse(value: &str) -> Self {
        match value {
            ...
            other => panic!("unsupported BHF_ADA_DIALECT={other}"),
        }
    }
...
fn run_bhf<I>(profile: &str, args: I)
where
