...
fn auto_accepts_reporting_and_attacking_modes() {
    ...
    let report_work_dir = temp.join("bhf_work_reporting");
    let attack_work_dir = temp.join("bhf_work_attacking");
...
fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-actionability-cli-{name}-{nonce}"));
