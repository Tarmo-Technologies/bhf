...
//! Regression test for #407: `bhf auto` stack-overflowed (aborted the whole
...
//! iterative and survives; the overflow was purely in bhf's recursive AST
...
fn tmpdir() -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-deep-src-{n}"));
