...
//! End-to-end: `bhf auto --differential clang:gcc` flags a cross-compiler
//! divergence as a BHF-301 finding.
//!
...
fn tempdir(name: &str) -> std::path::PathBuf {
    ...
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-diff-e2e-{name}-{nonce}"));
...
fn auto_differential_flags_cross_compiler_divergence() {
    ...
    let diff: Vec<_> = findings
        ...
        .filter(|f| {
            f["rule_id"] == "BHF-301"
                && f["analysis"]["engine"] == "bhf.dynamic.differential.replay"
        })
        .collect();
    assert!(
        !diff.is_empty(),
        "expected a BHF-301 differential finding (clang traps, gcc does not).\nstderr:\n{stderr}"
    );
