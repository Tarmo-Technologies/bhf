...
//! Regression for #411: `bhf auto` must not abort a buildable Ada
//! harness just because a user `.gpr` in the source root `with`s
//! bhf's own bundled `ada_runtime/adafuzz.gpr`.
//!
//! `prepare_layout` forwards the `with` clauses of top-level user gprs
//! into the generated `bhf_build.gpr`. That file lives several
...
//! base, and bhf reported its own runtime as a missing GPR import
...
fn ada_target_builds_when_user_gpr_withs_bundled_adafuzz_in_deep_workdir() {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("bhf-411-deep-workdir-")
        .tempdir()
        .expect("tempdir");
    // A source root buried several levels deep and in a completely
    // different filesystem subtree than the bhf install — the
    ...
    // A user harness project that `with`s bhf's bundled runtime via
    // a relative path. Before the fix this clause was forwarded verbatim
    // into bhf_build.gpr (deeper in the tree), where the relative
    ...
    // bhf's own bundled runtime must never be reported as a missing
    // import.
    let empty = Vec::new();
