...
//! (`BHF_COV_CNT_SHM`): the runtime saturating-increments each edge per exec,
...
/// match `BHF_COV_BITS` in the driver templates and the CoverageTracker
/// reader.
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-hitcount-{prefix}-{n}"));
...
fn standalone_footprint(
    ...
    assert_eq!(
        cnt_bytes.len(),
        BHF_COV_BITS,
        "count map must be BHF_COV_BITS-sized"
    );
...
fn hitcount_buckets_distinguish_loop_depth_and_grow_corpus() {
    ...
    let queue = find_named(&root.join("bhf_work/corpus"), "queue")
