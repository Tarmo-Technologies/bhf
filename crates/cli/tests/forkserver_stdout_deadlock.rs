...
//! deadlock that hung `bhf auto` indefinitely (observed: an Ada harness stuck
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-fs-stdout-{prefix}-{n}"));
...
/// Run `bhf auto src` in `root`, killing it after `deadline`. Returns
...
fn run_auto_bounded(root: &Path, per_target_time: &str, deadline: Duration) -> Option<i32> {
    let mut child = support::bhf_cargo_command()
        .current_dir(root)
    ...
    loop {
        match child.try_wait().expect("wait bhf auto") {
...
fn stdout_spamming_target_does_not_deadlock_fork_server() {
    ...
    let run_json: serde_json::Value =
        serde_json::from_slice(&std::fs::read(root.join("bhf_work/auto/run.json")).unwrap())
            .unwrap();
