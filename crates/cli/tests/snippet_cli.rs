...
//! End-to-end coverage for `bhf snippet`: paste ONE function with no project,
...
fn snippet_from_file_fuzzes_a_bare_c_function() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-snippet-file-{}", std::process::id()));
...
fn snippet_from_stdin_detects_language() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-snippet-stdin-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&tmp);
    std::fs::create_dir_all(&tmp).unwrap();
    let work = tmp.join("work");
