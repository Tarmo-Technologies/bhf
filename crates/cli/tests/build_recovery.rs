...
//! from a vendored upstream codebase, run `bhf auto`, and assert
...
struct Scenario {
    ...
    /// `bhf auto --target`. This keeps real-code fixtures from
...
/// the scenarios need. Bhf auto itself decides whether each
...
fn run_one(scenario_path: &Path) -> Result<(), String> {
    ...
    let auto_output = invoke_bhf_auto(&project_dir, &env_strip, &extra_ldflags, &scenario.targets)?;

    let run_json_path = project_dir.join("bhf_work/auto/run.json");
    let run_json_bytes = match std::fs::read(&run_json_path) {
        Ok(b) => b,
        Err(e) => {
            return Err(format!(
                "read {}: {e}; bhf auto exit={:?} stderr=\n{}",
...
fn invoke_bhf_auto(
    ...
    let work_dir = project_dir.join("bhf_work");
    let mut cmd = std::process::Command::new(env!("CARGO"));
    cmd.args(["run", "--quiet", "--release", "-p", "bhf", "--", "auto"])
        .arg(project_dir)
        .arg("--work-dir")
        .arg(&work_dir)
    ...
    // because bhf itself failed to compile).
...
fn smoke_extract_fixture_compiles() {
    ...
    let work = tempfile::Builder::new()
        .prefix("bhf-build-recovery-smoke-")
        .tempdir()
...
fn smoke_no_removals_does_not_fail_build() {
    ...
    let fixture = harness_root().join("fixtures/miniz");
    let work = tempfile::Builder::new()
        .prefix("bhf-build-recovery-no-removals-")
        .tempdir()
    ...
    invoke_bhf_auto(&project_dir, &[], &[], &["miniz_inflate_fuzz".to_owned()])
        .expect("invoke bhf auto in smoke test");
...
fn cpp_stdlib_type_resolves_via_real_include() {
    ...
    let out = invoke_bhf_auto(&project_dir, &[], &[], &[]).expect("invoke bhf auto");
    let run_json_path = project_dir.join("bhf_work/auto/run.json");
    let run_json: serde_json::Value =
...
/// struct produced `state _bhf_value_s;` → "missing_type: state". A clean
