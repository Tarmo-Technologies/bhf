...
fn run_auto(root: &Path, work: &Path, extra: &[&str]) -> (serde_json::Value, Output) {
    ...
    command
        ...
        .arg("--single-pass");
    command.args(extra);
    let output = command.output().expect("spawn bhf auto");
...
fn multi_idl_checked_in_servant_reopens_modules_without_collisions_and_fuzzes() {
    ...
    let temp = tempfile::Builder::new()
        .prefix("bhf-zero-multi-idl-servant-")
        .tempdir()
        .unwrap();
    let root = temp.path();
    ...
    let report_output = Command::new(env!("CARGO_BIN_EXE_bhf"))
        .arg("bug-report")
        .arg(&work)
        .arg("--output")
        .arg(&report_path)
...
fn empty_cpp_lifecycle_records_direct_fallback_and_fuzzes() {
    ...
    let temp = tempfile::Builder::new()
        .prefix("bhf-zero-empty-lifecycle-")
        .tempdir()
        .unwrap();
    let root = temp.path();
...
fn header_compile_database_context_fuzzes_and_support_report_is_private() {
    ...
    let temp = tempfile::Builder::new()
        .prefix("bhf-zero-header-context-")
        .tempdir()
        .unwrap();
    let root = temp.path();
...
fn incompatible_resume_and_clean_all_cannot_reuse_stale_generated_state() {
    ...
    let migration_report_output = Command::new(env!("CARGO_BIN_EXE_bhf"))
        .arg("bug-report")
        .arg(&work)
        .arg("--output")
    ...
    let clean = Command::new(env!("CARGO_BIN_EXE_bhf"))
        .arg("clean")
        .arg(&work)
        .arg("--all")
        .output()
