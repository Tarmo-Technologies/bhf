...
//
// `bhf auto --unsafe-search-and-run-build-commands`: the explicit opt-in to
// searching the tree for its own build and EXECUTING it to recover compile flags. The
// fixture's target only compiles with -DBHF_NEEDS_THIS, a define ONLY its build.sh
// sets — so without the flag it fails to build, and with it bhf finds+runs build.sh
...
fn unsafe_search_and_run_recovers_flags_from_build_sh() {
    let bin = bhf_bin();
    if !bin.exists() {
        eprintln!("skip: bhf binary not built");
        return;
    }
    if !have_toolchain() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-unsafe-search-{}", std::process::id()));
    ...
    // negative control: BHF repairs that shape by defining what the guard tests
    // (see `auto_config_guard.rs`). A compound condition names no single macro whose
    // definition decides it, so BHF refuses to guess and only the real flag —
    // recovered from build.sh — satisfies it.
    std::fs::write(
        src.join("gt.c"),
        "#if !defined(BHF_NEEDS_THIS) || BHF_NEEDS_THIS != 7\n\
         #error \"needs -DBHF_NEEDS_THIS=7, set only by build.sh\"\n\
         #endif\n\
    ...
    std::fs::write(
        &build_sh,
        "#!/bin/sh\ncc -DBHF_NEEDS_THIS=7 -c gt.c -o gt.o\n",
    )
    .unwrap();
    #[cfg(unix)]
    {
    ...
    assert!(
        !run(&tmp.join("w_off"), &[]),
        "without the flag the target must NOT build (missing -DBHF_NEEDS_THIS)"
    );
