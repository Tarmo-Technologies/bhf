...
//! reaching the network-egress (BHF-433), dynamic-library-load (BHF-435), and
//! destructive-filesystem (BHF-440) sinks, and reports nothing for the hardcoded
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-auto-sink-{prefix}-{n}"));
...
fn run_auto(root: &Path) {
    let status = support::bhf_cargo_command()
        .current_dir(root)
...
fn build_and_fuzz(prefix: &str, source: &str) -> Vec<serde_json::Value> {
    ...
    run_auto(&root);
    finding_jsons(&root.join("bhf_work"))
}

// ---- SSRF (BHF-433) — a fuzz-controlled unix-socket destination via connect ----
...
const SSRF_CONTROLLED: &str = "#include <sys/socket.h>\n\
     ...
     int run_connect(const unsigned char *d, unsigned long n) {\n\
         ...
         snprintf(sa.sun_path, sizeof(sa.sun_path), \"/nonexistent/bhf_%.*s\", (int)n, d);\n\
...
const SSRF_HARDCODED: &str = "#include <sys/socket.h>\n\
     ...
     int run_connect(const unsigned char *d, unsigned long n) {\n\
         ...
         memset(&sa, 0, sizeof(sa));\n\
         sa.sun_family = AF_UNIX;\n\
         strcpy(sa.sun_path, \"/nonexistent/bhf_fixed_socket\");\n\
         int fd = socket(AF_UNIX, SOCK_STREAM, 0);\n\
         if (fd < 0) return 0;\n\
         connect(fd, (struct sockaddr *)&sa, sizeof(sa));\n\
         close(fd);\n\
...
#[test]
fn auto_confirms_ssrf_controlled_bhf433() {
    ...
    let bhf433: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-433")
        .collect();
    assert!(
        !bhf433.is_empty(),
        "expected a runtime-confirmed BHF-433 SSRF finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf433
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-433 must be runtime-confirmed (#422)");
    assert!(
        taint_path_of(confirmed).contains("connect(address)"),
        "BHF-433 must carry a connect source→sink taint path, got: {:?}",
        taint_path_of(confirmed)
    );
}

#[test]
fn auto_hardcoded_destination_reports_no_bhf433() {
    ...
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-433"),
        "hardcoded destination must not produce a BHF-433 finding; findings: {:#?}",
        findings
    );
}

// ---- Controlled library load (BHF-435) — a fuzz-controlled dlopen path ----
...
const DLOPEN_CONTROLLED: &str = "#include <dlfcn.h>\n\
     ...
     int run_load(const unsigned char *d, unsigned long n) {\n\
         ...
         memcpy(lib, d, n);\n\
         lib[n] = 0;\n\
         snprintf(full, sizeof(full), \"/nonexistent/bhf_%s.so\", lib);\n\
...
const DLOPEN_HARDCODED: &str = "#include <dlfcn.h>\n\
     int run_load(const unsigned char *d, unsigned long n) {\n\
         (void)d;\n\
         void *h = dlopen(\"/nonexistent/bhf_fixed_plugin.so\", RTLD_NOW);\n\
         if (h) dlclose(h);\n\
         return (int)n;\n\
     }\n";

#[test]
fn auto_confirms_library_load_controlled_bhf435() {
    ...
    let bhf435: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-435")
        .collect();
    assert!(
        !bhf435.is_empty(),
        "expected a runtime-confirmed BHF-435 library-load finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf435
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-435 must be runtime-confirmed (#422)");
    assert!(
        taint_path_of(confirmed).contains("dlopen(path)"),
        "BHF-435 must carry a dlopen source→sink taint path, got: {:?}",
        taint_path_of(confirmed)
    );
}

#[test]
fn auto_hardcoded_library_reports_no_bhf435() {
    ...
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-435"),
        "hardcoded library path must not produce a BHF-435 finding; findings: {:#?}",
        findings
    );
}

// ---- Destructive filesystem op (BHF-440) — a fuzz-controlled unlink path ----
...
const UNLINK_CONTROLLED: &str = "#include <unistd.h>\n\
     ...
     int run_delete(const unsigned char *d, unsigned long n) {\n\
         ...
         memcpy(name, d, n);\n\
         name[n] = 0;\n\
         snprintf(full, sizeof(full), \"/nonexistent/bhf_%s\", name);\n\
         unlink(full);\n\
         return (int)n;\n\
     }\n";
...
const UNLINK_HARDCODED: &str = "#include <unistd.h>\n\
     int run_delete(const unsigned char *d, unsigned long n) {\n\
         (void)d;\n\
         unlink(\"/nonexistent/bhf_fixed_victim\");\n\
         return (int)n;\n\
     }\n";

#[test]
fn auto_confirms_destructive_path_controlled_bhf440() {
    ...
    let bhf440: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-440")
        .collect();
    assert!(
        !bhf440.is_empty(),
        "expected a runtime-confirmed BHF-440 destructive-path finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf440
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-440 must be runtime-confirmed (#422)");
    assert!(
        taint_path_of(confirmed).contains("unlink(path)"),
        "BHF-440 must carry an unlink source→sink taint path, got: {:?}",
        taint_path_of(confirmed)
    );
}

#[test]
fn auto_hardcoded_destructive_path_reports_no_bhf440() {
    ...
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-440"),
        "hardcoded destructive path must not produce a BHF-440 finding; findings: {:#?}",
        findings
    );
}

// ---- SQL injection (BHF-441) — a fuzz-controlled query reaching sqlite3_exec ----
//
...
#[test]
fn auto_confirms_sql_injection_controlled_bhf441() {
    if !support::libfuzzer_toolchain_available("sql-controlled") {
        eprintln!("skipping: clang+libfuzzer toolchain unavailable");
        return;
    }
    let findings = build_and_fuzz("sql-controlled", SQL_CONTROLLED);
    let bhf441: Vec<&serde_json::Value> = findings
        .iter()
        .filter(|f| f["rule_id"] == "BHF-441")
        .collect();
    assert!(
        !bhf441.is_empty(),
        "expected a runtime-confirmed BHF-441 SQL-injection finding; findings: {:#?}",
        findings
    );
    let confirmed = bhf441
        .iter()
        .find(|f| f["confirmation"] == "runtime")
        .expect("BHF-441 must be runtime-confirmed (#422)");
    assert!(
        taint_path_of(confirmed).contains("sqlite3_exec(sql)"),
        "BHF-441 must carry a sqlite3_exec source→sink taint path, got: {:?}",
        taint_path_of(confirmed)
    );
}

#[test]
fn auto_parameterized_query_reports_no_bhf441() {
    ...
    assert!(
        !findings.iter().any(|f| f["rule_id"] == "BHF-441"),
        "a constant query must not produce a BHF-441 finding; findings: {:#?}",
        findings
    );
}
