...
//
// End-to-end JVM sink-reachability oracle: build the real bhf agent jar (coverage
...
// deserialization sink into BHF_SINK_OUT — the input-reachable attack surface the
// crash-only JVM lane never reports. Then confirm the Rust oracle turns that report
// into a BHF-421 finding, and that a sink-free target records nothing (no FP).
...
/// Run `class_name` under the agent with BHF_SINK_OUT set, returning the report
...
fn agent_records_deserialization_sink_and_oracle_emits_finding() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-jsink-{}", std::process::id()));
    ...
    let agent = tmp.join("bhf-jvm-agent.jar");
    ...
    // The Rust oracle turns the recorded sink into a BHF-421 finding.
    ...
    assert!(finding.contains("BHF-421"), "finding:\n{finding}");
