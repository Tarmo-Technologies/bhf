...
// listed by `bhf auto`, then pre-skipped cleanly at attempt time (the
...
fn bhf_bin() -> PathBuf {
    ...
    // is two levels up at target/<profile>/bhf.
...
fn rust_target_builds_and_fuzzes_natively_and_finds_planted_crash() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-rust-m12-{}", std::process::id()));
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on rust_fuzz fixture");
    let combined = format!(
        "{}{}",
