...
fn an_unconstructible_receiver_skips_with_its_reason_and_its_sibling_still_fuzzes() {
    ...
    let src = std::env::temp_dir().join(format!("bhf_js_unconstructible_{}", std::process::id()));
    ...
    let work =
        std::env::temp_dir().join(format!("bhf_js_unconstructible_w_{}", std::process::id()));
    ...
    let out = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on js_unconstructible fixture");
    let combined = format!(
        "{}\n{}",
