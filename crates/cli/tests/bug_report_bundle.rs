...
//! #103: `bhf bug-report --bundle X.tar.gz` produces an offline, scrubbed
...
fn bundle_is_offline_scrubbed_and_self_tested_against_host_secrets() {
    ...
    let output = bhf()
        ...
        .arg(&out)
        .output()
        .expect("run bhf bug-report --bundle");
    assert!(
    ...
    let root = extract.join("bhf-bug-report");
...
fn preview_prints_inventory_without_writing_an_archive() {
    ...
    let base = secret_workspace("bhfPREVIEW");
    let (_src, work) = populate_work_dir(&base);
    ...
    let output = bhf()
        ...
        .arg("--preview")
        .output()
        .expect("run bhf bug-report --preview");
