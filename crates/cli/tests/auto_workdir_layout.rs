...
//! `bhf auto` should keep generated per-target harness artifacts under a
...
fn tmpdir(prefix: &str) -> PathBuf {
    let n = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = std::env::temp_dir().join(format!("bhf-layout-{prefix}-{n}"));
...
fn auto_writes_generated_harnesses_under_harnesses_dir() {
    ...
    assert!(
        ...
        "bhf auto should complete far enough to emit harness artifacts; status={:?}\nstderr=\n{}",
        output.status.code(),
