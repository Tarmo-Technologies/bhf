...
//! incompatible with bhf's standalone single-translation-unit harness compile.
//!
//! fmt's CMake declares `target_compile_options(<lib> PUBLIC -fmodules-ts)` (C++20
//! modules). bhf recovers a project's compile wiring from its CMakeLists when
...
//!
//! Shells the built `bhf` binary; gated on clang++ so a toolchain-less lane
//! skips cleanly.
...
fn write_fixture(root: &Path) {
    ...
    // bhf's CMakeLists inference must drop it from the harness compile.
    std::fs::write(
