...
/// Run `bhf auto` on the cpp-only-class fixture, optionally with `--force`,
...
fn run_auto(force: bool, work_dir: &Path) -> serde_json::Value {
    ...
    let mut cmd = std::process::Command::new(env!("CARGO_BIN_EXE_bhf"));
    cmd.arg("auto")
        .arg(&fixture)
        .arg("--work-dir")
        .arg(work_dir)
    ...
    let output = cmd.output().expect("spawn bhf auto");
...
/// exactly one non-`bhf_parser_version` target. We locate it structurally: the
/// fixture has exactly two targets, and `bhf_parser_version` always builds+fuzzes,
...
fn parser_scan_outcome(doc: &serde_json::Value) -> String {
    ...
    assert_eq!(
        targets.len(),
        2,
        "expected exactly two discovered targets (Parser::scan + bhf_parser_version); \
         run.json=\n{doc:#}"
    );

    // bhf_parser_version is a free function that always builds+fuzzes; Parser::scan
