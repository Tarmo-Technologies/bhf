...
//
// The planted BHF-201 sits behind a single-byte gate INSIDE the file the target
...
fn java_file_parameter_is_driven_through_a_temp_file_and_finds_the_planted_crash() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-java-fileparam-{}", std::process::id()));
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on java_file_param fixture");
    let combined = format!(
        "{}\n{}",
