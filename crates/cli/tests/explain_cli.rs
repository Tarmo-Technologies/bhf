...
//! End-to-end coverage for `bhf explain`: after fuzzing a crash that is gated
...
fn explain_joins_input_gates_faked_env_and_dataflow() {
    ...
    let work = std::env::temp_dir().join(format!("bhf-explain-{}", std::process::id()));
