...
//! failed to build. bhf now restricts the generated project to its staged Ada
//! closure, so the unrelated colliding C file is outside every `Source_Dirs`
//! entry and the Ada unit — the harness target — wins.
//!
//! Drives the real `bhf auto` against the bundled fixture and asserts the Ada
...
fn same_stem_c_source_excluded_so_ada_harness_builds() {
    ...
    let tmp = tempfile::Builder::new()
        .prefix("bhf-ada-object-collision-")
        .tempdir()
        .expect("tempdir");
    ...
    .expect("read bhf_build.gpr");
    assert!(gpr.contains("bhf_work/src_instrumented"), "{gpr}");
    assert!(
