...
"#,
    ...
    let profile = read_json(&output_dir.join("H-ADA-PROFILE/bhf-project-profile.json"));
    assert_eq!(profile["schema_version"], "bhf.ada_project_profile.v1");
    assert_eq!(
    ...
    assert!(gpr.contains("-- BHF project profile: bhf-project-profile.json"));
