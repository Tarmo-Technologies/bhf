...
// `parse(byte[])` is reachable solely through the fluent builder. `bhf auto`
...
// (BHF-201) — proving the emitted builder-receiver source compiles AND executes.
...
fn java_builder_only_class_synthesises_receiver_and_finds_crash() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-java-builder-{}", std::process::id()));
    ...
    let output = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto on java_builder fixture");
    let combined = format!(
        "{}\n{}",
    ...
    assert!(
        found_bhf201,
        "expected a BHF-201 finding under {} (reached via builder().build()), output:\n{combined}",
        findings.display()
    );
