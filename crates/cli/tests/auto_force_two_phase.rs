...
/// A tree with two endpoints bhf can drive from bytes and one it cannot (a
...
fn run(root: &Path, work: &Path, extra: &[&str]) -> (serde_json::Value, String) {
    ...
    command
        ...
        .arg("1");
    command.args(extra);
    let out = command.output().expect("spawn bhf auto");
