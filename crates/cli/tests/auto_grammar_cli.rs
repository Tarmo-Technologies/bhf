...
//
// `bhf auto --grammar` end-to-end. The grammar's every derivation starts with a
...
// token proves the grammar reached the fuzz loop (published via BHF_GRAMMAR and
...
fn auto_grammar_drives_generation_and_rejects_bad_grammar() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-auto-grammar-{}", std::process::id()));
    ...
    let bad_out = Command::new(&bin)
        ...
        .output()
        .expect("run bhf auto with bad grammar");
    assert!(
