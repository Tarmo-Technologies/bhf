...
fn release_packaging_includes_linux_runtrace_shim() {
    let root = repo_root();
    let shim_manifest = read(root.join("crates/bhf_runtrace_shim/Cargo.toml"));
    ...
    assert!(docs.contains("bhf_runtrace_shim"));
...
fn release_cli_is_self_contained_for_every_harness_runtime() {
    ...
    assert!(workflow.contains("archive=target/distrib/bhf-x86_64-unknown-linux-gnu.tar.xz"));
    assert!(workflow.contains("$archive = Get-Item target/distrib/bhf-x86_64-pc-windows-msvc.zip"));
    assert!(!workflow.contains("-name \"bhf-*.tar.xz\""));
    assert!(!workflow.contains("-Filter \"bhf-*.zip\""));
}
...
fn release_workflow_augments_the_unix_installer_with_rhel7_guidance() {
    ...
    assert!(workflow
        .contains("python3 scripts/augment-release-installer.py target/distrib/bhf-installer.sh"));
    assert!(workflow.contains("BHF_RHEL7_GUIDANCE_BEGIN"));
    assert!(augmenter.contains("BHF_RHEL7_GUIDANCE_BEGIN"));
    for required in [
        "rhel-server-rhscl-7-rpms",
        "llvm-toolset-7.0-clang",
        "llvm-toolset-7.0-compiler-rt",
        "bhf_runtrace_shim-installer.sh",
        "bhf_cc_intercept-installer.sh",
    ] {
...
fn release_workflow_fixes_library_installer_chmod_path() {
    ...
    assert!(workflow.contains("bhf_runtrace_shim-installer.sh"));
    assert!(workflow.contains("bhf_cc_intercept-installer.sh"));
    assert!(fixer_text.contains("$_lib_install_temp/$_lib_name"));

    let tmp =
        std::env::temp_dir().join(format!("bhf-library-installer-fix-{}", std::process::id()));
...
fn release_workflow_makes_powershell_installers_noninteractive_safe() {
    ...
    assert!(workflow.contains("bhf-installer.ps1"));
    assert!(workflow.contains("bhf-daemon-installer.ps1"));

    let tmp = std::env::temp_dir().join(format!(
        "bhf-powershell-installer-fix-{}",
        std::process::id()
    ));
    let _ = fs::remove_dir_all(&tmp);
...
fn release_shell_installers_check_for_xz() {
    ...
    let tmp = std::env::temp_dir().join(format!("bhf-shell-installer-fix-{}", std::process::id()));
    let _ = fs::remove_dir_all(&tmp);
...
fn ci_enforces_the_current_supported_os_matrix() {
    ...
    assert!(ci.contains("bhf-el7-release.tar.gz"));
    ...
    assert!(ci.contains("BHF_RELEASE_VERSION: \"1\""));
...
fn augmented_installer_prints_actionable_rhel7_prerequisites() {
    let root = repo_root();
    let tmp = std::env::temp_dir().join(format!(
        "bhf-release-installer-guidance-{}",
        std::process::id()
    ));
    let _ = fs::remove_dir_all(&tmp);
    fs::create_dir_all(&tmp).expect("create installer test directory");
    let installer = tmp.join("bhf-installer.sh");
...
PRINT_QUIET=0
INFERRED_HOME=/tmp/bhf-installer-test
say() { echo "$1"; }
warn() { say "WARN: $1" >&2; }
...
"#,
    ...
    assert_eq!(augmented.matches("BHF_RHEL7_GUIDANCE_BEGIN").count(), 1);

    let output = Command::new("sh")
        .arg(&installer)
        .env("BHF_OS_RELEASE_FILE", &os_release)
        .output()
