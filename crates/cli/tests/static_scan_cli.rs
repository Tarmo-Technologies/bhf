...
fn static_scan_since_scopes_to_changed_files() {
    ...
    std::env::set_var("BHF_SINCE_REV", "HEAD");
    ...
    std::env::remove_var("BHF_SINCE_REV");
    assert_eq!(exit, 0);
...
fn static_scan_reports_ada_c_and_cpp_findings_with_sarif() {
    ...
    assert_eq!(report["schema_version"], "bhf.static.v1");
    ...
    assert!(finding(&report, "BHF-401", "legacy.c").is_some());
    assert!(finding(&report, "BHF-401", "legacy.cpp").is_some());
    assert!(finding(&report, "BHF-402", "legacy.adb").is_some());

    let c_finding = finding(&report, "BHF-401", "legacy.c").unwrap();
...
fn static_scan_applies_suppressions_and_marks_baseline_status() {
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.static.suppressions.v1",
            "suppressions": [{
                "rule_id": "BHF-401",
                "path": "legacy.c",
                "line": 2,
    ...
    fs::write(
        &baseline,
        serde_json::to_vec_pretty(&json!({
            "schema_version": "bhf.static.v1",
            "findings": [
                {
                    "fingerprint": "BHF-401:legacy.cpp:2:unsafe-string-copy",
                    "rule_id": "BHF-401",
                    "path": "legacy.cpp"
                },
                {
                    "fingerprint": "BHF-999:old.adb:1:retired",
                    "rule_id": "BHF-999",
                    "path": "old.adb"
                }
            ]
    ...
    assert!(finding(&report, "BHF-401", "legacy.c").is_none());
    let cpp_finding = finding(&report, "BHF-401", "legacy.cpp").unwrap();
    assert_eq!(cpp_finding["baseline_status"], "unchanged");
    assert_eq!(
        report["baseline"]["resolved"][0]["fingerprint"],
        "BHF-999:old.adb:1:retired"
    );
...
fn static_scan_emits_interprocedural_taint_traces_and_gaps() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    let command_findings = findings_by_rule(&report, "BHF-304");
...
fn static_scan_resolves_cross_file_taint_calls_for_ada_c_and_cpp() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    let findings = findings_by_rule(&report, "BHF-304");
    assert_eq!(
        findings.len(),
        3,
    ...
    for (entry, stage) in [
        ...
        let finding = finding(&report, "BHF-304", stage).unwrap();
...
fn static_scan_rule_pack_reports_seeded_findings_and_honors_policy() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    for rule_id in [
        "BHF-405", "BHF-407", "BHF-408", "BHF-409", "BHF-410", "BHF-411", "BHF-412",
    ] {
        assert!(
    ...
    assert!(
        findings_by_rule(&report, "BHF-406").is_empty(),
        "policy should disable environment rule BHF-406"
    );
    assert_eq!(
        findings_by_rule(&report, "BHF-408")
            .iter()
    ...
    assert!(finding(&report, "BHF-411", "safe.adb").is_none());
    assert!(finding(&report, "BHF-412", "safe.adb").is_none());
}
...
fn static_scan_reduces_format_and_taint_false_positives() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    let format_findings = findings_by_rule(&report, "BHF-408");
    assert_eq!(
        format_findings.len(),
        2,
    ...
    let taint_findings = findings_by_rule(&report, "BHF-304");
    assert_eq!(
        taint_findings.len(),
        1,
...
fn static_scan_ignores_commented_out_rule_matches() {
    ...
    assert!(
        findings_by_rule(&report, "BHF-401").is_empty(),
        "commented-out unsafe string copies should be ignored: {report:#}"
    );
    assert!(
        findings_by_rule(&report, "BHF-402").is_empty(),
...
fn static_scan_preserves_triage_and_ci_gates_path_sensitive_findings() {
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&json!({
            ...
            "findings": [
                {
                    "fingerprint": "BHF-304:blocked.c:3:command-injection",
                    "rule_id": "BHF-304",
                    "path": "blocked.c",
                    "triage_state": "accepted_risk"
                },
                {
                    "fingerprint": "BHF-401:old.c:1:unsafe-string-copy",
                    "rule_id": "BHF-401",
                    "path": "old.c",
                    "triage_state": "fixed"
                }
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    let blocked = finding(&report, "BHF-304", "blocked.c").unwrap();
    ...
    assert!(markdown.contains("# BHF Static Scan"));
    ...
    let result = sarif["runs"][0]["results"]
        ...
        .unwrap()
        .iter()
        .find(|result| result["ruleId"] == "BHF-304")
        .unwrap();
...
fn static_scan_tracks_local_taint_aliases_across_ada_c_and_cpp() {
    ...
    for path in ["alias.c", "alias.cpp", "alias.adb"] {
        let finding = finding(&report, "BHF-304", path).unwrap();
        assert!(
...
fn static_scan_marks_multiline_impossible_guarded_taint_as_blocked() {
    ...
    let report: serde_json::Value =
        serde_json::from_slice(&fs::read(out.join("static-report.json")).unwrap()).unwrap();
    let finding = finding(&report, "BHF-304", "guard.c").unwrap();
...
fn static_scan_sloc_writes_text_table_and_json() {
    ...
    assert_eq!(report["schema_version"], "bhf.static.sloc.v1");
...
fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-static-scan-{name}-{nonce}"));
