...
/// limit. `BHF_MAX_SOURCE_FILE_BYTES` always overrides the derived value.
        ...
        return Err(std::io::Error::new(
            ...
            format!(
                "source file is {len} bytes, above the {limit}-byte safety limit; \
                 raise BHF_MAX_SOURCE_FILE_BYTES to parse it"
            ),
        ));
    }
...
mod tests {
    ...
    fn read_source_text_transcodes_latin1_file() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("bhf-latin1-{nonce}.ads"));
    ...
    fn read_source_text_propagates_missing_file() {
        let path = std::env::temp_dir().join("bhf-does-not-exist-zzz.ads");
