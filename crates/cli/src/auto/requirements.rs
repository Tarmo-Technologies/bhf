    ...
    for candidate in candidates {
        ...
        if candidate.lang == Lang::Ada {
            ...
            manifest.push_merge_detailed(
                ...
                false,
                Some(format!(
                    "stage a GNAT cross compiler and Ada runtime matching '{target_name}'; BHF auto does not yet drive foreign Ada targets automatically"
                )),
        ...
        if let Some(target) = crate::auto::cross_target::resolve_cross_target(guard) {
            ...
            manifest.push_merge_detailed(
                ...
                true,
                Some(format!(
                    "stage a compatible {} SDK/runtime and a runnable target environment; BHF used a host-side platform stub",
                    stub.platform
                )),
                RequirementBasis::Inferred,
            ...
            manifest.push_merge_detailed(
                ...
                RequirementBasis::Inferred,
                Some("BHF has no exact cross-target mapping for this guard".to_owned()),
            );
        }
    }
}
...
fn add_declared_generated_outputs(
    ...
    // A compile database in bhf's CMake probe directory is written only
        ...
        for token in tokens.iter().skip(1) {
            for source in token.split(';') {
                ...
                manifest.push_merge_detailed(
                    ...
                    false,
                    Some(format!(
                        "restore or transfer the exact source '{source}' declared by {}; BHF cannot synthesize its implementation semantics",
                        declared_by.display()
                    )),
                    RequirementBasis::Declared,
                    Some(format!(
