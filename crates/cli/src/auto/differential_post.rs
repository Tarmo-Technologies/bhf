...
//! hides. Each distinct divergence becomes a BHF-301 finding, so it flows through
//! the same report/confirmation path as any fuzz crash.
//!
//! Comparison is on **exit status** (including crash signals and timeout), not
//! stdout: bhf harnesses suppress the target's stdout under the fork-server
//! driver, so exit divergence is the reliable signal. (The standalone `bhf
...
fn differential_one(
    ...
    let bin_a = match build_side(hdir, is_cpp, &spec.cc_a, &spec.cxx_a, "a") {
        ...
        None => {
            bhfeprintln!(
                "bhf auto: differential — {harness_id}: build under {} failed; skipping",
