...
//! Rust-side reader for the JSONL events `libbhf_runtrace.so`
...
pub enum RuntraceEvent {
    ...
    /// destructive-path sink oracle (BHF-440).
    DestructiveFsOp {
        api: String,
        path: String,
        taint_offset: Option<u32>,
...
/// unclosed open of any path is still a BHF-306 leak).
...
pub enum SinkClass {
    /// A path reaching a file-open API (open/openat/fopen/...). BHF-405, CWE-22.
    OpenPath,
    ...
    /// posix_spawn). BHF-431, CWE-78.
    ProcessExec,
    /// A destination reaching a network-egress API (getaddrinfo/connect).
    /// BHF-433, CWE-918.
    NetworkEgress,
    /// A path reaching a dynamic library loader (dlopen/dlmopen). BHF-435,
    /// CWE-427.
    LibraryLoad,
    /// SQL text reaching a database-execution API. BHF-441, CWE-89.
    SqlQuery,
}
...
///
/// The "never untainted" clause is load-bearing: bhf harvests the target's
...
fn sink_tracker_limits() -> (usize, usize, usize) {
    let subject_bytes =
        crate::resource_limits::env_usize("BHF_MAX_SINK_SUBJECT_BYTES").unwrap_or(16 * 1024);
    let representative_input_bytes =
        crate::resource_limits::env_usize("BHF_MAX_SINK_INPUT_BYTES").unwrap_or(32 * 1024);
    let budget = crate::resource_limits::dynamic_bytes(
        "BHF_MAX_SINK_TRACKING_BYTES",
        128,
    ...
    let entry_limit =
        crate::resource_limits::env_usize("BHF_MAX_SINK_SUBJECTS").unwrap_or(derived_entries);
...
/// registry (BHF-405/431/433/435/439/440).
...
fn oracle_runtime_event(
    ...
    match event {
        ...
        // (BHF-304). Taint-confirmed command injection (BHF-431) is emitted at
...
/// File resources that bhf's own infrastructure opens during a run — the
...
/// fds (`/dev/null`). The BHF-306 resource-leak oracle is for resources the
/// *target* leaks; reporting one of these as a leak is a false positive on
/// bhf's own plumbing — the harness reads the input file / mmaps the
...
/// matches nothing here and still trips BHF-306.
pub fn is_bhf_owned_resource(path: &str) -> bool {
    ...
    // `bhf_make_tempfile()` materializes fuzz bytes at `/tmp/bhf_inXXXXXX`;
    // attempt/stub scratch and the fake-env dir live under `/tmp/bhf*`
    // (`std::env::temp_dir()` is `/tmp`).
    const OWNED_PREFIXES: &[&str] = &["/tmp/bhf"];
    ...
    for event in events {
        match event {
            RuntraceEvent::FileOpened {
                ...
                // Bhf's own input-plumbing / scaffolding opens must never
                // count toward a target resource leak (the FP this guards).
                if is_bhf_owned_resource(path) {
                    continue;
                }
                ...
                // opens an ATTACKER-CONTROLLED path and leaks it trips BHF-306.
...
/// untainted open carries `None` and never confirms BHF-405.
...
mod tests {
    ...
    #[test]
    fn internal_bhf_getenv_events_are_dropped() {
        let log = concat!(
            "{\"e\":\"getenv\",\"n\":\"BHF_FAKE_IDENTITY\",\"r\":null}\n",
            "{\"e\":\"getenv\",\"n\":\"ACME_HOME\",\"r\":null}\n",
        );
    ...
    fn oracle_hits_from_events_flags_world_writable_mkdir() {
        ...
        assert!(
            hits.iter()
                .any(|h| h.rule_id == "BHF-416" && h.api == "mkdir"),
            "world-writable mkdir must flag BHF-416"
        );
    }
    ...
    fn oracle_hits_from_events_maps_parent_path_file_events() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-101");
        assert_eq!(hits[0].api, "open");
        assert_eq!(hits[0].evidence_value("path"), Some("../secrets/token.txt"));
    }

    #[test]
    fn raw_tainted_open_does_not_emit_bhf405_per_input() {
        // BHF-405 is confirmed only by cross-execution correlation, never
        ...
        assert!(
            !hits.iter().any(|h| h.rule_id == "BHF-405"),
            "BHF-405 must not be emitted from the per-input oracle pass"
        );
    }
    ...
    fn sink_tracker_confirms_consistently_tainted_open_path() {
        ...
        // untainted) is a genuine fuzz-controlled open → BHF-405.
        ...
        let hit = confirmed_sink_hit(&confirmed[0]).expect("confirmed open yields BHF-405");
        assert_eq!(hit.rule_id, "BHF-405");
        ...
        assert!(
            ...
            "BHF-405 hit must carry a source→sink taint path"
        );
    }
    ...
    #[test]
    fn raw_tainted_command_does_not_emit_bhf431_per_input() {
        // BHF-431 is confirmed only by cross-execution correlation, never by a
        ...
        assert!(
            !hits.iter().any(|h| h.rule_id == "BHF-431"),
            "BHF-431 must not be emitted from the per-input oracle pass"
        );
    }
    ...
    fn sink_tracker_confirms_consistently_tainted_command() {
        ...
        // untainted) is a genuine fuzz-controlled shell exec → BHF-431.
        let mut tracker = SinkTaintTracker::default();
        tracker.observe(
            &[RuntraceEvent::CommandExecuted {
                api: "system".to_owned(),
        ...
        let hit = confirmed_sink_hit(&confirmed[0]).expect("confirmed command yields BHF-431");
        assert_eq!(hit.rule_id, "BHF-431");
        assert_eq!(hit.oracle_name, "command-controlled-runtime");
        assert_eq!(hit.api, "system");
        assert!(
            hit.evidence_value("taint_path")
                .is_some_and(|p| p.contains("→ system(command)")),
            "BHF-431 hit must carry a source→sink taint path"
        );
    }
    ...
    fn oracle_hits_from_events_correlates_toctou_check_then_open() {
        ...
        let toctou: Vec<_> = hits.iter().filter(|h| h.rule_id == "BHF-418").collect();
    ...
    fn oracle_hits_from_events_ignores_check_and_open_of_different_paths() {
        ...
        assert!(
            !hits.iter().any(|h| h.rule_id == "BHF-418"),
    ...
    fn oracle_hits_from_events_maps_network_egress_events() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-303");
    ...
    fn oracle_hits_from_events_maps_sensitive_env_access() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-305");
    ...
    fn oracle_hits_from_events_maps_relative_dlopen_library() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-413");
    ...
    fn oracle_hits_from_events_maps_command_injection() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-304");
    ...
    fn oracle_hits_from_events_maps_controlled_format_string() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-408");
    ...
    fn oracle_hits_from_events_maps_handled_runtime_constraint_check() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-102");
    ...
    fn oracle_hits_from_events_maps_handled_runtime_storage_error() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-103");
        assert_eq!(hits[0].api, "ada-runtime");
    ...
    fn oracle_hits_from_events_maps_handled_runtime_tasking_error() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-104");
        assert_eq!(hits[0].api, "ada-runtime");
    ...
    fn oracle_hits_from_events_maps_handled_runtime_user_exception() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-105");
        assert_eq!(hits[0].api, "ada-runtime");
        assert_eq!(
            hits[0].evidence_value("exception"),
    ...
    fn oracle_hits_from_events_maps_native_assertion_contract() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-415");
    ...
    fn oracle_hits_from_events_maps_unclosed_file_descriptor() {
        ...
        assert_eq!(hits[0].rule_id, "BHF-306");
        assert_eq!(hits[0].api, "open");
    ...
    #[test]
    fn is_bhf_owned_resource_ignores_infra_but_not_target_paths() {
        // bhf's own input-plumbing / scaffolding fds — never a leak.
        for owned in [
            "/home/u/campaign-work/work/mpack/fuzz_inputs/input-1545529-99.bin",
            "/tmp/bhf_inwZay20",
            "/tmp/bhf/fake_env/ACME_HOME",
            "/tmp/bhf-attempt-foo-3",
            ...
            assert!(
                is_bhf_owned_resource(owned),
                "{owned} must be treated as bhf-owned"
            );
        }
        ...
        for target in [
            ...
            assert!(
                !is_bhf_owned_resource(target),
                "{target} must NOT be treated as bhf-owned"
            );
        }
    }

    #[test]
    fn oracle_hits_from_events_ignores_bhf_owned_unclosed_open() {
        // The false positive: bhf's per-iteration fuzz INPUT file (and its
        // /tmp/bhf_* temp scaffolding) left open at testcase exit is bhf's
        // own plumbing, not a target leak — BHF-306 must not fire.
        for owned in [
            "/home/u/campaign-work/work/mpack/fuzz_inputs/input-1545529-99.bin",
            "/tmp/bhf_inwZay20",
        ] {
            ...
            assert!(
                !hits.iter().any(|h| h.rule_id == "BHF-306"),
                "{owned} must not be reported as a BHF-306 resource leak"
            );
        }
    }
    ...
    fn oracle_hits_from_events_still_flags_genuine_target_leak() {
        ...
        // genuine resource leak and MUST still trip BHF-306.
        let hits = oracle_hits_from_events(&[RuntraceEvent::FileOpened {
            syscall: "open".to_owned(),
            fd: 9,
        ...
        assert!(
            hits.iter().any(
                |h| h.rule_id == "BHF-306" && h.evidence_value("path") == Some("/etc/hostname")
            ),
            "an unclosed open of a genuine user-tree path must still trip BHF-306"
        );
    }
    ...
    fn interpreted_lane_leak_is_taint_gated() {
        ...
        // path) must NOT trip BHF-306 — it's not the target's leak.
        ...
        assert!(
            tainted.iter().any(|h| h.rule_id == "BHF-306"),
