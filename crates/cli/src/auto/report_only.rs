...
//! — bhf does not silently drop it. It runs the existing source static
...
//! [`Outcome::ReportOnly`].
//!
//! This reuses [`static_analysis::scan`] (the engine behind `bhf
...
fn static_finding_record(
    ...
    let entity = f
        ...
        .unwrap_or_default();
    // #1: source→sink dataflow. An interprocedural taint rule (BHF-304/405/419/…)
    // records the flow in `analysis.trace`; a pure pattern rule (BHF-401 unsafe
...
pub fn emit_tree_static_findings(root: &Path, work: &Path) -> usize {
    ...
    // The scan walks `root`; when the bhf work-dir lives INSIDE `root` (any
    // `--work-dir`, not just the name-excluded `bhf_work`), it would flag
    // bhf's OWN generated harnesses/stubs/runtime copies — CWE-22/CWE-15 noise
...
/// Whether a scan-reported path lies inside the bhf work-dir (its generated
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> PathBuf {
        ...
        p.push(format!("bhf-ro-{tag}-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&p);
        std::fs::create_dir_all(&p).unwrap();
        p
    }

    /// `--static` must not flag bhf's OWN generated harness: a `--work-dir`
    ...
    fn tree_scan_excludes_the_work_dir() {
        ...
        .unwrap();
        // The bhf work-dir lives INSIDE the tree, named `work` (NOT the
        // name-excluded `bhf_work`), holding a generated harness with the same
    ...
    /// (BHF-401 unsafe strcpy) has an EMPTY trace, so both stay empty — we do NOT
    /// fabricate a flow.
    #[test]
    fn taint_finding_records_data_flow_pattern_finding_does_not() {
        ...
        .unwrap();
        // A pure pattern hit (BHF-401 unsafe strcpy) with no traced origin.
        std::fs::write(
            root.join("weak.c"),
            "#include <string.h>\nint f(char *d, char *s){ strcpy(d, s); return 0; }\n",
        )
        ...
        assert!(saw_pattern, "expected a BHF-401 pattern finding with none");
    }
    ...
    fn report_only_emits_cwe_tagged_static_finding() {
        ...
        // A K&R-style definition with an unbounded strcpy — BHF-401 (CWE-120/787).
        std::fs::write(
            &src,
            "int copy(dst, s)\n    char *dst;\n    char *s;\n{\n    strcpy(dst, s);\n    return 0;\n}\n",
        )
