...
//! Native Perl fuzzing lane (M3.2): generate a bhf harness module, copy the
//! `perl_runtime` (driver + Devel::BhfCov), and emit a `harnesses/<id>/main`
//! launcher that runs the target under `perl -d:BhfCov` speaking the
//! `BHF_FRAMED` fork-server protocol with DB::DB edge coverage into the shared
...
/// Locate the bundled `perl_runtime/` (driver + Devel/BhfCov.pm).
fn locate_perl_runtime() -> Option<PathBuf> {
    crate::runtime_assets::locate("perl_runtime", "bhf_driver.pl")
}
...
pub fn build_perl_harness(
    ...
    let Some(runtime) = locate_perl_runtime() else {
        return PerlBuildResult::Failed {
            reason: "could not locate the bundled perl_runtime/ (driver + Devel::BhfCov)"
                .to_owned(),
            skip: false,
        };
    };
    ...
    if let Err(e) = std::fs::copy(
        runtime.join("bhf_driver.pl"),
        auto_dir.join("bhf_driver.pl"),
    ) {
        return PerlBuildResult::Failed {
    ...
    if let Err(e) = std::fs::copy(runtime.join("Devel/BhfCov.pm"), devel_dir.join("BhfCov.pm")) {
        return PerlBuildResult::Failed {
    ...
    let harness_path = auto_dir.join("bhfgen.pm");
    ...
    // Emit the launcher. Carries the BHF_FRAMED marker the engine greps for.
    let main_path = auto_dir.join("main");
    let driver = auto_dir.join("bhf_driver.pl");
    let top_pkg = sub.package.split("::").next().unwrap_or(&sub.package);
    let script = format!(
        "#!/bin/sh\n\
         # BHF_FRAMED BHF_PL_LAUNCHER bhf Perl driver launcher (native Perl lane).\n\
         # The engine sets BHF_FRAMED + BHF_COV_SHM; perl inherits them. Run under\n\
         # `perl -d:BhfCov` so DB::DB records edge coverage into the shared map.\n\
         BHF_HARNESS=\"{harness}\" \\\n\
         BHF_TARGET_PACKAGE=\"{pkg}\" \\\n\
         BHF_TRACE_PREFIX=\"{trace}\" \\\n\
         BHF_COVERED_LINES=\"{covered}\" \\\n\
         PERL5LIB=\"{perl5lib}:${{PERL5LIB}}\" \\\n\
         exec \"{perl}\" -d:BhfCov \"{driver}\" \"$@\"\n",
...
/// Emit `bhfgen.pm` exposing `bhfgen::bhf_run_one($bytes)`.
fn generate_harness(target_abs: &Path, call: &str, sub_name: &str) -> String {
    format!(
        "# SPDX-License-Identifier: Apache-2.0\n\
         # Generated by bhf (native Perl lane). Loads the target and passes the\n\
         ...
         BEGIN {{ *CORE::GLOBAL::exit = sub {{ die \"bhf_load_exit\\n\" }}; }}\n\
         {{\n\
         ...
         \x20   package main;\n\
         \x20   local @ARGV = ();\n\
         \x20   open(my $bhf_devnull, '>', '/dev/null');\n\
         \x20   my $bhf_saved;\n\
         \x20   $bhf_saved = select($bhf_devnull) if $bhf_devnull;\n\
         \x20   eval {{ require q{{{target}}}; 1 }};\n\
         \x20   my $bhf_err = $@;\n\
         \x20   select($bhf_saved) if defined $bhf_saved;\n\
         \x20   unless (defined &{{'{sub_name}'}}) {{\n\
         \x20       die $bhf_err || \"bhf: `{sub_name}` undefined after load\\n\";\n\
         \x20   }}\n\
         }}\n\
         \n\
         my $bhf_target_entered = 0;\n\
         sub bhf_mark_target_entry {{\n\
         \x20   return if $bhf_target_entered;\n\
         \x20   my $path = $ENV{{BHF_TARGET_ENTRY_SHM}} or return;\n\
         \x20   if (open(my $entry, '>', $path)) {{\n\
         \x20       binmode($entry);\n\
         \x20       print {{$entry}} \"\\x01\";\n\
         \x20       close($entry);\n\
         \x20       $bhf_target_entered = 1;\n\
         \x20   }}\n\
         }}\n\
         \n\
         sub bhf_run_one {{\n\
...
mod tests {
    ...
    fn harness_loads_target_and_calls_function() {
        ...
        assert!(h.contains("sub bhf_run_one"));
        assert!(
    ...
    fn bundled_driver_runs_on_perl_5_6_no_defined_or() {
        ...
        let driver = std::fs::read_to_string(runtime.join("bhf_driver.pl")).unwrap();
