...
//! Keyboard control plane for a live `bhf auto` run.
//!
...
pub fn apply(action: Action, status: &RunStatus) -> Option<String> {
    ...
    match action {
        Action::Quit => {
            ...
            status.request_quit();
            Some(
                "bhf auto: [q] stopping — no new targets will start; \
        ...
        Action::Jobs(delta) => {
            ...
            if after == before {
                ...
                return Some(format!("bhf auto: jobs unchanged ({before}) — {edge}"));
            }
            Some(format!(
                "bhf auto: jobs {before} → {after} (ceiling {})",
                status.jobs_ceiling()
            ))
        }
        Action::Pause => {
            ...
            Some(if paused {
                "bhf auto: [p] paused — in-flight targets finish, nothing new starts; \
                 press [p] again to resume"
                    .to_owned()
            } else {
                "bhf auto: [p] resumed".to_owned()
            })
        }
        Action::Cap(delta) => {
            ...
            if !changed {
                return Some(match (before, delta > 0) {
                    (None, true) => "bhf auto: no --max-targets cap on this run — every \
                                     candidate is already attempted"
                        .to_owned(),
                    _ => format!(
                        "bhf auto: --max-targets unchanged ({})",
        ...
        Action::TargetTime(delta) => {
            if status.budget_locked() {
                return Some(
                    "bhf auto: per-target time is set by the --campaign-time / \
            ...
            if after == before {
                return Some(format!(
                    "bhf auto: per-target time unchanged ({before}s — already at the floor)"
                ));
            }
            Some(format!(
                "bhf auto: --per-target-time {before}s → {after}s (applies from the next \
        ...
        Action::Force => {
            ...
            Some(if force {
                "bhf auto: --force ON — a second, forced pass will retry every target this \
                 pass could not fuzz (findings stamped low-confidence)"
                    .to_owned()
            } else {
                "bhf auto: --force OFF — no forced pass will run".to_owned()
            })
        }
        Action::Verbose => {
