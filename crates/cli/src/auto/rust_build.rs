...
//! Native Rust fuzzing lane: generate a bhf harness, build it as a
...
//!    `rust_runtime`, plus a copy of `c_runtime/bhf_driver.c` renamed
//!    `main.c` (the engine greps the sibling source for `BHF_FRAMED`).
    ...
    params
        ...
        .map(|p| {
            ...
            if let Some((leaf, is_mut, is_generic)) = scratch_slice(&p.ty) {
                ...
                {
                    return Some((
                        "core::array::from_fn::<_, 16, _>(|_bhf_idx| Default::default())"
                            .to_owned(),
                        by_ref,
                    ));
                }
...
fn resolve_target(candidate: &Candidate) -> Result<ResolvedTarget, String> {
    ...
    // input it violates the precondition and fabricates a false critical BHF-203/CWE-121
...
/// `bhf_decode.h`). Mirrors `generate_harness::locate_c_runtime` so the Rust
...
fn locate_c_runtime_dir() -> Option<PathBuf> {
    crate::runtime_assets::locate("c_runtime", "bhf_driver.c")
}
...
pub fn build_rust_harness(
    ...
    let Some(c_runtime_dir) = locate_c_runtime_dir() else {
        return RustBuildResult::Failed {
            reason: "could not locate c_runtime/bhf_driver.c for the Rust fork-server driver"
                .to_owned(),
            skip: false,
        };
    };
    ...
    // Copy the C driver beside the binary as `main.c` (the BHF_FRAMED marker
    // the engine greps for).
    let driver_src = c_runtime_dir.join("bhf_driver.c");
...
/// as a `#[doc(hidden)] pub mod __bhf_harness;` of its lib root (so its
...
fn build_in_crate(
    ...
    if let Err(e) = std::fs::write(src.join("__bhf_harness.rs"), &harness_module) {
        return RustBuildResult::Failed {
    ...
    text.push_str(
        "\n// GENERATED by bhf — in-crate fuzz harness module (§27.10).\n\
         #[doc(hidden)]\npub mod __bhf_harness;\n",
    );
    ...
    // Build the copied crate (now a staticlib exporting `bhf_run_one`).
...
/// consumed. `BHF_KEEP_BUILD_ARTIFACTS=1` is an emergency debugging escape
...
impl CargoTargetCleanup {
    fn new(path: PathBuf) -> Self {
        let keep = std::env::var("BHF_KEEP_BUILD_ARTIFACTS")
            .ok()
            .is_some_and(|value| {
                matches!(
...
/// Rewrite a copied workspace-MEMBER manifest so the DETACHED copy (bhf appends
...
/// target crate + `rust_runtime`, and `src/lib.rs` holding `bhf_run_one`.
fn emit_harness_crate(
    ...
    let cargo_toml = format!(
        "# SPDX-License-Identifier: Apache-2.0\n\
         # GENERATED by bhf — staticlib harness crate.\n\
         # An explicit empty [workspace] makes this crate its own workspace root so\n\
         # cargo does NOT walk up and bind it to an ancestor [workspace] — the bhf\n\
         # worktree (where bhf_work/ lives) or the target crate's own Cargo\n\
...
/// Locate the `rust_runtime` crate dir relative to the running `bhf` binary
/// or the source tree. Tries, in order: a `BHF_RUST_RUNTIME_DIR` override,
/// the workspace path relative to the executable, and a few source-tree guesses.
fn locate_rust_runtime() -> Option<PathBuf> {
    if let Ok(dir) = std::env::var("BHF_RUST_RUNTIME_DIR") {
...
/// Find the first `libbhf_rust_harness*.a` staticlib under
    ...
    for entry in entries.flatten() {
        ...
        if path.extension().and_then(|e| e.to_str()) == Some("a") {
            if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                if name.starts_with("libbhf_rust_harness") {
                    return Some(path);
                }
            }
        }
...
mod tests {
    ...
    fn emit_harness_crate_renames_a_hyphenated_target_package() {
        ...
        if locate_rust_runtime().is_none() {
            bhfeprintln!("skipping: rust_runtime crate not locatable");
            return;
        }
    ...
    fn find_staticlib_looks_under_target_triple_debug() {
        ...
        let ours = debug.join("libbhf_rust_harness.a");
    ...
    fn emitted_cargo_toml_detaches_from_ancestor_workspace() {
        ...
        // ancestor `[workspace]` — the bhf worktree root (where `bhf_work/`
    ...
    fn prepare_incrate_manifest_resolves_workspace_inherited_fields() {
        ...
        // (bhf appends its own `[workspace]`), still parse: NO `*.workspace = true`
    ...
    fn incrate_harness_uses_unsafe_no_mangle() {
        ...
        let harness = "#[no_mangle]\npub extern \"C\" fn bhf_run_one(d: *const u8, l: usize) \
                       -> i32 { 0 }\n";
        ...
        assert!(out.contains("pub extern \"C\" fn bhf_run_one"), "{out}");
    }
    ...
    fn resolve_target_skips_unsafe_fn_primary_target() {
        ...
        // fuzz input violates the contract and fabricates a false BHF-203 stack
