...
//!
//! Strategy (like the Ruby/Perl/Python lanes): reuse bhf's builtin engine over
...
//! `lua lua_runtime/bhf_driver.lua`, which `dofile`s the target, calls the
//! function with the fuzz bytes, records per-line edge coverage via a
//! `debug.sethook` line hook folded into the shared `BHF_COV_SHM` bitmap, and
