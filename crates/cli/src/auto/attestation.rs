...
//!
//! bhf's differentiator is correlating static + dynamic evidence: a finding is
...
pub(crate) fn write_attestation(
    ...
    let statement = json!({
        ...
        "predicateType": "https://buildharnessfuzz.dev/attestation/fuzz-assurance/v1",
        "predicate": {
            "tool": { "name": "bhf", "version": env!("CARGO_PKG_VERSION") },
...
mod tests {
    ...
    fn attestation_tiers_findings_and_self_anchors() {
        let tmp = std::env::temp_dir().join(format!("bhf-attest-{}", std::process::id()));
        ...
        write_finding(
            &tmp,
            "F-STATIC-0001",
            json!({"id":"F-STATIC-0001","rule_id":"BHF-401","cwe":"CWE-120","severity":"high",
                   "confirmation":"fuzz_confirmed","location":{"path":"a.c","line":8}}),
        );
        write_finding(
            &tmp,
            "F-STATIC-0002",
            json!({"id":"F-STATIC-0002","rule_id":"BHF-427","cwe":"CWE-918","severity":"medium",
        ...
        write_finding(
            &tmp,
            "F-0003",
            json!({"id":"F-0003","rule_id":"BHF-202","cwe":"CWE-416","severity":"critical",
        ...
        assert_eq!(
            doc["predicateType"],
            "https://buildharnessfuzz.dev/attestation/fuzz-assurance/v1"
        );
