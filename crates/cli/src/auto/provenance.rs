...
//!
//! This module distinguishes the two, and it is a bhf differentiator: no other
...
fn max_provenance_sidecar_bytes() -> u64 {
    crate::resource_limits::env_usize("BHF_MAX_PROVENANCE_SIDECAR_BYTES")
...
/// gets a `bhf__stub_fired("<name>")` call injected as its first statement (the
...
pub fn poison_stub_source(src: &str) -> Option<(String, Vec<String>)> {
        ...
        if let Some((rt, name)) = parse_weak_stub_open(line) {
            if !is_void_return(&rt) {
                ...
                if let Some(idx) = line.rfind('{') {
                    ...
                    out.push_str(head);
                    out.push_str(&format!(" bhf__stub_fired(\"{name}\");"));
                    out.push_str(tail);
                    out.push('\n');
                    poisoned.push(name);
                    continue;
    ...
    let prologue = "\
/* bhf build-recovery provenance: injected marker for value-returning stubs. */
#include <stdio.h>
#include <stdlib.h>
static void bhf__stub_fired(const char *name) {
    const char *p = getenv(\"BHF_STUB_TRACE\");
...
__attribute__((weak)) void audit_log(const char * _bhf_p0) {
    return;
}
__attribute__((weak)) char * acquire_scratch(void) {
    return NULL;
}
__attribute__((weak)) int compute_len(const char * _bhf_p0) {
    return 0;
}
";
    ...
    fn parses_weak_stub_openings() {
        assert_eq!(
            parse_weak_stub_open("__attribute__((weak)) void audit_log(const char * _bhf_p0) {"),
        ...
        assert_eq!(
            parse_weak_stub_open("__attribute__((weak)) int compute_len(const char * _bhf_p0) {"),
    ...
    fn poisons_only_value_stubs() {
        ...
        assert!(poisoned.contains("bhf__stub_fired(\"acquire_scratch\");"));
        assert!(poisoned.contains("bhf__stub_fired(\"compute_len\");"));
        // The void stub's body is untouched (no marker on its line).
        let audit_line = poisoned.lines().find(|l| l.contains("audit_log(")).unwrap();
        assert!(!audit_line.contains("bhf__stub_fired"));
        // Helper + headers are prepended, and the original bodies survive.
        assert!(poisoned.contains("static void bhf__stub_fired"));
    ...
    fn stub_artifact_reclassify_demotes_and_names_stub() {
        let dir = std::env::temp_dir().join(format!("bhf-prov-{}", std::process::id()));
    ...
    fn real_defect_certify_raises_confidence() {
        let dir = std::env::temp_dir().join(format!("bhf-prov-real-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let fj = dir.join("finding.json");
        write_json(
