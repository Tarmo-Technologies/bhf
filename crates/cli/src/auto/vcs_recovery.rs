...
/// `HEAD`, under the BHF work directory. The source tree remains untouched;
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let path = std::env::temp_dir().join(format!("bhf-vcs-recovery-{unique}"));
    ...
    fn deleted_tracked_file_is_recovered_only_into_work_dir() {
        ...
        assert!(git(
            ...
            &[
                ...
                "user.name=BHF Test",
                "-c",
                "user.email=bhf@example.invalid",
                "commit",
                "--quiet",
                "-m",
                "fixture",
