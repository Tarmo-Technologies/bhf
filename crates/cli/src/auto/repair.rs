...
/// `prepare_layout` copies these next to the generated `bhf_build.gpr` so
...
/// or a struct tag). When true, bhf must not force-include a `void *` alias
    ...
    match repair {
        ...
        Repair::HeaderForward {
            ...
            std::fs::write(
                &p,
                format!(
                    "#pragma once\n/* bhf: forward installed include spelling to real in-tree header */\n#include \"{escaped}\"\n"
                ),
            )?;
            Ok(ApplyOutcome {
                extra_sources: vec![],
        ...
        Repair::StubGprImport { project } => {
            ...
            // copies these next to bhf_build.gpr each round so the import is on
...
fn relative_include_search_dirs(base: &Path) -> std::io::Result<Vec<PathBuf>> {
    ...
    for depth in 1..=4 {
        nested = nested.join(format!(".bhf-relative-{depth}"));
...
fn synth_ada_package_spec_with_ops(
    ...
    content.push_str("--  Auto-stubbed by bhf from project/source inference.\n");
...
pub(crate) fn plan_repair_forced_with_source_policy(
    ...
    | BuildErrorKind::MissingMacro { name, .. } = error
    {
        ...
        //
        // — errors BHF created. When the tree owns the name, the ordinary type
    ...
    match error {
        ...
        BuildErrorKind::UndeclaredFunction { name, file, .. } => {
            ...
            if source_defines_function(file, name) {
                ...
                //
                // because BHF had written `#define syscall`. That is worse than
...
mod tests {
    ...
    fn tmpdir() -> std::path::PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-repair-{nonce}"));
    ...
    fn declared_stub_preserves_header_backed_parameter_and_enum_types() {
        ...
        let stubs = fs::read_to_string(repairs.join(AUTO_STUBS_FILE)).unwrap();
        assert!(
            stubs.contains("const allocator_t * _bhf_p1"),
            "allocator type must match its included prototype: {stubs}"
        );
        assert!(
            stubs.contains("enum parser_type _bhf_p1"),
    ...
    fn declared_stub_does_not_directly_include_umbrella_only_header() {
        ...
        assert!(
            stubs.contains("void string_free(void * _bhf_p0)"),
            "got: {stubs}"
        );
    }
    ...
    fn variadic_dependency_prefers_declared_stub_over_implementation_source() {
        ...
        let stubs = fs::read_to_string(repairs.join(AUTO_STUBS_FILE)).unwrap();
        assert!(
            stubs.contains("warningf(int _bhf_p0, const char * _bhf_p1, ...)"),
    ...
    fn undeclared_call_to_real_later_definition_restores_only_prototype() {
        ...
        let forced = fs::read_to_string(repairs.join(AUTO_CPP_INCLUDES_FILE)).unwrap();
        assert!(
            forced.contains("extern int target(int _bhf_p0);"),
            "{forced}"
        );
        assert!(
    ...
    fn standard_header_is_prepended_before_existing_prototype() {
        ...
        fs::write(
            ...
            "extern void consume(size_t _bhf_p0);\n",
        )
        .unwrap();
        apply_repair(
    ...
    fn a_standard_libc_call_is_never_defined_away_as_a_calling_convention_macro() {
        // An empty `#define mkstemp` erases the CALL in bhf's own decoder
        // (`c_runtime/bhf_decode.h`: `fd = mkstemp(path_buf);` becomes
    ...
    fn demangled_cpp_signature_uses_typed_cpp_declaration_stub() {
        ...
        assert!(stub.contains("const void * _bhf_p0"), "{stub}");
        assert!(!stub.contains("key _bhf_p0"), "{stub}");
    ...
    fn fundamental_declared_stub_is_republished_to_calling_tus() {
        ...
        let forced = fs::read_to_string(repairs.join(AUTO_CPP_INCLUDES_FILE)).unwrap();
        assert!(forced.contains("extern int rand_s(unsigned int * _bhf_p0);"));
    }
    ...
    fn header_placeholder_rejects_path_traversal() {
        ...
        for hostile in ["../../victim.txt", "/etc/cron.d/bhf_pwn", "../victim.txt"] {
