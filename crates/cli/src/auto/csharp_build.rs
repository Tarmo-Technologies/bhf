...
//!      `.csproj`.
//!   2. Generate a `BhfEntry.Run(byte[])` shim that statically calls the
...
//!      its deps, SharpFuzz.Common.dll, and `bhf_harness.dll`).
//!   4. Instrument the target dll's IL with `sharpfuzz` (edge coverage into the
//!      shared map).
//!   5. Emit the `harnesses/<id>/main` launcher: `exec dotnet .../bhf_harness.dll`
//!      with `BHF_CS_NAMESPACE` + `BHF_EXPECTED_EXCEPTIONS`. The engine
//!      sets `BHF_FRAMED` + `BHF_COV_SHM` across the exec and drives the
...
/// Generate the `BhfEntry.Run(byte[])` shim: a static call into the target.
fn generate_entry(method: &CSharpMethod, receiver_kind: Receiver) -> String {
    let (receiver_setup, receiver) = match receiver_kind {
        ...
        Receiver::New => (
            format!(
                "    var bhfReceiver = new global::{}();\n",
                method.type_name
            ),
            "bhfReceiver".to_owned(),
        ),
        Receiver::Uninitialized => (
            format!(
                "    var bhfReceiver = ((global::{}) BhfUninitialized(typeof(global::{})));\n",
    ...
    let arg_setup = decoded_args
        .iter()
        .enumerate()
        .map(|(index, expression)| format!("    var bhfArg{index} = {expression};\n"))
        .collect::<String>();
    let args = (0..decoded_args.len())
        .map(|index| format!("bhfArg{index}"))
        .collect::<Vec<_>>();
    ...
    let helper = if receiver_kind == Receiver::Uninitialized {
        "\x20   static object BhfUninitialized(System.Type t) {\n\
         ...
         \x20     throw new System.NotSupportedException(\"bhf: no uninitialized-object primitive\");\n\
         \x20   }\n"
    } else {
        ""
    };
    format!(
        "// SPDX-License-Identifier: Apache-2.0\n\
         // Generated by bhf — do not edit. Calls the discovered target with the\n\
         // fuzz bytes decoded to each parameter's static type.\n\
         namespace Bhfgen {{\n\
         \x20 public static class BhfEntry {{\n\
         {helper}\
         \x20   static bool BhfTargetEntered;\n\
         \x20   static void BhfMarkTargetEntry() {{\n\
         \x20     if (BhfTargetEntered) return;\n\
         \x20     var path = System.Environment.GetEnvironmentVariable(\"BHF_TARGET_ENTRY_SHM\");\n\
         \x20     if (string.IsNullOrEmpty(path)) return;\n\
         \x20     try {{ System.IO.File.WriteAllBytes(path, new byte[] {{ 1 }}); BhfTargetEntered = true; }}\n\
...
/// the fixed Driver.cs + the generated BhfEntry.cs. When the target project
...
fn generate_csproj(target_csproj: &Path, linkage: &TargetLinkage) -> String {
    let reference = match linkage {
        ...
        TargetLinkage::SourceInclusion { .. } => {
            "<ProjectReference Include=\"target/bhf_target.csproj\" />".to_owned()
        }
    };
    ...
    format!(
        "<Project Sdk=\"Microsoft.NET.Sdk\">\n\
         ...
         \x20   <AssemblyName>bhf_harness</AssemblyName>\n\
         ...
         \x20 </PropertyGroup>\n\
         \x20 <ItemGroup>\n\
         \x20   <Compile Include=\"Driver.cs;BhfEntry.cs\" />\n\
...
/// Target-only library for source inclusion. Keeping Driver/BhfEntry in a
...
fn generate_source_target_csproj(target_csproj: &Path, excluded: &[PathBuf]) -> String {
    ...
    format!(
        "<Project Sdk=\"Microsoft.NET.Sdk\">\n\
         ...
         \x20   <AllowUnsafeBlocks>true</AllowUnsafeBlocks>\n\
         \x20   <Nullable>disable</Nullable>\n\
         \x20   <AssemblyName>bhf_target</AssemblyName>\n\
         \x20   <GenerateAssemblyInfo>false</GenerateAssemblyInfo>\n\
...
pub fn build_csharp_harness(
    ...
    if let Err(e) = std::fs::write(
        proj_dir.join("BhfEntry.cs"),
        generate_entry(&method, receiver_kind),
    ) {
        return CSharpBuildResult::Failed(format!("write BhfEntry.cs: {e}"));
    }
    ...
    let linkage = if pinned_tfm.is_none() && !declares_tfm.is_empty() {
        bhfeprintln!(
            "bhf auto: C# project {} targets {} which the installed .NET SDK \
    ...
    if let Err(e) = std::fs::write(
        proj_dir.join("bhf_harness.csproj"),
    ...
    if let TargetLinkage::SourceInclusion { excluded } = &linkage {
        ...
        if let Err(error) = std::fs::write(
            source_target_dir.join("bhf_target.csproj"),
    ...
    for round in 0..=MAX_EJECT_ROUNDS {
        ...
        let build = crate::command_output::output_with_timeout(
            Command::new("dotnet")
                .arg("build")
                .arg(proj_dir.join("bhf_harness.csproj"))
                .arg("-c")
                .arg("Release")
                .arg("--nologo")
                .arg("-v")
        ...
        let recovered_path = source_target_dir.join("BhfRecoveredUsings.cs");
        ...
        bhfeprintln!(
            "bhf auto: C# source-inclusion: ejecting {} file(s) that do not compile \
    ...
    let asm = match linkage {
        TargetLinkage::SourceInclusion { .. } => "bhf_target".to_owned(),
    ...
    let harness_dll = out_dir.join("bhf_harness.dll");
    let script = format!(
        "#!/bin/sh\n\
         # BHF_FRAMED BHF_CS_LAUNCHER bhf .NET driver launcher (native C# lane).\n\
         # The engine sets BHF_FRAMED + BHF_COV_SHM in the environment; the CLR\n\
         # inherits them across this exec. The Driver mmaps the file-backed\n\
         # BHF_COV_SHM map into SharpFuzz.Common.Trace.SharedMem and speaks the\n\
         # framed fork-server protocol. BHF_CS_NAMESPACE = the target's root\n\
         # namespace, whose own exceptions are treated as declared input rejection.\n\
         BHF_CS_NAMESPACE=\"{ns}\" \\\n\
         BHF_EXPECTED_EXCEPTIONS=\"{expected}\" \\\n\
...
mod tests {
    ...
    fn entry_static_byte_array() {
        ...
        assert!(src.contains("var bhfArg0 = data;"));
        assert!(src.contains("global::Acme.Parser.Parse(bhfArg0)"));
        let checkpoint = src.find("BhfMarkTargetEntry();").unwrap();
        let call = src.find("global::Acme.Parser.Parse(bhfArg0)").unwrap();
        assert!(
            checkpoint < call,
            "checkpoint must precede target call: {src}"
        );
        assert!(!src.contains("BhfUninitialized"), "no forced helper: {src}");
    }
    ...
    fn entry_instance_string() {
        ...
        assert!(src.contains("var bhfReceiver = new global::Acme.Parser();"));
        assert!(src.contains("bhfReceiver.Parse(bhfArg0)"));
    ...
    fn forced_entry_allocates_an_uninitialized_receiver() {
        ...
        assert!(
            src.contains("var bhfReceiver = ((global::Acme.Parser) BhfUninitialized(typeof(global::Acme.Parser)));"),
            "{src}"
        );
        assert!(src.contains("bhfReceiver.Parse(bhfArg0)"), "{src}");
    ...
    fn an_unbuildable_project_falls_back_to_compiling_its_sources() {
        ...
        assert!(
            out.contains("<ProjectReference Include=\"target/bhf_target.csproj\""),
            "{out}"
        );
        assert!(
            out.contains("<Compile Include=\"Driver.cs;BhfEntry.cs\""),
            "{out}"
        );
    ...
    fn assembly_name_from_csproj() {
        let dir = std::env::temp_dir().join("bhf_cs_asmname_test");
