...
//!
//! Strategy (like the Python/Perl lanes): reuse bhf's builtin engine over the
...
//! `node js_runtime/bhf_driver.js`, which `require`s the target module, calls the
...
//! `BHF_COV_SHM` edge bitmap, and reports an uncaught non-rejection exception as a
...
";
    ...
    fn commonjs_exports_object_built_by_property_assignment() {
        ...
        // built by property assignment (bhf discovered 0 of these before).
        let src = "\
