...
//!
//! Strategy (like the Python/Perl/JS lanes): reuse bhf's builtin engine over the
...
//! `ruby ruby_runtime/bhf_driver.rb`, which `require`s the target file, calls the
//! method with the fuzz bytes, records per-line edge coverage via a `TracePoint`
//! folded into the shared `BHF_COV_SHM` bitmap, and reports an uncaught
