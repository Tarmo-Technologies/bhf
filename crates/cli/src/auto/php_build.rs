...
//! Native PHP fuzzing lane (M3.11): generate a bhf harness, copy the
...
//! `BHF_FRAMED` fork-server protocol — the SAME builtin-engine execution path as
...
fn locate_php_runtime() -> Option<PathBuf> {
    crate::runtime_assets::locate("php_runtime", "bhf_driver.php")
}
...
pub fn build_php_harness(
    ...
    if let Err(e) = std::fs::copy(
        runtime.join("bhf_driver.php"),
        auto_dir.join("bhf_driver.php"),
    ) {
        return PhpBuildResult::Failed {
    ...
    let harness_path = auto_dir.join("bhfgen.php");
    ...
    // carries the BHF_FRAMED + BHF_PHP_LAUNCHER markers.
    let main_path = auto_dir.join("main");
    let driver = auto_dir.join("bhf_driver.php");
    ...
    let script = format!(
        "#!/bin/sh\n\
         # BHF_FRAMED BHF_PHP_LAUNCHER bhf PHP driver launcher (native PHP lane).\n\
         # The engine sets BHF_FRAMED + BHF_COV_SHM; php inherits them. pcov (scoped\n\
...
fn resolve_target(candidate: &Candidate) -> Result<(PhpFunction, PhpCall), String> {
    ...
    let mut setup = format!("$bhf_arg = {argument};");
    let invocation = if f.class.is_empty() {
        // A free function; `f.name` is the (possibly namespaced) name.
        format!("return \\{}($bhf_arg);", f.name)
    } else if f.is_static {
        format!("return {}::{}($bhf_arg);", f.class, f.func)
    } else {
        setup.push_str(&format!("\n    $bhf_receiver = new {}();", f.class));
        format!("return $bhf_receiver->{}($bhf_arg);", f.func)
    };
...
/// Emit `bhfgen.php` returning a `run_one($data)` closure that `require`s the
...
fn generate_harness(target_abs: &Path, load_roots: &[PathBuf], call: &PhpCall) -> String {
    ...
    format!(
        "<?php\n\
         // SPDX-License-Identifier: Apache-2.0\n\
         // Generated by bhf (native PHP lane). Loads the target and passes the\n\
         // fuzz bytes as a string. Do not edit.\n\
         $bhf_roots = [{roots}];\n\
         foreach ($bhf_roots as $bhf_root) {{\n\
         \x20   $autoload = $bhf_root . '/vendor/autoload.php';\n\
         \x20   if (is_file($autoload)) {{ require_once $autoload; break; }}\n\
         }}\n\
         spl_autoload_register(function(string $class) use ($bhf_roots): void {{\n\
         \x20   $parts = explode('\\\\', ltrim($class, '\\\\'));\n\
         \x20   foreach ($bhf_roots as $root) {{\n\
         ...
         require_once '{target}';\n\
         $bhf_make_value = function(string $type, string $data, int $depth = 0) use (&$bhf_make_value) {{\n\
         ...
         \x20       $arguments[] = $bhf_make_value($named->getName(), $data, $depth + 1);\n\
         \x20   }}\n\
         \x20   return $reflection->newInstanceArgs($arguments);\n\
         }};\n\
         $bhf_target_entered = false;\n\
         $bhf_mark_target_entry = function() use (&$bhf_target_entered): void {{\n\
         \x20   if ($bhf_target_entered) return;\n\
         \x20   $path = getenv('BHF_TARGET_ENTRY_SHM');\n\
         \x20   if (!$path) return;\n\
         \x20   if (@file_put_contents($path, \"\\x01\", LOCK_EX) !== false) {{\n\
         \x20       $bhf_target_entered = true;\n\
         \x20   }}\n\
         }};\n\
         return function(string $data) use ($bhf_mark_target_entry, $bhf_make_value) {{\n\
         \x20   {setup}\n\
...
mod tests {
    ...
    fn object_argument_is_constructed_before_entry_checkpoint() {
        ...
        assert!(expression.contains("$bhf_make_value('\\Monolog\\LogRecord'"));
        let call = PhpCall {
            setup: format!("$bhf_arg = {expression};"),
            invocation: "return $receiver->format($bhf_arg);".to_owned(),
        };
        let harness = generate_harness(Path::new("/tmp/Formatter.php"), &[], &call);
        let argument = harness.find("$bhf_arg =").unwrap();
        let checkpoint = harness.find("$bhf_mark_target_entry();").unwrap();
    ...
    fn bundled_driver_is_locatable_in_tree() {
        ...
        assert!(runtime.join("bhf_driver.php").is_file());
    }
}
