...
const EXPERT_HARNESS_ENV: &str = "BHF_EXPERT_HARNESS";
const EXPERT_COVERAGE_ENV: &str = "BHF_EXPERT_COVERED_LINES";
...
fn replay_one(
    ...
    if export_text.contains("[bhf: subprocess output truncated]") {
        bhfeprintln!(
            "bhf: llvm-cov output for {harness_id} exceeded the bounded capture; \
        ...
        .enumerate()
    {
        match build_and_replay_expert(hdir, &expert, index, recipe, &root, &queue, profdata, cov) {
            Ok(expert_measurement) => {
                ...
                // The oracle is about the project implementation, not bhf's
...
fn build_and_replay_expert(
    ...
    for conventional in [
        ...
        project_root.join(".bhf-build"),
        project_root.join(".bhf-build-cov"),
        project_root.join(".bhf-build/include"),
        project_root.join(".bhf-build-cov/include"),
    ] {
...
fn parse_lcov_detailed(lcov: &str) -> LcovMeasurement {
    ...
    for line in lcov.lines() {
        if line.contains("[bhf: subprocess output truncated]") {
            file.clear();
