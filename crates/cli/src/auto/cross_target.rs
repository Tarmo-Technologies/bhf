    ...
    if contains_any(
        ...
        return Some(PlatformStub {
            ...
            headers: vec![
                ...
                SynthHeader {
                    name: "_bhf_crt_compat.h".to_owned(),
...
pub(crate) const WINDOWS_H_STUB: &str = r#"/* bhf synthesized fake <windows.h> — reduced-fidelity platform stub.
...
#ifndef BHF_FAKE_WINDOWS_H
#define BHF_FAKE_WINDOWS_H
#include <stdint.h>
#include <stddef.h>
...
/* File / memory-mapping flags the bhf driver's Windows input path passes. */
#ifndef GENERIC_READ
...
const IO_H_STUB: &str = r#"/* bhf synthesized fake <io.h> — reduced-fidelity platform stub.
 * Maps the MSVC CRT low-level I/O names onto POSIX so the host build links. */
#ifndef BHF_FAKE_IO_H
#define BHF_FAKE_IO_H
#include <unistd.h>
#include <fcntl.h>
#define _read   read
#define _write  write
...
const CRT_COMPAT_STUB: &str = r#"/* bhf synthesized MSVC CRT compat — reduced-fidelity platform stub.
...
#ifndef BHF_FAKE_CRT_COMPAT_H
#define BHF_FAKE_CRT_COMPAT_H
#include <stdio.h>
#include <string.h>
#include <strings.h>
#include <stdarg.h>
...
static inline int _bhf_crt_vsnprintf(char *_s, size_t _n, const char *_f, va_list _a) {
    return vsnprintf(_s, _n, _f, _a);
}
static inline int _bhf_crt_snprintf(char *_s, size_t _n, const char *_f, ...) {
    va_list _a; int _r;
    va_start(_a, _f); _r = vsnprintf(_s, _n, _f, _a); va_end(_a);
    return _r;
}
static inline int _bhf_crt_vscprintf(const char *_f, va_list _a) {
    return vsnprintf((char *)0, (size_t)0, _f, _a);
}
static inline int _bhf_crt_stricmp(const char *_a, const char *_b) {
    return strcasecmp(_a, _b);
}
static inline int _bhf_crt_strnicmp(const char *_a, const char *_b, size_t _n) {
    return strncasecmp(_a, _b, _n);
}
static inline char *_bhf_crt_strdup(const char *_s) { return strdup(_s); }
#define _vsnprintf _bhf_crt_vsnprintf
#define _snprintf _bhf_crt_snprintf
#define _vscprintf _bhf_crt_vscprintf
#define _stricmp _bhf_crt_stricmp
#define _strnicmp _bhf_crt_strnicmp
#define _strdup _bhf_crt_strdup
...
/// bhf-authored scaffolding, NOT Microsoft's headers, so nothing proprietary
...
pub(crate) const MFC_STUB: &str = r#"/* bhf synthesized MFC/ATL compat — reduced-fidelity platform stub.
...
#ifndef BHF_FAKE_MFC_H
#define BHF_FAKE_MFC_H
...
#endif /* __cplusplus */
#endif /* BHF_FAKE_MFC_H */
"#;

/// Win32 typedef/macro names bhf can satisfy with real underlying types from
...
const VXWORKS_H_STUB: &str = r#"/* bhf synthesized fake <vxWorks.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_VXWORKS_H
#define BHF_FAKE_VXWORKS_H
#include <stdint.h>
#include <stddef.h>
#ifndef __vxworks
#define __vxworks 1
...
#define NO_WAIT 0
#endif
#endif /* BHF_FAKE_VXWORKS_H */
"#;

/// Fake `<taskLib.h>`: the common VxWorks task API as declarations (calls
/// type-check; bhf's stub machinery resolves the link).
const TASKLIB_H_STUB: &str = r#"/* bhf synthesized fake <taskLib.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_TASKLIB_H
#define BHF_FAKE_TASKLIB_H
#include "vxWorks.h"
...
#endif /* BHF_FAKE_TASKLIB_H */
"#;

/// Fake `<semLib.h>`: VxWorks semaphore API + the option/timeout macros.
const SEMLIB_H_STUB: &str = r#"/* bhf synthesized fake <semLib.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_SEMLIB_H
#define BHF_FAKE_SEMLIB_H
#include "vxWorks.h"
...
#endif /* BHF_FAKE_SEMLIB_H */
"#;

/// Fake `<msgQLib.h>`: VxWorks message-queue API + priority macros.
const MSGQLIB_H_STUB: &str = r#"/* bhf synthesized fake <msgQLib.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_MSGQLIB_H
#define BHF_FAKE_MSGQLIB_H
#include "vxWorks.h"
...
const INTEGRITY_H_STUB: &str = r#"/* bhf synthesized fake <INTEGRITY.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_INTEGRITY_H
#define BHF_FAKE_INTEGRITY_H
#include <stdint.h>
#include <stddef.h>
#ifndef __INTEGRITY
...
const NEUTRINO_H_STUB: &str = r#"/* bhf synthesized fake <sys/neutrino.h> — reduced-fidelity RTOS stub. */
#ifndef BHF_FAKE_NEUTRINO_H
#define BHF_FAKE_NEUTRINO_H
#include <stdint.h>
#include <stddef.h>
#ifndef __QNX__
#define __QNX__ 1
...
"#;
    ...
    fn available_reflects_path_lookup() {
        ...
        assert!(!executable_on_path("bhf-cross-no-such-tool-zzz"));
        ...
        let absent = CrossTarget {
            cc: "bhf-cross-no-such-cc-zzz".to_owned(),
            runner: CrossRunner::Wine {
                exe: "bhf-cross-no-such-wine-zzz".to_owned(),
            },
            ..present.clone()
        };
    ...
    fn windows_stub_maps_msvc_crt_stdio_string_names_onto_glibc() {
        ...
        let crt = stub
            .headers
            .iter()
            .find(|h| h.name == "_bhf_crt_compat.h")
        ...
        for (msvc, wrapper, glibc) in [
            ("_vsnprintf", "_bhf_crt_vsnprintf", "vsnprintf"),
            ("_snprintf", "_bhf_crt_snprintf", "vsnprintf"),
            ("_stricmp", "_bhf_crt_stricmp", "strcasecmp"),
            ("_strnicmp", "_bhf_crt_strnicmp", "strncasecmp"),
            ("_strdup", "_bhf_crt_strdup", "strdup"),
        ] {
            assert!(
    ...
    fn crt_compat_header_compiles_and_resists_base_name_hijack() {
        ...
        if std::process::Command::new("clang")
            ...
            .is_err()
        {
            bhfeprintln!("skipping crt-compat compile: clang not on PATH");
            return;
        }
        let dir = std::env::temp_dir().join(format!("bhf-crt-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("_bhf_crt_compat.h"), CRT_COMPAT_STUB).unwrap();
        ...
        let tu = "#include \"_bhf_crt_compat.h\"\n\
        ...
        // bhf's generated Makefile actually builds with, where the POSIX
        ...
        let body = expanded
            ...
            .unwrap_or("");
        assert!(
            body.contains("_bhf_crt_vsnprintf(s, n, f, a)"),
    ...
    fn crt_compat_header_compiles_in_cpp_tu_with_cstddef() {
        ...
        if std::process::Command::new("clang++")
            ...
            .is_err()
        {
            bhfeprintln!("skipping crt-compat C++ compile: clang++ not on PATH");
            return;
        }
        let dir = std::env::temp_dir().join(format!("bhf-crt-cpp-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        ...
        let probe_ok = std::process::Command::new("clang++")
            ...
            .unwrap_or(false);
        if !probe_ok {
            bhfeprintln!("skipping crt-compat C++ compile: clang++ can't find the C++ stdlib");
            let _ = std::fs::remove_dir_all(&dir);
            return;
        }
        std::fs::write(dir.join("_bhf_crt_compat.h"), CRT_COMPAT_STUB).unwrap();
        ...
        let out = std::process::Command::new("clang++")
            ...
            .arg(dir.join("_bhf_crt_compat.h"))
            .arg(&tu)
            .output()
            .expect("spawn clang++");
