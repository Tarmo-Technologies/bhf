...
/// data remain visible, matching what bhf can use on a project with no
/// hand-written fuzzer.
fn recipe_text_is_visible(text: &str) -> bool {
    std::env::var_os("BHF_BLIND_EXPERT_HARNESSES").is_none()
...
/// tests — and bhf walked straight past them, seeding only from `--seed-dir`.
...
mod tests {
    ...
    fn mining_a_tree_reads_only_the_example_directories() {
        let root = std::env::temp_dir().join(format!("bhf-mine-{}", std::process::id()));
    ...
    fn seeds_are_mined_from_test_data_but_never_from_source() {
        ...
        let root = std::env::temp_dir().join(format!("bhf-seedmine-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn an_oversized_or_empty_file_is_not_a_seed() {
        let root = std::env::temp_dir().join(format!("bhf-seedcap-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
