...
fn precompile_project_modules(source_root: &Path, moddir: &Path) {
    let marker = moddir.join(".bhf_modules_done");
    if marker.exists() {
        return;
    }
...
/// (gfortran emits trace-pc, which the bhf Linux driver — trace-pc-guard —
...
fn glue_source(
        ...
        match a.kind {
            ...
            FortranArgKind::CharBuffer { len } => {
                if i == primary && len == 0 {
                    ...
                    frees.push_str(&format!("    free({var});\n"));
                    hidden_lengths.push("(size_t)bhf_primary_len".to_owned());
                } else if i == primary {
                    ...
                    fills.push_str(&format!(
                        "    if (!{var}) return 0;\n    memset({var}, ' ', {len});\n    bhf_primary_len = Size < {len} ? Size : {len};\n    if (bhf_primary_len) memcpy({var}, Data, bhf_primary_len);\n"
                    ));
                    frees.push_str(&format!("    free({var});\n"));
            ...
            FortranArgKind::Integer => {
                ...
                if Some(i) == len_idx {
                    fills.push_str(&format!("    {var} = (int)bhf_primary_len;\n"));
                }
    ...
    match result {
        ...
        FortranResult::ValueChar { fixed_len } => {
            ...
            let res_alloc = format!(
                "\x20   size_t bhf_res_len = (size_t)Size * 4;\n\
                 \x20   if (bhf_res_len < 4096) bhf_res_len = 4096;\n\
                 \x20   if (bhf_res_len < {fixed_len}u) bhf_res_len = {fixed_len}u;\n\
                 \x20   bhf_res_len += 256;\n\
                 \x20   if (bhf_res_len > (1u << 22)) bhf_res_len = (1u << 22);\n\
                 \x20   char *bhf_res = (char *)malloc(bhf_res_len);\n\
                 \x20   if (!bhf_res) return 0;\n\
                 \x20   memset(bhf_res, ' ', bhf_res_len);\n"
            );
            decls.push_str(&res_alloc);
            frees.push_str("    free(bhf_res);\n");
            all_args.insert(0, "bhf_res_len".to_owned());
            all_args.insert(0, "bhf_res".to_owned());
        ...
        FortranResult::AllocChar => {
            ...
            let res_alloc = "\x20   char *bhf_res_data = 0;\n\
                 \x20   size_t bhf_res_len = 0;\n";
            decls.push_str(res_alloc);
            frees.push_str("    free(bhf_res_data);\n");
            all_args.insert(0, "&bhf_res_len".to_owned());
            all_args.insert(0, "&bhf_res_data".to_owned());
            extern_sig.insert(0, "size_t *".to_owned());
            extern_sig.insert(0, "char **".to_owned());
        }
    }
    ...
    format!(
        "/* SPDX-License-Identifier: Apache-2.0 */\n\
         /* bhf Fortran glue: drives {entry} from fuzzer bytes. */\n\
         #include <stdint.h>\n\
         #include <stdlib.h>\n\
         #include <string.h>\n\
         #include <unistd.h>\n\
         ...
         \x20* bhf Linux driver only provides trace-pc-guard, so define the\n\
         \x20* trace-pc hook here, writing into the same shared edge map the engine\n\
         \x20* reads (BHF_COV_SHM). ASan reports memory bugs directly, so no\n\
         \x20* exit() interposition is needed (unlike the COBOL lane). */\n\
         #define BHF_COV_BITS (1u << 16)\n\
         static unsigned char *bhf_cov = 0;\n\
         static int bhf_cov_init = 0;\n\
         __attribute__((no_sanitize(\"coverage\"))) static void bhf_cov_open(void) {{\n\
         \x20   const char *p = getenv(\"BHF_COV_SHM\"); bhf_cov_init = 1;\n\
         \x20   if (!p || !*p) return;\n\
         \x20   int fd = open(p, O_RDWR | O_CREAT, 0600); if (fd < 0) return;\n\
         \x20   if (ftruncate(fd, BHF_COV_BITS) == 0) {{ void *m = mmap(0, BHF_COV_BITS, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0); if (m != MAP_FAILED) bhf_cov = (unsigned char *)m; }}\n\
         \x20   close(fd);\n\
         }}\n\
         /* WEAK: the bhf C driver also defines this hook (it serves the\n\
         ...
         \x20   if (!bhf_cov_init) bhf_cov_open();\n\
         \x20   if (!bhf_cov) return;\n\
         \x20   uintptr_t pc = (uintptr_t)__builtin_return_address(0);\n\
         \x20   unsigned h = ((unsigned)(pc * 2654435761u) >> 4) & (BHF_COV_BITS - 1);\n\
         \x20   bhf_cov[h] = 1;\n\
         }}\n\
         int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {{\n\
         \x20   size_t bhf_primary_len = 0; (void)bhf_primary_len; (void)Data;\n\
         {decls}\
         {fills}\
         \x20   bhf_target_enter();\n\
         \x20   {entry}({call});\n\
         {frees}\
         \x20   return 0;\n\
         }}\n"
...
fn array_wrapper_source(proc: &crate::auto::fortran::FortranProc) -> Option<String> {
    ...
    Some(format!(
        "! SPDX-License-Identifier: Apache-2.0\n\
         ...
         module bhf_array_wrapper_mod\n\
         \x20 use iso_c_binding\n\
         \x20 use {module}, only: bhf_target => {target}\n\
         \x20 implicit none\n\
         contains\n\
         \x20 subroutine bhf_fortran_array_entry(data, count) bind(C, name=\"bhf_fortran_array_entry\")\n\
         ...
         \x20   output = bhf_target(input)\n\
         \x20 end subroutine bhf_fortran_array_entry\n\
         end module bhf_array_wrapper_mod\n",
        target = proc.name,
    ))
}
...
pub fn build_fortran_harness(
    ...
    let fortran_o = if let Some(reused) = precompiled_o.filter(|p| p.is_file()) {
        ...
        // bounds error before the raw access, which bhf classifies as an input
    ...
    let entry = if has_array_arg {
        ...
        let wrapper_source = hdir.join("bhf_array_wrapper.f90");
        let wrapper_o = hdir.join("bhf_array_wrapper.o");
        ...
        "bhf_fortran_array_entry".to_owned()
    } else {
...
";
    ...
    fn glue_drives_buffer_len_and_hidden_length() {
        ...
        assert!(g.contains("scan_(bhf_a0, &bhf_a1, (size_t)bhf_primary_len);"));
        assert!(g.contains("bhf_a1 = (int)bhf_primary_len;"));
        assert!(g.contains("bhf_primary_len = Size;"));
        assert!(g.contains("char *bhf_a0 = (char *)malloc(Size ? Size : 1)"));
        // trace-pc coverage hook present.
        assert!(g.contains("void __sanitizer_cov_trace_pc(void)"));
        assert!(
            g.contains("bhf_target_enter();\n    scan_(bhf_a0, &bhf_a1, (size_t)bhf_primary_len);")
        );
    }
    ...
    fn glue_fixed_length_char_buffer_is_sized_to_the_declared_length() {
        ...
        assert!(g.contains("char *bhf_a0 = (char *)malloc(80);"));
        assert!(g.contains("memset(bhf_a0, ' ', 80);"));
        assert!(g.contains("bhf_primary_len = Size < 80 ? Size : 80;"));
        // Hidden length still the declared 80, now matched by the buffer.
        assert!(g.contains("nasopn_(bhf_a0, (size_t)80);"));
    ...
    fn glue_assumed_length_uses_byte_count_as_hidden_len() {
        ...
        assert!(g.contains("f_(bhf_a0, (size_t)bhf_primary_len);"));
    }
    ...
    fn glue_value_char_function_prepends_result_buffer_pair() {
        ...
        assert!(g.contains("__csv_utilities_MOD_lowercase_string(bhf_res, bhf_res_len, bhf_a0, (size_t)bhf_primary_len);"));
        // Result buffer is allocated, blank-filled, and freed.
        assert!(g.contains("char *bhf_res = (char *)malloc(bhf_res_len);"));
        assert!(g.contains("memset(bhf_res, ' ', bhf_res_len);"));
        assert!(g.contains("free(bhf_res);"));
    }
    ...
    fn glue_alloc_char_function_passes_and_frees_callee_allocated_result() {
        ...
        assert!(g.contains(
            "__m_strings_MOD_upper(&bhf_res_data, &bhf_res_len, bhf_a0, (size_t)bhf_primary_len);"
        ));
        assert!(g.contains("char *bhf_res_data = 0;"));
        assert!(g.contains("free(bhf_res_data);"));
        // The callee allocates; the glue must NOT pre-allocate a result buffer here.
        assert!(!g.contains("malloc(bhf_res_len)"));
    }
    ...
    fn assumed_shape_character_array_uses_fortran_descriptor_wrapper() {
        ...
        assert!(wrapper.contains("use functional, only: bhf_target => arrstr"));
        assert!(wrapper.contains("call c_f_pointer(data, input, [int(count)])"));
        assert!(wrapper.contains("output = bhf_target(input)"));

        let glue = glue_source(
            "bhf_fortran_array_entry",
            &proc.args,
            FortranResult::NonChar,
            true,
        );
        assert!(glue.contains("extern void bhf_fortran_array_entry(char *, size_t);"));
        assert!(glue.contains("malloc(Size ? Size : 1)"));
        assert!(glue.contains("bhf_fortran_array_entry(bhf_a0, Size);"));
