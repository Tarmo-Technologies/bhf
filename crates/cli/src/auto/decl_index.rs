...
impl DeclarationIndex {
    ...
    fn build_parsed(root: &Path) -> std::io::Result<Self> {
        ...
        if skipped_for_time > 0 {
            bhfeprintln!(
                "bhf auto: declaration index reached the discovery time budget and skipped \
                 {skipped_for_time} file(s); cross-file repair may be less complete. Raise \
                 --campaign-time or set BHF_DISCOVERY_TIME (seconds, 0 = unlimited)."
            );
        }
    ...
    pub fn include_root_for(&self, spelling: &str) -> Option<PathBuf> {
        ...
        // `parent/.bhf-up-0/../allocators.h` then resolves to the recovered blob.
        ...
        for level in 0..parent_count {
            root.push(format!(".bhf-relative-include-{level}"));
        }
        Some(root)
    }
...
/// directory-exclusion list as scan.rs (bhf_work, target, etc.).
...
";
    ...
    fn tmpdir() -> std::path::PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("bhf-declidx-{nonce}"));
    ...
    fn resolves_parent_relative_include_to_unique_recovered_header() {
        ...
        assert_eq!(
            ...
            Some(include.join(".bhf-relative-include-0"))
        );
    }
