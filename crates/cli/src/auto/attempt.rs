...
/// `bhf auto` CLI flags `--per-target-time` and `--no-stubs`.
#[derive(Debug, Clone)]
pub struct AttemptOptions {
    ...
    /// Canonical source root from `bhf auto <PATH>`. Ada build
    ...
    /// (BHF-209) instead of OOM-killing the host (#386). Mirrors libFuzzer's
    ...
    /// #421). When set, the driver's `BHF_CMP_PROGRESS_SHM` map is wired up so
    ...
    /// to before. Mirrors `bhf fuzz --sanitizers`. Inert on the Ada and
...
pub struct PassRun {
    ...
    /// harnesses without a bhf coverage runtime (only passthrough libFuzzer
...
/// Whether an undefined-symbol link error names a symbol bhf cannot satisfy
...
pub enum Outcome {
    BuiltAndFuzzed {
        ...
        /// Runtime audit events captured by libbhf_runtrace.so
    ...
    /// target, bhf degrades to discover + SBOM + static findings (each with
...
fn write_source_dictionary(
    ...
    let tokens = match lang {
        ...
        Lang::Js | Lang::Ts => {
            ...
            // fuzzer can reach an unsafe merge/set path (the BHF-509 detector then
...
/// make target + `#ifdef BHF_AFL` persistent harness exist only for the C/C++
...
/// #95: true when bhf's harness for this language injects a target-entry
/// probe that fires immediately before the selected project endpoint — C/C++
/// (`BHF_TARGET_ENTER`, compiled into `main` via `-DBHF_EXTERNAL_DRIVER`)
...
fn other_error_is_placeholder_class_misuse(tail: &str, placeholders: &[&String]) -> bool {
    ...
    for line in tail.lines() {
        ...
        match diagnostic_source_file(line) {
            // A diagnostic in the PROJECT's own source cannot be a bhf
            // codegen bug: bhf did not write that code, and it compiled
            // before a placeholder type was injected under it. This is what
            // makes the check independent of the placeholder's exact shape —
            // the member-inferred struct bhf synthesizes today yields
...
/// Whether a diagnostic's file is the harness bhf generated, as opposed to
...
fn run_attempt(
    ...
    // generate + compile a bhf harness, and emit `harnesses/<id>/main` — a launcher
    // that runs the target in a persistent JVM under bhf's own coverage agent.
    ...
    // Step 0 (Python, M3.1): the native Python lane. Generate a bhf harness
    ...
    // Step 0 (Perl, M3.2): the native Perl lane. Generate a bhf harness module
    // + copy the perl_runtime (driver + Devel::BhfCov), then emit
    // `harnesses/<id>/main` — a launcher that runs the target under `perl -d:BhfCov`
    ...
    // Step 0a: the native Rust lane (M1.2). Generate the bhf harness, build
    ...
    // the return by value (`<ret> R = _bhf_receiver.method(...)`), so an undefined
    ...
    if let ForeignStrategy::StubIsolated(stub) = &strategy {
        if let Err(error) = apply_platform_stub(stub, &harness_dir, &repairs_dir) {
            bhfeprintln!(
                "bhf auto: warning: {}: platform stub setup failed: {error}",
                candidate.harness_id
            );
        } else {
    ...
    for archive in crate::auto::build_probe::static_libraries_defining_any(
        ...
        bhfeprintln!(
            "bhf auto: {}: linking probe-built instrumented library {}",
            candidate.harness_id,
            archive.display()
        );
    ...
    for retry in 0..=max_repair_rounds {
        ...
        if retry > 0 && crate::command_output::campaign_budget_exhausted() {
            bhfeprintln!(
                "bhf auto: {}: --campaign-time budget exhausted after {retry} repair \
        ...
        match try_build(
            ...
            BuildOutcome::Success => {
                if matches!(
                    ...
                    let explicit_standard = is_cpp
                        .then(|| std::env::var("BHF_CXX_STD").ok())
                        .flatten()
                ...
                // those injections + their own BHF_RUNTRACE_MODE.
                ...
                for (pass_idx, pass) in builtin_passes.iter().enumerate() {
                    ...
                    // runtime_mode so `bhf replay` can replay it
                    ...
                    for fid in &summary.findings {
                        ...
                        {
                            bhfeprintln!("bhf auto: warning: stamp {fid} runtime_mode: {error}");
                        }
                    }
                ...
                if run_afl {
                    ...
                    if !afl_built {
                        bhfeprintln!(
                            "bhf auto: warning: `make afl` failed for {} — AFL skipped, \
                        ...
                        match crate::fuzz::run_afl_plus_plus_programmatic(
                            ...
                            Ok(summary) => {
                                ...
                                for fid in &summary.findings {
                                    ...
                                    if let Err(error) = stamp_runtime_mode(
                                        ...
                                        bhfeprintln!(
                                            "bhf auto: warning: stamp {fid} runtime_mode: {error}"
                                        );
                                    }
                                }
                                pass_runs.push(PassRun {
                            ...
                            Err(error) => bhfeprintln!(
                                "bhf auto: warning: AFL run failed for {}: {error}",
                                candidate.harness_id
                            ),
                        }
                    }
                ...
                if stub_exec.stub_only {
                    ...
                    bhfeprintln!(
                        "bhf auto: WARNING: {} fuzzed STUB-ONLY — {}/{} called symbols were \
                ...
                if let ForeignStrategy::StubIsolated(stub) = &strategy {
                    let findings: usize = pass_runs.iter().map(|p| p.findings.len()).sum();
                    bhfeprintln!(
                        "bhf auto: NOTE: {} fuzzed STUB-ISOLATED for `{}` — its platform \
                ...
                {
                    bhfeprintln!(
                        "bhf auto: {}: built but the target was NEVER entered ({entry_miss}) \
                ...
                if total_executions == 0 {
                    bhfeprintln!(
                        "bhf auto: {}: built but no fuzz iteration executed (0 executions) — \
                ...
                if harness_emits_target_entry_proof(candidate.lang) {
                    ...
                    if builtin_entered_execed && peak_builtin_edges == 0 {
                        bhfeprintln!(
                            "bhf auto: WARNING: {} entered and executed the target but recorded \
            ...
            BuildOutcome::Failed { errors } => {
                if retry == max_repair_rounds {
                    ...
                    bhfeprintln!(
                        "bhf auto: {}: repair cap reached ({max_repair_rounds} rounds) \
                ...
                if let Some(ejected) = eject_unbuildable_added_source(
                    ...
                    bhfeprintln!(
                        "bhf auto: {}: recovered source {} does not compile in \
                ...
                if !matching_archives.is_empty() {
                    ...
                    for archive in matching_archives {
                        ...
                        bhfeprintln!(
                            "bhf auto: {}: undefined externals unresolved in the swept tree; \
                    ...
                    && should_link_full_tu_set(&errors, decl_index, &candidate.source_path)
                {
                    ...
                    if linked_any {
                        bhfeprintln!(
                            "bhf auto: {}: undefined externals across sibling TUs and no \
                        ...
                        )
                    {
                        ...
                        if repair_replaces_candidate_target(&repair, candidate) {
                            ...
                            if record_refused_repair(&mut refused_repairs, &repair) {
                                bhfeprintln!(
                                    "bhf auto: refusing self-target repair {} for {}; \
                        ...
                        match crate::auto::repair::apply_repair_with_source(
                            ...
                            Err(e) => {
                                ...
                                // in last_errors.
                                bhfeprintln!(
                                    "bhf auto: skipping repair for {}: {e}",
                                    repair_key(&repair)
                                );
                                manifest.repairs.push(repair);
                            }
                            ...
                            outcome: Outcome::UnsupportedParams {
                                reason: format!(
                                    "target `{}` has no definition in the active build: its own \
                                     ...
                                     guard not enabled by bhf's portable `-O1` build). Re-adding \
...
mod full_tu_gate_tests {
    ...
    fn precise_definition_sources_prevent_unrelated_full_library_sweep() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-full-tu-gate-{nonce}"));
...
/// its `as_str()` becomes `BHF_RUNTRACE_MODE` in the child env.
///
...
> {
    ...
    let mut child_env: Vec<(String, String)> = vec![
        ...
        ("BHF_RUNTRACE_MODE".to_owned(), pass.as_str().to_owned()),
        ...
        ("BHF_CMPLOG".to_owned(), "1".to_owned()),
        // Edge-coverage bitmap shared by every harness child (#385). Only the
        // passthrough bhf driver carries a runtime that writes it; other
    ...
    // `BHF_DISABLE_REDQUEEN=1` is a kill-switch (used by the cold-solve
    // parity gate to A/B per-input capture against the dictionary-only path).
    if std::env::var("BHF_DISABLE_REDQUEEN").as_deref() != Ok("1") {
        child_env.push((
            "BHF_CMP_SHM".to_owned(),
    ...
    let owns_runtime_instrumentation = std::fs::read_to_string(harness_dir.join("main"))
        .map(|s| {
            s.contains("BHF_JVM_LAUNCHER")
                || s.contains("BHF_CS_LAUNCHER")
                || s.contains("BHF_JS_LAUNCHER")
    ...
    if cross_wrapper.is_some() || owns_runtime_instrumentation {
        ...
        bhfeprintln!(
            "bhf auto: libbhf_runtrace.so not found beside bhf or in a \
             sibling bhf_runtrace_shim archive directory; \
...
mod generation_outcome_tests {
    ...
    fn checked_in_derived_servant_operation_selects_servant_direct_lane() {
        let root = tempfile::Builder::new()
            .prefix("bhf-auto-servant-detect-")
            .tempdir()
            .unwrap();
...
mod c_auto_sequence_candidate_tests {
    ...
    fn returning_constructor_enables_opaque_handle_sequence() {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-sequence-ctor-{nonce}"));
    ...
    fn public_in_place_lifecycle_enables_sequence_with_opaque_source_receiver() {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-sequence-in-place-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn private_static_lifecycle_helpers_do_not_enable_auto_sequence() {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-sequence-private-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn destructor_only_output_contract_stays_direct() {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-output-only-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn zero_parameter_target_stays_direct_without_panicking() {
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-zero-param-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn static_operation_uses_proven_public_returning_lifecycle() {
        ...
        let nonce = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-c-static-sequence-ctor-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
...
fn try_build_c(
    ...
    // C++ dialect selection: an explicit `--cxx-std` (BHF_CXX_STD) wins; else a
    ...
    let explicit_std = std::env::var("BHF_CXX_STD")
        .ok()
        ...
        && which::which("gcc").is_ok()
    {
        ...
        if build_succeeded(&out) {
            ...
            bhfeprintln!(
                "bhf auto: {harness_id}: clang rejected this target and gcc \
...
fn cxx_dialect_cache_path(work_dir: &Path, harness_id: &str) -> PathBuf {
    ...
    hasher.update(b"bhf-cxx-dialect-context-v2\0");
...
fn resolve_foreign_candidate_target(
    ...
    let Some(target) = resolve_cross_target(guard) else {
        return Err(format!(
            "definition is guarded by platform conditional `{guard}`, which bhf has no \
    ...
    if matches!(candidate.lang, crate::auto::candidate::Lang::Ada) {
        return Err(format!(
            "foreign-platform Ada target (guard `{guard}`) needs a matching GNAT cross \
             toolchain; bhf auto currently cross-compiles only C/C++ (would target {})",
...
/// was simply lost, because bhf always used the same one.
///
...
fn try_build_ada(
    ...
    //     standalone `bhf instrument` subcommand creates this
...
fn stub_out_unbuildable_bodies(
    ...
    // bhf build extends the user .gpr, so merely deleting the staged copy
...
fn ensure_ada_src_instrumented(
    ...
    for source_path in &source_files {
        ...
        let mut source = match crate::source_text::read_source_text(&source_path) {
            ...
            Err(e) => {
                bhfeprintln!(
                    "bhf auto: skipping {}: read error: {e}",
                    source_path.display()
                );
                continue;
            }
        ...
        let ast = match ada_parser::reconcile::build_structural_ast(&source, None, &source_path) {
            ...
            Err(e) => {
                bhfeprintln!(
                    "bhf auto: skipping {}: parse error: {e}",
                    source_path.display()
                );
                continue;
            }
        ...
        let instrumented = match instrumenter::instrument_unit(instrumenter::InstrumentArgs {
            ...
            Err(e) => {
                bhfeprintln!(
                    "bhf auto: skipping {}: instrumenter: {e}",
                    source_path.display()
                );
                continue;
            }
        ...
        // the build project (`bhf_build.gpr`) compiles src_instrumented +
        ...
        if let Ok(sidecar) = instrumenter::line_map_sidecar_json(
            ...
            let sidecar_path = dst.join(format!("{dest_name}.bhf-lines.json"));
...
/// inside the bhf_work directory itself (so we don't pick up
    ...
    events
        ...
        .filter_map(|event| match event {
            crate::auto::runtrace::RuntraceEvent::EnvVarMissing { name, .. } => {
                ...
                {
                    Some((name.clone(), format!("/tmp/bhf/fake_env/{name}")))
                } else {
                    None
                }
            }
...
/// the TARGET's configuration dependency. Rejecting them stops bhf fabricating
...
/// so `bhf replay --finding <id>` can reconstruct the exact
...
mod undefined_external_types_tests {
    ...
    /// The residue of the placeholder bhf ACTUALLY synthesizes today. The
    ...
    fn a_codegen_bug_in_the_generated_harness_stays_failed_build() {
        ...
        assert_eq!(
            undefined_external_types(&errors, &idx, &synthesized),
            None,
            "a fault in bhf's own generated code must stay visible"
        );
    }
    ...
    fn mixed_residue_and_codegen_bug_stays_failed_build() {
        ...
        let errors = vec![BuildErrorKind::Other {
            tail: "handler.cpp:10:43: error: too many arguments to function call, expected 0, \
                   have 1\nmain.cpp:88:9: error: use of undeclared identifier 'bhf_decode_widget'"
                .to_owned(),
        }];
        let synthesized = BTreeSet::from(["CString".to_owned()]);
        assert_eq!(undefined_external_types(&errors, &idx, &synthesized), None);
...
mod corpus_reseed_tests {
    ...
    fn reseed_from_corpus_queue_loads_deduped_capped_inputs() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let work = std::env::temp_dir().join(format!("bhf-reseed-{nonce}"));
...
mod cmplog_accumulate_tests {
    ...
    fn append_cmplog_records_accumulates_only_cmplog_lines() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-cmplog-acc-{nonce}"));
        std::fs::create_dir_all(&dir).unwrap();
...
mod stamp_tests {
    ...
    fn tmp_finding() -> std::path::PathBuf {
        let n = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("bhf-stamp-{n}.json"));
        fs::write(
            &p,
    ...
    fn tmpdir(prefix: &str) -> std::path::PathBuf {
        let n = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("bhf-attempt-{prefix}-{n}"));
    ...
    fn stamps_pass_and_env_injected() {
        ...
        stamp_runtime_mode(
            ...
            &[(
                "ACME_HOME".to_owned(),
                "/tmp/bhf/fake_env/ACME_HOME".to_owned(),
            )],
            None,
        )
        .unwrap();
    ...
    fn env_injections_deduplicate_repeated_missing_names() {
        let events = vec![
            RuntraceEvent::EnvVarMissing {
                api: "getenv".to_owned(),
                name: "BHF_FAKE_IDENTITY".to_owned(),
            },
            RuntraceEvent::EnvVarMissing {
                api: "getenv".to_owned(),
                name: "BHF_FAKE_IDENTITY".to_owned(),
...
mod platform_stub_tests {
    ...
    fn tmp(prefix: &str) -> PathBuf {
        let n = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("bhf-pstub-{prefix}-{n}"));
...
mod refused_repair_tracking_tests {
    ...
    fn refused_self_source_dependency_falls_back_to_declaration_stub() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-self-source-fallback-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        fs::write(
            root.join("value.h"),
            "namespace Json {\n\
