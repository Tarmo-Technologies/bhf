...
fn parse_c_functions_preprocessed(
    ...
    if should_preprocess(mode, source, !defines.is_empty()) {
        ...
        if fns.is_empty() {
            ...
            if !raw.is_empty() {
                bhfeprintln!(
                    "bhf auto: note: preprocessing removed all {} discoverable \
...
fn parse_cpp_functions_preprocessed(
    ...
    if should_preprocess(mode, source, !defines.is_empty()) {
        ...
        if fns.is_empty() {
            ...
            if !raw.is_empty() {
                bhfeprintln!(
                    "bhf auto: note: C++ preprocessing removed all {} discoverable \
...
/// std versions. So a bhf rebuilt on a newer toolchain hashed byte-identical
...
/// on which bhf build computed it.
///
...
/// across bhf rebuilds (per-target corpus/build dirs survive a rebuild).
...
pub struct DirFilter {
    ...
    /// tree. Excluded by PATH (not just by the default `bhf_work` name) so a
    /// custom `--work-dir` under the tree — e.g. `bhf auto --work-dir wd .` —
...
fn discover_on_this_stack(
    ...
    if let Some(relaxed) = filter.without_organizational_exclusions() {
        ...
        if !empty_langs.is_empty() {
            ...
            if out.len() > before {
                ...
                bhfeprintln!(
                    "bhf auto: {} candidate(s) recovered from organizational \
    ...
    // top, but harnessing a harness is meaningless — and bhf emits
    ...
    // re-run over a directory of prior bhf output never targets itself. But when
...
/// IDL — the inputs that change how bhf recovers a build.
...
/// build-context file's content. Skips VCS, the bhf work dir, and common
...
fn collect_build_context_files(
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            if matches!(
                name,
                ".git" | ".hg" | ".svn" | "bhf_work" | "target" | "node_modules"
            ) {
                continue;
            }
...
/// be bhf's own generated output. It is dropped when a real library target
...
/// They are the only expert baseline there is: a bhf run over a tree that
...
/// Set the discovery wall-clock ceiling. `BHF_DISCOVERY_TIME` overrides
/// (in seconds; `0` disables the ceiling entirely).
pub(crate) fn set_time_budget(budget: Option<std::time::Duration>) {
    let resolved = match std::env::var("BHF_DISCOVERY_TIME")
        .ok()
...
fn walk(
    ...
    // and `auto` — bhf produced no target list at all. The static scan already
    ...
    if timed_out > 0 {
        bhfeprintln!(
            "bhf auto: discovery reached its time budget and skipped {timed_out} file(s); \
             the target list is PARTIAL. Raise --campaign-time, set BHF_DISCOVERY_TIME \
    ...
    if skipped > 0 {
        ...
        bhfeprintln!(
            "bhf auto: discovery reached its memory ceiling ({ceiling}) and skipped \
             {skipped} file(s); the target list is PARTIAL. Raise it with \
             BHF_MAX_MEMORY_KB, or scan a subdirectory."
        );
    }
    result
}
...
fn discovery_pool() -> &'static rayon::ThreadPool {
    ...
    POOL.get_or_init(|| {
        let threads = std::env::var("BHF_DISCOVERY_JOBS")
            .ok()
...
/// [`discover_file`] wrapped so a bhf-internal PANIC while parsing/ranking ONE
...
/// Stage timings to stderr when `BHF_PROFILE` is set. Off by default and
...
pub(crate) fn bhfprof(label: &str, start: std::time::Instant) {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    if *ON.get_or_init(|| std::env::var_os("BHF_PROFILE").is_some()) {
        bhfeprintln!("[prof] {label}: {:.3}s", start.elapsed().as_secs_f64());
    }
}
...
fn discover_file(path: &Path, out: &mut Vec<Candidate>, preprocess: PreprocessMode) -> Result<()> {
    ...
    bhfprof("pre:compile_db_defines", _tcd);
    bhfprof("pre:lang+dialect+defines", _tpre);
    ...
    match lang {
        ...
        Lang::Cpp => {
            ...
            bhfprof("cpp:parse", _t0);
            let _t1 = std::time::Instant::now();
            let fns = dedup_cpp_functions(fns);
            bhfprof("cpp:dedup", _t1);
...
fn is_excluded_dir(path: &Path, filter: &DirFilter) -> bool {
    ...
    if matches!(
        name,
        // `.bhf-build` = build_probe::PROBE_DIR (the --probe-build output:
...
mod tests {
    ...
    fn a_projects_own_harnesses_are_enumerable_even_though_they_are_not_targets() {
        ...
        let root = std::env::temp_dir().join(format!("bhf-experts-{}", std::process::id()));
    ...
    fn stable_hasher_and_ids_are_pinned_build_stable_constants() {
        ...
        h.write(b"bhf");
        assert_eq!(h.finish(), 0x0057_3219_1352_4043, "FNV-1a(\"bhf\")");
        // A harness id end-to-end (path + line + name): the `<work>/harnesses/<id>/` dir
        // name must be byte-identical across bhf builds so build artifacts and
    ...
    fn tmpdir() -> PathBuf {
        let n = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("bhf-discover-{n}"));
    ...
    fn excludes_a_custom_work_dir_nested_in_the_scanned_tree() {
        // A real library target, plus bhf's own work dir (a CUSTOM name, not
        // `bhf_work`) holding a generated harness that itself looks fuzzable.
        let root = tmpdir();
        fs::write(
            root.join("lib.c"),
    ...
    fn fuzz_driver_entry_points_are_never_targets() {
        ...
        // Excluded both by the `fuzz/` dir skip AND by NAME, so bhf can't target
    ...
    fn skips_excluded_dirs() {
        ...
        fs::write(
            ...
            "int bhf_run_one(const unsigned char *d, unsigned long n) { return 0; }\n",
        )
        .unwrap();
        let cands = discover(&root).unwrap();
        assert!(
            !cands.iter().any(|c| c.name == "bhf_run_one"),
