...
//!
//! `bhf auto` builds as far as it can, stubbing whatever a target needs but
...
//! dependencies at once — each marked whether bhf stubbed it (build
...
impl DepKind {
    ...
    /// artifacts even when BHF managed to substitute a reduced-fidelity
    /// implementation.
...
/// How BHF knows a requirement exists. This prevents a best-effort inference
...
pub struct DepEntry {
    ...
    /// True when bhf faked it so the build continued; false when it is still
...
impl DependencyManifest {
    ...
    pub fn push_merge_detailed(
            ...
            .find(|e| e.kind == kind && e.name == name)
        {
            ...
            // an actual target proves BHF substituted that same critical
    ...
    pub fn render_text(&self) -> String {
        ...
        out.push_str("BHF missing-dependency manifest\n");
        out.push_str("===================================\n\n");
        out.push_str(&format!(
...
pub fn acquisition_hint(kind: DepKind, name: &str) -> Option<String> {
    ...
    match kind {
            ...
            if is_configure_generated_header(leaf) {
                return Some(format!(
                    "run the project's configure step (`./configure`, `cmake`, or `autoreconf -i \
                     && ./configure`) to generate '{name}', then re-run bhf with --probe-build \
        ...
        DepKind::EnvVar => Some(format!(
            "set {name} before the build/run (bhf used an empty/fake value)"
        )),
        ...
        DepKind::IdlInterface => Some(format!(
            "bring '{name}' (and its IDL #includes), then re-run with --run-untrusted to compile it \
             (tao_idl / `bhf fake-corba --idl {name}`), or copy the generated C/S stub headers"
        )),
