...
//! Self-diagnostics: capture bhf's OWN internal failures (panics, codegen
...
//!     written as `<work>/auto/bug-report.json` + `.md`, stamped with the bhf
...
//! bug report is exclusively bhf's OWN defects.
...
pub fn init(debug: bool) {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        // (a genuine bhf bug outside per-target work), fall back to the
        ...
        if THREAD_IN_CATCH.with(|c| c.get()) {
            ...
            bhfeprintln!(
                "bhf: internal panic{}: {message} (recorded in bug-report.json{})",
                location
                    .as_ref()
                    .map(|l| format!(" at {l}"))
                    .unwrap_or_default(),
            ...
            if let Some(path) = flush_report() {
                bhfeprintln!(
                    "bhf: internal panic — bug report written to {}",
                    path.display()
                );
            }
            default_hook(info);
...
pub fn catch<T>(context: IssueContext, f: impl FnOnce() -> T) -> Result<T, String> {
    ...
    match outcome {
        ...
        Err(_payload) => {
            ...
            let reason = format!(
                "bhf internal error (see bug-report.json): {}{}",
                cap.message,
                cap.location
                    .as_ref()
                    .map(|l| format!(" at {l}"))
...
/// Where a bhf-internal issue happened, so the maintainer can reproduce it.
...
pub enum IssueCategory {
    /// bhf itself panicked while processing an input (highest priority — a bug).
    InternalPanic,
    /// A codegen/parser recovery artifact (a type bhf's own emitter left
    /// unresolved) — a defect in bhf's harness generation (a bug).
    CodegenDefect,
    /// bhf could not build a harness for a target — an unsupported parameter
    ...
    /// errors). May be a bhf gap or a missing dependency (see missing-deps.txt).
    FailedBuild,
    /// bhf could not fuzz the target and fell back to a source-only static
...
/// One boiled-down bhf defect, deduplicated across the run.
...
/// bhf version (git tag/describe) + short commit, stamped at build time
/// (see `build.rs`); the same string `bhf --version` prints.
pub fn version() -> &'static str {
    option_env!("BHF_VERSION_FULL").unwrap_or(env!("CARGO_PKG_VERSION"))
}
pub fn commit() -> &'static str {
    option_env!("BHF_GIT_COMMIT").unwrap_or("unknown")
}

/// Build the bug report from the recorded panics plus `extra` (codegen defects
/// and the per-target outcomes bhf couldn't fully fuzz), then write
...
pub fn write(auto_dir: &Path, generated_at: &str, extra: &[InternalIssue], always: bool) -> usize {
    ...
    let report = BugReport {
        schema_version: 1,
        bhf_version: version().to_owned(),
        bhf_commit: commit().to_owned(),
...
/// Human label + whether it's a bhf bug (vs a limitation/gap), per category.
...
/// explains why. These are the cases where bhf correctly SKIPS a parameter
...
/// expected gaps the user can't file as bhf bugs; the second is fixable in
    ...
    if s.contains("phase c") || s.contains("needs lifecycle support") {
        return Some(
            "working as intended: an opaque handle/pointer type whose construction needs \
             runtime lifecycle support (roadmap Phase C) — bhf skips this parameter and \
...
fn render_md(report: &BugReport) -> String {
    ...
    out.push_str("# bhf bug report\n\n");
    out.push_str(&format!(
        "bhf {} ({}) — generated {}\n\n",
        report.bhf_version, report.bhf_commit, report.generated_at
    ));
    out.push_str(
        "What bhf could NOT fully handle on your tree — its own bugs (`PANIC`, \
    ...
    if report.issue_count == 0 {
        out.push_str(
            "**No issues: bhf built and fuzzed every discovered target cleanly.** \
    ...
    {
        out.push_str(
            "> Rows marked **(working as intended)** are KNOWN limitations, not bhf bugs — \
             bhf skipped that parameter/target on purpose and kept going. You do NOT need to \
             report them.\n\n",
        );
    }
    for (n, issue) in report.issues.iter().enumerate() {
        ...
        // A panic also carries WHERE in bhf it fired (the pin) plus the full
        ...
        if issue.category == IssueCategory::InternalPanic {
            if let Some(bt) = &issue.backtrace {
                let frames = bhf_frames(bt, 3);
                if !frames.is_empty() {
                    out.push_str(&format!("   bhf: {}\n", frames.join(" <- ")));
                }
                out.push_str(
                ...
                out.push_str(&format!(
                    "   panicked at {detail} — rerun with `--debug` for the bhf frame\n"
                ));
            }
        }
        out.push('\n');
...
/// The first few BHF source frames from a captured backtrace (skipping std,
/// core, and the bug_report hook machinery), so a panic row names WHERE in
/// bhf it fired without the user re-typing the whole trace.
fn bhf_frames(backtrace: &str, max: usize) -> Vec<String> {
...
mod tests {
    ...
    fn catch_records_a_panic_and_returns_err_reason() {
        ...
        assert!(reason.contains("bhf internal error"), "{reason}");
    ...
    fn write_skips_clean_run_but_debug_writes_confirmation() {
        let _g = TEST_GUARD.lock().unwrap_or_else(|e| e.into_inner());
        reset_issues();
        let dir = std::env::temp_dir().join(format!("bhf-bugreport-a-{}", std::process::id()));
    ...
    fn write_emits_categorized_and_deduped_issues() {
        ...
        reset_issues();
        init(false);
        let dir = std::env::temp_dir().join(format!("bhf-bugreport-b-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn known_boundary_unsupported_types_are_tagged_working_as_intended() {
        ...
        reset_issues();
        init(false);
        let dir = std::env::temp_dir().join(format!("bhf-bugreport-wai-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
