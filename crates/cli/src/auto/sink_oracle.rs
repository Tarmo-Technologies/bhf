...
//!
//! bhf's coverage agent instruments the target's bytecode so each call site of a
...
//! `sink_report.txt` (see `java_runtime/com/bhf/Sink`). A sink reached while the
...
/// Map a `Sink` kind integer (must match `com.bhf.Sink`) to its rule id, CWE, and
/// a short human label.
fn kind_to_rule(kind: u32) -> Option<(&'static str, &'static str, &'static str)> {
    match kind {
        1 => Some(("BHF-421", "CWE-502", "untrusted deserialization")),
        2 => Some(("BHF-404", "CWE-78", "process execution")),
        3 => Some(("BHF-420", "CWE-94", "dynamic code evaluation")),
        4 => Some(("BHF-419", "CWE-89", "dynamic SQL execution")),
        5 => Some(("BHF-432", "CWE-90", "LDAP directory search")),
        _ => None,
    }
}
...
mod tests {
    ...
    fn kind_mapping_covers_the_agent_sink_kinds() {
        // Must stay in lockstep with com.bhf.Sink's constants.
        assert_eq!(kind_to_rule(1).unwrap().0, "BHF-421");
        assert_eq!(kind_to_rule(2).unwrap().0, "BHF-404");
        assert_eq!(kind_to_rule(3).unwrap().0, "BHF-420");
        assert_eq!(kind_to_rule(4).unwrap().0, "BHF-419");
        assert_eq!(kind_to_rule(5).unwrap().0, "BHF-432");
        assert!(kind_to_rule(99).is_none());
    }

    #[test]
    fn read_reached_kinds_dedupes_across_spawns() {
        let tmp = std::env::temp_dir().join(format!("bhf-sink-read-{}", std::process::id()));
    ...
    fn run_sink_oracle_emits_one_finding_per_reached_kind() {
        let tmp = std::env::temp_dir().join(format!("bhf-sink-oracle-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&tmp);
        ...
        assert!(
            joined.contains("BHF-421"),
            "deserialization finding:\n{joined}"
        );
        assert!(joined.contains("BHF-419"), "SQL finding:\n{joined}");
