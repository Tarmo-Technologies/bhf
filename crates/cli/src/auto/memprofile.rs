...
//! Memory-consumption replay — uncontrolled resource consumption (CWE-400, BHF-558),
//! the MemLock / MemConFuzz idea adapted to bhf's replay path.
//!
...
/// Profile every C/C++ harness's corpus, writing a BHF-558 finding for the worst
...
fn profile_one(work_dir: &Path, hdir: &Path, harness_id: &str, index: &mut usize) -> usize {
    // The ASan `main` built during fuzzing is the replay binary; the bhf C/Ada
...
fn write_mem_finding(
    ...
    let cluster_key_full = hex(&Sha256::digest(format!("BHF-558:{harness_id}").as_bytes()));
