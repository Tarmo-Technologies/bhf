...
//! and compile a bhf harness, then emit a `harnesses/<id>/main` launcher script
//! that runs the target in a persistent JVM under bhf's own coverage agent. The
//! builtin engine then drives that JVM over the `BHF_FRAMED` fork-server
...
//!    (the GNAT-less rule).
//! 2. **Agent jar** — ensure `bhf-jvm-agent.jar` (coverage agent + driver +
//!    BhfData + shaded ASM) is built (cached; built once by `build-agent.sh`).
...
//!    `bhfgen.Harness.bhfRunOne(byte[])` (a static call, a `new` for a
//!    constructor, or a no-arg-ctor receiver + method call for an instance method);
//!    compile it against the agent jar (for `BhfData`) + the target classpath.
//! 5. **Emit the launcher** — `harnesses/<id>/main`, an executable script that execs
//!    `java -javaagent:<agent> -cp <cp> com.bhf.Driver bhfgen.Harness`. It
//!    carries the `BHF_FRAMED` marker the engine greps for, and inherits the
//!    engine's `BHF_FRAMED` / `BHF_COV_SHM` env across the exec.
...
pub fn build_java_harness(
    ...
    let agent_jar = match ensure_agent_jar() {
        ...
        Err(e) => {
            return JavaBuildResult::Failed {
                reason: format!("could not build the bhf JVM agent jar: {e}"),
                skip: false,
            }
        }
    };
    ...
    // like org.json's JSONException) via BHF_EXPECTED_EXCEPTIONS.
    ...
    let script = format!(
        "#!/bin/sh\n\
         # BHF_FRAMED BHF_JVM_LAUNCHER bhf JVM driver launcher (native Java lane).\n\
         ...
         # file-backed BHF_COV_SHM map; the Driver speaks the framed protocol.\n\
         # BHF_EXPECTED_EXCEPTIONS = the target's declared `throws` (expected\n\
         # rejections of bad input, suppressed as non-findings by the Driver).\n\
         # BHF_SINK_OUT = where the agent records input-reachable dangerous sinks\n\
         # (deserialization / exec / eval / SQL / LDAP); bhf reads it after the run.\n\
         # BHF_SCALAR_ONLY_TARGET = the target has no byte/char channel, so an\n\
         ...
         BHF_SCALAR_ONLY_TARGET=\"{scalar_only}\" \\\n\
         BHF_SINK_OUT=\"$(dirname \"$0\")/sink_report.txt\" \\\n\
         exec \"{java}\"{preview} -javaagent:\"{agent}\" -cp \"{cp}\" \\\n\
         \x20   com.bhf.Driver {harness_class} \"$@\"\n",
...
fn is_non_source_dir(name: &str) -> bool {
    ...
    if matches!(
        n.as_str(),
        "bhf_work" | ".git" | "target" | "build" | "node_modules" | "out" | ".gradle"
    ) {
        return true;
    }
...
fn maven_classpath(module: &Path, work_dir: &Path) -> Result<String, String> {
    ...
    // bhf's offline classpath build deliberately ignores the project's CI
    ...
    // the whole phase fails before producing classes. bhf doesn't care about the
...
/// bhf's offline build doesn't honour a project's CI Maven-version policy.
...
fn gradle_runtime_classpath(module: &Path, gradle: &Path) -> Option<String> {
    let init = module.join(".bhf-init.gradle");
    let script = "allprojects { tasks.register('bhfCp') { doLast { try { \
...
/// Collect every `.java` file under `root`, skipping the bhf work dir and
...
fn agent_jar_cache_path() -> PathBuf {
    if let Ok(dir) = std::env::var("BHF_JVM_CACHE") {
        return PathBuf::from(dir).join("bhf-jvm-agent.jar");
    }
...
mod tests {
    ...
    fn agent_cache_path_honours_env() {
        // BHF_JVM_CACHE override is respected (don't mutate global env in the
        // test; just assert the default shape is sane).
        let p = agent_jar_cache_path();
        assert!(p.ends_with("bhf-jvm-agent.jar"), "{}", p.display());
    }
    ...
    fn collect_java_sources_finds_nested_and_skips_work_dir() {
        ...
        std::fs::create_dir_all(root.join("bhf_work/auto")).unwrap();
        std::fs::write(root.join("bhf_work/auto/Skip.java"), "class Skip {}").unwrap();
