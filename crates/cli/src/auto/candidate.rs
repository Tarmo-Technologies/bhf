...
pub enum Lang {
    ...
    /// persistent JVM whose bytecode is instrumented by bhf's own coverage
    ...
    /// persistent `perl` process (run under `perl -d:BhfCov`) that speaks the
    ...
    /// coverage into a shared map; bhf maps that map onto its own
    /// `BHF_COV_SHM` AFL-style bitmap and drives a warm, persistent CLR over
