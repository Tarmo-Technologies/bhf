...
//!
//! bhf's own rules are deliberately thin in some languages (Rust especially).
...
//! build-time `cargo deny` / license-audit guarantee: the bhf binary links
...
fn max_external_tool_output_bytes() -> usize {
    crate::resource_limits::dynamic_bytes(
        "BHF_MAX_EXTERNAL_TOOL_OUTPUT_BYTES",
        256,
...
/// One external analyzer bhf can drive.
struct Adapter {
...
/// tool's own license, because bhf never links any of them.
...
fn run_tool(adapter: &Adapter, root: &Path) -> Option<String> {
    ...
    if captured.timed_out {
        bhfeprintln!(
            "bhf: external analyzer '{}' exceeded its 30-minute timeout; skipping its output",
            adapter.binary
        );
        return None;
    }
    if captured.stdout_truncated {
        bhfeprintln!(
            "bhf: external analyzer '{}' exceeded the {} MiB output cap; skipping its \
             incomplete report (raise BHF_MAX_EXTERNAL_TOOL_OUTPUT_BYTES if needed)",
            adapter.binary,
    ...
    if captured.stderr_truncated {
        bhfeprintln!(
            "bhf: external analyzer '{}' stderr exceeded the bounded capture; diagnostics were truncated",
            adapter.binary
        );
    }
...
mod tests {
    ...
    fn write_finding_emits_static_record_with_tool_and_cwe() {
        let work = std::env::temp_dir().join(format!("bhf-ext-{}", std::process::id()));
