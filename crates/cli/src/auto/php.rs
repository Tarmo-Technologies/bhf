...
//!
//! Strategy (like the Ruby/Lua/Perl lanes): reuse bhf's builtin engine over the
...
//! launcher execs `php -d pcov.enabled=1 bhf_driver.php`, which `require`s the
//! target, calls the function, records per-line edge coverage via the `pcov` extension
//! folded into the shared `BHF_COV_SHM` bitmap, and reports an uncaught bug-class
