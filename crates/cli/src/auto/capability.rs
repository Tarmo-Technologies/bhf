...
//!
//! Crash-only fuzzers report where the program *breaks*. bhf's runtrace shim
...
//! BHF-668 finding per `(harness, kind)` whose input-triggered operand set is
...
/// Profile every native harness's capabilities and emit BHF-668 findings for the
...
pub fn run_capability_profile(work_dir: &Path) -> usize {
    ...
    if !profiles.is_empty() {
        let out = json!({ "schema": "bhf.capabilities/v1", "harnesses": profiles });
...
fn classify(ev: &RuntraceEvent) -> Option<Capability> {
    let cap = match ev {
        ...
        | RuntraceEvent::FileMissing {
            ...
            if runtrace::is_bhf_owned_resource(path) {
                return None;
            }
            Capability {
...
/// Write one clustered BHF-668 finding for a `(harness, kind)` with a non-empty
...
fn write_capability_finding(
    ...
    let cluster_key_full = hex(&Sha256::digest(
        format!("BHF-668:{harness_id}:{kind}").as_bytes(),
    ));
...
fn is_native_harness(hdir: &Path, harness_id: &str) -> bool {
    ...
    !std::fs::read_to_string(hdir.join("main"))
        .map(|s| s.contains("BHF_JVM_LAUNCHER"))
        .unwrap_or(false)
}
...
mod tests {
    ...
    fn write_finding_has_no_sink_and_carries_cwe() {
        let work = std::env::temp_dir().join(format!("bhf-cap-{}", std::process::id()));
