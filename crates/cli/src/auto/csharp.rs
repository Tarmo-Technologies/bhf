...
//!
//! Strategy (like the COBOL/Fortran lanes): reuse bhf's builtin engine. A
...
//! bhf maps that map onto its own `BHF_COV_SHM` AFL-style bitmap and
...
//! `parse_csharp_finding` maps the exception type to a BHF rule + CWE.
//!
...
pub struct CSharpMethod {
    ...
    /// namespace). Drives `BHF_CS_NAMESPACE` so the target library's own
