...
pub fn run(args: FakeCorbaArgs) -> i32 {
    ...
    if let Err(error) = std::fs::create_dir_all(&work_dir) {
        bhfeprintln!("create work directory '{}': {error}", work_dir.display());
        return 1;
    }
...
/// bhf's own IDL parser — it executes no project code — so it runs by default
...
pub fn auto_generate_from_tree(source_root: &Path, work_dir: &Path, force: bool) -> usize {
    ...
    if let Err(error) = ::fake_corba::generate_fake_corba_with(source_root, &output_dir, force) {
        bhfeprintln!("bhf auto: fake-corba base generation: {error}");
    }
    ...
    for idl in &idls {
        ...
        match idl_parser::parse_idl_file_recovering_with_options(
            ...
            Err(error) => bhfeprintln!("bhf auto: skipping {}: {error}", idl.display()),
        }
    }
    ...
    if mapped > 0 {
        if let Err(error) = write_idl_ast(
            ...
            bhfeprintln!("bhf auto: write aggregate IDL mapping: {error}");
            return 0;
        }
    }
    ...
    match prune_generated_real_unit_collisions(source_root, &output_dir, Some(work_dir)) {
        Ok(removed) => {
            if let Err(error) = checkpoint_real_fake_collisions(&output_dir, removed) {
                bhfeprintln!("bhf auto: fake-corba collision checkpoint: {error}");
            }
        }
        Err(error) => {
            bhfeprintln!("bhf auto: fake-corba collision pruning: {error}");
        }
    }
    mapped
}
...
/// `#pragma bhf_warning "include ... not found"` breadcrumb and the mapping
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                let skip = path.file_name().and_then(|n| n.to_str()).is_some_and(|n| {
                    matches!(n, ".git" | "bhf_work" | "target" | "build" | "node_modules")
                });
                if !skip {
                    stack.push(path);
                }
            ...
            if path.is_dir() {
                let skip = entry.file_name().to_string_lossy().starts_with('.')
                    || matches!(
                        ...
                        "target" | "build" | "node_modules" | "bhf_work"
                    );
                if !skip {
                    stack.push(path);
                }
...
fn report_idl_warnings(warnings: &[String]) {
    for line in reportable_idl_warnings(warnings) {
        bhfeprintln!("IDL mapping warning: {line}");
    }
}
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-fake-corba-{name}-{nonce}"));
