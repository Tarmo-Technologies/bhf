...
//! `bhf snippet` — fuzz ONE pasted function with no project, build, or
...
//! machinery: paste a single function (from stdin or a file), and bhf detects
...
pub struct SnippetArgs {
    ...
    /// `./bhf_snippet`. Kept after the run so you can inspect the generated
    /// harness and re-run `bhf report` over `<work-dir>`.
    #[arg(long = "work-dir", default_value = "bhf_snippet")]
            ...
            if buf.trim().is_empty() {
                anyhow::bail!(
                    "no snippet on stdin — pass a file path or pipe a function in, e.g. \
                     `bhf snippet fn.c` or `cat fn.c | bhf snippet --lang c`"
                );
            }
            Ok(buf)
        }
...
pub fn run(args: SnippetArgs) -> i32 {
    ...
    bhfeprintln!(
        "bhf snippet: detected {}, fuzzing for up to {}s (project at {})",
        lang_token(lang),
    ...
    let auto_args = match AutoArgsCarrier::try_parse_from(&argv) {
        ...
        Err(error) => {
            // A parse failure here is a bhf bug (we built the argv), not user
            // error — surface it rather than printing clap usage for `auto`.
            bhfeprintln!("internal error: could not construct auto args: {error}");
            return 2;
        }
    };
...
mod tests {
    ...
    fn materialize_go_adds_module_and_package() {
        let dir = std::env::temp_dir().join(format!("bhfsnip-go-{}", std::process::id()));
    ...
    fn materialize_rust_adds_manifest() {
        let dir = std::env::temp_dir().join(format!("bhfsnip-rs-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        materialize(
            LangSelector::Rust,
