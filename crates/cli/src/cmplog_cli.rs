...
//! `bhf cmplog` subcommand: ingest a runtrace audit log and
//! print recovered cmplog operands as a JSON token list.
//!
//! Use case: after `BHF_CMPLOG=1 BHF_RUNTRACE_LOG=audit.jsonl
//! ./harness < input` writes runtime cmp operands to audit.jsonl,
//! `bhf cmplog ingest --log audit.jsonl` prints the deduplicated
//! token list. Pipe that into a future `bhf fuzz --dictionary
...
pub struct IngestArgs {
    /// Path to a `BHF_RUNTRACE_LOG` JSONL file produced by
    /// the runtrace_shim with BHF_CMPLOG=1 set.
...
fn run_ingest(args: IngestArgs) -> i32 {
    ...
    match args.format.as_str() {
        ...
        other => {
            bhfeprintln!("error: unknown --format {other}");
            return 2;
        }
    }
    0
