...
/// exactly as it did.
#[macro_export]
macro_rules! bhfeprintln {
    () => { $crate::auto::dashboard::emit_line("") };
    ($($arg:tt)*) => { $crate::auto::dashboard::emit_line(&format!($($arg)*)) };
}

/// [`bhfeprintln`] without the trailing newline, for pre-rendered blocks that
/// carry their own.
#[macro_export]
macro_rules! bhfeprint {
...
`bhf binary scan`). TIP: the list is greppable — `bhf --help | grep -i sarif`.")]
struct Args {
    ...
    /// Debug mode: capture a backtrace for any bhf-internal panic (sets
    /// RUST_BACKTRACE), keep going past a file that crashes bhf, and enrich
    /// the generated bug-report.json. Use this when bhf itself errors on your
...
// written so `bhf --help | grep -i <keyword>` surfaces the right command
...
enum Command {
    ...
    /// Import, export, and merge corpora across AFL++, libFuzzer, Honggfuzz, and bhf layouts
    ...
    // ── Reference data ──────────────────────────────────────────────────────
    /// Browse the rule catalog: BHF-NNNN ids, CWE/CERT/MISRA mappings, severity
    Rules(rules::RulesArgs),

    // ── Governance & air-gapped distribution ────────────────────────────────
    /// Explain, validate, or dry-run BHF policy-as-code governance rules
...
/// `bhf list <targets|oracles>` — enumerate discovered fuzzable targets or
...
mod tests {
    ...
    fn strict_permissive_without_probe_exits_zero() {
        assert_eq!(run_from(["bhf", "--profile", "strict-permissive"]), 0);
    }

    #[test]
    fn rules_list_exits_zero() {
        assert_eq!(run_from(["bhf", "rules", "list"]), 0);
    }

    #[test]
    fn rules_show_known_id_exits_zero() {
        assert_eq!(run_from(["bhf", "rules", "show", "BHF-201"]), 0);
    }

    #[test]
    fn rules_show_unknown_id_exits_two() {
        assert_eq!(run_from(["bhf", "rules", "show", "BHF-999-unknown"]), 2);
    }
    ...
    fn build_subcommand_discovers_compiler_and_invokes_build() {
        ...
        let _log = EnvVarGuard::set("BHF_BUILD_LOG", log_path.as_os_str());

        let exit = run_from(["bhf", "build", work_dir.to_str().expect("utf-8 work dir")]);

        let argv = fs::read_to_string(log_path).expect("build argv log is readable");
        assert_eq!(exit, 0);
        assert!(argv.contains("-P"));
        assert!(argv.contains("bhf_build.gpr"));
        assert!(work_dir.join("build/H-TEST/bhf_build.gpr").is_file());
    }
    ...
    fn build_subcommand_includes_fake_corba_source_root_when_present() {
        ...
        assert_eq!(exit, 0);
        let project = fs::read_to_string(work_dir.join("build/H-TEST/bhf_build.gpr"))
    ...
    fn build_subcommand_writes_build_local_runtime_project() {
        ...
        let project =
            fs::read_to_string(build_dir.join("bhf_build.gpr")).expect("project file is readable");
    ...
    fn build_instruments_target_with_trace_pc_but_not_runtime() {
        // #412: a real `bhf build` must produce a self-consistent coverage
        ...
        let runtime_project = fs::read_to_string(build_dir.join("adafuzz_runtime.gpr"))
            .expect("build-local runtime project is readable");
        let sentinel = build_dir.join(".bhf_ada_cov");
        ...
        assert_eq!(
            ...
            sentinel.is_file(),
            "trace-pc flag in Bhf_Build must match the coverage sentinel:\n{project}"
        );
    }
    ...
    fn build_subcommand_writes_cross_attributes_and_uses_prefixed_probe() {
        ...
        let _path = PathEnvGuard::prepend(&bin_dir);
        let _build_log = EnvVarGuard::set("BHF_BUILD_LOG", build_log_path.as_os_str());
        let _gnat_log = EnvVarGuard::set("BHF_RAW_GNAT_LOG", gnat_log_path.as_os_str());
        ...
        assert!(build_argv.contains("bhf_build.gpr"));
    ...
    fn build_subcommand_returns_three_when_work_dir_missing() {
        ...
        let exit = run_from(["bhf", "build", missing.to_str().expect("utf-8 work dir")]);
    ...
    fn stub_subcommand_returns_three_when_work_dir_missing() {
        ...
        let exit = run_from(["bhf", "stub", missing.to_str().expect("utf-8 work dir")]);
    ...
    fn stub_subcommand_writes_manifest_on_success() {
        ...
        let exit = run_from(["bhf", "stub", work_dir.to_str().expect("utf-8 work dir")]);
    ...
    fn clean_subcommand_without_scope_is_noop() {
        ...
        let exit = run_from(["bhf", "clean", work_dir.to_str().expect("utf-8")]);
    ...
    fn clean_subcommand_missing_workdir_returns_three() {
        ...
        let exit = run_from(["bhf", "clean", missing.to_str().expect("utf-8")]);
    ...
    fn create_minimal_work_dir(parent: &Path) -> PathBuf {
        let work_dir = parent.join("bhf_work");
