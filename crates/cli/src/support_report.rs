...
//! Compact, privacy-preserving diagnostics for an offline `bhf auto` run.
//!
...
pub struct SupportReportArgs {
    /// Existing `bhf auto --work-dir` to summarize. The source tree is never read.
    ...
    /// (e.g. `--bundle bhf-bug-report.tar.gz`). The archive contains the
    /// scrubbed report, bhf's own internal-issue log (bug-report.json, if any),
...
/// excluded. Called after `.bhf.toml` has been applied so the checkpoint
...
pub fn run(args: SupportReportArgs) -> i32 {
    ...
    match run_inner(args) {
        Ok(path) => {
            if !bundle_mode {
                bhfeprintln!(
                    "bhf: compact scrubbed support report written to {}",
                    path.display()
                );
            }
            0
        ...
        Err(error) => {
            bhfeprintln!("bhf bug-report: {error:#}");
            2
        }
    }
}
...
/// bhf's own internal-issue log if present, and a manifest — plus a self-test
...
fn emit_bundle(args: &SupportReportArgs, report: &str) -> Result<PathBuf> {
    ...
    // bhf's own internal-issue log (its OWN defects), already scrubbed: file
    ...
    if args.preview {
        println!("bhf bug-report — bundle preview (NO archive written)");
        println!(
    ...
    // up the staging tree. A single top-level `bhf-bug-report/` dir keeps the
    ...
    let stage = stage_parent.join("bhf-bug-report");
    ...
    bhfeprintln!(
        "bhf: bug-report bundle written to {} ({} scrubbed field(s), self-test PASS)",
        bundle_path.display(),
        files.len()
    );
    Ok(bundle_path)
...
fn bundle_manifest() -> String {
    format!(
        "bhf bug-report bundle — manifest (schema {SCHEMA_VERSION})\n\
         \n\
         Included fields (all scrubbed + bounded):\n\
         - support-report.txt : bhf version/build + toolchain versions; sanitized run\n\
         ...
         - bug-report.json    : bhf's OWN internal issues (panics/codegen/discovery\n\
...
fn collect_evidence(work_dir: &Path) -> Result<Evidence> {
    ...
    {
        bail!(
            "no completed target checkpoints found under {}; run this against the work directory passed to `bhf auto`",
            work_dir.display()
        );
    }
    Ok(evidence)
...
fn render_report(
    ...
    body.push_str(&format!(
        "schema=bhf.support.v{SCHEMA_VERSION}\nversion={}\ncommit={}\nhost={}/{}\n",
