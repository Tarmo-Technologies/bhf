...
//! `bhf introspect` surfaces the first practical coverage-blocker
//! view: discovered targets versus a prior `bhf auto` run.
...
fn recommendation_for(blocker_kind: &str, candidate: &Candidate) -> String {
    match blocker_kind {
        ...
        "no_prior_run" => {
            "No prior auto run found; run bhf auto to collect build and fuzz outcomes."
                .to_owned()
        }
...
fn recommendations(
    ...
    if !prior.summary.present {
        out.push(format!(
            "Run `bhf auto {} --work-dir {}` to collect build and fuzz outcomes.",
            source_root.display(),
            work_dir.display()
        ));
        return out;
...
fn render_markdown(report: &IntrospectionReport) -> String {
    ...
    out.push_str("# BHF Introspection\n\n");
