...
const BUILD_CONTEXT_CONFIDENCE_PREFIX: &str = "@bhf-build-context-confidence=";
const BUILD_CONTEXT_RECOVERY_PREFIX: &str = "@bhf-build-context-recovery=";
const BUILD_CONTEXT_LDFLAG_PREFIX: &str = "@bhf-build-context-ldflag=";
const BUILD_CONTEXT_CXX_STANDARD_PREFIX: &str = "@bhf-build-context-cxx-standard=";
...
/// flattened into both `bhf auto` and `bhf generate-harness`. Each flag
...
/// Programmatic shim around `run()` used by `bhf auto`'s attempt
...
fn generate_private_child_bridge_direct(
    ...
    // PRIVATE descendants, so a public `<parent>.Bhf_Bridge` body may not `with`
...
/// subprogram* of the parent (`private procedure UnZip.Bhf_Harness`): its body
...
fn generate_private_child_subprogram_direct(
    ...
    let harness_unit = format!("{parent}.Bhf_Harness");
...
fn write_project_profile_if_requested(
    ...
    let profile_path = output_dir.join("bhf-project-profile.json");
...
fn annotate_gpr_with_project_profile(gpr_path: &Path, project: &Path) -> Result<()> {
    ...
    if original.contains("-- BHF project profile: bhf-project-profile.json") {
        return Ok(());
    }
    let annotated = format!(
        "-- BHF project profile: bhf-project-profile.json\n-- GPR provenance: {}\n{}",
        project.display(),
        original
    );
...
fn ada_project_profile(
    ...
    Ok(json!({
        "schema_version": "bhf.ada_project_profile.v1",
        "project": {
...
fn harness_source_plan(args: &GenerateHarnessArgs) -> Result<HarnessSourcePlan> {
    ...
    // no explicit roots/trees were given — which is how `bhf auto` invokes
...
fn run_c_direct(args: &GenerateHarnessArgs) -> Result<()> {
    ...
    // cluster handle and drives it through `&_bhf_handle`, which is only valid when the
    ...
    // (`hashmap_new`, which needs caller-supplied hash/compare function pointers bhf
    ...
    let result = if args.kind == "sequence" {
        ...
        // opt-in via BHF_DRIVE_DECODERS for targets where the caller has
        // structurally-valid seeds and specifically wants decoder depth.
        let drive_plan = if std::env::var_os("BHF_DRIVE_DECODERS").is_some() {
    ...
    while let Some(dir) = dirs.pop() {
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                ...
                if !matches!(
                    name.as_str(),
                    ".git" | ".bhf-build" | ".bhf-build-cov" | "build" | "target"
                ) {
                    dirs.push(path);
                }
                continue;
...
pub(crate) fn c_lifecycle_handle_key(
    ...
    // `bhf_c_string` buffer underflows that header (a guaranteed BHF-201 OOB FP).
...
/// Locate the `c_runtime` directory (holding `bhf_decode.h` +
/// `bhf_driver.c`). Installed distributions should prefer the runtime staged
...
pub(crate) fn locate_c_runtime() -> PathBuf {
    crate::runtime_assets::locate("c_runtime", "bhf_decode.h").unwrap_or_else(|| {
...
fn detect_success_result_object_cleanup(
    ...
    Some(format!(
        "if (R) {{ {object_type} _bhf_result_object = {getter_name}({receiver_name}); \
         if (_bhf_result_object) {}(_bhf_result_object); }}",
        release.name
    ))
}
...
/// pipeline bhf deliberately disables — see [`is_harness_incompatible_flag`]),
...
/// BHF's placeholder header and expose only a misleading partial program.
...
///      (the library's `src/`), so a default `bhf auto` (no probe) still links
///      the whole library.
///
...
/// CMakeLists, Makefile) that must NOT be forwarded to bhf's standalone
...
/// without `-fopenmp` (which also predefines `_OPENMP`); bhf builds harnesses
...
fn standalone_header_include_plan(
    ...
    if force {
        bhfeprintln!(
            "bhf generate-harness: --force: '{}' is not self-contained ({}), \
...
fn preflight_header_includes(
    ...
    while let Some(flag) = compile_flags.get(index) {
        if let Some(value) = flag.strip_prefix(BUILD_CONTEXT_COMPILER_PREFIX) {
            ...
            // Remaining @bhf entries are Makefile/report metadata, never
            // compiler arguments.
        } else if flag == "-x" {
...
fn run_cpp_direct(args: &GenerateHarnessArgs) -> Result<()> {
    ...
    let stdlib_probe_flags = compile_flags
        .iter()
        .filter(|flag| !flag.starts_with("@bhf-"))
        .cloned()
        .collect::<Vec<_>>();
...
fn cpp_receiver_constructor_params(
    ...
    // `Abstract _bhf_receiver;` which fails "variable type is an abstract class").
    ...
    if closure_texts.iter().any(|t| {
        ...
        // _bhf_receiver;` and the virtual call dispatches to its override.
        ...
        // arguments: `Subclass _bhf_receiver(args); _bhf_receiver.method(..)`.
    ...
    if constructors.is_empty() {
        ...
        // Default-construct (`T _bhf_receiver;`) ONLY when the class is actually
    ...
    // The factory's owner is stack-allocated so `_bhf_owner` outlives the
...
/// library — but bhf links each harness against the static CRT (clang's default,
...
mod tests {
    ...
    fn harness_incompatible_flags_drop_modules_and_pch_keep_the_rest() {
        ...
        // project's build wiring may declare break or mean nothing to bhf's
    ...
    fn compile_database_preserves_compiler_abi_and_extension_context() {
        ...
        for expected in [
            "@bhf-build-context-compiler=/opt/toolchain/bin/g++-12",
            "-fms-extensions",
            "-fpermissive",
            "-fpack-struct=1",
            "-fshort-enums",
    ...
    fn detect_top_level_namespaces_ignores_macro_body_namespaces() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-ns-macro-{nonce}"));
...
"#;
    ...
    fn ada_param_typed_with_dependency_package_array_resolves_via_ada_deps() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-ada-dep-array-{nonce}"));
    ...
    fn collect_c_type_defs_follows_includes_transitively() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-transitive-typedef-{nonce}"));
    ...
    fn boolean_stream_feed_acquires_and_releases_completed_object() {
        ...
        assert!(
            cleanup.contains("ucl_object_t * _bhf_result_object = ucl_parser_get_object(parser)")
        );
        assert!(cleanup.contains("ucl_object_unref(_bhf_result_object)"));
    ...
    fn paired_deallocator_detected_from_header_for_owning_pointer_return() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-dealloc-{nonce}"));
        ...
        let cbor_root = std::env::temp_dir().join(format!("bhf-cbor-{nonce}"));
    ...
    fn owning_returns_pair_via_read_verb_and_structural_inference() {
        ...
        let yroot = std::env::temp_dir().join(format!("bhf-yyjson-{nonce}"));
        ...
        let mroot = std::env::temp_dir().join(format!("bhf-mpc-{nonce}"));
...
";
    ...
    fn auto_detect_project_includes_finds_sibling_include_dir() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-incdir-{nonce}"));
    ...
    fn auto_detect_project_includes_skips_source_dir_itself() {
        ...
        let root = std::env::temp_dir().join(format!("bhf-incnone-{nonce}"));
    ...
    fn self_prefixed_include_roots_adds_parent_of_prefix_dir() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-selfinc-{nonce}"));
    ...
    fn self_prefixed_include_roots_ignores_plain_and_angle_includes() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-selfinc2-{nonce}"));
    ...
    fn auto_detect_c_headers_includes_quoted_project_headers_from_source() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-source-headers-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn harness_project_includes_picks_angle_project_headers_and_skips_partials() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-proj-inc-{nonce}"));
        let inc = root.join("include");
    ...
    fn harness_project_includes_skips_foreign_platform_guarded_includes() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-foreign-inc-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn harness_project_includes_pulls_transitive_deps_dependencies_first() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-trans-inc-{nonce}"));
    ...
    fn harness_project_includes_leaves_umbrella_only_children_to_the_umbrella() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-umbrella-inc-{nonce}"));
    ...
    fn is_openmp_translation_unit_detects_omp_runtime_calls() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-omp-tu-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn auto_detect_c_headers_excludes_openmp_translation_unit() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-omp-headers-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn harness_project_includes_excludes_openmp_translation_unit() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-omp-proj-inc-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        fs::write(
            root.join("lib_openmp.c"),
    ...
    fn cleanup_flag_overrides_auto_detect_in_emitted_harness() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-cleanup-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    /// count bound (`bhf_bounded_length(&Cur, 0, N)`). Exercises the full CLI
    ...
    fn container_size_max_flag_shrinks_emitted_cpp_container_decode() {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-ctrcap-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(
            default_main.contains("bhf_bounded_length(&Cur, 0, 16)"),
        ...
        assert!(
            capped_main.contains("bhf_bounded_length(&Cur, 0, 8)"),
    ...
    fn generic_package_target_is_blocked_with_clear_reason() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-generic-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn private_child_direct_target_uses_child_subprogram_harness() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-privchild-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(!out.join("H-PRIV/parent-bhf_bridge.ads").exists());
        let spec = fs::read_to_string(out.join("H-PRIV/parent-bhf_harness.ads"))
            .unwrap()
            .to_ascii_lowercase();
        let body = fs::read_to_string(out.join("H-PRIV/parent-bhf_harness.adb"))
            .unwrap()
            .to_ascii_lowercase();
        assert!(spec.contains("private procedure parent.bhf_harness;"));
        assert!(body.contains("procedure parent.bhf_harness is"));
    ...
    fn private_child_target_with_private_type_param_uses_child_subprogram_harness() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-privtype-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        fs::write(
            root.join("pp.ads"),
        ...
        assert!(!out.join("H-PT/pp-bhf_bridge.ads").exists());
        let spec = fs::read_to_string(out.join("H-PT/pp-bhf_harness.ads"))
            .unwrap()
            .to_ascii_lowercase();
        let body = fs::read_to_string(out.join("H-PT/pp-bhf_harness.adb"))
            .unwrap()
            .to_ascii_lowercase();
        assert!(spec.contains("private procedure pp.bhf_harness;"));
        assert!(body.contains("procedure pp.bhf_harness is"));
    ...
    fn private_child_sequence_kind_is_still_blocked() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-privchild-seq-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        let source = root.join("parent-secret.ads");
        fs::write(
            &source,
    ...
    fn generic_codec_package_is_instantiated_with_fuzz_callbacks() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-codec-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(
            main_adb.contains("Bhf_Generic_Instance.run"),
    ...
    fn generic_package_operation_with_record_param_is_synthesized() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-codec-rec-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        let source = root.join("codec.ads");
        fs::write(
            &source,
        ...
        assert!(
            main_adb.contains("Bhf_Generic_Instance.Hints'("),
        ...
        assert!(
            main_adb.contains("Bhf_Generic_Instance.decompress"),
            "the target must be called through the instance: {main_adb}"
        );
    }
    ...
    fn out_param_of_private_type_is_declared_bare() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-outpriv-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn generic_encoder_subprogram_is_instantiated_and_called_with_defaults() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-enc-{nonce}"));
        fs::create_dir_all(&root).unwrap();
        ...
        assert!(
            main_adb.contains("procedure Bhf_Generic_Instance is new enc.encode"),
            "expected a generic-subprogram instantiation: {main_adb}"
        );
        assert!(
            main_adb.contains("Bhf_Generic_Instance;")
                && !main_adb.contains("Bhf_Generic_Instance ("),
    ...
    fn generic_subprogram_target_is_blocked_with_clear_reason() {
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("bhf-genproc-{nonce}"));
        fs::create_dir_all(&root).unwrap();
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-cli-{name}-{nonce}"));
        fs::create_dir_all(&dir).unwrap();
        dir
    }
    ...
    fn generate_harness_resolves_parent_package_types_for_child_units() {
        ...
        let main = fs::read_to_string(temp.join("generated_harnesses/H-CHILD-PARENT/main.adb"))
            .expect("generated main is readable");
        assert!(main.contains("Bhf_Result : constant Parent.Thing := Parent.Child.Get;"));
    }
    ...
    fn generate_c_direct_harness_keeps_header_typedefs_when_including_main_tu() {
        ...
        assert!(
            !main.contains("char *s = bhf_c_string"),
    ...
    fn generate_c_sequence_harness_models_handle_lifecycle_functions() {
        ...
        assert!(main.contains("session_init(&_bhf_handle"));
        ...
        assert!(main.contains("_bhf_lifecycle_count"));
        ...
        if std::process::Command::new("clang")
            ...
            .is_err()
        {
            bhfeprintln!("skipping generated C sequence compile: clang not on PATH");
            return;
        }
    ...
    fn mined_protocol_without_configuration_keeps_api_defaults() {
        ...
        assert!(main.contains("parser_parse(&_bhf_handle"), "{main}");
        assert!(main.contains("parser_fini(&_bhf_handle"), "{main}");
        assert!(main.contains("parser_free(&_bhf_handle"), "{main}");
        assert!(
            !main.contains("parser_set_mode(&_bhf_handle"),
    ...
    fn mined_protocol_preserves_mutually_exclusive_if_else_arms() {
        ...
        assert_eq!(
            main.matches("parser_parse(&_bhf_handle").count(),
            2,
    ...
    fn cpp_build_context_extracts_last_standard_into_single_control() {
        ...
        assert!(encoded.contains(&"@bhf-build-context-cxx-standard=c++14".to_owned()));
    ...
    fn cpp_build_compiles_each_translation_unit_with_its_own_database_flags() {
        if std::process::Command::new("g++")
            ...
            .is_err()
        {
            bhfeprintln!("skipping per-TU context test: g++ not on PATH");
            return;
        }
...
{{"directory":"{}","file":"{}","arguments":["g++","-I",".","-DMODE_SUPPORT=1","-std=c++17","-c","{}"]}}
    ...
    fn cpp_header_preflight_selects_a_compiling_umbrella() {
        if std::process::Command::new("clang++")
            ...
            .is_err()
        {
            bhfeprintln!("skipping header preflight test: clang++ not on PATH");
            return;
        }
    ...
    fn generate_c_harness_for_file_pointer_target() {
        ...
        assert!(main.contains("unsigned char *_bhf_file_buf_stream"));
        ...
        assert!(main.contains("free(_bhf_file_buf_stream);"));
    }
...
"#;
    ...
    fn generate_cpp_direct_harness_default_constructs_target_class_parameter() {
        ...
        assert!(main.contains("options.mode = bhf_i32(&Cur)"), "{main}");
        assert!(
    ...
    fn generate_cpp_direct_harness_uses_supported_parameterized_constructor_for_method() {
        ...
        assert!(main.contains("std::string _bhf_ctor_seed"));
        assert!(main.contains("gov::Parser _bhf_receiver(_bhf_ctor_seed);"));
        assert!(main.contains("int R = _bhf_receiver.parse(input);"));
    }
    ...
    fn generate_cpp_sequence_harness_models_object_lifecycle_methods() {
        ...
        assert!(main.contains("gov::Parser _bhf_receiver;"));
        assert!(main.contains("for (size_t _bhf_lifecycle_index = 0;"));
        assert!(main.contains("_bhf_receiver.reset();"));
        assert!(main.contains("_bhf_receiver.feed("));
        assert!(main.contains("int R = _bhf_receiver.parse(input);"));
        assert!(main.contains("catch (...)"));
        assert!(main.contains("RAII destructors run when _bhf_receiver leaves scope"));
        assert!(
            !makefile.contains(source.to_str().unwrap()),
    ...
    fn generate_cpp_sequence_keeps_registry_decodable_setup_method() {
        ...
        assert!(main.contains("Options _bhf_step0_options{};"), "{main}");
        assert!(
            main.contains("_bhf_step0_options.mode = bhf_i32(&Cur)"),
            "{main}"
        );
        assert!(
            main.contains("_bhf_receiver.configure(_bhf_step0_options)"),
            "{main}"
        );
        if std::process::Command::new("g++")
            .arg("--version")
    ...
    fn generate_cpp_sequence_harness_excludes_private_lifecycle_methods() {
        ...
        assert!(main.contains("_bhf_receiver.feed("));
        assert!(
            !main.contains("_bhf_receiver.reset("),
    ...
    fn generate_cpp_harness_skips_abstract_receiver_class() {
        ...
        // members). It must be an honest skip, not `ClientHook _bhf_receiver;`
    ...
    fn generate_cpp_sequence_harness_constructs_receiver_from_defaultable_ctor_dependency() {
        ...
        assert!(
            main.contains("Dependency _bhf_ctor_dependency{};"),
            "{main}"
        );
        assert!(
            main.contains("gov::Parser _bhf_receiver(_bhf_ctor_dependency);"),
            "{main}"
        );
    }
    ...
    fn generate_cpp_factory_receiver_via_owner_method_pointer_return() {
        ...
        assert!(
            main.contains("XMLDocument _bhf_owner;"),
            "owner must be stack-allocated:\n{main}"
        );
        assert!(
            main.contains("auto _bhf_receiver = _bhf_owner.NewElement("),
            "factory call must use owner:\n{main}"
        );
        assert!(
            main.contains("if (_bhf_receiver)"),
            "pointer receiver must be null-guarded:\n{main}"
        );
        assert!(
            main.contains("_bhf_receiver->IntAttribute("),
            "method on pointer receiver must use -> access:\n{main}"
        );
        // Owner stays in scope (declared before the if-guard block).
        let owner_pos = main.find("XMLDocument _bhf_owner;").unwrap();
        let guard_pos = main.find("if (_bhf_receiver)").unwrap();
        assert!(
            owner_pos < guard_pos,
    ...
    fn cpp_receiver_uses_header_declared_static_factory_defined_in_sibling_tu() {
        ...
        assert!(
            main.contains("auto _bhf_receiver = Widget::Create("),
            "{main}"
        );
        assert!(!main.contains("Widget _bhf_owner"), "{main}");
