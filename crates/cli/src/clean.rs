...
pub struct CleanArgs {
    /// Path to the bhf work directory.
    ...
    /// Remove all known BHF-owned workdir subtrees.
    #[arg(long)]
    pub all: bool,
}
...
pub fn run(args: CleanArgs) -> i32 {
    if !args.work_dir.is_dir() {
        bhfeprintln!("work directory not found: {}", args.work_dir.display());
        return 3;
    }
    ...
    if (args.compact || args.build) && !args.all {
        match crate::auto::storage::compact_work_dir(&args.work_dir) {
            ...
            Err(error) => {
                bhfeprintln!("compact {}: {error}", args.work_dir.display());
                return 1;
            }
        }
    }
    ...
    for relative in targets {
        ...
        match remove_owned_path(&args.work_dir, relative) {
            ...
            Err(error) => {
                bhfeprintln!("clean {}: {error}", path.display());
                return 1;
            }
        }
    }
