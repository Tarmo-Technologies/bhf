...
pub struct ScanArgs {
    ...
    /// BHF work directory where scan_index.json is written.
    ...
    match scan_and_write(&args.path, &args.work_dir) {
        Ok(index) => {
            match serde_json::to_string_pretty(&index) {
                ...
                Err(error) => {
                    bhfeprintln!("failed to render scan summary: {error}");
                    return 1;
                }
            }
...
fn scan_path(path: &Path) -> Result<ScanIndex> {
    ...
    for path in walk_source_files(path)? {
        match scan_file(&path) {
            ...
            Err(reason) => {
                bhfeprintln!("skipping source {}: {reason}", path.display());
...
/// Skip bhf-owned output trees, common build/CI bookkeeping
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-scan-{name}-{nonce}"));
