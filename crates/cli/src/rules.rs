...
//! `bhf rules` subcommand - inspect the rule catalog.
//!
//! The rule catalog drives finding identifiers (BHF-NNNN), CWE / CERT-C /
...
//! bhf emits without grepping source.
...
#[derive(Debug, Args)]
pub struct ShowArgs {
    /// Rule identifier - either the numeric id (`BHF-201`) or the slug
    /// (`bhf.c/heap-buffer-overflow`).
    pub rule: String,
...
fn list(args: ListArgs) -> i32 {
    if args.json {
        match serde_json::to_string_pretty(RULES) {
            ...
            Err(error) => {
                bhfeprintln!("serialize rule catalog: {error}");
                1
            }
        }
    } else {
...
fn show(args: ShowArgs) -> i32 {
    let rule = match finding_rules::by_id(&args.rule).or_else(|| finding_rules::by_slug(&args.rule))
    {
        ...
        None => {
            bhfeprintln!(
                "rule '{}' not found. Try `bhf rules list` for known ids.",
                args.rule
            );
            return 2;
        }
    ...
    if args.json {
        match serde_json::to_string_pretty(rule) {
            ...
            Err(error) => {
                bhfeprintln!("serialize rule: {error}");
                1
            }
        }
    } else {
