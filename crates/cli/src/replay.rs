...
pub fn run(args: ReplayArgs) -> i32 {
    ...
    match replay_min::replay_with_runner(&finding_dir, &runner) {
        ...
        Ok(replay_min::ReplayResult::Mismatch { recorded, actual }) => {
            ...
            bhfeprintln!("MISMATCH recorded={} actual={actual}", recorded.hex());
            3
        }
        Err(error) => {
            bhfeprintln!("error: {error}");
...
fn replay_c_afl(finding_dir: &Path, harness: &Path) -> i32 {
    ...
    let recorded_rule = match fs::read(&finding_path).and_then(|bytes| {
        ...
        Err(error) => {
            bhfeprintln!("read {}: {error}", finding_path.display());
            return 1;
        }
    };
    ...
    let input = match fs::read(&testcase_path) {
        Ok(b) => b,
        Err(error) => {
            bhfeprintln!("read {}: {error}", testcase_path.display());
            return 1;
        }
    };
    ...
    let mut child = match spawn_harness(&mut stdin_replay) {
        Ok(c) => c,
        Err(error) => {
            bhfeprintln!("spawn {}: {error}", harness.display());
            return 1;
        }
    };
    ...
    loop {
        match child.try_wait() {
            ...
            Err(error) => {
                bhfeprintln!("poll {}: {error}", harness.display());
                return 1;
            }
        }
    }
    let output = match child.wait_with_output() {
        Ok(o) => o,
        Err(error) => {
            bhfeprintln!("wait {}: {error}", harness.display());
            return 1;
        }
    };
    ...
    match actual_rule {
        ...
        Some(rule) => {
            bhfeprintln!("MISMATCH recorded={recorded_rule} actual={rule}");
            3
        }
        None => {
            bhfeprintln!("MISMATCH recorded={recorded_rule} actual=<none>");
            3
        }
    }
}
