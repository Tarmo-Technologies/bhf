...
fn status(args: StatusArgs) -> i32 {
    ...
    let report = StatusReport {
        ...
        response_byte_limit: memory_aware_byte_limit("BHF_LLM_MAX_RESPONSE_BYTES"),
        note: "installed does not prove authentication; use `bhf llm test`. MCP mode needs no BHF API token.",
    };
    if args.json {
...
fn test_provider(args: TestArgs) -> i32 {
    ...
    let completion = match provider.complete(
        "This is a BHF provider connectivity test. Follow the requested exact output.",
        "Reply with exactly BHF_LLM_OK and no other text.",
    ) {
        Ok(completion) => completion,
        Err(error) => {
            bhfeprintln!("LLM connection failed: {error}");
            return 1;
        }
    };
    if !completion.contains("BHF_LLM_OK") {
        bhfeprintln!("LLM connection responded but did not return the verification marker");
        return 1;
    }
...
fn assist(args: AssistArgs) -> i32 {
    ...
    match provider.complete(&prompt.system_prompt, &prompt.user_prompt) {
        ...
        Err(error) => {
            bhfeprintln!("LLM assistance failed: {error}");
            1
        }
    }
}
...
fn build_prompt(args: AssistanceArgs) -> Result<llm_harness_gen::PreparedPrompt, String> {
    let budget = memory_aware_byte_limit("BHF_LLM_MAX_EVIDENCE_BYTES");
    ...
    for path in args.inputs {
        ...
        let text = decode_bounded_lossy(&bytes, remaining).map_err(|()| {
            format!(
                "decoded evidence {} exceeds the memory-aware {remaining}-byte remaining budget; set BHF_LLM_MAX_EVIDENCE_BYTES to override",
                path.display()
            )
        })?;
...
fn read_bounded(path: &Path, remaining: usize) -> Result<Vec<u8>, String> {
    if remaining == 0 {
        return Err(
            "LLM evidence budget exhausted; set BHF_LLM_MAX_EVIDENCE_BYTES to override".to_owned(),
        );
    }
    ...
    if bytes.len() > remaining {
        return Err(format!(
            "evidence exceeds the memory-aware {remaining}-byte remaining budget; set BHF_LLM_MAX_EVIDENCE_BYTES to override"
        ));
    }
    Ok(bytes)
}
...
fn required_model(args: &ProviderArgs) -> Result<String, String> {
    args.model.clone().ok_or_else(|| {
        format!(
            "--model is required for {:?}; model defaults change, so BHF does not silently select one",
            args.provider
        )
    })
}
