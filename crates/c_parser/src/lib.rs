...
fn collect_typedef(node: tree_sitter::Node<'_>, source: &[u8], defs: &mut CTypeDefs) {
    ...
    // was cast from a scalar (`(nd_uint8_t)bhf_u8(...)` — illegal) instead of
    // filled element-wise.
...
fn split_declarator(raw: &str) -> (String, String) {
    ...
    // and emitted `internal_hooks _bhf_value_const hooks;`.
    loop {
    ...
    // emitted `te_expr _bhf_value_parameters[];` — an unsized-array decl that does
