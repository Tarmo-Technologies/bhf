...
///
/// This is not a test-only concern. bhf BUILDS a harness and then replays it,
...
pub(crate) fn signatures_for_input_with_runner(
    ...
    command
        .env("BHF_EVENTS_PATH", events_path.path())
        .stdin(Stdio::piped())
        .stdout(Stdio::null())
        .stderr(Stdio::piped());
    let mut child =
...
fn temp_event_path_from(clock_reading: u128) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "bhf-replay-events-{}-{clock_reading}-{sequence}.bin",
        std::process::id()
    ))
}
...
mod tests {
    ...
    fn temp_event_path_uses_replay_event_name() {
        ...
        assert!(path
            ...
            .is_some_and(|name| name.starts_with("bhf-replay-events-")));
    }
    ...
    fn replay_pipes_testcase_bytes_to_harness_stdin() {
        ...
        let testcase = b"bhf-stdin-marker\0mismatch\n".to_vec();
    ...
    fn qemu_user_runner_wraps_harness_and_preserves_replay_contract() {
        ...
        let testcase = b"bhf-qemu-stdin-marker\n".to_vec();
    ...
    fn firejail_sandbox_runner_wraps_harness_and_preserves_replay_contract() {
        ...
        let testcase = b"bhf-sandbox-stdin-marker\n".to_vec();
    ...
    fn bubblewrap_installed_but_broken_degrades_to_direct_run_when_non_strict() {
        ...
        let testcase = b"bhf-broken-bwrap-marker\n".to_vec();
    ...
    fn bubblewrap_drops_unshare_net_when_netns_denied() {
        ...
        let testcase = b"bhf-fs-only-marker\n".to_vec();
...
fn main() -> std::io::Result<()> {{
    ...
    let event_path = std::env::var("BHF_EVENTS_PATH").map_err(std::io::Error::other)?;
...
"#,
    ...
    fn replay_event_files() -> Vec<PathBuf> {
        let prefix = format!("bhf-replay-events-{}-", std::process::id());
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-replay-unit-{name}-{nonce}"));
