...
mod tests {
    ...
    #[test]
    fn parse_idl_surfaces_bhf_warning_pragma_as_clean_message() {
        // bhf's own recovery breadcrumb must NOT be re-flagged as an unknown
        // pragma; its message is surfaced verbatim (the confusing double-diagnostic
        // the Ada/CORBA dogfood produced).
        let ast = parse_idl(
            "#pragma bhf_warning \"include 'common.idl' not found\"\ninterface Root {};\n",
        )
        .expect("IDL parses");
        assert!(
            ast.warnings
        ...
        assert!(
            ...
            "bhf_warning must not be labelled unknown: {:?}",
            ast.warnings
        );
    }
    ...
    fn parse_idl_file_parses_declarations_from_includes() {
        let root = std::env::temp_dir().join(format!("bhf-idl-parse-file-{}", std::process::id()));
    ...
    fn parse_idl_file_searches_configured_include_dirs() {
        let root = std::env::temp_dir().join(format!(
            "bhf-idl-parse-file-include-dir-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn parse_idl_file_parses_include_inside_active_ifdef() {
        let root =
            std::env::temp_dir().join(format!("bhf-idl-active-include-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("create fixture dir");
        std::fs::write(root.join("common.idl"), "interface Common {};\n").expect("write include");
        let root_file = root.join("root.idl");
