...
fn shared(job: FuzzJob) -> SharedState {
    SharedState {
        inner: Mutex::new(InnerState {
            queue: if job.state == JobState::Queued {
                VecDeque::from([job.clone()])
            } else {
                VecDeque::new()
            },
            seen: vec![job],
...
fn wait_for(mut predicate: impl FnMut() -> bool) {
    ...
    while !predicate() {
        assert!(
            ...
            "bounded condition did not become true"
        );
...
fn idle_health_and_explicit_shutdown_are_distinct_from_storage_failure() {
    ...
    assert!(matches!(
        scheduler.submit(dir.path().to_owned(), "H".into(), Duration::ZERO),
...
fn health_counts_states_without_exposing_project_or_harness_names() {
    ...
    let scheduler = Scheduler::start(&config(dir.path())).unwrap();
    {
        ...
        .into_iter()
        .enumerate()
        .map(|(index, state)| FuzzJob {
            job_id: format!("J-{index:06}"),
            ..job(state)
        })
        .collect();
    }
    ...
    assert_eq!(
        (
            health.retained_jobs,
            health.queued_jobs,
            health.running_jobs,
            health.complete_jobs,
            health.failed_jobs
        ),
        (4, 1, 1, 1, 1)
    );
    ...
    assert_eq!(
        serde_json::from_str::<SchedulerHealth>(&encoded).unwrap(),
        health
    );
}
...
fn health_marks_retained_capacity_without_calling_it_a_storage_fault() {
    ...
    fs::write(
        ...
        format!(
            "{}\n",
            serde_json::to_string(&job(JobState::Complete)).unwrap()
        ),
    )
    .unwrap();
...
fn rejected_admission_latches_fault_without_consuming_id_or_record() {
    let dir = tempfile::tempdir().unwrap();
    let scheduler = Scheduler::start(&config(dir.path())).unwrap();
    let result = scheduler.submit_with_persistence(
        dir.path().to_owned(),
        "H".into(),
        Duration::ZERO,
        |_, _| Err(failed(false)),
    );
    ...
    assert_eq!(
        guard.storage_fault.as_ref().unwrap().phase,
        PersistencePhase::Admission
    );
    drop(guard);
    assert!(!scheduler.health().accepting_submissions);
    assert!(matches!(
        scheduler.submit(dir.path().to_owned(), "next".into(), Duration::ZERO),
        Err(DaemonError::Shutdown)
    ));
}
...
fn uncertain_admission_reserves_id_and_bytes_and_exposes_installed_flag() {
    ...
    let result = scheduler.submit_with_persistence(
        ...
        Duration::ZERO,
        |path, jobs| {
            crate::persist_jobs_with_sync(path, jobs, |_| {
    ...
    assert!(matches!(
        result,
        Err(DaemonError::DurabilityUncertain { .. })
    ));
    let health = scheduler.health();
...
fn start_failure_never_dequeues_or_authorizes_dispatch() {
    ...
    for installed in [false, true] {
        ...
        assert!(claim_job(dir.path(), &state, &mut guard, |_, _| panic!(
            "must not attempt another write"
        ))
        .is_none());
    }
}
...
fn successful_claim_persists_running_before_dequeue() {
    ...
    let saved: FuzzJob = serde_json::from_slice(
        fs::read(dir.path().join("jobs.jsonl"))
            .unwrap()
            .trim_ascii(),
    )
    .unwrap();
...
fn completion_failure_blocks_notification_and_further_writes() {
    ...
    for installed in [false, true] {
        ...
        assert_eq!(fault.snapshot_installed, installed);
        assert!(!serde_json::to_string(fault)
            .unwrap()
        ...
        assert!(!finish_job(
            ...
            "J-000000",
            JobState::Queued,
            |_, _| panic!("must not overwrite uncertain snapshot")
        ));
    }
}
...
fn duplicate_scheduler_is_rejected_even_through_a_path_alias() {
    ...
    let scheduler = Scheduler::start(&config(dir.path())).unwrap();
    assert!(matches!(
        Scheduler::start(&config(dir.path())),
        Err(DaemonError::DataDirInUse(_))
    ));
    assert!(matches!(
        Scheduler::start(&config(&dir.path().join("."))),
        Err(DaemonError::DataDirInUse(_))
    ));
    drop(scheduler);
...
fn lock_child(dir: &Path, mode: &str) -> ChildGuard {
    ChildGuard(
        Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "health::tests::storage_lock_child",
                "--nocapture",
            ])
...
fn storage_lock_child() {
    let Some(mode) = std::env::var_os("BHF_TEST_STORAGE_CHILD") else {
        return;
    };
...
fn real_worker_completion_storage_failure_stops_the_next_job() {
    ...
    scheduler
        .submit(dir.path().to_owned(), "first".into(), Duration::ZERO)
        .unwrap();
    wait_for(|| dir.path().join("entered-first").exists());
    scheduler
        .submit(dir.path().to_owned(), "second".into(), Duration::ZERO)
        .unwrap();
    ...
    let health = scheduler.health();
    assert_eq!(
        health.storage_fault.unwrap().phase,
        PersistencePhase::Completion
    );
    assert!(!health.accepting_submissions);
