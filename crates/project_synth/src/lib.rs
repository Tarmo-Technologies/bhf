...
mod tests {
    ...
    fn project_spec_validate_accepts_valid_ada_identifier() {
        assert!(spec_named("Bhf_Build_1").validate().is_ok());
    }
    ...
    fn compile_c_declares_c_language_and_switch() {
        ...
        let mut spec = spec_named("Bhf_Build");
        spec.compile_c = true;
        ...
        let ada_only = render_project(&spec_named("Bhf_Build")).unwrap();
        assert!(
    ...
    fn excluded_source_files_emitted_only_when_populated() {
        ...
        let none = render_project(&spec_named("Bhf_Build")).unwrap();
        assert!(
    ...
    fn project_spec_validate_rejects_name_with_hyphen() {
        assert!(spec_named("Bhf-Build").validate().is_err());
    }

    #[test]
    fn project_spec_validate_rejects_name_starting_with_digit() {
        assert!(spec_named("1Bhf_Build").validate().is_err());
    }
    ...
    fn render_project_includes_spdx_header() {
        let rendered = render_project(&spec_named("Bhf_Build")).expect("project renders");
    ...
    fn render_project_can_extend_a_governing_project() {
        ...
        let rendered = render_project(&spec).unwrap();
        assert!(rendered.contains("project Bhf_Build extends \"/project/app.gpr\" is"));
    }
    ...
    fn render_project_rejects_invalid_project_name() {
        let error = render_project(&spec_named("Bhf-Build")).expect_err("name is invalid");
    ...
    fn write_project_writes_to_disk_and_round_trips_through_render() {
        let spec = spec_named("Bhf_Build");
        let path = std::env::temp_dir().join(format!(
            "bhf-project-synth-test-{}-{}.gpr",
            std::process::id(),
            "write_project"
        ));
