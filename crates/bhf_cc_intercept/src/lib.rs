...
//! `libbhf_cc_intercept.so` — a build-time LD_PRELOAD shim that catches
...
//! `$BHF_CC_LOG` — the SAME `DIR`/`CC`/`ARG`/`ENDREC` format the Make-tier and
...
/// `$BHF_CC_LOG`, copied NUL-terminated at load. Written once before any hook can
...
/// Append one `DIR/CC/ARG.../ENDREC` record for `path` + `argv` to `$BHF_CC_LOG`
