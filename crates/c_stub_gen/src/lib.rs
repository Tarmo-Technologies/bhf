...
fn stub_body_for_declaration<D: DeclarationView>(decl: &D, return_type: &str) -> Option<String> {
    ...
    match decl.name() {
        "_mi_prim_mem_init" if params.len() == 1 => {
            return Some(
                "if (_bhf_p0) { _bhf_p0->page_size = 4096; _bhf_p0->large_page_size = 0; \
                 _bhf_p0->alloc_granularity = 4096; _bhf_p0->physical_memory_in_kib = 0; \
                 _bhf_p0->virtual_address_bits = 48; _bhf_p0->has_overcommit = true; \
                 _bhf_p0->has_partial_free = false; _bhf_p0->has_virtual_reserve = false; } return;"
                    .to_owned(),
            );
        }
        "_mi_prim_alloc" if result == "int" && params.len() == 8 => {
            return Some(
                "(void)_bhf_p0; (void)_bhf_p3; (void)_bhf_p4; \
                 size_t _bhf_align = _bhf_p2 < sizeof(void *) ? sizeof(void *) : _bhf_p2; \
                 size_t _bhf_size = _bhf_p1 ? _bhf_p1 : 1; \
                 size_t _bhf_rem = _bhf_size % _bhf_align; \
                 if (_bhf_rem) _bhf_size += _bhf_align - _bhf_rem; \
                 void *_bhf_mem = aligned_alloc(_bhf_align, _bhf_size); \
                 if (!_bhf_mem) { if (_bhf_p7) *_bhf_p7 = NULL; return 12; } \
                 for (size_t _bhf_i = 0; _bhf_i < _bhf_size; ++_bhf_i) \
                     ((unsigned char *)_bhf_mem)[_bhf_i] = 0; \
                 if (_bhf_p5) *_bhf_p5 = false; if (_bhf_p6) *_bhf_p6 = true; \
                 if (_bhf_p7) *_bhf_p7 = _bhf_mem; else free(_bhf_mem); return _bhf_p7 ? 0 : 12;"
                    .to_owned(),
            );
        }
        "_mi_prim_free" if result == "int" && params.len() == 2 => {
            return Some("(void)_bhf_p1; free(_bhf_p0); return 0;".to_owned());
        }
        "_mi_prim_commit" if result == "int" && params.len() == 3 => {
            return Some(
                "(void)_bhf_p0; (void)_bhf_p1; if (_bhf_p2) *_bhf_p2 = false; return 0;".to_owned(),
            );
        }
        ...
        "_mi_prim_alloc_huge_os_pages" if result == "int" && params.len() == 5 => {
            return Some(
                "(void)_bhf_p0; (void)_bhf_p2; void *_bhf_mem = calloc(1, _bhf_p1 ? _bhf_p1 : 1); \
                 if (_bhf_p3) *_bhf_p3 = true; if (_bhf_p4) *_bhf_p4 = _bhf_mem; \
                 else free(_bhf_mem); return (_bhf_mem && _bhf_p4) ? 0 : 12;"
                    .to_owned(),
            );
        }
        ...
        "_mi_allocator_init" if result == "bool" && params.len() == 1 => {
            return Some("(void)_bhf_p0; return true;".to_owned());
        }
        _ => {}
    }
    ...
    {
        return Some("return calloc(1, _bhf_p0 ? _bhf_p0 : 1);".to_owned());
    }
    if result == "void*" && first_is_size && allocator_parameter && decl.name().ends_with("_alloc")
    {
        return Some("return malloc(_bhf_p0 ? _bhf_p0 : 1);".to_owned());
    }
    if result == "void"
        && params
            .first()
    ...
    {
        return Some("free(_bhf_p0); return;".to_owned());
    }
    if result.contains('*')
        && result != "void*"
        && decl
    ...
    {
        return Some("return _bhf_p0;".to_owned());
    }
...
/// malformed declarator), so each type gets a synthetic `_bhf_pN`. Empty / a sole
/// `void` stays `void`.
fn named_param_list(param_types: &[String]) -> String {
    ...
    param_types
        ...
        .map(|(i, ty)| {
            ...
            name_abstract_param(emitted, &format!("_bhf_p{i}"))
        })
        .collect::<Vec<_>>()
        .join(", ")
}
    ...
    param_types
        ...
        .map(|(i, ty)| {
            let cleaned = ty
                ...
                .collect::<Vec<_>>()
                .join(" ");
            name_abstract_param(&cleaned, &format!("_bhf_p{i}"))
        })
        .collect::<Vec<_>>()
        .join(", ")
}
...
fn cpp_reference_return_body(return_type: &str) -> Option<String> {
    ...
    Some(format!(
        "static {value_type} _bhf_value{{}}; return _bhf_value;"
    ))
}
...
/// Render an exact C forward declaration without requiring that BHF can
...
pub fn synth_placeholder_header(virtual_path: &str) -> String {
    ...
    format!(
         ...
         * bhf auto could not find this header on the include path.\n\
...
pub fn synth_idl_placeholder_header(virtual_path: &str, decls: &[String]) -> String {
    ...
    let mut out = format!(
         ...
         * bhf auto could not find this IDL-generated stub header (the source\n\
         ...
         * header (tao_idl / `bhf fake-corba`) for faithful CORBA semantics.\n\
         */\n\
         #pragma once\n\
...
pub fn synth_minimal_config_h() -> String {
    let mut out = String::from(
        "/* auto-synthesised minimal config.h (project ships only config.h.in) */\n\
         #ifndef BHF_AUTO_CONFIG_H\n#define BHF_AUTO_CONFIG_H 1\n\
...
pub fn is_standard_libc_symbol(name: &str) -> bool {
    matches!(
        ...
        // temp-file and truncation calls. bhf's own `c_runtime` decoder
...
mod tests {
    ...
    fn miniz_ulong_returns_zero() {
        // Concrete regression: bhf auto failed to stub
...
mod synth_tests {
    ...
    fn header_backed_stub_zero_initialises_an_aggregate_return() {
        ...
        assert!(
            stub.contains(
                "Shader LoadShaderFromMemory(const char * _bhf_p0, const char * _bhf_p1)"
            ),
            "{stub}"
        );
        assert!(stub.contains("Shader _bhf_ret = {0};"), "{stub}");
        assert!(stub.contains("return _bhf_ret;"), "{stub}");
    }
    ...
    fn aggregate_return_body_only_accepts_a_complete_value_spelling() {
        for ty in ["Shader", "struct widget", "const point_t", "union tag"] {
            ...
            assert!(body.ends_with("return _bhf_ret;"), "{ty}: {body}");
        }
        for ty in [
            "",
            "void",
    ...
    fn qualified_cpp_value_return_keeps_its_own_default_initialiser() {
        ...
        assert!(!stub.contains("_bhf_ret"), "{stub}");
    }
    ...
    fn pointer_preserving_stub_returns_first_argument() {
        ...
        assert!(stub.contains("return _bhf_p0;"), "{stub}");
    ...
    fn allocator_wrapper_stubs_preserve_heap_semantics() {
        ...
        assert!(
            alloc_stub.contains("return malloc(_bhf_p0 ? _bhf_p0 : 1);"),
            "{alloc_stub}"
        );
        assert!(alloc_stub.contains("const void * _bhf_p1"), "{alloc_stub}");
        assert!(free_stub.contains("free(_bhf_p0); return;"), "{free_stub}");
        assert!(free_stub.contains("const void * _bhf_p1"), "{free_stub}");
    }
    ...
    fn mimalloc_primitive_stubs_assign_success_outputs() {
        ...
        assert!(alloc_stub.contains("*_bhf_p7 = _bhf_mem"), "{alloc_stub}");
        assert!(alloc_stub.contains("*_bhf_p6 = true"), "{alloc_stub}");
        assert!(!alloc_stub.contains("{\n    return 0;\n}"), "{alloc_stub}");

        let free_stub = synth_c_stub(&free).unwrap();
        assert!(
            free_stub.contains("free(_bhf_p0); return 0;"),
            "{free_stub}"
        );
    ...
    fn synthesises_qualified_cpp_constructor_and_destructor_stubs() {
        ...
        assert_eq!(
            constructor_stub,
            "__attribute__((weak)) leveldb::Status::Status(const Status & _bhf_p0) {}\n"
        );
        assert_eq!(
            destructor_stub,
    ...
    fn synthesises_default_cpp_value_return_with_namespace_qualification() {
        ...
        assert_eq!(
            stub,
            "__attribute__((weak)) auto leveldb::BuildTable(const std::string & _bhf_p0) -> Status {\n    return {};\n}\n"
        );
    }
    ...
    fn keeps_partially_qualified_cpp_value_return_in_function_scope() {
        ...
        assert!(
            stub.contains(
                "YAML::Utils::ComputeBinaryFormat(const Binary & _bhf_p0) -> StringFormat::value"
            ),
            "{stub}"
        );
    ...
    fn renders_real_param_list() {
        ...
        assert!(
            stub.contains(
                "int decoder_feed(decoder_t * _bhf_p0, const uint8_t * _bhf_p1, size_t _bhf_p2)"
            ),
            "stub: {stub}"
        );
    ...
    fn unwraps_export_macro_return_type_and_names_params() {
        ...
        assert_eq!(
            stub.trim(),
            "__attribute__((weak)) void * cJSON_malloc(size_t _bhf_p0) {\n    return NULL;\n}",
            "stub: {stub}"
        );
    }
    ...
    fn callback_parameter_name_is_inserted_inside_abstract_declarator() {
        // lacc: appending `_bhf_p3` produced the invalid
        // `void * (*)(void *, String *) _bhf_p3` in auto_stubs.c.
        let decl = FakeDecl {
            name: "hash_insert",
        ...
        let stub = synth_c_stub(&decl).expect("pointer return is stubbable");
        assert!(
            stub.contains("void * (* _bhf_p3)(void *, String *)"),
    ...
    fn strips_zlib_decorations_from_isolated_stub_parameters() {
        ...
        assert!(
            stub.contains("inflate_fixed(struct inflate_state * _bhf_p0)"),
            "{stub}"
        );
    ...
    fn compile_stub_tu_or_skip(stub: &str) {
        ...
        let dir = std::env::temp_dir().join(format!("bhf_stub_tu_{}_{n}", std::process::id()));
    ...
    fn incomplete_enum_value_param_demoted_to_int_and_compiles() {
        ...
        assert!(
            stub.contains("size_t _bhf_p3, int _bhf_p4, const char * _bhf_p5"),
    ...
    fn incomplete_enum_param_variants_handled() {
        ...
        let stub = synth_c_stub(&decl).expect("void return is stubbable");
        assert!(
            stub.contains("int _bhf_p0, enum json_error_code * _bhf_p1"),
    ...
    fn force_include_prototype_requires_fundamental_types() {
        ...
        assert_eq!(
            ...
            Some("extern int rand_s(unsigned int * _bhf_p0);\n")
        );
