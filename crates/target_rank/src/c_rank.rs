...
//
// Bhf's Ada ranker reasons over the full structural AST (visibility,
...
///
/// bhf fuzzes a target by driving its *parameters* with the fuzz input. That
...
///   `void`/`byte`) — those are the attacker buffer — nor a `FILE` (bhf
...
fn is_prebuilt_input_context(name: &str, raw: &str) -> bool {
    ...
    if base.is_empty()
        ...
        || is_scalar_base(&base)
        // A config/option/context/settings TYPE is default-constructible (bhf
