...
//! output must not change. Set `BHF_BLESS=1` to (re)write the golden files.
...
fn redact_root(pretty: &str, root: &Path) -> String {
    ...
    // bhf stamps its OWN version (the workspace version) into the SBOM tool
    ...
    // `CARGO_PKG_VERSION` is bhf's version. (Safe: the fixture packages are
    // demo@0.1.0 / demo-ui@2.0.0, neither equal to bhf's version.)
    out = out.replace(env!("CARGO_PKG_VERSION"), "<BHF_VERSION>");
    out
}
...
fn assert_golden(name: &str, actual: &serde_json::Value, root: &Path) {
    ...
    if std::env::var("BHF_BLESS").is_ok() {
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        fs::write(&path, format!("{pretty}\n")).unwrap();
        return;
    }
    let expected = fs::read_to_string(&path)
        .unwrap_or_else(|_| panic!("missing golden {name}; run with BHF_BLESS=1"));
    assert_eq!(
        format!("{pretty}\n"),
        expected,
