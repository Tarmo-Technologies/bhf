...
//! Offline enterprise governance primitives for BHF.
...
pub const UPDATE_PACK_VERIFICATION_SCHEMA_VERSION: &str = "bhf.update_pack.verification.v1";
pub const UPDATE_PACK_INSPECTION_SCHEMA_VERSION: &str = "bhf.update_pack.inspection.v1";
pub const UPDATE_PACK_INSTALL_SCHEMA_VERSION: &str = "bhf.update_pack.install.v1";
pub const EXPORT_SCHEMA_VERSION: &str = "bhf.export.v1";
pub const POLICY_EXPLAIN_SCHEMA_VERSION: &str = "bhf.policy.explain.v1";
pub const POLICY_DRY_RUN_SCHEMA_VERSION: &str = "bhf.policy.dry_run.v1";
pub const RUNNER_SELECTION_SCHEMA_VERSION: &str = "bhf.runners.selection.v1";
pub const RUNNER_HANDOFF_SCHEMA_VERSION: &str = "bhf.runners.handoff.v1";
pub const RUNNER_LEASE_SCHEMA_VERSION: &str = "bhf.runners.lease.v1";
pub const RUNNER_COMPLETION_SCHEMA_VERSION: &str = "bhf.runners.completion.v1";
pub const RUNNER_PLAN_SCHEMA_VERSION: &str = "bhf.runners.plan.v1";
pub const AUDIT_EVENT_SCHEMA_VERSION: &str = "bhf.audit.event.v1";
pub const AUDIT_LOG_SCHEMA_VERSION: &str = "bhf.audit.log.v1";
pub const DASHBOARD_SCHEMA_VERSION: &str = "bhf.dashboard.v1";
pub const CI_DASHBOARD_SCHEMA_VERSION: &str = "bhf.ci.dashboard.v1";
pub const SBOM_SCHEMA_VERSION: &str = "bhf.sbom.v1";
pub const VULNERABILITY_SCHEMA_VERSION: &str = "bhf.vulnerabilities.v1";
...
pub enum EmitKind {
    /// `sbom.json` — the native BHF component inventory.
    Sbom,
...
fn build_update_pack(
    ...
    let mut manifest = json!({
        "schema_version": "bhf.update_pack.v1",
        "pack_id": pack_id,
...
/// manifest. The `bom-ref` stays `bhf:scanned-root` and the `bhf:root`
...
fn render_cyclonedx_sbom(root: &Path, components: &[Component]) -> Value {
    let root_ref = "bhf:scanned-root";
    let bhf_tool_ref = format!("pkg:cargo/bhf@{}", env!("CARGO_PKG_VERSION"));
...
fn render_spdx_document(root: &Path, components: &[Component]) -> Value {
    ...
    let namespace = format!("https://buildharnessfuzz.dev/spdx/{doc_name}");
    ...
    json!({
        ...
        "creationInfo": {
            ...
            "creators": [
                format!("Tool: bhf-{}", env!("CARGO_PKG_VERSION")),
...
fn render_openvex_document(vulnerability_report: &Value) -> Value {
    let statements = vulnerability_report
        ...
        .unwrap_or_default();
    vex::render_openvex("bhf:vex", SBOM_TIMESTAMP, statements)
}
...
fn cyclonedx_vulnerability_json(finding: &Value) -> Value {
    ...
    let severity = finding
        ...
        .unwrap_or("unknown");
    let mut item = json!({
        "bom-ref": format!("bhf:vulnerability:{id}:{component_ref}"),
        "id": id,
        "source": {
            "name": "BHF offline vulnerability database"
        },
...
fn cyclonedx_component_json(component: &Component) -> Value {
    let mut item = json!({
        ...
        "properties": [
            ...
            {
                "name": "bhf:component_type",
                "value": component.component_type
            },
            {
                "name": "bhf:identity_confidence",
                "value": component.identity_confidence
            },
            {
                "name": "bhf:matching_method",
...
fn component_scope_path(path: &str) -> String {
    ...
    if normalized.rsplit('/').next().is_some_and(|name| {
        matches!(
            name,
            "Cargo.toml" | "package.json" | "component.json" | "bhf-component.json" | "VERSION"
        )
    }) {
        normalized
            .rsplit_once('/')
...
fn dir_is_excluded(path: &Path) -> bool {
    path.file_name()
        .and_then(|name| name.to_str())
        .is_some_and(|name| matches!(name, ".git" | "target" | "bhf_work" | "build"))
}
...
mod install_security_tests {
    ...
    fn cyclonedx_sbom_renders_components_hashes_purls_and_dependencies() {
        ...
        assert_eq!(tool["name"], "bhf");
        assert_eq!(tool["supplier"]["name"], "Tarmo Technologies");
        assert_eq!(
            tool["purl"],
            format!("pkg:cargo/bhf@{}", env!("CARGO_PKG_VERSION"))
        );
    ...
    fn metadata_component_adopts_root_self_manifest_identity() {
        ...
        // its name/version/purl/licenses, keeping bom-ref + bhf:root.
        ...
        assert_eq!(meta["bom-ref"], "bhf:scanned-root");
        assert_eq!(meta["properties"][0]["name"], "bhf:root");
    }
    ...
    fn sbom_includes_runtime_dlopen_components_with_reachability() {
        ...
        assert!(runtime_component["properties"]
            ...
            .unwrap()
            .iter()
            .any(|property| property["name"] == "bhf:matching_method"
    ...
    fn cyclonedx_sbom_includes_offline_vulnerability_findings() {
        ...
        assert!(vulnerability["properties"]
            ...
            .iter()
            .any(
                |property| property["name"] == "bhf:matching_method" && property["value"] == "purl"
            ));
        assert!(vulnerability["properties"]
            .as_array()
            .unwrap()
            .iter()
            .any(|property| property["name"] == "bhf:kev_known_exploited"
                && property["value"] == "true"));
    }
    ...
    fn install_rejects_pack_id_path_traversal() {
        ...
        for hostile in ["../escape_zone", "/tmp/bhf-zipslip", "../../escape_zone"] {
...
mod vex_e2e_tests {
    ...
    fn emit_spdx_json_writes_valid_spdx_2_3_document() {
        ...
        assert!(doc["creationInfo"]["creators"]
            ...
            .unwrap()
            .iter()
            .any(|c| c.as_str().unwrap().contains("bhf")));
