...
//! Bounded, provider-neutral LLM assistance for BHF.
//!
...
/// deterministic BHF command that can verify its conclusions.
pub fn prepare_assistance(request: &AssistanceRequest) -> Result<PreparedPrompt, LlmError> {
    ...
    let task = match request.kind {
        AssistanceKind::PlanRun => {
            "Plan a resource-aware BHF run. Identify the cheapest deterministic discovery step first, then propose explicit commands, budgets, stop conditions, and expected artifacts. Do not invent CLI flags."
        }
        ...
        AssistanceKind::AnalyzeFindings => {
            "Triage BHF findings. Separate reproduced behavior from static inference, group likely duplicates, assess input reachability, and propose replay/minimization commands. Do not claim exploitability from a crash alone."
        }
        ...
        AssistanceKind::DiagnoseError => {
            "Root-cause the supplied BHF, compiler, linker, harness, or fuzzer diagnostics. Quote the decisive diagnostic briefly, rank hypotheses, and give the smallest deterministic command or edit that distinguishes them."
        }
    };
    let system_prompt = format!(
        "You are an advisory assistant for BHF, a deterministic fuzzing tool. {task} \
...
/// still run BHF's normal generate/build/fuzz validation path.
pub fn preflight_harness(
    ...
    diagnostics.push(
        "structural preflight cannot prove compilation, linking, target reachability, or coverage; run bhf generate-harness/build/fuzz"
            .to_owned(),
    );
    HarnessPreflight {
        source_nonempty,
...
pub enum LlmError {
    ...
    Provider(String),
    #[error("provider response exceeded the memory-aware {limit}-byte limit; set BHF_LLM_MAX_RESPONSE_BYTES to override")]
...
pub fn default_system_prompt() -> &'static str {
    "You are generating a fuzz harness for bhf. Produce a single \
