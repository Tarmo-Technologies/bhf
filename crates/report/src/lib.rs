...
pub struct FindingReport {
    ...
    /// Numeric rule id (`BHF-NNNN`) the finding maps to, or `None` when no rule
...
/// issue id.
///
/// Two fields are intentionally NOT projected here: the internal `BHF-NNNN`
...
/// `bhf auto` sweep writes next to the findings directory
...
fn render_markdown_inner(document: &ReportDocument, collapse_clusters: bool) -> String {
    ...
    let mut out = String::new();
    out.push_str(&format!("# BHF Report: {}\n\n", document.run.id));
    ...
    for finding in &document.findings {
        ...
        // synthetic bhf harness entry — it reads as a real attacker entry
...
pub fn render_sarif_report(document: &ReportDocument) -> Value {
    ...
    let mut run = json!({
        ...
        "results": document
            ...
            .collect::<Vec<_>>(),
        "properties": {
            "bhfRunId": document.run.id,
            "bhfReportSchemaVersion": document.schema_version,
        }
    });
...
fn sarif_rules_for_document(document: &ReportDocument) -> Vec<Value> {
    ...
    if has_generic || rules.is_empty() {
        rules.push(json!({
            "id": "bhf.finding",
            "name": "BHF finding",
            "shortDescription": { "text": "BHF finding" },
            "fullDescription": {
                "text": "A BHF input triggered an exception-related finding."
            },
...
fn sarif_rule_entry(rule: &Rule) -> Value {
    ...
    tags.push(format!("bhf.slug:{}", rule.slug));

    let help_uri = rule
        .references
        .first()
        .copied()
        .unwrap_or("https://github.com/Tarmo-Technologies/bhf");
...
pub fn validate_sarif_report(report: &Value) -> Result<(), ReportError> {
    ...
    for (run_index, run) in runs.iter().enumerate() {
        ...
        for (result_index, result) in results.iter().enumerate() {
            ...
            expect_string(
                result,
                &["properties", "bhfExceptionSignature"],
                &result_path,
            )?;
            expect_string(result, &["properties", "bhfFindingId"], &result_path)?;
...
pub fn render_junit_report(document: &ReportDocument) -> String {
    ...
    out.push_str(&format!(
        ...
        xml_attr(&format!("bhf.{}", document.run.id)),
        tests,
        failures
    ));
    ...
    for issue in &issues {
        ...
        let classname = format!("bhf.{}", target_summary(finding));
        let failure_type = finding.classification.as_deref().unwrap_or("bhf_finding");
...
///      (`Rule.cwe`) — this is where behavioral oracle findings (`BHF-NNNN`),
...
# SPDX-License-Identifier: Apache-2.0
#
# Auto-generated by bhf. Standalone reproducer for this finding.
__HEADER__
#
# Usage:
...
def main():
    ...
    # bhf driver harnesses take it as argv[1]. Detect from the harness name,
    # else default to argv[1] (the bhf convention).
...
fn fallback_replay(id: &str) -> Value {
    json!({ "command": format!("bhf replay --finding {id}") })
}
...
fn sarif_result(
    ...
    let rule_id = rule.map(|rule| rule.id).unwrap_or("bhf.finding");
    let mut properties = json!({
        "bhfFindingId": finding.id,
        "bhfExceptionSignature": signature,
    ...
    if let Some(rule) = rule {
        ...
        obj.insert("bhf.rule_slug".to_owned(), json!(rule.slug));
        obj.insert(
            "precision".to_owned(),
    ...
    let mut result = json!({
        ...
        "partialFingerprints": {
            "bhfExceptionSignature": signature,
            "bhfRuleSignature": rule_signature(rule_id, signature),
    ...
    if let Some(cluster_key) = finding.cluster_key.as_deref() {
        ...
        pf.insert("bhfClusterKey".to_owned(), json!(cluster_key));
        if let Some(full) = finding.cluster_key_full.as_deref() {
            pf.insert("bhfClusterKeyFull".to_owned(), json!(full));
        }
        let props = result
            .get_mut("properties")
            .and_then(|v| v.as_object_mut())
    ...
    // patch stashed in `properties.bhfSuggestedPatch` — valid SARIF that
    ...
    if let Some((description, diff)) = sarif_fix_for(finding) {
        result.as_object_mut().expect("result is an object").insert(
            ...
            json!([{
                ...
                "properties": { "bhfSuggestedPatch": diff },
            }]),
        );
    }
    result
...
/// Boundary line inserted where the bhf harness scaffold begins, so a reader
/// can see the real target call path stops and the synthetic driver takes over.
const HARNESS_BOUNDARY_MARKER: &str = "… ↑ bhf harness (synthetic driver; frames omitted)";

/// Render the crash stack for the human-facing writeup, showing only the
/// target's call path. The bhf harness scaffolding (`bhf_run_one`,
...
/// `bhfRuleSignature`, which lets SARIF consumers (GitHub Code Scanning,
...
fn junit_system_out(finding: &FindingReport) -> String {
    ...
    if let Some(signature) = finding.signature.as_deref() {
        lines.push(format!("bhfExceptionSignature={signature}"));
    }
    if let Some(mode) = string_at(&finding.sandbox, &["mode"]) {
...
mod tests {
    ...
    fn write_reports_emits_sorted_json_and_markdown() {
        ...
        assert_eq!(report["schema_version"], "bhf.report.v2");
        ...
        assert!(markdown.contains("# BHF Report: unit"));
        assert!(
    ...
    fn load_findings_adds_calibrated_confidence_when_missing() {
        ...
        assert_eq!(loaded[0].confidence["calibration_id"], "bhf.calibrated.v1");
    ...
    fn sarif_result_carries_confirmation_property() {
        ...
        let prop = |id: &str| -> String {
            results
                .iter()
                .find(|r| r["properties"]["bhfFindingId"] == id)
    ...
    fn build_report_rejects_tampered_confidence_model_id() {
        ...
        mutate_confidence_model(&model_path, |model| {
            model["model_id"] = json!("bhf.learned.v1.tampered");
        });
    ...
    fn write_reports_emits_valid_sarif_when_requested() {
        ...
        assert_eq!(
            sarif["runs"][0]["results"][0]["properties"]["bhfExceptionSignature"],
            "abcd1234"
        );
        assert_eq!(
    ...
    fn sarif_emits_rule_catalog_entry_for_each_finding_rule() {
        ...
        let rule_ids: Vec<&str> = rules
            ...
            .collect();
        assert!(rule_ids.contains(&"BHF-101"));
        assert!(rule_ids.contains(&"BHF-102"));
        assert_eq!(rule_ids.iter().filter(|id| **id == "BHF-101").count(), 1);

        let bhf102 = rules
            .iter()
            .find(|rule| rule["id"] == "BHF-102")
            .expect("BHF-102 rule present");
        assert_eq!(bhf102["properties"]["cwe"], "CWE-129");
        assert_eq!(
            bhf102["properties"]["security-severity"].as_str().unwrap(),
            "7.0"
        );
        let tags = bhf102["properties"]["tags"].as_array().unwrap();
        assert!(tags.iter().any(|tag| tag == "CWE-129"));
        assert!(tags.iter().any(|tag| tag == "cert-c:ARR30-C"));
        assert!(tags
            .iter()
            .any(|tag| tag == "bhf.slug:bhf.ada/swallowed-range-or-index-check"));

        let result = &sarif["runs"][0]["results"]
            .as_array()
            .unwrap()
            .iter()
            .find(|r| r["properties"]["bhfFindingId"] == "F-0002-swallowed-constraint")
            .expect("finding result present")
            .clone();
        assert_eq!(result["ruleId"], "BHF-102");
    ...
    fn sarif_falls_back_to_generic_rule_when_classification_unmapped() {
        ...
        let rules = sarif["runs"][0]["tool"]["driver"]["rules"]
            .as_array()
            .unwrap();
        assert!(rules.iter().any(|rule| rule["id"] == "bhf.finding"));
        assert_eq!(sarif["runs"][0]["results"][0]["ruleId"], "bhf.finding");
    }
    ...
    fn sarif_top25_finding_carries_top25_tag() {
        ...
        assert_eq!(result["ruleId"], "BHF-202");
    ...
    fn sarif_rule_signature_is_stable_across_runs_with_same_rule_and_exception() {
        let sig1 = rule_signature("BHF-102", "abcd1234");
        let sig2 = rule_signature("BHF-102", "abcd1234");
        assert_eq!(sig1, sig2);
    ...
    fn sarif_rule_signature_differs_when_rule_differs() {
        let sig_a = rule_signature("BHF-101", "abcd1234");
        let sig_b = rule_signature("BHF-102", "abcd1234");
        assert_ne!(sig_a, sig_b);
    }

    #[test]
    fn sarif_rule_signature_differs_when_exception_signature_differs() {
        let sig_a = rule_signature("BHF-201", "deadbeef");
        let sig_b = rule_signature("BHF-201", "cafebabe");
        assert_ne!(sig_a, sig_b);
    }
    ...
    fn sarif_result_emits_rule_signature_in_partial_fingerprints() {
        ...
        assert_eq!(fingerprints["bhfExceptionSignature"], "deadbeefcafe");
        let rule_sig = fingerprints["bhfRuleSignature"]
            .as_str()
            .expect("rule signature present");
        assert_eq!(rule_sig.len(), 16);
        assert_eq!(rule_sig, rule_signature("BHF-202", "deadbeefcafe"));
    }
    ...
    fn sarif_iso_tr_24772_codes_each_emitted_as_separate_tag() {
        ...
        let bhf102 = rules
            ...
            .find(|rule| rule["id"] == "BHF-102")
            .expect("BHF-102 rule present");
        let tags: Vec<&str> = bhf102["properties"]["tags"]
            .as_array()
            .unwrap()
            .iter()
    ...
    #[test]
    fn markdown_stack_trims_bhf_harness_frames() {
            ...
            json!({
                ...
                "exception": {
                    ...
                    "stack": [
                        ...
                        { "function": "bhf_run_one", "file": "/work/auto/main.cpp", "line": 41 },
        ...
        // ... but the bhf harness scaffolding is trimmed and replaced by a
        // single boundary marker.
        assert!(
            !stack_block.contains("bhf_run_one"),
        ...
        assert!(
            stack_block.contains("bhf harness (synthetic driver; frames omitted)"),
        ...
        assert!(
            stack.iter().any(|frame| frame["function"] == "bhf_run_one"),
    ...
    fn markdown_flags_unproven_reachability_without_naming_harness() {
        ...
        // the synthetic bhf harness entry — the Sink / fix-location below
    ...
    fn write_reports_emits_junit_when_requested() {
        ...
        assert!(junit.contains(
            "<testsuite name=\"bhf.unit\" tests=\"1\" failures=\"1\" errors=\"0\" skipped=\"0\">"
        ));
        assert!(junit.contains(
            "<testcase classname=\"bhf.Pkg.Parse (H-1)\" name=\"F-0001-junit [likely_reachable]\">"
        ));
        assert!(junit.contains("type=\"explicit_raise\""));
        // JUnit failure message leads with the finding's CWE (BHF-106 -> CWE-754).
        ...
        assert!(junit.contains("bhfExceptionSignature=abcd1234"));
    }
    ...
    fn junit_generation_uses_current_finding_emitter_exception_shape() {
        ...
        assert!(junit.contains("bhfExceptionSignature=aabbccdd"));
    }
    ...
    fn junit_generation_replaces_invalid_xml_control_chars() {
        ...
        write_finding(
            ...
            json!({
                ...
                "replay": { "command": "bhf\u{001F} replay" }
            }),
        );
        let document = build_report(&ReportOptions::new(&findings, root.join("reports"))).unwrap();
        ...
        assert!(junit.contains("bhfExceptionSignature=sig?"));
        assert!(junit.contains("replay=bhf? replay"));
    }
    ...
    fn finding_report_with_actionability(
        ...
        FindingReport {
            ...
            replay: json!({ "command": "bhf replay --finding F-real" }),
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-report-{name}-{nonce}"));
    ...
    fn sarif_result_carries_cluster_key_in_partial_fingerprints() {
        ...
        let result = &sarif["runs"][0]["results"][0];
        assert_eq!(
            result["partialFingerprints"]["bhfClusterKey"],
            "k1aaaaaaaaaaaaaa"
        );
    ...
    fn render_csv_report_emits_header_and_one_row_per_issue() {
        ...
        for row in &lines[1..] {
            ...
            if !row.contains('"') {
                ...
                assert_eq!(cols[3], "CWE-248", "cwe column (BHF-101): {row}");
    ...
    fn sarif_result_carries_cwe_and_cluster_fingerprint() {
        ...
        assert_eq!(pf["bhfClusterKey"], "k1aaaaaaaaaaaaaa");
        assert_eq!(
            pf["bhfIssueKey"],
    ...
    fn load_finding_backfills_cwe_from_rule_for_oracle_finding() {
        ...
        write_finding(
            ...
            json!({
                ...
                "exception": { "name": "BHF-405", "message": "attacker-controlled path open" }
            }),
        );
        let findings = load_findings(&findings_dir).unwrap();
        let rule_cwe = rules::by_id("BHF-405").unwrap().cwe;
    ...
    fn markdown_and_sarif_surface_patch_diff_and_fix() {
        ...
        assert!(fix["properties"]["bhfSuggestedPatch"]
            .as_str()
            .unwrap()
