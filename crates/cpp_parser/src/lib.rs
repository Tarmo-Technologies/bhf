...
/// method (`_bhf_receiver.XXH128`). Only KNOWN decoration-macro tokens are blanked,
...
/// abstract must be skipped, never emitted as `Abstract _bhf_receiver;`.
...
fn function_param_types(declarator: tree_sitter::Node<'_>, source: &[u8]) -> Vec<String> {
    ...
    for child in params_node.children(&mut cursor) {
        ...
        // (`const void * key _bhf_p0`), which is invalid C++.
...
"#;
    ...
    fn free_function_in_capitalized_namespace_is_not_a_method() {
        ...
        // bogus `Json` class (which produced `Json _bhf_receiver`).
    ...
    fn function_leading_decoration_macros_stripped_from_return_type() {
        ...
        // made tree-sitter model it as a phantom method `_bhf_receiver.XXH128`).
        let functions = parse_cpp_functions(
    ...
    /// basisu_transcoder.cpp took >8 min in `bhf auto`). The template state is
