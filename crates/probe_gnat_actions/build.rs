...
fn main() {
    ...
    // skip re-running this script when BHF_PROFILE changes, which
    // would silently bypass the profile gate on subsequent invocations.
    println!("cargo:rerun-if-env-changed=BHF_PROFILE");
    ...
    let profile = std::env::var("BHF_PROFILE").unwrap_or_default();
    if !matches!(profile.as_str(), "external-tools" | "research-lab") {
        panic!(
            "probe_gnat_actions 'enabled' feature requires BHF_PROFILE=external-tools or research-lab"
        );
    }
}
