...
fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let dir = std::env::temp_dir().join(format!("bhf-harness-snapshot-{name}-{nonce}"));
...
fn direct_harness_result_var_does_not_collide_with_a_param_named_r() {
    ...
    assert!(
        text.contains("Bhf_Result : constant"),
...
fn direct_harness_drives_root_stream_type_class_param_with_fuzz_source() {
    ...
    assert!(text.contains("with Bhf_Source_Streams;"), "{text}");
    assert!(
        text.contains("Bhf_Source_Streams.Fuzz_Stream"),
        "harness must declare a fuzz source stream: {text}"
    );
    assert!(
        text.contains("Bhf_Source_Streams.Set (From, Buf'Unchecked_Access, Last)"),
        "harness must load the source stream from the fuzz input: {text}"
    );

    let dir = main_adb.parent().unwrap();
    assert!(
        dir.join("bhf_source_streams.ads").exists() && dir.join("bhf_source_streams.adb").exists(),
...
fn direct_harness_constructs_private_type_via_out_param_ctor_with_fuzz_stream() {
    ...
    assert!(
        text.contains("Bhf_Source_Streams.Set"),
