...
pub struct CppDecoderLimits {
    ...
    /// (`bhf_bounded_length(&Cur, 0, N)` for `vector`/`set`/`map`/`span`/…);
...
fn select_cpp_file_path_decoder(stripped: &str, name: &str) -> Option<CParamEmission> {
    ...
    let make = format!(
        "char {name}_path[bhf_tempfile_path_len]; \
         const char *{name}_made = bhf_make_tempfile(Data, Size, {name}_path)"
    );
...
pub(crate) fn select_cpp_decoder_limited(
    ...
    match stripped.as_str() {
        ...
        "std::string" => Some(CParamEmission {
            support: None,
            decl: format!(
                "char *_tmp_{name} = bhf_c_string(&Cur, 4096); std::string {name}(_tmp_{name}); free(_tmp_{name})",
            ),
            arg: name.to_owned(),
        ...
        // the bhf MFC stub's `CString(const char *)` (see cross_target.rs
        // MFC_STUB). Covers `CString` / `const CString &` / `CStringA`.
        "CString" | "CStringA" | "ATL::CStringA" | "ATL::CString" => Some(CParamEmission {
            support: None,
            decl: format!(
                "char *_tmp_{name} = bhf_c_string(&Cur, 4096); CString {name}(_tmp_{name} ? _tmp_{name} : \"\"); free(_tmp_{name})",
            ),
            arg: name.to_owned(),
        ...
        "std::optional<std::string>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::optional<std::string> {name}; if (bhf_u8(&Cur) & 1) {{ char *_tmp_{name} = bhf_c_string(&Cur, 4096); {name}.emplace(_tmp_{name} ? _tmp_{name} : \"\"); free(_tmp_{name}); }} (void){name}",
            ),
            arg: name.to_owned(),
        ...
        "std::filesystem::path" => Some(CParamEmission {
            support: None,
            decl: format!(
                "char *_tmp_{name} = bhf_c_string(&Cur, 4096); std::filesystem::path {name}(_tmp_{name}); free(_tmp_{name})",
            ),
            arg: name.to_owned(),
        ...
        "std::string_view" => Some(CParamEmission {
            support: None,
            decl: format!(
                "const uint8_t *_bhf_sv_ptr_{name} = nullptr; size_t _bhf_sv_len_{name} = 0; bhf_data_slice(&Cur, &_bhf_sv_ptr_{name}, &_bhf_sv_len_{name}); std::string_view {name}(reinterpret_cast<const char *>(_bhf_sv_ptr_{name}), _bhf_sv_len_{name})",
            ),
            arg: name.to_owned(),
        ...
        "std::u8string_view" => Some(CParamEmission {
            support: None,
            decl: format!(
                "const uint8_t *_bhf_sv_ptr_{name} = nullptr; size_t _bhf_sv_len_{name} = 0; bhf_data_slice(&Cur, &_bhf_sv_ptr_{name}, &_bhf_sv_len_{name}); std::u8string_view {name}(reinterpret_cast<const char8_t *>(_bhf_sv_ptr_{name}), _bhf_sv_len_{name})",
            ),
            arg: name.to_owned(),
        ...
        "std::u8string" => Some(CParamEmission {
            support: None,
            decl: format!(
                "const uint8_t *_bhf_u8s_ptr_{name} = nullptr; size_t _bhf_u8s_len_{name} = 0; bhf_data_slice(&Cur, &_bhf_u8s_ptr_{name}, &_bhf_u8s_len_{name}); std::u8string {name}(reinterpret_cast<const char8_t *>(_bhf_u8s_ptr_{name}), _bhf_u8s_len_{name})",
            ),
            arg: name.to_owned(),
        ...
        "std::vector<std::string>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<std::string> {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ char *_tmp_{name} = bhf_c_string(&Cur, 256); {name}.emplace_back(_tmp_{name} ? _tmp_{name} : \"\"); free(_tmp_{name}); }} (void){name}",
        ...
        "std::span<uint8_t>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<uint8_t> _bhf_span_storage_{name}(Data, Data + Size); std::span<uint8_t> {name}(_bhf_span_storage_{name}.data(), _bhf_span_storage_{name}.size()); (void){name}",
            ),
            arg: name.to_owned(),
        ...
        "std::span<std::uint8_t>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<std::uint8_t> _bhf_span_storage_{name}(Data, Data + Size); std::span<std::uint8_t> {name}(_bhf_span_storage_{name}.data(), _bhf_span_storage_{name}.size()); (void){name}",
            ),
            arg: name.to_owned(),
        ...
        "std::span<unsigned char>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<unsigned char> _bhf_span_storage_{name}(Data, Data + Size); std::span<unsigned char> {name}(_bhf_span_storage_{name}.data(), _bhf_span_storage_{name}.size()); (void){name}",
            ),
            arg: name.to_owned(),
        ...
        "std::span<char>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<char> _bhf_span_storage_{name}(reinterpret_cast<const char *>(Data), reinterpret_cast<const char *>(Data) + Size); std::span<char> {name}(_bhf_span_storage_{name}.data(), _bhf_span_storage_{name}.size()); (void){name}",
            ),
            arg: name.to_owned(),
        ...
        "std::span<std::byte>" => Some(CParamEmission {
            support: None,
            decl: format!(
                "std::vector<std::byte> _bhf_span_storage_{name}(reinterpret_cast<const std::byte *>(Data), reinterpret_cast<const std::byte *>(Data) + Size); std::span<std::byte> {name}(_bhf_span_storage_{name}.data(), _bhf_span_storage_{name}.size()); (void){name}",
            ),
            arg: name.to_owned(),
...
fn select_std_chrono_duration_decoder(
    ...
    Some(CParamEmission {
        support: None,
        decl: format!("{cpp_type} {name}(bhf_bounded_i32(&Cur, 0, {max_value})); (void){name}"),
        arg: name.to_owned(),
        c_type,
        free: None,
    })
...
fn select_std_bitset_decoder(
    ...
    Some(CParamEmission {
        support: None,
        decl: format!(
            "{bitset_type} {name}; for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < {name}.size(); ++_bhf_i_{name}) {{ {name}.set(_bhf_i_{name}, (bhf_u8(&Cur) & 1) != 0); }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_fixed_byte_array_decoder(
    ...
    Some(CParamEmission {
        support: None,
        decl: format!(
            "std::array<{elem}, {len}> {name}{{}}; size_t _bhf_copy_{name} = Size < {name}.size() ? Size : {name}.size(); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_copy_{name}; ++_bhf_i_{name}) {{ {name}[_bhf_i_{name}] = static_cast<{elem}>(Data[_bhf_i_{name}]); }} (void){name}",
        ),
        arg: name.to_owned(),
...
fn select_std_array_decoder(
    ...
    let element_name = format!("_bhf_array_{name}_elt");
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "std::array<{element_type}, {len}> {name}{{}}; for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < {name}.size(); ++_bhf_i_{name}) {{ {decl}; {name}[_bhf_i_{name}] = {arg}; }} (void){name}"
        ),
        arg: name.to_owned(),
...
fn select_std_array_decoder_with_registry(
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "{array_type} {name}{{}}; for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < {name}.size(); ++_bhf_i_{name}) {{ {decl}; {name}[_bhf_i_{name}] = {arg}; }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_std_optional_decoder(
    ...
    let inner_name = format!("_bhf_optional_{name}");
    ...
    Some(CParamEmission {
        ...
        decl: format!(
            "std::optional<{inner_type}> {name}; if (bhf_u8(&Cur) & 1) {{ {}; {name}.emplace({}); }} (void){name}",
...
fn select_std_pair_decoder(
    ...
    let first_name = format!("_bhf_pair_{name}_first");
    let second_name = format!("_bhf_pair_{name}_second");
...
fn select_std_vector_decoder(
    ...
    let element_name = format!("_bhf_vector_{name}_elt");
    let element = select_cpp_decoder_limited(element_type, &element_name, limits)?;
    let CParamEmission {
        support,
        decl,
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "std::vector<{element_type}> {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {name}.emplace_back({arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
...
fn select_std_vector_decoder_with_registry(
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "{vector_type} {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {name}.emplace_back({arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_std_sequence_container_decoder(
    ...
    let element_name = format!("_bhf_{variable_prefix}_{name}_elt");
    let element = select_cpp_decoder_limited(element_type, &element_name, limits)?;
    let CParamEmission {
        support,
        decl,
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "{container}<{element_type}> {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {name}.{insertion_method}({arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
...
fn select_std_sequence_container_decoder_with_registry(
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "{container_type} {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {name}.{insertion_method}({arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_std_associative_set_decoder(
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "{container}<{element_type}> {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {name}.emplace({arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type: format!("{container}<{element_type}>"),
        free: None,
...
fn select_std_associative_map_decoder(
    ...
    let key_name = format!("_bhf_{variable_prefix}_{name}_key");
    let value_name = format!("_bhf_{variable_prefix}_{name}_value");
    ...
    Some(CParamEmission {
        ...
        decl: format!(
            "{container}<{key_type}, {value_type}> {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {key_decl}; {value_decl}; {name}.emplace({key_arg}, {value_arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
...
fn select_std_associative_map_decoder_with_registry(
    ...
    Some(CParamEmission {
        support: combine_support(key_support, value_support),
        decl: format!(
            "{map_type} {name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {key_decl}; {value_decl}; {name}.emplace({key_arg}, {value_arg}); }} (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_std_span_decoder_with_registry(
    ...
    let element_name = format!("_bhf_span_{name}_elt");
    let element =
    ...
    let storage_name = format!("_bhf_span_storage_{name}");
    let c_type = if is_cpp_reference(raw_type) {
        raw_type.trim().to_owned()
    } else {
        span_type.clone()
    ...
    Some(CParamEmission {
        support,
        decl: format!(
            "std::vector<{storage_type}> {storage_name}; size_t _bhf_count_{name} = bhf_bounded_length(&Cur, 0, {cap}); for (size_t _bhf_i_{name} = 0; _bhf_i_{name} < _bhf_count_{name}; ++_bhf_i_{name}) {{ {decl}; {storage_name}.emplace_back({arg}); }} {span_type} {name}({storage_name}.data(), {storage_name}.size()); (void){name}"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_std_tuple_decoder(
    ...
    for (index, element_type) in element_types.iter().enumerate() {
        let element_name = format!("_bhf_tuple_{name}_{index}");
...
fn select_std_variant_decoder(
    ...
    for (index, alternative_type) in alternative_types.iter().enumerate() {
        let alternative_name = format!("_bhf_variant_{name}_{index}");
    ...
    Some(CParamEmission {
        support: combine_supports(supports),
        decl: format!(
            "{variant_type} {name} = [&]() -> {variant_type} {{ switch (bhf_u8(&Cur) % {}) {{ {} default: {{ {default_body} }} }} }}(); (void){name}",
...
fn unicode_string_view_emission(
    ...
    let ptr = format!("_bhf_sv_ptr_{name}");
    let len = format!("_bhf_sv_len_{name}");
    let buf = format!("_bhf_sv_buf_{name}");
    let idx = format!("_bhf_sv_i_{name}");
    let combine = (0..width)
        .map(|b| {
            if b == 0 {
    ...
    CParamEmission {
        support: None,
        decl: format!(
            "const uint8_t *{ptr} = nullptr; size_t {len} = 0; bhf_data_slice(&Cur, &{ptr}, &{len}); {owned_type} {buf}; for (size_t {idx} = 0; {idx} + {width} <= {len}; {idx} += {width}) {{ {buf}.push_back(({char_type})({combine})); }} {view_type} {name}({buf})"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn string_view_alias_emission(raw_type: &str, value_type: &str, name: &str) -> CParamEmission {
    ...
    CParamEmission {
        support: None,
        decl: format!(
            "const uint8_t *_bhf_sv_ptr_{name} = nullptr; size_t _bhf_sv_len_{name} = 0; bhf_data_slice(&Cur, &_bhf_sv_ptr_{name}, &_bhf_sv_len_{name}); {value_type} {name}(reinterpret_cast<const char *>(_bhf_sv_ptr_{name}), _bhf_sv_len_{name})"
        ),
        arg: name.to_owned(),
        c_type,
        free: None,
...
fn select_cpp_output_sink_decoder(
    ...
    let scratch = format!("_bhf_out_{name}");
    Some(CParamEmission {
        support: None,
...
fn select_std_function_decoder(
    ...
    let lambda_params = param_types
        .iter()
        .enumerate()
        .map(|(index, param_type)| format!("{} _bhf_{name}_arg{index}", param_type.trim()))
        .collect::<Vec<_>>();
    let void_params = (0..param_types.len())
        .map(|index| format!("(void)_bhf_{name}_arg{index};"))
        .collect::<Vec<_>>()
        .join(" ");
...
mod tests {
    ...
    fn class_construction_recipes_emit_factory_and_constructor_for_opaque_params() {
        ...
        assert!(
            ctor.decl.contains("int w_gfa0 = bhf_i32(&Cur)"),
    ...
    fn select_cpp_decoder_handles_std_string() {
        ...
        assert!(e.decl.contains("bhf_c_string"));
    ...
    fn select_cpp_decoder_drives_mfc_cstring_param_from_fuzz_string() {
        ...
        for ty in ["const CString &", "CString", "CStringA"] {
            ...
            assert!(
                e.decl.contains("bhf_c_string") && e.decl.contains("CString cmd(_tmp_cmd"),
                "{ty:?} -> {}",
                e.decl
            );
        }
    ...
    fn select_cpp_decoder_handles_win32_bool_scalar() {
        ...
        assert!(
            e.decl
                .contains("BOOL bValidateWithSchema = (BOOL)bhf_i32(&Cur)"),
            "{}",
            e.decl
        );
    ...
    fn select_cpp_decoder_drives_file_path_params_with_tempfile() {
        ...
        assert!(
            s.decl
                .contains("bhf_make_tempfile(Data, Size, filePath_path)"),
            "got {}",
            s.decl
        );
        assert!(
        ...
        assert!(
            c.decl
                .contains("bhf_make_tempfile(Data, Size, filename_path)"),
            "got {}",
            c.decl
        );
        assert!(
        ...
        // A non-path-named std::string is UNCHANGED (still bhf_c_string).
        let plain = select_cpp_decoder("const std::string &", "input").expect("plain string");
        assert!(
            plain.decl.contains("bhf_c_string"),
            "plain string param must stay bhf_c_string"
        );
        assert!(!plain.decl.contains("bhf_make_tempfile"));
    }
    ...
    fn qualify_std_type_names_preserves_utf8_before_an_identifier() {
        // Regression for BHF-210: fuzzed replacement/non-ASCII characters can
    ...
    fn select_cpp_decoder_handles_string_view() {
        ...
        assert!(e.decl.contains("bhf_data_slice"));
    }
    ...
    fn select_cpp_decoder_handles_direct_nonstd_string_view() {
        ...
        assert!(e.decl.contains("bhf_data_slice"), "{}", e.decl);
    ...
    fn select_cpp_decoder_handles_string_vectors() {
        ...
        assert!(e.decl.contains("bhf_bounded_length(&Cur, 0, 16)"));
        assert!(e
            .decl
    ...
    fn select_cpp_decoder_handles_vector_of_supported_values() {
        ...
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_vector_items_elt = (std::uint32_t)bhf_bounded_i32"));
        assert!(e.decl.contains("items.emplace_back(_bhf_vector_items_elt)"));
    ...
    fn select_cpp_decoder_handles_deque_and_list_of_supported_values() {
        ...
        assert!(deque.decl.contains("bhf_bounded_length(&Cur, 0, 16)"));
        assert!(deque
            .decl
            .contains("std::uint32_t _bhf_deque_items_elt = (std::uint32_t)bhf_bounded_i32"));
        assert!(deque
            .decl
            .contains("items.emplace_back(_bhf_deque_items_elt)"));
        ...
        assert!(list.decl.contains("std::string_view _bhf_list_views_elt("));
        assert!(list
            .decl
            .contains("views.emplace_back(_bhf_list_views_elt)"));
    ...
    fn select_cpp_decoder_handles_forward_list_of_supported_values() {
        ...
        assert!(items.decl.contains("bhf_bounded_length(&Cur, 0, 16)"));
        assert!(items.decl.contains(
            "std::uint32_t _bhf_forward_list_items_elt = (std::uint32_t)bhf_bounded_i32"
        ));
        assert!(items
            .decl
            .contains("items.emplace_front(_bhf_forward_list_items_elt)"));
    ...
    fn select_cpp_decoder_handles_set_and_map_of_supported_values() {
        ...
        assert!(set.decl.contains("bhf_bounded_length(&Cur, 0, 16)"));
        assert!(set
            .decl
            .contains("std::uint32_t _bhf_set_ids_elt = (std::uint32_t)bhf_bounded_i32"));
        assert!(set.decl.contains("ids.emplace(_bhf_set_ids_elt)"));
        ...
        assert!(map.decl.contains("std::string_view _bhf_map_lookup_key("));
        assert!(map
            .decl
            .contains("std::uint32_t _bhf_map_lookup_value = (std::uint32_t)bhf_bounded_i32"));
        assert!(map
            .decl
            .contains("lookup.emplace(_bhf_map_lookup_key, _bhf_map_lookup_value)"));
    ...
    fn select_cpp_decoder_handles_unordered_set_and_map_of_supported_values() {
        ...
        assert!(set
            .decl
            .contains("std::uint32_t _bhf_unordered_set_ids_elt = (std::uint32_t)bhf_bounded_i32"));
        assert!(set.decl.contains("ids.emplace(_bhf_unordered_set_ids_elt)"));
        ...
        assert!(map
            .decl
            .contains("std::string_view _bhf_unordered_map_lookup_key("));
        assert!(map.decl.contains(
            "std::uint32_t _bhf_unordered_map_lookup_value = (std::uint32_t)bhf_bounded_i32"
        ));
        assert!(map.decl.contains(
            "lookup.emplace(_bhf_unordered_map_lookup_key, _bhf_unordered_map_lookup_value)"
        ));
        assert_eq!(
            map.c_type,
    ...
    fn select_cpp_decoder_handles_fixed_byte_arrays() {
        ...
        assert!(key
            .decl
            .contains("key[_bhf_i_key] = static_cast<std::byte>(Data[_bhf_i_key])"));
        ...
        assert!(raw
            .decl
            .contains("raw[_bhf_i_raw] = static_cast<std::uint8_t>(Data[_bhf_i_raw])"));
    }
    ...
    fn select_cpp_decoder_handles_array_of_supported_values() {
        ...
        assert!(values
            .decl
            .contains("std::uint32_t _bhf_array_values_elt = (std::uint32_t)bhf_bounded_i32"));
        assert!(values
            .decl
            .contains("values[_bhf_i_values] = _bhf_array_values_elt"));
    ...
    fn select_cpp_decoder_handles_bitset_flags() {
        ...
        assert!(flags.decl.contains("flags.set(_bhf_i_flags"));
        assert!(flags.decl.contains("(bhf_u8(&Cur) & 1) != 0"));
    ...
    fn select_cpp_decoder_handles_mutable_byte_spans() {
        ...
        assert!(byte_span
            .decl
            .contains("std::vector<std::byte> _bhf_span_storage_bytes("));
        assert!(byte_span
            .decl
            .contains("std::span<std::byte> bytes(_bhf_span_storage_bytes.data(), _bhf_span_storage_bytes.size())"));
        ...
        assert!(u8_span
            .decl
            .contains("std::vector<std::uint8_t> _bhf_span_storage_raw(Data, Data + Size)"));
        assert!(u8_span.decl.contains(
            "std::span<std::uint8_t> raw(_bhf_span_storage_raw.data(), _bhf_span_storage_raw.size())"
        ));
    }
    ...
    fn select_cpp_decoder_handles_optional_string() {
        ...
        assert!(e.decl.contains("bhf_u8(&Cur) & 1"));
        assert!(e
            .decl
    ...
    fn select_cpp_decoder_handles_optional_standard_scalar() {
        ...
        assert!(e.decl.contains("if (bhf_u8(&Cur) & 1)"));
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_optional_maybe_count = (std::uint32_t)bhf_bounded_i32"));
        assert!(e
            .decl
            .contains("maybe_count.emplace(_bhf_optional_maybe_count)"));
    ...
    fn select_cpp_decoder_with_registry_handles_optional_visible_aggregate() {
        ...
        assert!(maybe.decl.contains("Config _bhf_optional_maybe"));
        assert!(maybe
            .decl
            .contains("_bhf_optional_maybe.mode = bhf_i32(&Cur)"));
        assert!(maybe.decl.contains("maybe.emplace(_bhf_optional_maybe)"));
    ...
    fn select_cpp_decoder_with_registry_resolves_string_alias_for_file_path() {
        ...
        assert!(value.decl.contains("bhf_c_string"), "got {}", value.decl);
        assert!(!value.decl.contains("bhf_make_tempfile"));
    }
    ...
    fn select_cpp_decoder_with_registry_handles_vector_visible_aggregate() {
        ...
        assert!(items.decl.contains("Config _bhf_vector_items_elt"));
        assert!(items
            .decl
            .contains("_bhf_vector_items_elt.mode = bhf_i32(&Cur)"));
        assert!(items
            .decl
            .contains("items.emplace_back(_bhf_vector_items_elt)"));
    ...
    fn select_cpp_decoder_with_registry_handles_array_visible_aggregate() {
        ...
        assert!(items.decl.contains("Config _bhf_array_items_elt"));
        assert!(items
            .decl
            .contains("_bhf_array_items_elt.mode = bhf_i32(&Cur)"));
        assert!(items
            .decl
            .contains("items[_bhf_i_items] = _bhf_array_items_elt"));
        assert_eq!(items.arg, "items");
    ...
    fn select_cpp_decoder_with_registry_handles_sequence_visible_aggregates() {
        ...
        for (cpp_type, container, prefix, insertion) in [
            ...
            assert!(items
                .decl
                .contains(&format!("Config _bhf_{prefix}_items_elt")));
            assert!(items
                .decl
                .contains(&format!("_bhf_{prefix}_items_elt.mode = bhf_i32(&Cur)")));
            assert!(items
                .decl
                .contains(&format!("items.{insertion}(_bhf_{prefix}_items_elt)")));
            assert_eq!(items.arg, "items");
    ...
    fn select_cpp_decoder_with_registry_handles_composed_visible_aggregates() {
        ...
        assert!(pair.decl.contains("Config _bhf_pair_entry_first"));
        assert!(pair
            .decl
            .contains("_bhf_pair_entry_first.mode = bhf_i32(&Cur)"));
        assert!(pair.decl.contains(
            "std::pair<Config, std::uint32_t> entry(_bhf_pair_entry_first, _bhf_pair_entry_second)"
        ));
        ...
        assert!(tuple.decl.contains("Config _bhf_tuple_entry_0"));
        assert!(tuple
            .decl
            .contains("_bhf_tuple_entry_0.mode = bhf_i32(&Cur)"));
        assert!(tuple.decl.contains(
            "std::tuple<Config, std::uint32_t> entry(_bhf_tuple_entry_0, _bhf_tuple_entry_1)"
        ));
        ...
        assert!(variant.decl.contains("Config _bhf_variant_choice_0"));
        assert!(variant
            .decl
            .contains("_bhf_variant_choice_0.mode = bhf_i32(&Cur)"));
        assert!(variant.decl.contains(
            "std::variant<Config, std::uint32_t>(std::in_place_index<0>, _bhf_variant_choice_0)"
        ));
        assert_eq!(
            variant.c_type,
            "const std::variant<Config, std::uint32_t> &"
    ...
    fn select_cpp_decoder_with_registry_handles_map_visible_aggregate_values() {
        ...
        assert!(map.decl.contains("std::string_view _bhf_map_lookup_key("));
        assert!(map.decl.contains("Config _bhf_map_lookup_value"));
        assert!(map
            .decl
            .contains("_bhf_map_lookup_value.mode = bhf_i32(&Cur)"));
        assert!(map
            .decl
            .contains("lookup.emplace(_bhf_map_lookup_key, _bhf_map_lookup_value)"));
        ...
        assert!(unordered
            .decl
            .contains("std::string _bhf_unordered_map_lookup_key"));
        assert!(unordered
            .decl
            .contains("Config _bhf_unordered_map_lookup_value"));
        assert!(unordered
            .decl
            .contains("_bhf_unordered_map_lookup_value.mode = bhf_i32(&Cur)"));
    ...
    fn select_cpp_decoder_with_registry_handles_span_visible_aggregates() {
        ...
        assert!(mutable
            .decl
            .contains("std::vector<Config> _bhf_span_storage_items;"));
        assert!(mutable.decl.contains("Config _bhf_span_items_elt"));
        assert!(mutable
            .decl
            .contains("_bhf_span_items_elt.mode = bhf_i32(&Cur)"));
        assert!(mutable
            .decl
            .contains("_bhf_span_storage_items.emplace_back(_bhf_span_items_elt)"));
        assert!(mutable.decl.contains(
            "std::span<Config> items(_bhf_span_storage_items.data(), _bhf_span_storage_items.size())"
        ));
        ...
        assert!(constant
            .decl
            .contains("std::vector<Config> _bhf_span_storage_items;"));
        assert!(constant.decl.contains("Config _bhf_span_items_elt"));
        assert!(constant.decl.contains(
            "std::span<const Config> items(_bhf_span_storage_items.data(), _bhf_span_storage_items.size())"
        ));
    ...
    fn select_cpp_decoder_handles_smart_pointers_to_supported_values() {
        ...
        assert!(unique.decl.contains(
            "std::uint32_t _bhf_unique_ptr_owned_value = (std::uint32_t)bhf_bounded_i32"
        ));
        assert!(unique.decl.contains(
            "std::unique_ptr<std::uint32_t> owned = std::make_unique<std::uint32_t>(_bhf_unique_ptr_owned_value)"
        ));
        ...
        assert!(shared
            .decl
            .contains("std::string _bhf_shared_ptr_name_value"));
        assert!(shared.decl.contains(
            "std::shared_ptr<std::string> name = std::make_shared<std::string>(_bhf_shared_ptr_name_value)"
        ));
    ...
    fn select_cpp_decoder_with_registry_handles_smart_pointer_to_visible_aggregate() {
        ...
        assert!(owned.decl.contains("Config _bhf_unique_ptr_owned_value"));
        assert!(owned
            .decl
            .contains("_bhf_unique_ptr_owned_value.mode = bhf_i32(&Cur)"));
        assert!(owned.decl.contains(
            "std::unique_ptr<Config> owned = std::make_unique<Config>(_bhf_unique_ptr_owned_value)"
        ));
    ...
    fn select_cpp_decoder_handles_pair_of_supported_values() {
        ...
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_pair_entry_first = (std::uint32_t)bhf_bounded_i32"));
        assert!(e.decl.contains("std::string_view _bhf_pair_entry_second("));
        assert!(e.decl.contains(
            "std::pair<std::uint32_t, std::string_view> entry(_bhf_pair_entry_first, _bhf_pair_entry_second)"
        ));
    ...
    fn select_cpp_decoder_handles_tuple_of_supported_values() {
        ...
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_tuple_entry_0 = (std::uint32_t)bhf_bounded_i32"));
        assert!(e.decl.contains("std::string_view _bhf_tuple_entry_1("));
        assert!(e
            .decl
            .contains("bool _bhf_tuple_entry_2 = (bool)(bhf_u8(&Cur) & 1)"));
        assert!(e.decl.contains(
            "std::tuple<std::uint32_t, std::string_view, bool> entry(_bhf_tuple_entry_0, _bhf_tuple_entry_1, _bhf_tuple_entry_2)"
        ));
        assert_eq!(
            e.c_type,
    ...
    fn select_cpp_decoder_handles_variant_of_supported_values() {
        ...
        assert!(e.decl.contains("switch (bhf_u8(&Cur) % 3)"));
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_variant_choice_0 = (std::uint32_t)bhf_bounded_i32"));
        assert!(e.decl.contains("std::string_view _bhf_variant_choice_1("));
        assert!(e.decl.contains(
            "return std::variant<std::uint32_t, std::string_view, bool>(std::in_place_index<1>, _bhf_variant_choice_1)"
        ));
        assert_eq!(
            e.c_type,
    ...
    fn select_cpp_decoder_handles_monostate_variant_alternative() {
        ...
        assert!(e.decl.contains("std::monostate _bhf_variant_choice_0{}"));
        assert!(e.decl.contains(
            "return std::variant<std::monostate, std::uint32_t>(std::in_place_index<0>, _bhf_variant_choice_0)"
        ));
        assert!(e
            .decl
            .contains("std::uint32_t _bhf_variant_choice_1 = (std::uint32_t)bhf_bounded_i32"));
    ...
    fn select_cpp_decoder_handles_filesystem_path() {
        ...
        assert!(e
            .decl
            .contains("char *_tmp_path = bhf_c_string(&Cur, 4096)"));
    ...
    fn select_cpp_decoder_falls_back_to_c_for_scalars() {
        ...
        assert!(e.decl.contains("int n = bhf_i32(&Cur)"));
    }
    ...
    fn select_cpp_decoder_handles_standard_scalar_aliases() {
        ...
        assert!(count
            .decl
            .contains("std::size_t count = (std::size_t)bhf_bounded_length(&Cur, 0, 65536)"));
        ...
        assert!(flags
            .decl
            .contains("std::uint32_t flags = (std::uint32_t)bhf_bounded_i32(&Cur, 0, 0x7fffffff)"));
        ...
        assert!(limit
            .decl
            .contains("std::size_t limit = (std::size_t)bhf_bounded_length(&Cur, 0, 65536)"));
        ...
        assert!(
            pos.decl.contains(
                "std::string::size_type parser_stop_pos = \
                 (std::string::size_type)bhf_bounded_length(&Cur, 0, 65536)"
            ),
            "decl: {}",
            pos.decl
        );
    ...
    fn select_cpp_decoder_handles_const_reference_scalars() {
        ...
        assert!(b.decl.contains("bhf_u8"), "{}", b.decl);
    }
    ...
    fn select_cpp_decoder_handles_u8_string_types() {
        ...
        assert!(view.decl.contains("bhf_data_slice"), "{}", view.decl);
        ...
        assert!(owned.decl.contains("bhf_data_slice"), "{}", owned.decl);
    ...
    fn select_cpp_decoder_handles_u16_u32_string_views() {
        ...
        assert!(
            u16.decl.contains("std::u16string _bhf_sv_buf_v"),
            "{}",
            u16.decl
        );
        assert!(
            u16.decl.contains("std::u16string_view v(_bhf_sv_buf_v)"),
            "{}",
            u16.decl
        );
        assert!(u16.decl.contains("+ 2 <= _bhf_sv_len_v"), "{}", u16.decl);
        assert_eq!(u16.c_type, "std::u16string_view");

        let u32 =
            select_cpp_decoder("std::u32string_view", "v").expect("u32string_view is supported");
        assert!(
            u32.decl.contains("std::u32string _bhf_sv_buf_v"),
            "{}",
            u32.decl
        );
        assert!(
            u32.decl.contains("std::u32string_view v(_bhf_sv_buf_v)"),
            "{}",
            u32.decl
        );
        assert!(u32.decl.contains("+ 4 <= _bhf_sv_len_v"), "{}", u32.decl);
    }
    ...
    fn select_cpp_decoder_with_registry_stack_allocates_output_sink_pointers() {
        ...
        assert!(
            warn.decl.contains("std::string _bhf_out_warn{}"),
            "{}",
            warn.decl
        );
        assert!(
            warn.decl.contains("std::string * warn = &_bhf_out_warn"),
            "{}",
            warn.decl
        );
        ...
        assert!(
            shapes
                .decl
                .contains("std::vector<shape_t> _bhf_out_shapes{}"),
            "{}",
            shapes.decl
        );
        assert!(
            shapes
                .decl
                .contains("std::vector<shape_t> * shapes = &_bhf_out_shapes"),
            "{}",
            shapes.decl
        );
        ...
        assert!(
            attrib.decl.contains("attrib_t _bhf_out_attrib{}"),
            "{}",
            attrib.decl
        );
    ...
    fn cpp_decoder_limits_default_is_byte_identical() {
        ...
        assert!(
            legacy.decl.contains("bhf_bounded_length(&Cur, 0, 16)"),
    ...
    fn cpp_custom_container_cap_shrinks_emitted_bound() {
        ...
        for ty in [
            ...
            assert!(
                e.decl.contains("bhf_bounded_length(&Cur, 0, 4)"),
    ...
    fn cpp_oom_guard_clamps_huge_cap_and_skips_huge_array() {
        ...
        assert!(
            e.decl
                .contains(&format!("bhf_bounded_length(&Cur, 0, {clamp})")),
        ...
        assert!(
            e.decl.contains("bhf_bounded_length(&Cur, 0, 1000000)"),
