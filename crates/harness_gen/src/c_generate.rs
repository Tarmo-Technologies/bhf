...
const BUILD_CONTEXT_COMPILER_PREFIX: &str = "@bhf-build-context-compiler=";
const BUILD_CONTEXT_PROVENANCE_PREFIX: &str = "@bhf-build-context-provenance=";
const BUILD_CONTEXT_DROPPED_PREFIX: &str = "@bhf-build-context-dropped=";
...
pub struct GenerateCSequenceArgs {
    ...
    pub source_path: PathBuf,
    pub target: CFunction,
    /// C type stored in `_bhf_handle`. For an in-place lifecycle this is the
...
struct CSequenceStepEmission {
    ...
    /// Expression for the object this op is called ON. `&_bhf_handle` unless the
...
fn strip_redundant_callback_typedef(mut param: CParamEmission) -> CParamEmission {
    let stripped = param.support.as_deref().and_then(|support| {
        ...
        // `_bhf_cb_<name>` typedef is the ONLY declaration of the callback's decl
        // type, and the included header cannot supply it. Dropping it would leave
        // the decl referencing an undeclared `_bhf_cb_*` type (failed_build). Only a
        ...
        if typedef_line.contains("_bhf_cb_") {
            return None;
        }
        Some(body.to_owned())
    });
...
fn build_param_decoders(
    ...
    while i < params.len() {
        ...
        // pair, the string falls through to the NUL-terminating `bhf_c_string` decoder.
        if !pair_consumed
            && i + 1 < params.len()
        ...
        // specifiers with bhf_c_format_string. This complements the NAME-based
...
fn out_param_handle_scratch(
    ...
    let local = format!("_bhf_out_{name}");
...
fn parser_config_struct_scratch(
    ...
    let local = format!("_bhf_cfg_{name}");
    Some(CParamEmission {
        support: None,
...
fn integer_index_out_pointer(c_type: &str, name: &str) -> Option<CParamEmission> {
    ...
    let storage = format!("_bhf_out_{name}");
    Some(CParamEmission {
        support: None,
...
fn pair_stream_count_byte_cursor(
    ...
    let len_storage = format!("_bhf_stream_{len_name}");
    ...
    match kind {
        ByteDoublePointerKind::Input => {
            let inner_type = cursor_type
                ...
                .trim();
            let ptr_storage = format!("_bhf_stream_{cursor_name}");
        ...
        ByteDoublePointerKind::Output => {
            let inner_type = cursor_type
                ...
                .expect("double pointer has outer star")
                .trim();
            let cap = format!("_bhf_cap_{cursor_name}");
            let buffer = format!("_bhf_buf_{cursor_name}");
            let ptr_storage = format!("_bhf_stream_{cursor_name}");
            let len_emission = CParamEmission {
                support: None,
                decl: format!(
...
fn pair_buffer_length(
    ...
    if is_const_pointer_type(&canonical_c_type(&buffer.c_type)) {
        ...
        if nul_terminate {
            let copy = format!("_bhf_ntbuf_{buf_name}");
            let copy_len = format!("_bhf_ntlen_{buf_name}");
            let buf_decl = format!(
    ...
    let cap_name = format!("_bhf_cap_{buf_name}");
    let buf_decl = format!(
...
fn pair_typed_array_count(
    ...
    let n_name = format!("_bhf_n_{arr_name}");

    let arr_decl = format!(
        "size_t {n_name} = bhf_bounded_length(&Cur, 0, 64); \
...
fn pair_string_array_count(
    ...
    let count_type = emit_c_type(&count.c_type);
    let count_name = &count.name;
    let n = format!("_bhf_n_{arr_name}");
    let actual = format!("_bhf_count_{arr_name}");
    let i = format!("_bhf_i_{arr_name}");

    let arr_decl = format!(
        "size_t {n} = bhf_bounded_length(&Cur, 0, 16); \
         {elem} **{arr_name} = ({elem} **)calloc({n} ? {n} : 1, sizeof({elem} *)); \
         size_t {actual} = 0; \
         for (size_t {i} = 0; {arr_name} && {i} < {n}; ++{i}) {{ \
             {arr_name}[{i}] = bhf_c_string(&Cur, 256); \
             if (!{arr_name}[{i}]) break; \
             {actual}++; \
         }}"
    );
...
fn file_path_array_param(param: &CParameter) -> Option<CParamEmission> {
    ...
    let path = format!("_bhf_path_{name}");
    let made = format!("_bhf_path_made_{name}");
    let storage = format!("_bhf_paths_{name}");
    let decl = if wide {
        let wide_path = format!("_bhf_wpath_{name}");
        let index = format!("_bhf_wpath_i_{name}");
        format!(
            "char {path}[bhf_tempfile_path_len]; \
             const char *{made} = bhf_make_tempfile(Data, Size, {path}); \
             wchar_t {wide_path}[bhf_tempfile_path_len]; \
             size_t {index} = 0; \
             if ({made}) {{ \
                 while ({index} + 1 < bhf_tempfile_path_len && {path}[{index}]) {{ \
...
fn pair_output_length_buffer(
    ...
    let len_base = pointer_base(&len_type)
        ...
        .to_owned();
    let buf_type = emit_c_type(&buffer.c_type);
    let cap_name = format!("_bhf_cap_{}", buffer.name);
    let len_storage = format!("_bhf_out_{}", length.name);
    let capacity = output_buffer_capacity_expression(function_name);
...
fn is_synthetic_param_name(name: &str) -> bool {
    // Sequence emission scopes every name (`_bhf_step5_arg1`), so inspect the
    ...
    if let Some(rest) = stem.strip_prefix("bhf_") {
        stem = rest;
    }
...
fn pair_input_buffer_length_pointer(
    ...
    let len_base = pointer_base(&len_type)
        ...
        .to_owned();
    let len_name = &length.name;
    let len_storage = format!("_bhf_in_{len_name}");
    if is_const_pointer_type(&canonical_c_type(&buffer.c_type)) {
        let buf_emission = CParamEmission {
            support: None,
...
fn pair_output_buffer_length(
    ...
    let len_name = &length.name;
    let cap_name = format!("_bhf_cap_{buf_name}");
    let len_storage = format!("_bhf_out_{len_name}");
...
fn bind_webp_interleaved_output_geometry(
    ...
    emissions[2].decl = format!(
        "int _bhf_image_width = 0; int _bhf_image_height = 0; \
         int _bhf_image_geometry_valid = WebPGetInfo({input}, {input_size}, \
             &_bhf_image_width, &_bhf_image_height) && \
             _bhf_image_width > 0 && _bhf_image_height > 0 && \
             _bhf_image_width <= 4096 && _bhf_image_height <= 4096; \
         size_t _bhf_image_stride = 0; size_t _bhf_image_capacity = 0; \
         if (_bhf_image_geometry_valid) {{ \
             _bhf_image_stride = (size_t)_bhf_image_width * {bytes_per_pixel}u; \
             if ((size_t)_bhf_image_height <= (16u << 20) / _bhf_image_stride) \
                 _bhf_image_capacity = _bhf_image_stride * (size_t)_bhf_image_height; \
             else _bhf_image_geometry_valid = 0; \
         }} \
         size_t _bhf_generic_capacity = Size < (1024 * 1024) ? \
             Size + 65536 : (1024 * 1024 + 65536); \
         size_t _bhf_output_capacity = _bhf_image_geometry_valid ? \
             _bhf_image_capacity : _bhf_generic_capacity; \
         {output_type} {output} = ({output_type})malloc(\
             _bhf_output_capacity ? _bhf_output_capacity : 1)"
    );
    emissions[3].decl = format!(
        "{capacity_type} {capacity} = ({capacity_type})(\
         {output} ? _bhf_output_capacity : 0)"
    );
    emissions[4].decl = format!(
        "{stride_type} {stride} = _bhf_image_geometry_valid ? \
         ({stride_type})_bhf_image_stride : ({stride_type})bhf_i32(&Cur)"
    );
}
...
fn build_c_context(args: &GenerateCDirectArgs) -> Result<CTemplateContext, HarnessGenError> {
    ...
    // canonical contract). The scratch decoder names these objects `_bhf_out_*`.
    ...
    if return_type_present && c_type_is_status_int(&return_type) {
        for (source_param, emission) in decoder_params.iter().zip(params.iter_mut()) {
            if emission.free.is_some() || !emission.arg.starts_with("&_bhf_out_") {
                continue;
            }
...
fn build_c_sequence_context(
    ...
    let registry = TypeRegistry::from_defs(args.type_defs.iter());
    // An in-place sequence stack-allocates `{handle_type} _bhf_handle;` and drives
    // the lifecycle through `&_bhf_handle`, which REQUIRES a complete handle type.
    ...
    let mut init_step = args
        ...
        .map(|step| {
            if args.init_returns_handle {
                ...
                build_c_out_handle_init_emission(step, output_argument, "_bhf_init_result")
            } else {
    ...
    for (index, step) in ordered_op_specs.iter().enumerate() {
        match build_c_sequence_step_emission(
            step,
            &format!("_bhf_step{index}"),
            &format!("_bhf_step{index}_result"),
            &registry,
            args.decoder_limits,
    ...
    if handle_is_pointer_value {
        for step in &mut op_steps {
            step.receiver = "_bhf_handle".to_owned();
        }
    }
        ...
        .enumerate()
    {
        if let Ok(mut emission) = build_c_sequence_step_emission(
            step,
            &format!("_bhf_cfg{index}"),
            &format!("_bhf_cfg{index}_result"),
            &registry,
            args.decoder_limits,
            false,
        ) {
            if handle_is_pointer_value {
                emission.receiver = "_bhf_handle".to_owned();
            }
    ...
    for (index, observed) in args.protocol_steps.iter().enumerate() {
        match build_c_protocol_step_emission(
            observed,
            &format!("_bhf_protocol{index}"),
            &format!("_bhf_protocol{index}_result"),
            &registry,
            args.decoder_limits,
            if handle_is_pointer_value {
                "_bhf_handle"
    ...
    if !mined_protocol_steps.is_empty() && end_step.is_some() {
        for (object_index, object) in args.protocol_objects.iter().enumerate() {
            let built = (|| -> Result<CProtocolObjectEmission, HarnessGenError> {
                ...
                let handle_name = format!("_bhf_protocol_object{object_index}");
                ...
                let mut constructor = build_c_sequence_step_emission(
                    &decoder_constructor,
                    &format!("_bhf_object{object_index}_ctor"),
                    &handle_name,
                    &registry,
                    args.decoder_limits,
                    false,
                ...
                for (step_index, observed) in args.protocol_steps.iter().enumerate() {
                    let emission = build_c_protocol_step_emission(
                        observed,
                        &format!("_bhf_object{object_index}_protocol{step_index}"),
                        &format!("_bhf_object{object_index}_protocol{step_index}_result"),
                        &registry,
                        args.decoder_limits,
                        &handle_name,
                        &object_protocol,
                ...
                .filter_map(|step| {
                    ...
                    step.name, param.name
                        ...
                        action_args.push(format!(
                            "({c_type})((_bhf_callback_choice >> {}) & 1u)",
                            index % 8
                        ));
                    }
...
fn output_drain_pump(
    ...
    let output_value = "_bhf_drain_output".to_owned();
    let output_pointer = "_bhf_drain_output_ptr".to_owned();
...
fn libarchive_expert_stream_pump(
    ...
    Some(CExpertStreamPump {
        ...
        data_step,
        skip_step,
        data_buffer_name: "_bhf_archive_data_buffer".to_owned(),
...
fn set_sequence_callback_action(param: &mut CParamEmission, active: bool) -> bool {
    ...
    if !support.contains("BHF_CALLBACK_OBSERVE") {
        return false;
    }
    if !active {
        *support = support.replace("BHF_CALLBACK_OBSERVE", "BHF_CALLBACK_IGNORE");
    }
    active
}
    ...
    if any {
        ...
        for (index, step) in op_steps.iter_mut().enumerate() {
            step.receiver = format!(
                "((_bhf_derived_live && (_bhf_recv{index} & 1)) ? _bhf_derived : {primary})"
            );
            step.receiver_selector = Some(format!("_bhf_recv{index}"));
        }
    }
    any
}
...
fn build_c_sequence_step_emission(
    ...
    // `&_bhf_handle` element and dropped from `step.params` by `c_lifecycle_function
    ...
    // arbitrary `bhf_i64`, so `op(&_bhf_handle, strlen, ...)` reads `strlen` bytes out
    ...
    // unchanged (`bhf_bounded_length` reads the same `bhf_i64`), so the corpus format
    ...
    if let (Some(first_param), Some(first_emit)) = (step.params.first(), params.first_mut()) {
        ...
        if resolves_length && looks_like_count_name(&first_param.name) {
            ...
            first_emit.decl = format!(
                "{ty} {arg} = ({ty})bhf_bounded_length(&Cur, 0, sizeof _bhf_handle)",
                arg = first_emit.arg
            );
        }
    }
    ...
    // an illegal `__declspec(dllexport)` on the local `_bhf_stepN_result` var.
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-cgen-{name}-{nonce}"));
    ...
    fn variadic_format_param_uses_neutralised_decoder() {
        ...
        // bhf_c_format_string (which strips `%`), NOT bhf_c_string — the harness
        ...
        assert!(
            main.contains("char *message = bhf_c_format_string(&Cur, 4096)"),
    ...
    fn file_loader_path_param_is_driven_via_tempfile() {
        ...
        // string. Decoded as a random bhf_c_string it ENOENTs and the parser never
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("bhf_make_tempfile") || main.contains("_bhf_tmp"),
            "a file-loader path param must be driven via a tempfile, not bhf_c_string: {main}"
        );
        assert!(
            !main.contains("path_ = bhf_c_string"),
    ...
    fn integer_index_out_pointer_with_byte_buffer_is_pinned_zero() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("int _bhf_out_p = (int)0;"),
            "the index out-pointer must be pinned to 0: {main}"
        );
        assert!(
            !main.contains("bhf_i32(&Cur)") || !main.contains("_bhf_out_p = (int)bhf_i32"),
    ...
    fn non_variadic_trailing_string_stays_plain() {
        ...
        // string and must keep the plain bhf_c_string decoder (no over-trigger of
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("char *name = bhf_c_string(&Cur, 4096)"),
            "an ordinary trailing string must stay on bhf_c_string: {main}"
        );
        assert!(
            !main.contains("bhf_c_format_string"),
    ...
    fn char_double_pointer_with_count_allocates_a_string_array() {
        ...
        assert!(
            main.contains("const char **strings = (const char **)calloc(_bhf_n_strings"),
            "{main}"
        );
        assert!(
            main.contains("strings[_bhf_i_strings] = bhf_c_string(&Cur, 256)"),
            "{main}"
        );
        assert!(
            main.contains("int count = (int)_bhf_count_strings"),
            "{main}"
        );
        // The strings and the array are freed.
        assert!(
            main.contains("free((void *)strings[_bhf_i_strings])"),
            "{main}"
        );
    }
    ...
    fn plural_file_paths_use_one_real_null_terminated_tempfile_vector() {
        for (name, c_type, expected_storage) in [
            (
                "filenames",
                "const char **",
                "const char *_bhf_paths_filenames[2]",
            ),
            (
                "wfilenames",
                "const wchar_t **",
                "const wchar_t *_bhf_paths_wfilenames[2]",
            ),
        ] {
            let params = vec![
                CParameter {
            ...
            assert!(
                emissions[0].decl.contains("bhf_make_tempfile(Data, Size")
            ...
            assert!(
                emissions[1].decl.contains("block_size")
                    ...
                    && !emissions[1].decl.contains("_bhf_count")
                    && !emissions[1].decl.contains("_bhf_n_filenames"),
    ...
    fn const_input_buffer_is_nul_terminated_only_with_a_nullterm_mode_enum() {
        ...
        assert!(
            m.contains("char *_bhf_ntbuf_str = (char *)malloc(_bhf_ntlen_str + 1)")
                && m.contains("_bhf_ntbuf_str[_bhf_ntlen_str] = 0")
                && m.contains("len = (size_t)(_bhf_ntbuf_str ? _bhf_ntlen_str : 0)"),
    ...
    fn whole_tu_source_include_renames_its_main() {
        ...
        assert!(
            main.contains("#define main bhf_included_main_")
        ...
        let define_idx = main.find("#define main bhf_included_main_").unwrap();
    ...
    fn sequence_orphan_length_param_is_bounded_to_the_handle_size() {
        ...
        // `&_bhf_handle`, a single element) and excluded from the step's decoded
        ...
        // decode it as an arbitrary `bhf_i64` — and `op(&_bhf_handle, strlen, ...)`
        ...
        assert!(
            strlen_decl.contains("bhf_bounded_length(&Cur, 0, sizeof _bhf_handle)"),
            "orphan length must be bounded to the handle size, got: {strlen_decl}"
        );
        assert!(
            !strlen_decl.contains("bhf_i64(&Cur)"),
            "orphan length must NOT be an unbounded bhf_i64: {strlen_decl}"
        );
    }
    ...
    fn sequence_leading_non_length_param_is_not_bounded() {
        ...
        .expect("emission");
        assert!(
            !emission.params[0].decl.contains("sizeof _bhf_handle"),
    ...
    fn out_param_struct_scratch_zero_inits_nonconst_struct_pointer() {
        ...
        assert!(e.decl.contains("wav_t _bhf_out_pWav;"), "{}", e.decl);
        assert!(
            e.decl
                .contains("memset(&_bhf_out_pWav, 0, sizeof _bhf_out_pWav)"),
            "{}",
            e.decl
        );
        assert_eq!(e.arg, "&_bhf_out_pWav");
    ...
    fn out_param_struct_scratch_uses_visible_inplace_lifecycle() {
        ...
        assert!(
            emission
                .decl
                .contains("msgpack_unpacked_init(&_bhf_out_result)"),
            "{}",
            emission.decl
        );
        assert_eq!(
            emission.free.as_deref(),
            Some("msgpack_unpacked_destroy(&_bhf_out_result)")
        );
    }
    ...
    fn status_constructed_output_uses_delete_only_lifecycle_after_success() {
        ...
        assert!(
            main.contains("if (R == 0) regfree(&_bhf_out_preg)"),
            "{main}"
        );
        let _ = fs::remove_dir_all(out);
    }
    ...
    fn byte_stream_decoder_emits_per_byte_loop() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("for (size_t _bhf_i = 0; _bhf_i < Size; ++_bhf_i)"),
            "byte-stream loop missing:\n{main}"
        );
        assert!(
            main.contains("st24_decode((Data[_bhf_i])"),
    ...
    fn generate_c_direct_harness_emits_main_and_makefile() {
        ...
        assert!(
            main.contains("#if defined(_WIN32)\n#define BHF_PUBLISH_INPUT(data, size) ((void)0)")
                && main.contains("BHF_PUBLISH_INPUT(Data, Size);"),
        ...
        assert!(
            mk.contains("DBHF_AFL"),
            "Makefile should compile AFL build with -DBHF_AFL"
        );
        assert!(
    ...
    fn project_include_dirs_use_iquote_so_system_headers_win() {
        ...
        assert!(
            mk.contains("-I /rt"),
            "the bhf c_runtime include must stay -I: {mk}"
        );
    }
    ...
    fn existing_libfuzzer_entrypoint_is_not_wrapped_recursively() {
        ...
        // harness provides a lightweight bhf driver `main` with a per-spawn and
        // a persistent framed mode, plus a SanitizerCoverage edge-coverage runtime.
        assert!(
            main.contains("int main(int argc, char **argv)"),
            "passthrough harness must provide a bhf driver main: {main}"
        );
        assert!(
            main.contains("BHF_FRAMED") && main.contains("__sanitizer_cov_trace_pc_guard"),
    ...
    fn buffer_length_pair_emits_coherent_data_size_decoder() {
        ...
        assert!(
            !main.contains("bhf_c_string(&Cur, 4096)"),
            "pair-detected buffer should NOT use bhf_c_string",
        );
        assert!(
            !main.contains("bhf_bounded_length(&Cur"),
    ...
    fn begin_end_pointer_pair_brackets_one_span() {
        ...
        assert!(
            main.contains("key = bhf_c_string(&Cur"),
    ...
    fn cstring_with_int_out_param_nul_terminates_and_does_not_mispair() {
        ...
        assert!(
            main.contains("bhf_c_string(&Cur"),
        ...
        // (the mis-pair emitted `int _bhf_in_error = (int)Size`).
        assert!(
            !main.contains("_bhf_in_error = (int)Size"),
    ...
    fn raw_buffer_with_non_count_int_does_not_mispair() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("char *file = bhf_c_string(&Cur"),
            "file must be a NUL-terminated string, not the raw Data span:\n{main}"
        );
        assert!(
            main.contains("int line = bhf_i32(&Cur)"),
    ...
    fn east_const_buffer_length_pair_is_coalesced_not_independently_fuzzed() {
        ...
        // this, `size` was an INDEPENDENT bhf_bounded_length and the callee read far
    ...
    fn const_input_buffer_with_length_pointer_binds_length_to_size() {
        ...
        assert!(
            main.contains("(mz_ulong)Size") && main.contains("*pSource_len = &_bhf_in_pSource_len"),
    ...
    fn streaming_byte_cursors_are_coupled_to_their_in_out_counts() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("_bhf_stream_available_in = (size_t)Size"),
            "{main}"
        );
        assert!(
            main.contains("const uint8_t * _bhf_stream_next_in = (const uint8_t *)Data"),
            "{main}"
        );
        assert!(
            main.contains(
                "_bhf_stream_available_out = (size_t)(_bhf_buf_next_out ? _bhf_cap_next_out : 0)"
            ),
            "{main}"
        );
        assert!(
            main.contains("uint8_t * _bhf_buf_next_out = (uint8_t *)malloc"),
            "{main}"
        );
        assert!(
            main.contains("uint8_t * * next_out = &_bhf_stream_next_out"),
            "{main}"
        );
        assert!(main.contains("free(_bhf_buf_next_out)"), "{main}");
    }
    ...
    fn typed_output_array_with_typedef_count_is_sized_not_independently_fuzzed() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("calloc(_bhf_n_out"),
            "output array must be sized to the count (calloc'd), got:\n{main}",
        );
        assert!(
            main.contains("element_size = (cgltf_size)(out ? _bhf_n_out : 0)"),
            "count must equal the array's element count, got:\n{main}",
        );
        assert!(
            !main.contains("bhf_bounded_length(&Cur, 0, 0x7fffffff)"),
    ...
    fn double_pointer_output_handle_in_parser_gets_null_scratch() {
        ...
        assert!(
            main.contains("result * _bhf_out_out = 0") && main.contains(", &_bhf_out_out)"),
    ...
    fn parser_config_struct_is_zeroed_not_fuzzed() {
        ...
        let main = fs::read_to_string(&result_h.main_c).unwrap();
        assert!(
            main.contains("cgltf_options _bhf_cfg_o; memset(&_bhf_cfg_o, 0, sizeof _bhf_cfg_o)"),
            "config struct must be a zeroed library default, got:\n{main}",
        );
        assert!(
            !main.contains("bhf_bounded_length"),
    ...
    fn out_handle_with_known_deallocator_is_freed_to_avoid_leak_on_valid_input() {
        ...
        assert!(
            main.contains("if (_bhf_out_out) result_free(_bhf_out_out)"),
    ...
    fn typed_array_count_pair_sizes_array_to_count_not_a_fabricated_singleton() {
        ...
        assert!(
            main.contains("(size_t)(tokens ? _bhf_n_tokens : 0)"),
    ...
    fn reverse_count_typed_array_pair_preserves_order_and_shared_extent() {
        ...
        assert!(emissions[0].decl.contains("_bhf_n_pmatch"));
    ...
    fn in_memory_parser_uses_null_for_optional_url_and_encoding_metadata() {
        ...
        assert!(standalone[0].decl.contains("bhf_c_string"));
        assert!(standalone[1].decl.contains("bhf_c_string"));
    }
    ...
    fn webp_interleaved_decode_derives_bounded_output_geometry() {
        ...
        assert!(emissions[2]
            .decl
            .contains("WebPGetInfo(data, data_size, &_bhf_image_width, &_bhf_image_height)"));
        assert!(emissions[2]
            .decl
            .contains("_bhf_image_stride = (size_t)_bhf_image_width * 4u"));
        assert!(emissions[2].decl.contains("(16u << 20)"));
        assert!(emissions[2].decl.contains("_bhf_generic_capacity"));
        assert!(emissions[3].decl.contains("_bhf_output_capacity"));
        assert!(emissions[4].decl.contains("_bhf_image_geometry_valid ?"));
        assert!(emissions[4].decl.contains("bhf_i32(&Cur)"));
        ...
        assert!(planar[4].decl.contains("bhf_i32"));
    }
    ...
    fn typed_pointer_with_non_count_neighbor_is_not_mis_paired_as_array() {
        ...
        // typed-array pairing must NOT fire (no `_bhf_n_out`); the params fall to the
        // standalone decoders.
    ...
    fn non_const_output_buffer_pair_allocates_instead_of_aliasing_const_data() {
        ...
        assert!(
            main.contains("memcpy(buffer, Data, _bhf_cap_buffer)"),
            "allocated buffer should be seeded with the fuzz bytes:\n{main}",
        );
        assert!(
            main.contains("buffer_size = (size_t)(buffer ? _bhf_cap_buffer : 0)"),
    ...
    fn buffer_with_non_length_neighbor_falls_back_to_individual_decoders() {
        ...
        assert!(
            main.contains("bhf_c_string(&Cur, 4096)"),
            "no length neighbor: buffer should use bhf_c_string",
        );
        assert!(
    ...
    fn zlib_style_output_and_input_buffers_are_supported() {
        ...
        assert!(
            main.contains("uLongf _bhf_out_destLen = (uLongf)(dest ? _bhf_cap_dest : 0)"),
            "output length pointer should be initialized to buffer capacity, got:\n{main}",
        );
        assert!(
            main.contains("uLongf *destLen = &_bhf_out_destLen"),
    ...
    fn reverse_size_buffer_pairs_match_brotli_one_shot_contract() {
        ...
        assert!(
            main.contains("size_t _bhf_cap_decoded_buffer = 1024 * 1024"),
            "one-shot decompression needs a bounded expansion budget:\n{main}"
        );
        assert!(
            main.contains(
                "size_t _bhf_out_decoded_size = (size_t)(decoded_buffer ? _bhf_cap_decoded_buffer : 0)"
            ),
    ...
    fn scalar_output_capacity_does_not_consume_input_buffer_pair() {
        ...
        assert!(
            main.contains("size_t out_buf_len = (size_t)(pOut_buf ? _bhf_cap_pOut_buf : 0)"),
        ...
        assert!(
            !main.contains("size_t src_buf_len = bhf_bounded_length"),
    ...
    fn unnamed_read_buffer_capacity_stays_coupled_to_its_allocation() {
        ...
        assert!(buffer_emission.decl.contains("size_t _bhf_cap_arg1"));
        assert!(buffer_emission.decl.contains("malloc(_bhf_cap_arg1"));
        assert_eq!(
            length_emission.decl,
            "size_t arg2 = (size_t)(arg1 ? _bhf_cap_arg1 : 0)"
        );
    }
    ...
    fn generate_c_direct_harness_drives_file_pointer_with_fmemopen() {
        ...
        assert!(main.contains("fmemopen(_bhf_file_buf_stream, Size, \"r+\")"));
        assert!(main.contains("if (stream) fclose(stream); free(_bhf_file_buf_stream);"));
    ...
    fn generate_c_direct_harness_drives_miniz_file_macro_pointer() {
        ...
        assert!(
            main.contains("MZ_FILE * stream = (MZ_FILE *)(_bhf_file_buf_stream ? (Size ? fmemopen")
        );
    ...
    fn strip_redundant_callback_typedef_drops_leading_typedef_keeps_trampoline() {
        let param = CParamEmission {
            support: Some(
                "typedef int (*cb_t)(void *, int);\n\
                 static int _bhf_cb_trampoline(void *u, int n) { (void)u; (void)n; return 0; }"
                    .to_owned(),
            ),
            decl: "cb_t cb = (cb_t)_bhf_cb_trampoline".to_owned(),
            arg: "cb".to_owned(),
        ...
        assert!(
            support.contains("static int _bhf_cb_trampoline(void *u, int n)"),
    ...
    fn strip_redundant_callback_typedef_keeps_synthesized_inline_typedef() {
        ...
        // the decl type `_bhf_cb_<name>` is the synthesized `typedef …;` line. The
        // included header CANNOT supply it, so stripping the typedef would leave the
        // decl referencing an undeclared `_bhf_cb_*` type (md4c md_html). It must be
        // KEPT even when the target's header is included.
        let param = CParamEmission {
            support: Some(
                "typedef void (*_bhf_cb_process_output)(const char*, int, void*);\n\
                 static void _bhf_process_output_trampoline(const char* a, int b, void* c) \
                 { (void)a; (void)b; (void)c; }"
                    .to_owned(),
            ),
            decl: "_bhf_cb_process_output process_output = \
                   (_bhf_cb_process_output)_bhf_process_output_trampoline"
                .to_owned(),
            arg: "process_output".to_owned(),
            c_type: "_bhf_cb_process_output".to_owned(),
            free: None,
        };
        let support = strip_redundant_callback_typedef(param).support.unwrap();
        assert!(
            support.contains("typedef void (*_bhf_cb_process_output)"),
            "synthesized inline typedef must be kept: {support}"
        );
        assert!(
            support.contains("static void _bhf_process_output_trampoline("),
    ...
    fn generate_c_direct_harness_emits_callback_trampoline() {
        ...
        let result = generate_c_direct_harness(args).unwrap();
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(main.contains("static int _bhf_cb_trampoline(void *opaque, const char *name)"));
        assert!(main.contains("visit_cb cb = (visit_cb)_bhf_cb_trampoline;"));
    ...
    fn generate_c_direct_harness_emits_inline_callback_typedef_when_header_included() {
        ...
        // `_bhf_cb_process_output` typedef AND the trampoline definition — even
        ...
        // `_bhf_cb_process_output` undeclared (failed_build).
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("typedef void (*_bhf_cb_process_output)"),
            "synthesized inline callback typedef must be emitted:\n{main}"
        );
        assert!(
            main.contains("_bhf_cb_process_output process_output = (_bhf_cb_process_output)"),
    ...
    fn generate_c_direct_harness_drives_variadic_callback_param() {
        ...
        assert_callback_harness_compiles(
            ...
            &[
                "_bhf_fn_trampoline(int level, ...)",
                "(log_fn)_bhf_fn_trampoline",
            ],
        );
    }
    ...
    fn generate_c_direct_harness_emits_trampoline_for_pointer_returning_funcptr_param() {
        ...
        assert!(
            main.contains("typedef void * (*_bhf_cb_alloc_func_ptr)(void *, unsigned)"),
            "synthesized funcptr typedef must be emitted:\n{main}"
        );
        assert!(
            main.contains("static void * _bhf_alloc_func_ptr_trampoline("),
            "no-op trampoline returning a pointer must be emitted:\n{main}"
        );
        assert!(
            main.contains(
                "_bhf_cb_alloc_func_ptr alloc_func_ptr = (_bhf_cb_alloc_func_ptr)\
                 _bhf_alloc_func_ptr_trampoline"
            ),
    ...
    fn generate_c_direct_harness_constructs_handle_via_returning_constructor() {
        ...
        assert!(!main.contains("_bhf_lc_g"), "no stack storage: {main}");
        ...
        // runtime's own `&bhf_*` symbols (e.g. `&bhf_cmpp_open`).
    ...
    fn generate_c_direct_harness_compiles_struct_enum_array_decoder() {
        ...
        assert!(main.contains("cfg.count = bhf_i32(&Cur)"));
        assert!(main.contains("cfg.name = bhf_c_string(&Cur, 256)"));
        assert!(main.contains("cfg.mode = (enum mode)MODE_A"));
        assert!(main.contains("cfg.tag[_bhf_i_cfg_tag] = (char)bhf_u8(&Cur)"));
        assert!(main.contains("struct config _bhf_value_out"));
        assert!(main.contains("struct config * out = &_bhf_value_out"));
        assert!(main.contains("free((void *)cfg.name)"));
        assert!(main.contains("free((void *)_bhf_value_out.name)"));
    ...
    fn mined_protocol_keeps_stream_then_finalize_calls_and_safe_literals() {
        ...
        assert!(
            main.contains("_bhf_protocol0_result = XML_Parse(_bhf_handle"),
            "{main}"
        );
        assert!(
            main.contains("_bhf_protocol1_result = XML_Parse(_bhf_handle"),
            "{main}"
        );
        assert!(main.contains(", 0);"), "{main}");
        assert!(main.contains(", 1);"), "{main}");
        assert!(
            main.contains("XML_GetCurrentLineNumber(_bhf_handle)"),
            "{main}"
        );
        assert!(
            main.contains("XML_ErrorString(_bhf_protocol2_result)"),
            "{main}"
        );
        assert!(
            main.contains("if (_bhf_protocol1_result == XML_STATUS_ERROR)"),
            "{main}"
        );
        assert!(
            main.contains("XML_ParserReset(_bhf_handle, NULL)"),
            "{main}"
        );
        assert!(
        ...
        assert!(
            main.contains("XML_Parser _bhf_protocol_object0 = XML_ParserCreateNS(NULL, '!')"),
            "{main}"
        );
        assert!(
            main.contains("XML_Parse(_bhf_protocol_object0"),
            "the mined namespace object must run the same parse protocol: {main}"
        );
        assert!(
            main.contains("_bhf_protocol_lane == 1")
                && main.contains("bhf_cursor Cur = bhf_open_data(Data, Size, 42)"),
        ...
        assert!(
            main.contains("XML_ParserFree(_bhf_protocol_object0)"),
            "the additional object must be released: {main}"
        );
        assert!(
            main.contains("switch ((int)bhf_ctrl_op"),
    ...
    fn sequence_harness_guards_oploop_and_teardown_on_init_status() {
        ...
        let run_one_decl = main
            .find("int bhf_run_one(const uint8_t *Data, size_t Size);")
        ...
        let guard = main
            .find("if (_bhf_init_result == 0) {")
            .expect("op-loop must be guarded on constructor success");
        let loop_pos = main.find("_bhf_lifecycle_count").unwrap();
        let close_pos = main.find("mtar_close(&_bhf_handle)").unwrap();
        assert!(
            guard < loop_pos,
        ...
        assert!(
            !main.contains("if (_bhf_step0_result != 0) {"),
    ...
    fn configuration_runs_between_construction_and_use() {
        ...
        let initialized = main
            .find("_bhf_handle.fmt = 7;")
            .expect("maintained public-field precondition must be emitted");
        assert!(initialized < main.find("arc_new(&_bhf_handle)").unwrap());
        assert!(!main.contains("_bhf_handle.not_declared"));

        // Configuration is unconditional, and sits after construction but before
        // the op loop.
        let configured = main
            .find("arc_support_format_all(&_bhf_handle)")
            .expect("configuration must be emitted");
        let constructed = main.find("arc_new(&_bhf_handle)").expect("constructor");
        let loop_start = main.find("_bhf_lifecycle_count").expect("op loop");
        assert!(
    ...
    fn libarchive_memory_reader_uses_the_expert_nested_drain_protocol() {
        ...
        let opened = main.find("archive_read_open_memory(_bhf_handle").unwrap();
        let header = main.find("archive_read_next_header(_bhf_handle").unwrap();
        let data = main.find("archive_read_data(_bhf_handle").unwrap();
        let freed = main.rfind("archive_read_free(_bhf_handle").unwrap();
        ...
        assert!(main.contains("_bhf_entry_index < 256"), "{main}");
        assert!(main.contains("_bhf_data_index < 4096"), "{main}");
        assert!(main.contains("== ARCHIVE_RETRY"), "{main}");
        assert!(main.contains("== ARCHIVE_FATAL"), "{main}");
        assert!(
            main.contains("_bhf_stream_lane")
                && main.contains("archive_read_data_skip(_bhf_handle)"),
        ...
        assert!(
            main.contains("const void * _bhf_step0_arg1 = (const void *)Data")
                && main.contains("size_t _bhf_step0_arg2 = (size_t)Size"),
            "the unnamed memory-image pointer and length must describe the same fuzz span:\n{main}"
        );
        assert!(
            !main.contains("_bhf_lifecycle_count"),
            "the proven reader protocol must replace random call ordering:\n{main}"
        );
        assert!(
            main.contains("unsigned char _bhf_archive_data_buffer[4096]")
                && main.contains(
                    "archive_read_data(_bhf_handle, _bhf_archive_data_buffer, sizeof(_bhf_archive_data_buffer))"
                )
                && !main.contains("malloc(_bhf_cap__bhf_step2_arg1"),
        ...
        assert!(wide.contains("archive_read_support_filter_all(_bhf_handle)"));
        assert!(wide.contains("archive_read_support_format_all(_bhf_handle)"));
        assert!(wide.contains("const wchar_t *_bhf_paths__bhf_step0_wfilenames[2]"));
        assert!(wide.contains("archive_read_open_filenames_w(_bhf_handle"));
        assert!(wide.contains("archive_read_next_header(_bhf_handle"));
        assert!(wide.contains("archive_read_data(_bhf_handle"));
        assert!(
            !wide.contains("_bhf_lifecycle_count"),
    ...
    fn an_op_returning_a_child_handle_lets_later_ops_drive_the_child() {
        ...
        assert!(
            main.contains("parser * _bhf_derived = 0;")
                && main.contains("int _bhf_derived_live = 0;"),
        ...
        assert!(
            main.contains("_bhf_derived = _bhf_step1_result;")
                && main.contains("_bhf_derived_live = 1;"),
            "the child must be captured:\n{main}"
        );
        assert!(
            main.contains("if (_bhf_step1_result) {"),
            "a NULL child must not be captured:\n{main}"
        );
        assert!(
            !main.contains("if (_bhf_handle_live) { break; }"),
        ...
        assert!(
            main.contains("? _bhf_derived : &_bhf_handle"),
            "ops must be able to run on the child:\n{main}"
        );
        assert!(
            main.contains("parser_feed(((_bhf_derived_live"),
    ...
    fn an_ops_return_value_can_become_a_later_ops_argument() {
        ...
        assert!(
            main.contains("pool_id _bhf_slot_pool_id;"),
            "slot declared:\n{main}"
        );
        assert!(
            main.contains("_bhf_slot_pool_id = _bhf_step0_result;")
                && main.contains("_bhf_slot_pool_id_live = 1;"),
            "the producer must store its result:\n{main}"
        );
        // The consumer can take the slot instead of a freshly decoded value...
        assert!(
            main.contains("_bhf_slot_pool_id_live && ") && main.contains("? _bhf_slot_pool_id :"),
    ...
    fn open_and_close_ops_cycle_the_handle_without_use_after_close() {
        ...
        assert!(
            main.contains("arc_open_memory(&_bhf_handle"),
        ...
        assert!(main.contains("_bhf_handle_live = 0;"), "{main}");
        assert!(
            main.contains("_bhf_handle_live = (_bhf_step1_result == 0);"),
        ...
        let free_pos = main
            .find("arc_free(&_bhf_handle")
            .expect("teardown emitted");
        let guard_pos = main[..free_pos]
            .rfind("if (_bhf_handle_live) {")
    ...
    fn generate_c_sequence_harness_emits_init_loop_and_end() {
        ...
        let result = generate_c_sequence_harness(args).unwrap();
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(main.contains("struct session _bhf_handle"));
        assert!(main.contains("memset(&_bhf_handle, 0, sizeof _bhf_handle)"));
        assert!(main.contains("int _bhf_init_result = session_init(&_bhf_handle, _bhf_init_seed)"));
        ...
        assert!(main.contains("size_t _bhf_lifecycle_count = bhf_ctrl_step_count(Data, Size, 8)"));
        assert!(main.contains("if (Size < 41) { _bhf_lifecycle_count = 1; }"));
        assert!(main.contains("switch ((int)bhf_ctrl_op(Data, Size, _bhf_lifecycle_index, 8, 2))"));
        assert!(
            main.contains("bhf_cursor Cur = bhf_open_data(Data, Size, 41)"),
            "the argument cursor must be clamped away from the control region:\n{main}"
        );
        assert!(main.contains("session_step(&_bhf_handle"));
        assert!(main.contains("session_reset(&_bhf_handle)"));
        assert!(main.contains("session_end(&_bhf_handle)"));
    ...
    fn returning_opaque_handle_sequence_constructs_drives_and_destroys_values() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("parser_t _bhf_handle = parser_create(NULL);"),
            "{main}"
        );
        assert!(main.contains("if (_bhf_handle) {"), "{main}");
        assert!(!main.contains("memset(&_bhf_handle"), "{main}");
        assert!(main.contains("parser_parse(((_bhf_derived_live"), "{main}");
        assert!(main.contains("parser_free(_bhf_derived)"), "{main}");
        assert!(main.contains("parser_free(_bhf_handle)"), "{main}");
        assert!(!main.contains("parser_free(&_bhf_handle)"), "{main}");
    ...
    fn out_parameter_handle_sequence_opens_drives_and_closes_pointer_value() {
        ...
        let result = generate_c_sequence_harness(args).unwrap();
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(main.contains("db * _bhf_handle = NULL;"), "{main}");
        assert!(
            main.contains("db_open(\":memory:\", &_bhf_handle)"),
            "{main}"
        );
        assert!(main.contains("db_exec(_bhf_handle"), "{main}");
        assert!(main.contains("db_close(_bhf_handle)"), "{main}");
        assert!(!main.contains("db_close(&_bhf_handle)"), "{main}");
    ...
    fn jpeg_header_reader_uses_public_error_and_scanline_protocol() {
        ...
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(
            main.contains("_bhf_error.pub.error_exit = _bhf_jpeg_error_exit"),
            "{main}"
        );
        assert!(
            main.contains("jpeg_create_decompress(&_bhf_cinfo)"),
            "{main}"
        );
        assert!(main.contains("jpeg_mem_src(&_bhf_cinfo, Data"), "{main}");
        assert!(
            main.contains("jpeg_read_header(&_bhf_cinfo, TRUE)"),
            "{main}"
        );
        assert!(main.contains("jpeg_read_scanlines(&_bhf_cinfo"), "{main}");
        assert!(
            main.contains("jpeg_destroy_decompress(&_bhf_cinfo)"),
            "{main}"
        );
    ...
    fn sqlite_prepare_uses_nul_terminated_multi_statement_vm_protocol() {
        ...
        let result = generate_c_sequence_harness(args).unwrap();
        let main = fs::read_to_string(&result.main_c).unwrap();
        assert!(main.contains("_bhf_sql[Size] = '\\0'"), "{main}");
        assert!(
            main.contains("sqlite3_open(\":memory:\", &_bhf_db)"),
            "{main}"
        );
        assert!(main.contains("sqlite3_prepare_v2("), "{main}");
        assert!(main.contains("sqlite3_step(_bhf_stmt)"), "{main}");
        assert!(main.contains("sqlite3_finalize(_bhf_stmt)"), "{main}");
        assert!(main.contains("sqlite3_close(_bhf_db)"), "{main}");
        assert!(!main.contains("bhf_open_data"), "{main}");
    ...
    fn generate_c_sequence_harness_skips_incomplete_handle() {
        ...
        // the harness does not include. Stack-allocating `struct hashmap _bhf_handle;`
    ...
    fn generate_c_sequence_harness_skips_unsupported_secondary_ops() {
        ...
        assert!(main.contains("switch ((int)bhf_ctrl_op(Data, Size, _bhf_lifecycle_index, 8, 1))"));
    }
}
