...
//! `rust_parser`) to a bhf-native decode: a `rust_runtime::Cursor` call plus
...
impl std::fmt::Display for RustDecodeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Rust parameter type '{}' has no bhf-native byte decoder",
            self.ty
        )
    }
}
...
pub fn select_rust_decoder(ty: &str) -> Result<RustParamEmission, RustDecodeError> {
    ...
    if compact == "&mut[u8]" {
        ...
        return Ok(RustParamEmission {
            bounded_expr: format!(
                "{{ let mut _bhf_buf = c.bytes({RUST_OUTPUT_BUF_LEN}); \
                 _bhf_buf.resize({RUST_OUTPUT_BUF_LEN}, 0u8); _bhf_buf }}"
            ),
            rest_expr: None,
        ...
        .and_then(|s| s.strip_suffix(']'))
    {
        if let Some(zero) = scalar_zero_literal(elem) {
            return Ok(RustParamEmission {
                bounded_expr: format!(
                    "{{ let mut _bhf_buf = vec![{zero}; {RUST_OUTPUT_BUF_LEN}]; _bhf_buf }}"
                ),
                rest_expr: None,
                by_ref: ArgPass::RefMut,
                rest_eligible: false,
...
fn decode_owned_expr(c: &str, depth: u8) -> Option<String> {
    ...
    if let Some(inner) = generic_inner(c, "Vec") {
        ...
        return Some(format!(
            "{{ let _bhf_n = c.bounded_len(256); \
             let mut _bhf_v = Vec::with_capacity(_bhf_n); \
             for _ in 0.._bhf_n {{ _bhf_v.push({e}); }} _bhf_v }}"
        ));
    }
...
mod tests {
    ...
    fn vec_of_decodable_element_loops_a_bounded_count() {
        ...
        assert!(e.bounded_expr.contains("_bhf_v.push(c.u32())"));
