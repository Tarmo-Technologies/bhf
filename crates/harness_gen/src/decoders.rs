...
/// which `Bhf_Callbacks` declares a subtype-conformant subprogram.
fn callback_constructor_neutral(type_name: &str) -> Option<&'static str> {
    ...
    if lower.contains("item : out string") && lower.contains("last : out natural") {
        return Some("Bhf_Callbacks.Src_String'Access");
    }
    if lower.contains("item : in string") && !lower.contains(';') {
        return Some("Bhf_Callbacks.Snk_String'Access");
    }
    None
}
...
mod tests {
    ...
    fn private_param_uses_public_constant_as_neutral() {
        ...
        // Ada is case-insensitive; bhf title-cases the emitted identifier.
        assert!(
    ...
    fn constructor_is_usable_rejects_non_synthesizable_required_params() {
        ...
        assert_eq!(
            ...
            "YAML.Create(Bhf_Callbacks.Src_String'Access)"
        );
