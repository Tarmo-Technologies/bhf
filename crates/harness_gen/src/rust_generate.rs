...
//! Generate a bhf-native Rust fuzzing harness: a `staticlib` source file
//! exposing
//!
//! ```ignore
//! #[no_mangle]
...
//! fork-server driver (`c_runtime/bhf_driver.c`, copied beside the harness as
...
/// Build the body of `bhf_run_one` for a direct call. Decodes each parameter
...
fn build_call_body(
    ...
    body.push_str("    unsafe { bhf_target_enter(); }\n");
    body.push_str(&call);
    body.push('\n');
    Ok(body)
}
...
/// Wrap a `bhf_run_one` body in the staticlib crate root. The `#[no_mangle]
/// extern "C"` ABI matches the C driver's `extern int bhf_run_one(...)`; a
...
fn render_harness_rs(body: &str) -> String {
    format!(
        "// SPDX-License-Identifier: Apache-2.0\n\
         //\n\
         // GENERATED by bhf harness_gen::rust_generate. Do not edit.\n\
         //\n\
         // A bhf-native Rust fuzzing harness: decodes raw fuzz bytes into\n\
         ...
         \n\
         unsafe extern \"C\" {{ fn bhf_target_enter(); }}\n\
         \n\
...
mod tests {
    ...
    fn byte_slice_param_consumes_rest_and_is_borrowed() {
        ...
        assert!(h
            .harness_rs
            .contains("unsafe extern \"C\" { fn bhf_target_enter(); }"));
        assert!(h
            .harness_rs
            .contains("unsafe { bhf_target_enter(); }\n    let _ = mycrate::parse(&a0);"));
    ...
    fn ufcs_write_method_backs_output_buffer_not_empty_slice() {
        ...
        assert!(
            h.harness_rs
                .contains("_bhf_buf.resize(64, 0u8); _bhf_buf };"),
