...
pub const INSTANCE_NAME: &str = "Bhf_Generic_Instance";
...
/// `with package`, or `is <>` formal subprogram has no value bhf can invent,
...
fn classify_formal(formal: &str) -> Option<Formal> {
    ...
    if lower.starts_with("type ") {
        ...
        let decl = if def_low.starts_with("(<>)") {
            Some(format!("type Stub_{name} is (Bhf_E0, Bhf_E1, Bhf_E2);"))
...
";
    ...
    fn synthesizes_bzip2_decoding_codec() {
        ...
        assert_eq!(
            inst.instantiation,
            "package Bhf_Generic_Instance is new BZip2.Decoding (Read_Byte => Stub_Read_Byte, Write_Byte => Stub_Write_Byte, check_crc => False);"
        );
...
";
        ...
        assert_eq!(
            inst.instantiation,
            "procedure Bhf_Generic_Instance is new LZMA.Encoding.Encode (Read_Byte => Stub_Read_Byte, More_Bytes => Stub_More_Bytes, Write_Byte => Stub_Write_Byte);"
        );
    }
...
";
        ...
        assert!(
            inst.stub_decls
                .iter()
                .any(|d| d == "type Stub_Categories is (Bhf_E0, Bhf_E1, Bhf_E2);"),
            "stub_decls: {:?}",
            inst.stub_decls
        );
        assert!(
