...
/// emitted decl (`BOOL flag = (BOOL)bhf_i32(&Cur)`), which is what compiles once
...
/// unaffected. The C path of `bhf auto` uses this; the permissive
...
fn select_c_decoder_inner(
    ...
    // emits a broken `bhf_i32` harness that fails to BUILD; skip the target cleanly
    ...
    // read the `s[-1]` header out of bounds (a guaranteed BHF-201 FP). This runs
...
fn bzip2_read_handle(c_type: &str, name: &str) -> Option<CParamEmission> {
    ...
    let file = format!("_bhf_bz_file_{id}");
    let error = format!("_bhf_bz_error_{id}");
    Some(CParamEmission {
        support: None,
        decl: format!(
...
fn build_callback_trampoline(
    ...
    // project typedef to name, so a `_bhf_cb_<name>` is synthesized below. A
    ...
    let trampoline = format!("_bhf_{}_trampoline", sanitize_ident(name));
    ...
    for (index, param) in parsed.params.iter().enumerate() {
        ...
        // ...)`) cannot be named or referenced — `... _bhf_arg2` is a syntax error
        // ("expected ')'") and `(void)_bhf_arg2;` then fails. Emit the ellipsis
        ...
        match callback_param_name(param) {
            ...
            None => {
                let synthesized = format!("_bhf_arg{index}");
    ...
    if let Some(context) = callback_context.as_deref() {
        body.push_str(&format!("    BHF_CALLBACK_OBSERVE({context});\n"));
    }
    ...
    // inline funcptr has no project typedef, so a fresh `_bhf_cb_<name>` is
    // synthesized; the parameter path recomputes the same name for the decl type.
    let typedef_name = if inline {
        format!("_bhf_cb_{}", sanitize_ident(name))
    } else {
        c_type
            .split_whitespace()
    ...
    let callback_observer_fallback = if callback_context.is_some() {
        "#ifndef BHF_CALLBACK_OBSERVE\n#define BHF_CALLBACK_OBSERVE(context) ((void)(context))\n#endif\n"
    } else {
        ""
    };
    let support = format!(
...
fn callback_trampoline(
    ...
    Ok(CParamEmission {
        ...
        // For an inline funcptr the cast is to the synthesized `_bhf_cb_<name>` type
...
/// suffixes after the name ("int [4]" -> "int _bhf_arg0[4]").
...
fn legacy_select_c_decoder(c_type: &str, name: &str) -> Option<CParamEmission> {
    ...
    // full-range `bhf_i32` was valid ~0% of the time and coverage stalled at the
    ...
    if is_channel_count_name(name) {
        if let Some(ty) = small_int_decl_type(normalized.as_str()) {
            return Some(scalar(
                name,
                ty,
                &format!("({ty})bhf_bounded_i32(&Cur, 0, 8)"),
            ));
        }
    }
    match normalized.as_str() {
        "_Bool" => Some(scalar(name, "_Bool", "(_Bool)(bhf_u8(&Cur) & 1)")),
        "bool" => Some(scalar(name, "bool", "(bool)(bhf_u8(&Cur) & 1)")),
        ...
        // emission (bhf_i32/bhf_i64 reinterpreted to the float type).
        "float" => Some(scalar(name, "float", "(float)bhf_i32(&Cur)")),
        "double" => Some(scalar(name, "double", "(double)bhf_i64(&Cur)")),
        "long double" => Some(scalar(name, "long double", "(long double)bhf_i64(&Cur)")),
        "short" | "signed short" | "short int" => {
            Some(scalar(name, "short", "(short)bhf_i32(&Cur)"))
        }
        "unsigned short" | "unsigned short int" => Some(scalar(
            name,
            "unsigned short",
            "(unsigned short)bhf_bounded_i32(&Cur, 0, 0xffff)",
        )),
        "int" | "signed int" => Some(scalar(name, "int", "bhf_i32(&Cur)")),
        "unsigned" | "unsigned int" => Some(scalar(
            name,
            "unsigned int",
            "(unsigned int)bhf_bounded_i32(&Cur, 0, 0x7fffffff)",
        )),
        "long" | "signed long" => Some(scalar(name, "long", "(long)bhf_i64(&Cur)")),
        "unsigned long" => Some(scalar(
            name,
            "unsigned long",
            "(unsigned long)bhf_bounded_length(&Cur, 0, 0x7fffffff)",
        )),
        "uLong" | "uLongf" => Some(scalar(
            name,
            normalized.as_str(),
            "(uLong)bhf_bounded_length(&Cur, 0, 0x7fffffff)",
        )),
        "uInt" => Some(scalar(
            name,
            "uInt",
            "(uInt)bhf_bounded_i32(&Cur, 0, 0x7fffffff)",
        )),
        "z_size_t" => Some(scalar(
            name,
            "z_size_t",
            "(z_size_t)bhf_bounded_length(&Cur, 0, 65536)",
        )),
        "mz_ulong" => Some(scalar(
            name,
            "mz_ulong",
            "(mz_ulong)bhf_bounded_length(&Cur, 0, 0x7fffffff)",
        )),
        "long long" | "signed long long" => Some(scalar(name, "long long", "bhf_i64(&Cur)")),
        "size_t" => Some(scalar(name, "size_t", "bhf_bounded_length(&Cur, 0, 65536)")),
        "uint8_t" | "unsigned char" => Some(scalar(name, "uint8_t", "bhf_u8(&Cur)")),
        "int8_t" | "signed char" | "char" => Some(scalar(name, "int8_t", "(int8_t)bhf_u8(&Cur)")),
        "uint16_t" => Some(scalar(
            name,
            "uint16_t",
            "(uint16_t)bhf_bounded_i32(&Cur, 0, 0xffff)",
        )),
        "int16_t" => Some(scalar(name, "int16_t", "(int16_t)bhf_i32(&Cur)")),
        "int32_t" => Some(scalar(name, "int32_t", "bhf_i32(&Cur)")),
        "uint32_t" => Some(scalar(
            name,
            "uint32_t",
            "(uint32_t)bhf_bounded_i32(&Cur, 0, 0x7fffffff)",
        )),
        "int64_t" => Some(scalar(name, "int64_t", "bhf_i64(&Cur)")),
        "uint64_t" => Some(scalar(name, "uint64_t", "(uint64_t)bhf_i64(&Cur)")),
        ...
        "__int64" | "signed __int64" => Some(scalar(name, "__int64", "bhf_i64(&Cur)")),
        "unsigned __int64" => Some(scalar(
            name,
            "unsigned __int64",
            "(unsigned __int64)bhf_i64(&Cur)",
        )),
        "__int32" | "signed __int32" => Some(scalar(name, "__int32", "bhf_i32(&Cur)")),
        "unsigned __int32" => Some(scalar(
            name,
            "unsigned __int32",
            "(unsigned __int32)bhf_bounded_i32(&Cur, 0, 0x7fffffff)",
        )),
        "__int16" | "signed __int16" => Some(scalar(name, "__int16", "(__int16)bhf_i32(&Cur)")),
        "unsigned __int16" => Some(scalar(
            name,
            "unsigned __int16",
            "(unsigned __int16)bhf_bounded_i32(&Cur, 0, 0xffff)",
        )),
        "__int8" | "signed __int8" => Some(scalar(name, "__int8", "(__int8)bhf_u8(&Cur)")),
        "unsigned __int8" => Some(scalar(
            name,
            "unsigned __int8",
            "(unsigned __int8)bhf_u8(&Cur)",
        )),
        ...
        "const char *" | "const char*" | "char const *" => Some(CParamEmission {
            support: None,
            decl: format!("char *{name} = bhf_c_string(&Cur, 4096)"),
            arg: name.to_owned(),
        ...
        "char *" | "char*" => Some(CParamEmission {
            support: None,
            decl: format!("char *{name} = bhf_c_string_out(&Cur, 4096)"),
            arg: name.to_owned(),
        ...
        "const wchar_t *" | "const wchar_t*" | "wchar_t const *" => Some(CParamEmission {
            support: None,
            decl: format!("wchar_t *{name} = bhf_wc_string(&Cur, 2048)"),
            arg: name.to_owned(),
...
fn emit_top_level_value(
    ...
    match shape {
        ...
        TypeShape::CString => {
            ...
            // ALLOCATION must not be fuzzer-sized — see `bhf_c_string_out`. A
...
fn emit_top_level_pointer(
    ...
    match inner {
        ...
        TypeShape::Struct { name: struct_name, fields } => {
            ...
            let storage = format!("_bhf_value_{name}");
        ...
        TypeShape::Opaque(raw) => {
            ...
            if let Some(entry) = lookup_lifecycle(lifecycle, handle_key) {
                let storage = format!("_bhf_lc_{name}");
...
fn emit_struct_init(
    ...
    if depth >= ctx.limits.depth {
        ctx.push(format!(
            "/* bhf: {lvalue} left zeroed after decoder depth cap */"
        ));
        return;
    }
    if !ctx.active_aggregates.insert(struct_name.to_owned()) {
        ctx.push(format!(
            "/* bhf: {lvalue} left zeroed at recursive aggregate {struct_name} */"
        ));
        return;
    }
    for field in fields {
        ...
        if !is_c_identifier(&field.name) {
            ctx.push(format!(
                "/* bhf: field {lvalue}.<unnamed> left zeroed: unresolved member name */"
            ));
            continue;
        }
            ...
            emit_lvalue_decode(ctx, &field_lvalue, &field.c_type, &field.shape, depth + 1)
        {
            *ctx = before;
            ctx.push(format!(
                "/* bhf: field {field_lvalue} left zeroed: {err} */"
            ));
        }
    }
...
fn emit_union_init(
    ...
    ctx.push(format!(
        "/* bhf: union {lvalue} left zeroed: no decodable member */"
    ));
    Ok(())
}
...
fn emit_lvalue_decode(
    ...
    match shape {
        ...
        TypeShape::CString => {
            ctx.push(format!("{lvalue} = bhf_c_string(&Cur, 256)"));
...
/// (`{T} _bhf_pf_<field> = <decoded>; field = &_bhf_pf_<field>;`). The storage is a
...
fn emit_pointer_field_storage(
    ...
    let storage_type = storage_c_type(&base);
    let storage = format!("_bhf_pf_{}", sanitize_ident(lvalue));
    match inner {
        TypeShape::Scalar(kind) => {
            ctx.push(format!(
                "{storage_type} {storage} = {}",
...
fn emit_array_decode(
    ...
    let idx = format!("_bhf_i_{}", sanitize_ident(lvalue));
    ...
    match elem {
        ...
        TypeShape::Scalar(kind) => {
            if len > ctx.limits.array_elems {
                ...
                let count = format!("_bhf_n_{}", sanitize_ident(lvalue));
                ctx.push(format!(
                    "size_t {count} = (size_t)(bhf_u8(&Cur) % {})",
                    capped + 1
                ));
                ctx.push(format!(
...
fn emit_enum_decode(
    ...
    if members.is_empty() {
        ...
        let expr = format!("({decl_type})bhf_bounded_i32(&Cur, 0, 255)");
        if declare {
    ...
    let sel = format!("_bhf_sel_{}", sanitize_ident(lvalue));
    ctx.push(format!("int {sel} = bhf_bounded_i32(&Cur, 0, {max})"));
    if declare {
...
fn scalar_expr(kind: ScalarKind, c_type: &str) -> String {
    match kind {
        ScalarKind::Bool => format!("({c_type})(bhf_u8(&Cur) & 1)"),
        ScalarKind::I8 => {
            if c_type.trim() == "int8_t" {
                "(int8_t)bhf_u8(&Cur)".to_owned()
            } else {
                format!("({c_type})bhf_u8(&Cur)")
            }
        }
        ScalarKind::U8 => {
        ...
        ScalarKind::I16 => format!("({c_type})bhf_i32(&Cur)"),
        ScalarKind::U16 => format!("({c_type})bhf_bounded_i32(&Cur, 0, 0xffff)"),
        ScalarKind::I32 => {
            if matches!(c_type.trim(), "int" | "signed int" | "int32_t") {
                "bhf_i32(&Cur)".to_owned()
            } else {
                format!("({c_type})bhf_i32(&Cur)")
            }
        }
        ScalarKind::U32 => format!("({c_type})bhf_bounded_i32(&Cur, 0, 0x7fffffff)"),
        ScalarKind::I64 => {
            if matches!(c_type.trim(), "long long" | "int64_t") {
                "bhf_i64(&Cur)".to_owned()
            } else {
                format!("({c_type})bhf_i64(&Cur)")
            }
        }
        ScalarKind::U64 => format!("({c_type})bhf_bounded_length(&Cur, 0, 0x7fffffff)"),
        ScalarKind::F32 => format!("({c_type})bhf_i32(&Cur)"),
        ScalarKind::F64 => format!("({c_type})bhf_i64(&Cur)"),
    }
}
...
fn typed_character_string(c_type: &str, name: &str) -> Option<CParamEmission> {
    ...
    let len = format!("_bhf_len_{name}");
    let index = format!("_bhf_i_{name}");
    Some(CParamEmission {
        support: None,
        decl: format!(
            "size_t {len} = bhf_bounded_length(&Cur, 0, 4096); \
             {base} *{name} = ({base} *)calloc({len} + 1, sizeof({base})); \
             for (size_t {index} = 0; {name} && {index} < {len}; ++{index}) {{ \
                 {name}[{index}] = ({base})bhf_u8(&Cur); \
             }}"
        ),
        arg: name.to_owned(),
...
fn mutable_byte_pointer(c_type: &str, name: &str) -> CParamEmission {
    let cap = format!("_bhf_cap_{name}");
    let copy = format!("_bhf_copy_{name}");
    CParamEmission {
        support: None,
        decl: format!(
...
fn wide_string_vector(name: &str) -> CParamEmission {
    let value = format!("_bhf_wvalue_{name}");
    let items = format!("_bhf_witems_{name}");
    CParamEmission {
        support: None,
        decl: format!(
            "wchar_t *{value} = bhf_wc_string(&Cur, 2048); \
...
fn char_double_pointer(is_const: bool, name: &str) -> CParamEmission {
    ...
    if is_out {
        CParamEmission {
            support: None,
            decl: format!("{elem} *_bhf_end_{name} = 0; {elem} **{name} = &_bhf_end_{name}"),
            arg: name.to_owned(),
            c_type: format!("{elem} **"),
            free: None,
        }
        ...
        CParamEmission {
            support: None,
            decl: format!(
                "char *_bhf_buf_{name} = bhf_c_string(&Cur, 4096); \
                 {elem} *_bhf_p_{name} = _bhf_buf_{name}; \
                 {elem} **{name} = &_bhf_p_{name}"
            ),
            arg: name.to_owned(),
            c_type: format!("{elem} **"),
            free: Some(format!("free(_bhf_buf_{name})")),
        }
    }
}
...
fn out_pointer_decoder(c_type: &str, name: &str) -> Option<CParamEmission> {
    ...
    let (decoder_expr, storage_init) = match base.as_str() {
        "int" | "signed int" => ("bhf_i32(&Cur)", "0"),
        "unsigned" | "unsigned int" => ("(unsigned int)bhf_bounded_i32(&Cur, 0, 0x7fffffff)", "0"),
        "long" | "signed long" => ("(long)bhf_i64(&Cur)", "0"),
        "unsigned long" => (
            "(unsigned long)bhf_bounded_length(&Cur, 0, 0x7fffffff)",
            "0",
        ),
        "uLong" | "uLongf" => ("(uLong)bhf_bounded_length(&Cur, 0, 0x7fffffff)", "0"),
        "uInt" => ("(uInt)bhf_bounded_i32(&Cur, 0, 0x7fffffff)", "0"),
        "z_size_t" => ("(z_size_t)bhf_bounded_length(&Cur, 0, 65536)", "0"),
        "mz_ulong" => ("(mz_ulong)bhf_bounded_length(&Cur, 0, 0x7fffffff)", "0"),
        "long long" | "signed long long" => ("bhf_i64(&Cur)", "0"),
        "size_t" => ("bhf_bounded_length(&Cur, 0, 65536)", "0"),
        "uint8_t" | "unsigned char" => ("bhf_u8(&Cur)", "0"),
        "int8_t" | "signed char" | "char" => ("(int8_t)bhf_u8(&Cur)", "0"),
        "int32_t" => ("bhf_i32(&Cur)", "0"),
        "uint32_t" => ("(uint32_t)bhf_bounded_i32(&Cur, 0, 0x7fffffff)", "0"),
        "int64_t" => ("bhf_i64(&Cur)", "0"),
        "uint64_t" => ("(uint64_t)bhf_i64(&Cur)", "0"),
        _ => return None,
    };
    let storage_name = format!("_bhf_out_{name}");
    let _ = storage_init;
    let decl =
...
/// pointer API is left as the normal `bhf_c_string` decoder. Recognises the common
...
/// Without this a fuzzed `bhf_c_string` path ENOENTs and the parser never runs — a
...
/// `%` neutralised (`bhf_c_format_string`) rather than the plain `bhf_c_string`. A
...
pub(crate) fn c_format_string_param(name: &str, is_const: bool) -> CParamEmission {
    CParamEmission {
        support: None,
        decl: format!("char *{name} = bhf_c_format_string(&Cur, 4096)"),
        arg: name.to_owned(),
...
fn file_pointer(c_type: &str, name: &str) -> CParamEmission {
    let buffer = format!("_bhf_file_buf_{name}");
    let setup = format!(
...
pub fn best_effort_param_emission(c_type: &str, name: &str) -> CParamEmission {
    ...
    if trimmed.ends_with('*') {
        let buf = format!("_bhf_force_{name}");
        return CParamEmission {
            support: None,
            decl: format!(
...
mod tests {
    ...
    fn control_flag_discriminator_is_pinned_to_zero() {
        ...
        assert!(w.decl.contains("bhf_i32(&Cur)"), "{}", w.decl);
    }
    ...
    fn strip_type_decoration_drops_specifiers_attrs_and_decoration_macros() {
        ...
        // ';' after top level declarator" cascade, on a line BHF wrote itself.
    ...
    fn sanitize_param_name_drops_misparsed_leading_qualifiers() {
        ...
        // became the local name `_bhf_out___restrict row_buf`).
        ...
        assert_eq!(sanitize_or_synthesize_param_name("", 2), "_bhf_arg2");
    ...
    fn character_typedef_pointer_gets_a_typed_nul_terminated_string() {
        ...
        assert!(!e.decl.contains("_bhf_out_context"), "{}", e.decl);
    ...
    fn a_writable_char_buffer_gets_a_fixed_capacity_not_a_fuzzer_chosen_one() {
        ...
        assert!(
            out.decl.contains("bhf_c_string_out(&Cur, 4096)"),
        ...
        assert!(
            input.decl.contains("bhf_c_string(&Cur, 4096)")
                && !input.decl.contains("bhf_c_string_out"),
    ...
    fn format_string_param_uses_neutralised_decoder() {
        ...
        // bhf_c_format_string (which strips `%`), NOT bhf_c_string: a variadic
        ...
        assert!(
            msg.decl.contains("bhf_c_string(&Cur") && !msg.decl.contains("bhf_c_format_string"),
            "a non-format char* must stay on bhf_c_string: {}",
            msg.decl
        );
    }
    ...
    fn select_c_decoder_handles_int() {
        ...
        assert!(e.decl.contains("int x = bhf_i32(&Cur)"));
    ...
    fn select_c_decoder_handles_win32_bool_and_dword_scalars() {
        ...
        assert!(
            b.decl.contains("BOOL flag = (BOOL)bhf_i32(&Cur)"),
            "{}",
            b.decl
        );
        ...
        assert!(
            d.decl
                .contains("DWORD count = (DWORD)bhf_bounded_i32(&Cur, 0, 0x7fffffff)"),
            "{}",
            d.decl
        );
    }
    ...
    fn select_c_decoder_drives_null_terminated_wide_string_vector() {
        ...
        assert!(e.decl.contains("bhf_wc_string(&Cur, 2048)"), "{}", e.decl);
        assert!(
            e.decl.contains("const wchar_t *_bhf_witems_wfilenames[2]") && e.decl.contains("NULL"),
            "{}",
            e.decl
        );
        assert_eq!(e.arg, "wfilenames");
        assert_eq!(e.free.as_deref(), Some("free(_bhf_wvalue_wfilenames)"));
    }
    ...
    fn select_c_decoder_drives_const_char_double_pointer_cursor() {
        ...
        assert!(e.decl.contains("bhf_c_string"), "{}", e.decl);
        assert!(
            e.decl.contains("const char **string = &_bhf_p_string"),
            "{}",
            e.decl
        );
        assert_eq!(e.free.as_deref(), Some("free(_bhf_buf_string)"));
    }
    ...
    fn select_c_decoder_drives_const_char_double_pointer_out_param() {
        ...
        assert!(
            e.decl.contains("const char *_bhf_end_return_parse_end = 0"),
            "{}",
            e.decl
        );
        assert!(!e.decl.contains("bhf_c_string"), "{}", e.decl);
    ...
    fn select_c_decoder_handles_float_double_and_short_scalars() {
        ...
        assert!(
            d.decl.contains("double v = (double)bhf_i64(&Cur)"),
            "{}",
            d.decl
        );
        let f = select_c_decoder("float", "f").expect("float is supported");
        assert!(
            f.decl.contains("float f = (float)bhf_i32(&Cur)"),
            "{}",
            f.decl
        );
        let s = select_c_decoder("short", "s").expect("short is supported");
        assert!(
            s.decl.contains("short s = (short)bhf_i32(&Cur)"),
            "{}",
            s.decl
        );
        ...
        assert!(
            us.decl
                .contains("unsigned short u = (unsigned short)bhf_bounded_i32(&Cur, 0, 0xffff)"),
            "{}",
            us.decl
        );
    }
    ...
    fn channel_count_int_params_are_bounded_small_not_full_range() {
        ...
        // entry actually runs (jpgd req_comps must be 0/1/3/4): a full-range bhf_i32
        ...
        for n in [
            ...
            assert!(
                e.decl.contains("bhf_bounded_i32(&Cur, 0, 8)"),
        ...
        assert!(
            u.decl.contains("bhf_bounded_i32(&Cur, 0, 8)"),
            "got: {}",
            u.decl
        );
        ...
        assert!(
            generic.decl.contains("int count = bhf_i32(&Cur)"),
        ...
        assert!(
            w.decl.contains("int width = bhf_i32(&Cur)"),
            "got: {}",
            w.decl
        );
    }
    ...
    fn select_c_decoder_handles_zlib_length_output_pointer_alias() {
        ...
        assert!(e.decl.contains("uLongf _bhf_out_destLen"));
        assert!(e.decl.contains("uLongf *destLen = &_bhf_out_destLen"));
        assert!(e.free.is_none());
    }
    ...
    fn select_c_decoder_handles_file_pointer_with_fmemopen() {
        ...
        assert!(e.decl.contains("FILE * stream = _bhf_file_buf_stream ?"));
        assert!(e.decl.contains("memcpy(_bhf_file_buf_stream, Data, Size)"));
        assert!(e.decl.contains("Size"));
        assert!(e
            .decl
            .contains("fmemopen(_bhf_file_buf_stream, Size, \"r+\")"));
        assert!(e.decl.contains("tmpfile()"));
        assert_eq!(e.arg, "stream");
        assert_eq!(e.c_type, "FILE *");
        assert_eq!(
            e.free.as_deref(),
            Some("if (stream) fclose(stream); free(_bhf_file_buf_stream)")
        );
    }
    ...
    fn select_c_decoder_drives_file_path_param_with_tempfile() {
        ...
        for n in [
            ...
            assert!(
                e.decl.contains("bhf_make_tempfile(Data, Size"),
        ...
        // A plain string param is UNCHANGED — still the bhf_c_string decoder, so we
        // don't regress ordinary `const char *` inputs.
        let s = select_c_decoder("const char *", "name").expect("string supported");
        assert!(
            s.decl.contains("bhf_c_string(&Cur"),
    ...
    fn select_c_decoder_handles_miniz_file_macro_pointer_with_fmemopen() {
        ...
        assert!(e
            .decl
            .contains("MZ_FILE * stream = (MZ_FILE *)(_bhf_file_buf_stream ? (Size ? fmemopen"));
        assert!(e.decl.contains("memcpy(_bhf_file_buf_stream, Data, Size)"));
        assert!(e.decl.contains("Size"));
        assert_eq!(e.arg, "stream");
    ...
    fn win32_pointer_typedefs_are_drivable() {
        ...
        assert!(name.decl.contains("bhf_c_string"));
    ...
    fn select_c_decoder_with_registry_decodes_enum() {
        ...
        assert!(e
            .decl
            .contains("int _bhf_sel_mode = bhf_bounded_i32(&Cur, 0, 1)"));
    ...
    fn x_macro_enum_decodes_as_bounded_int_not_phantom_member() {
        ...
        assert!(
            e.decl
                .contains("enum http_errno err = (enum http_errno)bhf_bounded_i32(&Cur, 0, 255)"),
            "{}",
            e.decl
        );
        assert!(
    ...
    fn select_c_decoder_with_registry_decodes_struct_by_value() {
        ...
        assert!(e.decl.contains("cfg.count = bhf_i32(&Cur)"));
        assert!(e.decl.contains("cfg.name = bhf_c_string(&Cur, 256)"));
        assert!(e.decl.contains("cfg.mode = (enum mode)MODE_A"));
        assert!(e.decl.contains("size_t _bhf_i_cfg_tag = 0"));
        assert!(e
            .decl
            .contains("cfg.tag[_bhf_i_cfg_tag] = (char)bhf_u8(&Cur)"));
    ...
    fn select_c_decoder_fuzzes_fill_count_for_over_cap_scalar_array() {
        ...
        assert!(
            e.decl
                .contains("size_t _bhf_n_s_t = (size_t)(bhf_u8(&Cur) % 65)"),
            "{}",
            e.decl
        );
        assert!(e.decl.contains("_bhf_i_s_t < _bhf_n_s_t"), "{}", e.decl);
        assert!(
            e.decl.contains("s.t[_bhf_i_s_t] = bhf_i32(&Cur)"),
            "{}",
            e.decl
        );
    }
    ...
    fn bzip2_file_handle_is_opened_over_fuzz_bytes_and_closed() {
        ...
        assert!(emission.decl.contains("FILE * _bhf_bz_file_b = tmpfile()"));
        assert!(emission
            .decl
            .contains("fwrite(Data, 1, Size, _bhf_bz_file_b)"));
        assert!(emission
            .decl
            .contains("BZ2_bzReadOpen(&_bhf_bz_error_b, _bhf_bz_file_b, 0, 0, NULL, 0)"));
        let cleanup = emission.free.expect("BZFILE cleanup");
        assert!(cleanup.contains("BZ2_bzReadClose(&_bhf_bz_error_b, b)"));
        assert!(cleanup.contains("fclose(_bhf_bz_file_b)"));
    }
    ...
    fn select_c_decoder_with_registry_decodes_pointer_to_scalar_field() {
        ...
        assert!(
            e.decl.contains("int _bhf_pf_v_count = bhf_i32(&Cur)"),
            "{}",
            e.decl
        );
        assert!(e.decl.contains("v.count = &_bhf_pf_v_count"), "{}", e.decl);
    ...
    fn select_c_decoder_with_registry_decodes_pointer_to_struct_field() {
        ...
        assert!(
            e.decl.contains("struct point _bhf_pf_s_origin"),
            "{}",
            e.decl
        );
        assert!(e.decl.contains("_bhf_pf_s_origin.x ="), "{}", e.decl);
        assert!(
            e.decl.contains("s.origin = &_bhf_pf_s_origin"),
            "{}",
            e.decl
        );
    }
    ...
    fn recursive_struct_pointer_fields_are_zeroed_without_exponential_decoder_growth() {
        ...
        assert!(!e.decl.contains("_bhf_pf_node_next"), "{}", e.decl);
        assert!(
            e.decl.len() < 8 * 1024,
    ...
    fn select_c_decoder_skips_malformed_split_funcptr_fragment() {
        ...
        // unbalanced-paren -> clean skip, not a broken `bhf_i32` scalar harness that
        // fails to build.
        let reg = registry("");
    ...
    fn select_c_decoder_with_registry_emits_callback_trampoline_for_typedef_param() {
        ...
        assert!(support.contains("static int _bhf_cb_trampoline(void *opaque, const char *name)"));
        ...
        assert_eq!(e.decl, "visit_cb cb = (visit_cb)_bhf_cb_trampoline");
        assert_eq!(e.arg, "cb");
    ...
    fn select_c_decoder_emits_trampoline_for_inline_function_pointer_param() {
        ...
        // typedef — synthesize `_bhf_cb_<name>` and use it as the parameter's type.
        let reg = registry("");
        ...
        let support = e.support.as_deref().expect("callback support code");
        assert!(
            support.contains("typedef int (*_bhf_cb_callback)(int, int);"),
            "{support}"
        );
        assert!(support.contains("static int _bhf_callback_trampoline("));
        // The trampoline is cast to the (here synthesized, no-op) callback type.
        assert_eq!(
            e.decl,
            "_bhf_cb_callback callback = (_bhf_cb_callback)_bhf_callback_trampoline"
        );
        assert_eq!(e.c_type, "_bhf_cb_callback");
    ...
    fn callback_trampoline_uses_exact_header_signature_no_guess() {
        ...
        assert!(
            support.contains(
                "static int _bhf_handler_trampoline(void* user, const char* section, const char* name, const char* value)"
            ),
    ...
    fn callback_trampoline_synthesizes_names_for_unnamed_params() {
        ...
        // a synthesized name.
        assert!(
            support.contains("(void)_bhf_arg0;") && support.contains("(void)_bhf_arg1;"),
        ...
        assert!(
            support.contains("_bhf_arg0,") && support.contains("_bhf_arg1)"),
    ...
    fn variadic_callback_trampoline_keeps_ellipsis_unnamed() {
        ...
        // named — emitting `... _bhf_arg2` is a syntax error ("expected ')'") and
        // `(void)_bhf_arg2;` then fails. The trampoline must keep a bare ellipsis.
        ...
        assert!(
            !support.contains("(void)_bhf_arg2;"),
    ...
    fn select_c_decoder_with_registry_decodes_struct_pointer() {
        ...
        assert!(e.decl.contains("struct config _bhf_value_out"));
        assert!(e
            .decl
            .contains("memset(&_bhf_value_out, 0, sizeof _bhf_value_out)"));
        assert!(e.decl.contains("_bhf_value_out.count = bhf_i32(&Cur)"));
        assert!(e.decl.contains("struct config * out = &_bhf_value_out"));
        assert_eq!(e.arg, "out");

        let alias = select_c_decoder_with_registry("config_ptr", "alias", &reg)
            .expect("struct pointer typedef is supported");
        assert!(alias.decl.contains("struct config _bhf_value_alias"));
        assert!(alias
            .decl
            .contains("_bhf_value_alias.count = bhf_i32(&Cur)"));
        assert!(alias.decl.contains("config_ptr alias = &_bhf_value_alias"));
    ...
    fn select_c_decoder_decodes_typed_output_handle_double_pointer() {
        ...
        assert!(e.decl.contains("_bhf_out_out = NULL"), "{}", e.decl);
        assert!(e.decl.contains("out = &_bhf_out_out"), "{}", e.decl);
        assert_eq!(e.arg, "out");
        assert_eq!(
            e.free.as_deref(),
            Some("if (_bhf_out_out) foo_free(_bhf_out_out)")
        );
        ...
        assert!(e2.decl.contains("_bhf_out_out = NULL"));
    ...
    fn select_c_decoder_with_registry_decodes_scalar_output_pointer_alias() {
        ...
        assert!(e
            .decl
            .contains("mz_uint32 _bhf_out_pIndex = (mz_uint32)bhf_bounded_i32"));
        assert!(e.decl.contains("mz_uint32 * pIndex = &_bhf_out_pIndex"));
    ...
    fn select_c_decoder_with_registry_decodes_enum_output_pointer_alias() {
        ...
        assert!(e
            .decl
            .contains("mz_zip_error _bhf_out_pErr = (mz_zip_error)MZ_ZIP_NO_ERROR"));
        assert!(e
            .decl
            .contains("case 1: _bhf_out_pErr = (mz_zip_error)MZ_ZIP_INVALID_PARAMETER; break"));
        assert!(e.decl.contains("mz_zip_error * pErr = &_bhf_out_pErr"));
    ...
    fn select_c_decoder_with_registry_decodes_const_miniz_time_pointer_alias() {
        ...
        assert!(e
            .decl
            .contains("MZ_TIME_T _bhf_out_pFile_time = (MZ_TIME_T)bhf_i64"));
        assert!(e
            .decl
            .contains("const MZ_TIME_T * pFile_time = &_bhf_out_pFile_time"));
    ...
    fn select_c_decoder_with_registry_decodes_void_pointer_output_slot() {
        ...
        assert!(e.decl.contains("void * _bhf_out_ppBuf = NULL"));
        assert!(e.decl.contains("void * * ppBuf = &_bhf_out_ppBuf"));
    ...
    fn select_c_decoder_floors_lone_const_byte_pointer_to_max_scalar_width() {
        ...
        assert!(
            e.decl
                .contains("if (_bhf_cap_source < 8) _bhf_cap_source = 8"),
            "{}",
            e.decl
        );
        assert!(
            e.decl
                .contains("memset((void *)source, 0, _bhf_cap_source)"),
            "{}",
            e.decl
        );
    ...
    #[test]
    fn select_c_decoder_routes_wide_string_pointer_to_bhf_wc_string() {
        ...
        assert!(e.decl.contains("bhf_wc_string(&Cur"), "{}", e.decl);
        ...
        assert!(m.decl.contains("bhf_wc_string(&Cur"), "{}", m.decl);
    }
    ...
    fn select_c_decoder_with_registry_decodes_union_first_decodable_member() {
        ...
        assert!(e.decl.contains("payload.code = bhf_i32(&Cur)"));
        assert!(!e.decl.contains("payload.name = bhf_c_string"));
    }
    ...
    fn lifecycle_constructs_opaque_handle_via_init_and_delete() {
        ...
        assert!(e.decl.contains("widget_t _bhf_lc_w"), "{}", e.decl);
        assert!(
            e.decl.contains("widget_initialize(&_bhf_lc_w)"),
            "{}",
            e.decl
        );
        assert!(e.decl.contains("widget_t * w = &_bhf_lc_w"), "{}", e.decl);
        assert_eq!(e.arg, "w");
        assert_eq!(e.free.as_deref(), Some("widget_delete(&_bhf_lc_w)"));
    }
    ...
    fn strict_lifecycle_skips_opaque_handle_incomplete_in_headers() {
        ...
        // stack-allocate `struct hashmap _bhf_lc_map;` + memset — an illegal "variable
    ...
    fn strict_lifecycle_constructs_opaque_handle_complete_in_headers() {
        ...
        assert!(
            e.decl.contains("struct yaml_token_s _bhf_lc_tok"),
            "{}",
            e.decl
        );
        assert!(
            e.decl
                .contains("memset(&_bhf_lc_tok, 0, sizeof _bhf_lc_tok)"),
            "{}",
            e.decl
        );
        assert_eq!(e.free.as_deref(), Some("yaml_token_delete(&_bhf_lc_tok)"));
    }
    ...
    fn interior_pointer_typedef_handle_built_via_returning_constructor_not_raw_string() {
        ...
        // bhf_c_string buffer (which underflows the header -> BHF-201 FP).
    ...
    fn lifecycle_lookup_is_insensitive_to_struct_tag_spelling() {
        ...
        assert!(
            !e.decl.contains("struct libdeflate_decompressor _bhf_lc"),
    ...
    fn lifecycle_returning_constructor_outranks_complete_private_struct_synthesis() {
        ...
        assert!(!e.decl.contains("_bhf_value"), "{}", e.decl);
    }
    ...
    fn lifecycle_without_delete_still_constructs_with_no_cleanup() {
        ...
        assert!(e.decl.contains("widget_create(&_bhf_lc_w)"), "{}", e.decl);
    ...
    fn lifecycle_delete_only_zero_inits_output_struct() {
        ...
        assert!(e.decl.contains("widget_t _bhf_lc_w"), "{}", e.decl);
        assert!(
            e.decl.contains("memset(&_bhf_lc_w, 0, sizeof _bhf_lc_w)"),
            "{}",
            e.decl
        );
        assert!(e.decl.contains("widget_t * w = &_bhf_lc_w"), "{}", e.decl);
