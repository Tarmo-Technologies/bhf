...
pub struct GenerateDirectArgs<'a> {
    ...
    /// (e.g. `UnZip.Bhf_Harness`) instead of `procedure Main`, plus a matching
...
pub fn generate_direct_harness(
    ...
    // A child-subprogram harness lives in `<parent>-bhf_harness.ad{s,b}` (GNAT
    // crunch of `UnZip.Bhf_Harness`) with a `private` spec; an ordinary harness
    ...
    if context.needs_stream_sink {
        let (spec, body) = bhf_sink_streams_sources();
        fs::write(args.output_dir.join("bhf_sink_streams.ads"), spec)?;
        fs::write(args.output_dir.join("bhf_sink_streams.adb"), body)?;
    }
    ...
    if context.needs_source_stream {
        let (spec, body) = bhf_source_streams_sources();
        fs::write(args.output_dir.join("bhf_source_streams.ads"), spec)?;
        fs::write(args.output_dir.join("bhf_source_streams.adb"), body)?;
    }
    // Emit the callback package when an access-to-subprogram parameter is backed
    // by a generated `Bhf_Callbacks` subprogram.
    if context.needs_callbacks {
        let (spec, body) = bhf_callbacks_sources();
        fs::write(args.output_dir.join("bhf_callbacks.ads"), spec)?;
...
struct TemplateContext {
    ...
    /// private child like `UnZip.Bhf_Harness`.
    harness_unit: String,
    ...
    /// `Bhf_Free_<name> (<name>);` statements run once after the input loop ends,
    ...
    /// the generated `Bhf_Sink_Streams` package (emitted beside `main.adb`).
    ...
    /// `Bhf_Source_Streams` package (emitted beside `main.adb`).
    needs_source_stream: bool,
    /// True when an access-to-subprogram parameter is backed by a generated
    /// `Bhf_Callbacks` subprogram, so the harness `with`s the `Bhf_Callbacks`
    /// package (emitted beside `main.adb`) and catches its `BHF_Fuzz_EOF`.
    ...
    for (param_index, param) in target_params.iter().enumerate() {
        ...
        if let Some(root) = stream_init::class_wide_root(&param.type_ref.name_path) {
                ...
                && matches!(param.mode, ParamMode::AccessMode)
            {
                ...
                params.push(ParamContext {
                    pre_call_lines: vec![format!(
                        "Bhf_Source_Streams.Set ({backing}, Buf'Unchecked_Access, Last);"
                    )],
                    name: name.clone(),
                    ada_type_name: "access Bhf_Source_Streams.Fuzz_Stream".to_owned(),
                    decoder_expr: String::new(),
                    setup_lines: Vec::new(),
                    needs_initializer: false,
                    post_call_lines: Vec::new(),
                    once_decl: Some(format!(
                        "{backing} : aliased Bhf_Source_Streams.Fuzz_Stream;\n   \
                         {name} : constant access Bhf_Source_Streams.Fuzz_Stream := \
                         {backing}'Access;"
                    )),
                });
                ...
                && stream_sink(args.ast, &resolved_param.type_ref).is_none()
            {
                ...
                params.push(ParamContext {
                    pre_call_lines: vec![format!(
                        "Bhf_Source_Streams.Set ({name}, Buf'Unchecked_Access, Last);"
                    )],
                    name,
                    ada_type_name: "Bhf_Source_Streams.Fuzz_Stream".to_owned(),
                    decoder_expr: String::new(),
                    setup_lines: Vec::new(),
                    needs_initializer: false,
                    post_call_lines: Vec::new(),
        ...
        if let Some(sink) = access_to_array_sink(args.ast, &resolved_param.type_ref) {
            ...
            let once_decl = if sink.is_constant {
                ...
                let free_proc = format!("Bhf_Free_{name}");
        ...
        // moment the callee uses it. Back it with a concrete `Bhf_Callbacks`
        // subprogram whose `'Access` we pass: a source callback feeds the fuzz
        // bytes (raising `BHF_Fuzz_EOF` at end-of-input), a sink callback discards.
        ...
        if let Some(cb) = access_to_subprogram_callback(args.ast, &resolved_param.type_ref) {
            ...
            let pre_call_lines = if callback_set_emitted {
                ...
                vec!["Bhf_Callbacks.Set (Buf'Unchecked_Access, Last);".to_owned()]
            };
            params.push(ParamContext {
                name,
        ...
        let uses_callbacks = decoder.call_expr.contains("Bhf_Callbacks.")
            || decoder
                .setup_lines
                .iter()
                .any(|line| line.contains("Bhf_Callbacks."));
...
struct OutParamConstructor {
    ...
    /// `Bhf_Free_<n> (<n>);` statements freeing those backing streams after the
    ...
    /// stream argument from the input bytes (`Bhf_Source_Streams.Set (...)`).
    setup_lines: Vec<String>,
    /// True when a stream argument used the generated `Bhf_Sink_Streams` package.
    needs_stream_sink: bool,
    /// True when a by-reference `Root_Stream_Type'Class` argument is backed by
    /// the generated `Bhf_Source_Streams` fuzz source stream.
    needs_source_stream: bool,
}
...
fn discover_out_param_constructor(
    ...
    for sp in &ast.subprograms {
        ...
        for (index, param) in sp.params.iter().enumerate() {
            ...
            {
                let backing = format!("Bhf_Ctor_Stream_{index}");
                arg_decls.push(format!("{backing} : Bhf_Source_Streams.Fuzz_Stream;"));
            ...
            if matches!(param.mode, ParamMode::In | ParamMode::InOut) {
                if let Some(sink) = stream_sink(ast, &resolved.type_ref) {
                    let backing = format!("Bhf_Ctor_Stream_{index}");
                    let free_proc = format!("Bhf_Free_{backing}");
...
/// (`[Bhf_Generic_Instance, Word]`). A type declared inside the generic must
...
/// (`Json.Types.Create_Null` -> `Bhf_Generic_Instance.Create_Null`). A
...
struct StreamSink {
    ...
    /// `Bhf_Sink_Streams.Null_Stream` package beside the harness.
...
/// generated `Bhf_Sink_Streams.Null_Stream` (Write discards, Read reports EOF);
...
fn stream_sink(ast: &StructuralAst, type_ref: &TypeRef) -> Option<StreamSink> {
    ...
    if root_simple == "root_stream_type" {
        return Some(StreamSink {
            ...
            allocator: "new Bhf_Sink_Streams.Null_Stream".to_owned(),
            extra_withs: Vec::new(),
...
fn bhf_sink_streams_sources() -> (&'static str, &'static str) {
    (
        "--  SPDX-License-Identifier: Apache-2.0\n\
         with Ada.Streams;\n\
         package Bhf_Sink_Streams is\n\
         ...
         end Bhf_Sink_Streams;\n",
        "--  SPDX-License-Identifier: Apache-2.0\n\
         package body Bhf_Sink_Streams is\n\
...
fn bhf_source_streams_sources() -> (&'static str, &'static str) {
    (
        "--  SPDX-License-Identifier: Apache-2.0\n\
         with Ada.Streams;\n\
         package Bhf_Source_Streams is\n\
         ...
         \x20  end record;\n\
         end Bhf_Source_Streams;\n",
        "--  SPDX-License-Identifier: Apache-2.0\n\
         package body Bhf_Source_Streams is\n\
         \x20  use type Ada.Streams.Stream_Element_Offset;\n\
         \x20  procedure Set\n\
         \x20    (S    : in out Fuzz_Stream;\n\
         \x20     Data : Buffer_Ref;\n\
...
///     raise `BHF_Fuzz_EOF` once the input is consumed. A read-until-EOF decode
...
///     on a fixed buffer — the harness catches `BHF_Fuzz_EOF` as normal
///     end-of-input, NOT a finding.
///   * Sink callbacks (`Snk_*`, `Noop`) discard — the callee's output channel.
fn bhf_callbacks_sources() -> (&'static str, &'static str) {
    (
        "--  SPDX-License-Identifier: Apache-2.0\n\
         pragma Ada_2012;\n\
         with Ada.Streams;\n\
         with Interfaces;\n\
         package Bhf_Callbacks is\n\
         ...
         \x20  BHF_Fuzz_EOF : exception;\n\
         \x20  type Buffer_Ref is access all Ada.Streams.Stream_Element_Array;\n\
         \x20  --  Install the fuzz input as the byte source the callbacks serve.\n\
         \x20  procedure Set\n\
         \x20    (Data : Buffer_Ref;\n\
         \x20     Last : Ada.Streams.Stream_Element_Offset);\n\
         \x20  --  Source callbacks: yield the next fuzz byte, raise BHF_Fuzz_EOF at end.\n\
        ...
        "--  SPDX-License-Identifier: Apache-2.0\n\
         ...
         \x20     Last : Ada.Streams.Stream_Element_Offset) is\n\
         \x20  begin\n\
         \x20     Bhf_Callbacks.Data := Data;\n\
         \x20     Bhf_Callbacks.Pos  := (if Data = null then 1 else Data'First);\n\
         \x20     Bhf_Callbacks.Last := Last;\n\
         \x20  end Set;\n\
         \x20  function Next return Ada.Streams.Stream_Element is\n\
         \x20  begin\n\
         \x20     if Data = null or else Pos > Last then\n\
         \x20        raise BHF_Fuzz_EOF;\n\
         \x20     end if;\n\
         ...
         \x20        exit when Data = null or else Pos > Bhf_Callbacks.Last;\n\
...
/// library-level `Bhf_Callbacks` subprogram whose `'Access` the harness passes,
...
struct CallbackSynth {
    ...
    /// (C : out Character)`).
    decl_type: String,
    /// The initializer expression: `Bhf_Callbacks.Src_Char'Access`.
    access_expr: String,
}

/// If `type_ref` is an access-to-subprogram parameter whose profile we can back
/// with a concrete callback, describe the `Bhf_Callbacks` subprogram to pass
...
fn access_to_subprogram_callback(ast: &StructuralAst, type_ref: &TypeRef) -> Option<CallbackSynth> {
    ...
    Some(CallbackSynth {
        decl_type,
        access_expr: format!("Bhf_Callbacks.{subprogram}'Access"),
    })
}

/// Map a lower-cased access-to-subprogram profile (`procedure (c : out
/// character)`, `function return boolean`) to the `Bhf_Callbacks` subprogram that
...
/// Whether a lower-cased type mark is one of the byte types `Bhf_Callbacks`'
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-harness-gen-{name}-{nonce}"));
    ...
    fn instance_qualified_name_path_rewrites_generic_local_types() {
        ...
        // the instance (`Bhf_Generic_Instance.Word`), not the generic package
        ...
        assert_eq!(
            ...
            Some(vec!["Bhf_Generic_Instance".to_owned(), "Word".to_owned()])
        );
    ...
    fn instance_qualified_constructor_path_rewrites_generic_owned_ctors() {
        ...
        assert_eq!(
            ...
            Some("Bhf_Generic_Instance.Create_Null".to_owned())
        );
        ...
        assert_eq!(
            ...
            Some("Bhf_Generic_Instance.Sub.Make".to_owned())
        );
    ...
    fn out_param_constructor_synthesizes_stream_argument() {
        ...
        let main_adb = fs::read_to_string(generated.main_adb).unwrap();
        assert!(
            main_adb.contains("Bhf_Ctor_Stream_1"),
            "constructor stream arg must be backed by a declared stream: {main_adb}"
        );
        assert!(
            main_adb.contains("Make (W, Bhf_Ctor_Stream_1)")
                || main_adb.contains("Pkg.Make (W, Bhf_Ctor_Stream_1)"),
    ...
    fn anonymous_access_classwide_stream_uses_persistent_fuzz_stream() {
        ...
        let generated = generate_for(&ast, &target, &output_dir).unwrap();
        let main_adb = fs::read_to_string(generated.main_adb).unwrap();
        assert!(main_adb.contains("with Bhf_Source_Streams;"), "{main_adb}");
        assert!(
            main_adb.contains("Back_Backing : aliased Bhf_Source_Streams.Fuzz_Stream;"),
            "{main_adb}"
        );
        assert!(
            main_adb.contains("Back_Backing'Access;")
                && main_adb.contains("Bhf_Source_Streams.Set (Back_Backing")
    ...
    fn generate_lowercase_package_target_uses_conventional_ada_casing() {
        ...
        assert!(main_adb.contains("Bhf_Result : constant Integer := Pkg.Parse (S);"));
    }
    ...
    fn generate_qualifies_package_owned_return_type() {
        ...
        assert!(main_adb.contains("Bhf_Result : constant Pkg.Model := Pkg.Make;"));
    }
    ...
    fn out_access_constant_to_string_gets_initialized_buffer_no_free() {
        ...
        assert!(
            !main_adb.contains("Unchecked_Deallocation") && !main_adb.contains("Bhf_Free_Dst"),
    ...
    fn out_access_to_stream_element_array_gets_bounded_heap_sink() {
        ...
        assert!(main_adb.contains(
            "procedure Bhf_Free_Out_Buf is new Ada.Unchecked_Deallocation\n     \
             (Ada.Streams.Stream_Element_Array, P_Stream_Element_Array);"
        ));
        assert!(main_adb.contains("Bhf_Free_Out_Buf (Out_Buf);"));
    }
    ...
    fn access_to_root_stream_class_gets_discard_stream_sink() {
        ...
        assert!(main_adb.contains("with Bhf_Sink_Streams;"));
        assert!(main_adb.contains("Out_Stream : P_Stream := new Bhf_Sink_Streams.Null_Stream;"));
        assert!(main_adb.contains("Bhf_Free_Out_Stream (Out_Stream);"));
        assert!(!main_adb.contains("Out_Stream : P_Stream := null"));
        // The discard-stream package is emitted beside the harness.
        let pkg = output_dir.join("bhf_sink_streams.ads");
        assert!(pkg.exists(), "Bhf_Sink_Streams spec must be emitted");
        let pkg_body = fs::read_to_string(output_dir.join("bhf_sink_streams.adb")).unwrap();
    ...
    fn access_to_custom_stream_root_allocates_inmemory_derivation() {
        ...
        assert!(main_adb.contains("Bhf_Free_Z (Z);"));
        // No discard-stream package for a custom root.
        assert!(!output_dir.join("bhf_sink_streams.ads").exists());
        assert!(!main_adb.contains("Bhf_Sink_Streams"));
    }
    ...
    fn access_to_subprogram_param_backed_by_callback() {
        ...
        assert!(main_adb.contains("with Bhf_Callbacks;"));
        assert!(main_adb.contains("Get : Getchar_Ptr := Bhf_Callbacks.Src_Char'Access;"));
        assert!(main_adb.contains("Bhf_Callbacks.Set (Buf'Unchecked_Access, Last);"));
        // BHF_Fuzz_EOF is the normal end-of-input, caught silently (not a finding).
        assert!(main_adb.contains("when Bhf_Callbacks.BHF_Fuzz_EOF =>"));
        // The callback package is emitted beside the harness.
        assert!(output_dir.join("bhf_callbacks.ads").exists());
        assert!(output_dir.join("bhf_callbacks.adb").exists());
    }
    ...
    fn access_to_object_param_is_not_treated_as_callback() {
        ...
        let main_adb =
            fs::read_to_string(generate_for(&ast, &target, &output_dir).unwrap().main_adb).unwrap();
        assert!(!main_adb.contains("Bhf_Callbacks"));
    }
}
