...
/// mode the host Codex/Claude session does the reasoning; BHF exposes
...
fn mcp_tools() -> Vec<Value> {
    ...
    vec![
        ...
        json!({
            "name": "bhf_load_findings",
            "description": "Load normalized BHF findings for evidence-grounded triage. Read-only; top defaults from the current MCP message budget and can be set explicitly.",
            "annotations": read_only_annotations(),
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        json!({
            "name": "bhf_preflight_harness",
            "description": "Perform a cheap structural check of an LLM-produced harness candidate. This never claims compilation, linking, reachability, or coverage; follow with deterministic BHF build/fuzz validation.",
            "annotations": read_only_annotations(),
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        match authorize_request(security, request.auth.as_ref()).and_then(|identity| {
            ...
            Ok((result, identity)) => {
                ...
                if let Some(audit) = identity.audit_metadata(&request.method) {
                    response["bhfAudit"] = serde_json::to_value(audit)
...
fn read_frame_with_limit<R: BufRead>(
    ...
    if length > limit {
        return Err(JsonRpcServerError::InvalidFrame(format!(
            "JSON-RPC frame exceeds the memory-aware {limit}-byte limit; set BHF_DAEMON_MAX_MESSAGE_BYTES to override"
        )));
    }
...
struct FindingsResult {
    findings: Vec<bhf_report::FindingReport>,
...
fn static_scan(params: StaticScanParams, limit: usize) -> Result<Value, String> {
    let out_dir = params
        .out
        .clone()
        .unwrap_or_else(|| PathBuf::from("bhf_work/static"));
...
pub struct BhfCliRunner;

impl CliRunner for BhfCliRunner {
    fn run(&self, argv: Vec<OsString>) -> i32 {
        cli::run_from(argv)
    }
}
...
#[derive(Debug, Clone)]
pub struct CliRpcService<R = BhfCliRunner> {
    runner: R,
}

impl Default for CliRpcService<BhfCliRunner> {
    fn default() -> Self {
        Self {
            runner: BhfCliRunner,
        }
...
impl<R: CliRunner> BhfRpc for CliRpcService<R> {
    fn build(&self, request: BuildRequest) -> RpcCommandResult {
...
fn command_argv(options: &RpcOptions, command: &'static str) -> Vec<OsString> {
    let mut argv = vec![OsString::from("bhf")];
...
mod tests {
    use super::{
        BhfRpc, BuildRequest, CliRpcService, CliRunner, FakeCorbaRequest, GenerateHarnessRequest,
        HarnessKind, InstrumentRequest, ListTargetsFormat, ListTargetsRequest, MinimizeRequest,
        MinimizeStrategy, ModelTrainRequest, ReplayRequest, ReportRequest, RpcOptions,
        StaticScanFailOn, StaticScanRequest, StubRequest,
    };
    ...
    fn static_scan_rpc_delegates_policy_triage_and_ci_flags_to_cli_argv() {
        ...
        let result = service.static_scan(StaticScanRequest {
            ...
            enabled_rules: vec!["BHF-408".to_owned()],
            disabled_rules: vec!["BHF-406".to_owned()],
            sarif: true,
    ...
    fn json_rpc_multi_tenant_mode_enforces_roles_and_records_audit_metadata() {
        ...
        assert_eq!(responses[0]["bhfAudit"]["tenant"], "viewer");
        assert_eq!(responses[0]["bhfAudit"]["role"], "viewer");
        assert_eq!(responses[0]["bhfAudit"]["method"], "rankAt");
    ...
    fn mcp_negotiates_lists_tools_and_prepares_session_prompt() {
        ...
        assert!(tools
            .iter()
            .any(|tool| tool["name"] == "bhf_prepare_assistance"));
        assert!(tools
            .iter()
            .find(|tool| tool["name"] == "bhf_list_targets")
            .and_then(|tool| tool["inputSchema"]["properties"]["top"]["default"].as_u64())
            .is_some_and(|top| top > 0));
        assert!(tools
            .iter()
            .find(|tool| tool["name"] == "bhf_load_findings")
            .and_then(|tool| tool["inputSchema"]["properties"]["top"]["default"].as_u64())
            .is_some_and(|top| top > 0));
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-daemon-{name}-{nonce}"));
        fs::create_dir_all(&dir).unwrap();
        dir
    }
}
