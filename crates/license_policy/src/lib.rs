...
const DEFAULT_AUDIT_PACKAGE: &str = "bhf";
...
mod tests {
    ...
    fn audit_metadata_reports_missing_third_party_matrix_entries() {
        ...
        let report =
            audit_metadata_json(&root, Profile::StrictPermissive, "bhf", fixture_metadata())
                .unwrap();
    ...
    fn fixture_metadata() -> &'static [u8] {
        br#"{
          "packages": [
            {
              "id": "path+file:///repo/crates/cli#bhf@0.1.0",
              "name": "bhf",
              "version": "0.1.0",
              "source": null,
              "license": "Apache-2.0",
    ...
    fn temp_dir(name: &str) -> std::path::PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("bhf-license-policy-{name}-{nonce}"));
