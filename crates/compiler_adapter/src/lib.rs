...
mod tests {
    ...
    fn build_invokes_compiler_with_dash_p_project_path() {
        ...
        let _path = PathEnvGuard::set(dir.path());
        let _arg_log = EnvVarGuard::set("BHF_ARG_LOG", log_path.as_os_str());

        let adapter = CompilerAdapter::discover().expect("adapter is discovered");
        let project = dir.file_path("bhf_build.gpr");
    ...
    fn check_passes_dash_gnatc_via_dash_cargs_to_compiler() {
        ...
        // the project, and bhf's build project declares C for the coverage
    ...
    fn build_captures_stdout_and_stderr() {
        ...
        let adapter = CompilerAdapter::discover().expect("adapter is discovered");
        let result = adapter
            .build(&dir.file_path("bhf_build.gpr"))
            .expect("build runs");
    ...
    fn build_returns_exit_code_of_subprocess() {
        ...
        dir.write_executable("gprbuild", fake_compiler_script());
        let _path = PathEnvGuard::set(dir.path());
        let _exit = EnvVarGuard::set("BHF_FAKE_EXIT", "7");
    ...
    fn build_records_duration() {
        ...
        dir.write_executable("gprbuild", fake_compiler_script());
        let _path = PathEnvGuard::set(dir.path());
        let _sleep = EnvVarGuard::set("BHF_FAKE_SLEEP", "0.02");
    ...
    fn probe_compiler_for_target_runs_prefixed_gnat_canary() {
        ...
        let _path = PathEnvGuard::set(dir.path());
        let _log = EnvVarGuard::set("BHF_RAW_GNAT_LOG", log_path.as_os_str());
    ...
    fn canary_probes_ada2022_when_supported() {
        ...
        let _path = PathEnvGuard::set(dir.path());
        let _log = EnvVarGuard::set("BHF_CANARY_LOG", log_path.as_os_str());
    ...
    fn canary_falls_back_to_ada2012_when_2022_rejected() {
        ...
        let _path = PathEnvGuard::set(dir.path());
        let _log = EnvVarGuard::set("BHF_CANARY_LOG", log_path.as_os_str());
        let _reject = EnvVarGuard::set("BHF_REJECT_ADA2022", "1");
    ...
    fn probe_caches_result_after_first_call() {
        ...
        let _path = PathEnvGuard::set(dir.path());
        let _counter = EnvVarGuard::set("BHF_PROBE_COUNT", count_path.as_os_str());
    ...
    impl TestPathDir {
        fn new(test_name: &str) -> Self {
            let path = std::env::temp_dir().join(format!(
                "bhf-compiler-adapter-{test_name}-{}",
                std::process::id()
            ));
            if path.exists() {
    ...
    fn fake_compiler_script() -> &'static str {
        r#"#!/bin/sh
if [ -n "$BHF_ARG_LOG" ]; then
  printf '%s\n' "$@" > "$BHF_ARG_LOG"
fi
echo "fake compiler stdout"
echo "fake compiler stderr" >&2
if [ -n "$BHF_FAKE_SLEEP" ]; then
  /bin/sleep "$BHF_FAKE_SLEEP"
fi
exit "${BHF_FAKE_EXIT:-0}"
"#
    }
...
"#
    ...
    fn fake_canary_compiler_script() -> &'static str {
        r#"#!/bin/sh
if [ -n "$BHF_PROBE_COUNT" ]; then
  count=0
  if [ -f "$BHF_PROBE_COUNT" ]; then
    IFS= read -r count < "$BHF_PROBE_COUNT"
  fi
  count=$((count + 1))
  printf '%s\n' "$count" > "$BHF_PROBE_COUNT"
fi
