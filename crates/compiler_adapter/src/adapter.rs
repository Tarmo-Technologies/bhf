...
impl CompilerAdapter {
    ...
    fn run(&self, project: &Path, mode: BuildMode) -> Result<BuildResult, CompilerError> {
        ...
        // process CWD. This is where bhf drops a synthesized stub project for a
        ...
        if mode == BuildMode::CheckOnly {
            ...
            // language in the project, and bhf's build project declares C
...
/// parallelism; `BHF_BUILD_JOBS` overrides it for a cgroup-constrained host or
...
pub(crate) fn build_job_count() -> usize {
    if let Some(raw) = std::env::var_os("BHF_BUILD_JOBS") {
...
/// `BHF_SUBPROCESS_TIMEOUT_SECS`. Unset (or 0) means unbounded, which is the
/// right default for a one-off `bhf build`; the sweep sets it from the
/// campaign budget.
fn subprocess_timeout() -> Option<Duration> {
    std::env::var("BHF_SUBPROCESS_TIMEOUT_SECS")
        .ok()
...
/// only covers the direct child, and only while bhf's spawning thread lives.
