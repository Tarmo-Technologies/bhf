...
//! bhf-native byte→typed decode runtime for generated Rust harnesses.
//!
//! This is the Rust analog of `c_runtime/bhf_decode.h`: a [`Cursor`] over a
//! `&[u8]` fuzz input that yields the typed argument values a generated
//! `bhf_run_one` feeds to the target function. It deliberately has **no**
//! `arbitrary` / `libfuzzer-sys` dependency — it is first-party, dependency-free
//! bhf code so the generated harness staticlib stays clean of any
//! third-party fuzzer in the licensable artifact (the same posture as the C
//! decoder header).
//!
//! ## Decode model (matches `bhf_decode.h`)
//!
...
//!   exactly `bhf_u8`/`bhf_i32`/`bhf_i64`'s behavior and is what lets the engine
...
//!   so later parameters still see fresh input. Mirrors `bhf_c_string`.
...
//!   `bhf_data_slice`). This is the high-value path: a `parse(&[u8])` target gets
...
impl<'a> Cursor<'a> {
    ...
    /// One byte (`u8`), `0` when exhausted. Mirrors `bhf_u8`.
    #[inline]
    ...
    /// `usize`/`isize` decode as their 64-bit cousins (bhf targets 64-bit
    ...
    /// still see fresh input. Mirrors `bhf_c_string` (sans the NUL terminator).
    #[inline]
    ...
    /// `bhf_data_slice` (but owned, so the harness can pass `&v` or `v`).
    #[inline]
