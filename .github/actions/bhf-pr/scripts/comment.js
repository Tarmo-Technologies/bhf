...
// Upsert a sticky bhf summary comment on the pull request. Keyed by an
...
module.exports = async ({ github, context, core, ciJsonPath }) => {
  ...
  try {
    ...
    core.info(`bhf: no ci-json at ${ciJsonPath} (${e}); skipping comment`);
    return;
  }

  const issue_number = context.payload.pull_request && context.payload.pull_request.number;
  if (!issue_number) {
    core.info('bhf: not a pull_request event; skipping comment');
    return;
  }
...
<sub>Scoped to this PR's diff under a bounded time budget — inline annotations are in the **Files changed** tab. Absence of findings is not proof of safety.</sub>`;
  ...
  if (mine) {
    ...
    core.info(`bhf: updated PR comment ${mine.id}`);
  } else {
    await github.rest.issues.createComment({ owner, repo, issue_number, body });
    core.info('bhf: created PR comment');
  }
};
