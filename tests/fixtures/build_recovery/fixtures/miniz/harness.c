...
//
// bhf fuzz target for miniz. Auto discovers `miniz_inflate_fuzz`
...
//   - open("/etc/miniz_bhf_config.conf") -> missing_files
//   - dlopen("libbhf_vendor...")  -> dlopen_failures
//   - connect("/tmp/bhf_missing.sock") -> network_endpoints
//
...
int miniz_inflate_fuzz(const unsigned char *data, unsigned long len) {
    ...
    // accidentally exist.
    int fd = open("/etc/miniz_bhf_config.conf", O_RDONLY);
    if (fd >= 0) {
        close(fd);
    }
    ...
    void *h = dlopen("libbhf_vendor_does_not_exist.so.42", RTLD_NOW);
    if (h) {
        dlclose(h);
    }
    ...
    if (sock >= 0) {
        ...
        strncpy(addr.sun_path, "/tmp/bhf_missing.sock", sizeof(addr.sun_path) - 1);
