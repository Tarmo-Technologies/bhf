...
impl Parser {
    ...
    /// Parse a record fed the whole fuzz input. A two-byte magic gate (`b"BHF"`)
    /// guards a planted out-of-bounds index bug (BHF-201): the attacker-supplied
    ...
    pub fn parse(&mut self, data: &[u8]) -> u32 {
        ...
        // Magic gate: only inputs starting with "BHF" reach the deeper logic.
