...
typedef struct bhf_cursor {
    const uint8_t *data;
    size_t size;
    size_t pos;
} bhf_cursor;

static inline bhf_cursor bhf_open(const uint8_t *data, size_t size) {
...
static inline size_t bhf_remaining(const bhf_cursor *c) {
 ...
 * the same forward cursor makes the program byte-fragile — `bhf_c_string`
 ...
 *
 * Each slot is BHF_CTRL_SLOT bytes wide.
 *
...
#define BHF_CTRL_SLOT 5
/* One count byte, then one slot per possible step. */
#define BHF_CTRL_LEN(max_steps) ((size_t)1 + (size_t)(max_steps) * BHF_CTRL_SLOT)
...
static inline bhf_cursor bhf_open_data(const uint8_t *data, size_t size, size_t ctrl_len) {
    bhf_cursor c;
    c.data = data;
...
static inline size_t bhf_ctrl_step_count(const uint8_t *data, size_t size, size_t max_steps) {
    size_t ctrl_len = BHF_CTRL_LEN(max_steps);
...
static inline uint32_t bhf_ctrl_bounded(const uint8_t *data, size_t at, uint32_t min,
                                       uint32_t max) {
    uint8_t selector;
    uint32_t raw;
...
static inline uint32_t bhf_ctrl_op(const uint8_t *data, size_t size, size_t step,
                                  size_t max_steps, uint32_t op_count) {
    size_t ctrl_len = BHF_CTRL_LEN(max_steps);
    size_t at;
    if (op_count == 0 || step >= max_steps || size < ctrl_len) return 0;
    at = size - ctrl_len + 1 + step * BHF_CTRL_SLOT;
    return bhf_ctrl_bounded(data, at, 0, op_count - 1);
}

static inline uint8_t bhf_u8(bhf_cursor *c) {
    if (bhf_remaining(c) == 0) return 0;
    return c->data[c->pos++];
}

static inline int32_t bhf_i32(bhf_cursor *c) {
    uint32_t v = 0;
    int i;
    for (i = 0; i < 4 && bhf_remaining(c) > 0; ++i) {
        v |= ((uint32_t)c->data[c->pos++]) << (i * 8);
    }
    return (int32_t)v;
}

static inline int64_t bhf_i64(bhf_cursor *c) {
    uint64_t v = 0;
    int i;
    for (i = 0; i < 8 && bhf_remaining(c) > 0; ++i) {
        v |= ((uint64_t)c->data[c->pos++]) << (i * 8);
    }
    return (int64_t)v;
}

static inline int32_t bhf_bounded_i32(bhf_cursor *c, int32_t lo, int32_t hi) {
    uint32_t range = (uint32_t)(hi - lo) + 1u;
    uint32_t raw = (uint32_t)bhf_i32(c);
    if (hi <= lo) return lo;
    return lo + (int32_t)(raw % range);
}

static inline size_t bhf_bounded_length(bhf_cursor *c, size_t lo, size_t hi) {
    size_t range = (hi - lo) + 1;
    size_t raw = (size_t)bhf_i64(c);
    if (hi <= lo) return lo;
    return lo + (raw % range);
}
 ...
 * free(). */
static inline char *bhf_c_string(bhf_cursor *c, size_t max) {
    uint16_t prefix = 0;
    size_t avail;
    size_t want;
    char *out;
    prefix |= (uint16_t)bhf_u8(c);
    prefix |= ((uint16_t)bhf_u8(c)) << 8;
    avail = bhf_remaining(c);
    want = (size_t)prefix;
...
/* Like bhf_c_string, but the ALLOCATION is a fixed `cap` rather than a
...
static inline char *bhf_c_string_out(bhf_cursor *c, size_t cap) {
    size_t avail;
    size_t want;
    char *out;
...
/* Like bhf_c_string, but NEUTRALISES printf-style format specifiers by replacing
...
static inline char *bhf_c_format_string(bhf_cursor *c, size_t max) {
    char *out = bhf_c_string(c, max);
    if (out) {
        char *p;
        for (p = out; *p; ++p) {
            if (*p == '%') {
...
/* Wide-char (wchar_t) analog of bhf_c_string: a NUL-terminated heap wchar_t
...
static inline wchar_t *bhf_wc_string(bhf_cursor *c, size_t max) {
    uint16_t prefix = 0;
    size_t avail_chars;
    size_t want;
    size_t nbytes;
    ...
    want = (size_t)prefix;
    if (want > max) want = max;
    avail_chars = bhf_remaining(c) / sizeof(wchar_t);
 ...
 * in caller-owned `path_buf`, which MUST be at least `bhf_tempfile_path_len`
...
#define bhf_tempfile_path_len 20
#if defined(__unix__) || defined(__APPLE__) || defined(__linux__)
#include <unistd.h>
static inline const char *bhf_make_tempfile(const uint8_t *data, size_t size, char *path_buf) {
    static const char tmpl[bhf_tempfile_path_len] = "/tmp/bhf_inXXXXXX";
    int fd;
...
static inline void bhf_data_slice(bhf_cursor *c, const uint8_t **ptr, size_t *size) {
    *ptr = c->data + c->pos;
    *size = bhf_remaining(c);
    c->pos = c->size;
}
