...
/*
 * bhf custom-allocator ASan poison bridge (#436).
 *
 ...
 *
 *   1. At startup, poison the WHOLE pool   -> bhf_asan_pool_init(pool, sizeof pool)
 *   2. On each allocation of `n` bytes     -> bhf_asan_on_alloc(ptr, n)
 *   3. On each free of a region of `n`     -> bhf_asan_on_free(ptr, n)
 *
...
#ifdef BHF_ASAN_ENABLED

/* Provided by the ASan runtime (declared here to stay header-only / C89). */
void __asan_poison_memory_region(void const volatile *addr, size_t size);
void __asan_unpoison_memory_region(void const volatile *addr, size_t size);

#define BHF_ASAN_POISON(addr, size) __asan_poison_memory_region((addr), (size))
#define BHF_ASAN_UNPOISON(addr, size) __asan_unpoison_memory_region((addr), (size))

#else /* !BHF_ASAN_ENABLED */

#define BHF_ASAN_POISON(addr, size) ((void)(addr), (void)(size))
#define BHF_ASAN_UNPOISON(addr, size) ((void)(addr), (void)(size))

#endif /* BHF_ASAN_ENABLED */
...
#define BHF_ASAN_UNUSED __attribute__((unused))
#else
#define BHF_ASAN_UNUSED
#endif
...
BHF_ASAN_UNUSED static void bhf_asan_pool_init(void *base, size_t size) {
    BHF_ASAN_POISON(base, size);
}

/* Mark `size` bytes at `ptr` as a live allocation (callable from the custom
 * allocator's success path). */
BHF_ASAN_UNUSED static void bhf_asan_on_alloc(void *ptr, size_t size) {
    BHF_ASAN_UNPOISON(ptr, size);
}
...
BHF_ASAN_UNUSED static void bhf_asan_on_free(void *ptr, size_t size) {
    BHF_ASAN_POISON(ptr, size);
}
